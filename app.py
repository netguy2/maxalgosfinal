...
if __name__ == "__main__":
    ...
    if not _is_reloader_parent:
        print("\033[93mStarting maxalgos...\033[0m", flush=True)
...
from blueprints.ai_insight import ai_insight_bp  # Import the AI insight blueprint
from blueprints.ai_settings import ai_settings_bp  # Import the AI settings blueprint
from blueprints.analytics import analytics_bp  # Import the analytics blueprint
...
from blueprints.chart_drawings import (
    chart_drawings_bp,
)  # Import the chart drawings blueprint
from blueprints.chart_watchlists import (
    chart_watchlists_bp,
)  # Import the chart watchlists blueprint
from blueprints.chartink import chartink_bp  # Import the chartink blueprint
from blueprints.core import core_bp
from blueprints.custom_indicators import (
    custom_indicators_bp,
)  # Import the custom indicators blueprint
...
from blueprints.indicator_presets import (
    indicator_presets_bp,
)  # Import the indicator presets blueprint
...
from blueprints.killswitch import killswitch_bp  # Import the kill switch blueprint
...
from blueprints.payments import payments_bp  # Import the Razorpay payments blueprint
...
from database.ai_settings_db import init_db as ensure_ai_settings_tables_exists
from database.analyzer_db import init_db as ensure_analyzer_tables_exists
from database.apilog_db import init_db as ensure_api_log_tables_exists
from database.auth_db import init_db as ensure_auth_tables_exists
from database.chart_drawing_db import init_db as ensure_chart_drawing_tables_exists
from database.chart_watchlist_db import init_db as ensure_chart_watchlist_tables_exists
from database.chartink_db import init_db as ensure_chartink_tables_exists
from database.custom_indicator_db import init_db as ensure_custom_indicator_tables_exists
from database.deployment_db import init_db as ensure_deployment_tables_exists
...
from database.indicator_preset_db import init_db as ensure_indicator_preset_tables_exists
from database.instrument_registry import init_db as ensure_instrument_registry_tables_exists
from database.latency_db import init_latency_db as ensure_latency_tables_exists
from database.leverage_db import init_db as ensure_leverage_tables_exists
from database.payment_db import init_db as ensure_payment_tables_exists
...
def create_app():
    # Ensure the users table schema (including the uuid/user_code/status/
    # email_verified/created_at/last_login_at identity columns) is up to
    # date BEFORE the app can serve a single request. This used to run only
    # inside _init_databases_and_schedulers()'s background thread below,
    # which raced against socketio.run() starting to accept connections --
    # any /auth/login request that landed before that thread finished hit
    # "no such column" on every User query (looks like an unrelated "session
    # expired" failure to the user, since the frontend treats any non-JSON
    # 500 response the same way). Login is the single most request-critical
    # path in the app, so this one init is synchronous; every other DB
    # (master contracts, chartink, sandbox, etc.) is fine staying on the
    # background thread since nothing serves a request against them this
    # early.
    ...
    # Default-namespace per-user room: every browser tab's Socket.IO
    # connection (useSocket.ts, no namespace) joins a room scoped to its own
    # username. Multi-tenant emits (force_logout on password change/reset,
    # admin suspend, etc.) target this room instead of broadcasting to every
    # connected client -- previously `socketio.emit("force_logout", ...)`
    # with no `room=` kicked EVERY logged-in user on the platform whenever
    # ANY single user reset/changed their password, which is exactly why
    # unrelated users saw random logouts once this stopped being a
    ...
    app.register_blueprint(payments_bp)  # Register Razorpay payments blueprint
    app.register_blueprint(killswitch_bp)  # Register kill switch blueprint
    app.register_blueprint(master_risk_bp)  # Register Master SL/Target blueprint
    app.register_blueprint(analyzer_bp)
    app.register_blueprint(settings_bp)
    app.register_blueprint(chartink_bp)
    app.register_blueprint(traffic_bp)
    app.register_blueprint(latency_bp)
    app.register_blueprint(analytics_bp)  # Register performance analytics blueprint
    ...
    app.register_blueprint(custom_indicators_bp)  # Register Custom Indicators blueprint
    app.register_blueprint(chart_watchlists_bp)  # Register Chart Watchlists blueprint
    app.register_blueprint(chart_drawings_bp)  # Register Chart Drawings blueprint
    app.register_blueprint(indicator_presets_bp)  # Register Indicator Presets blueprint
    app.register_blueprint(ai_settings_bp)  # Register AI Settings blueprint
    app.register_blueprint(ai_insight_bp)  # Register AI Insight blueprint
    ...
    if os.getenv("MCP_HTTP_ENABLED", "False").lower() == "true":
        ...
        # Crucial ordering: set MAX_ALGOS_MCP_HTTP_BOOT BEFORE importing the
        ...
        os.environ["MAX_ALGOS_MCP_HTTP_BOOT"] = "1"
        ...
        # maxalgos session cookie. Flask-WTF's global CSRFProtect would
        ...
        # browser-driven from the maxalgos session and uses the
    ...
    with app.app_context():
        ...
        # Exempt the Razorpay webhook -- it is server-to-server (Razorpay
        # cannot carry a CSRF token) and authenticates itself via the
        # X-Razorpay-Signature HMAC check inside the route (see
        # blueprints/payments.py::webhook), which MUST pass before any event
        # is processed.
        csrf.exempt(app.view_functions["payments_bp.webhook"])
        ...
        # Exempt email verification from CSRF. The user clicking a signup
        # verification link is NOT logged in and has no session, so there is
        # no CSRF token to send -- a CSRF-protected POST here fails for every
        # user with a 400 before the token is even checked (this is why
        # every fresh verification link showed "Verification failed"). CSRF
        # protection adds nothing here anyway: the single-use, unguessable,
        # expiring verification token IS the security credential, and there
        # is no authenticated session state a forged request could abuse.
        csrf.exempt(app.view_functions["auth.verify_email_api"])
        ...
        # Daily unattended re-login for opt-in password+TOTP brokers
        # (angel/fivepaisa/motilal). Off unless BROKER_AUTO_REFRESH_ENABLED.
        from services.broker_auto_refresh_scheduler import start_broker_auto_refresh_scheduler

        start_broker_auto_refresh_scheduler()

        # Master Target/SL account-wide combined-P&L auto-exit monitor.
        # Starts unconditionally (like the scheduler itself) -- the actual
        # enabled/disabled toggle is read fresh from settings on every tick,
        # so starting the scheduler here just means the poll loop exists;
        # it no-ops every tick until a user turns it on.
        from services.master_risk_monitor_service import start_master_risk_monitor

        start_master_risk_monitor()

        # Platform-managed trailing stops. Same "always start, no-op until
        # armed" shape as the monitor above: the poll loop simply exists and
        # does nothing until a filled entry registers a trailing stop.
        # Required because most brokers don't honour a trailing field, so
        # without this the Trailing Stop setting silently protects nothing.
        from services.trailing_stop_service import start_trailing_stop_monitor

        start_trailing_stop_monitor()
    ...
    def check_session_expiry():
        ...
        if (
            ...
            in [
                ...
                "/setup",
                "/payments/config",
                "/payments/setup/create-order",
                "/payments/setup/verify",
                "/download",
                "/faq",
...
def setup_environment(app):
    ...
    def _init_databases_and_schedulers():
        with app.app_context():
            ...
            # Strategy DB creates strategies/strategy_versions/deployments (one shared
            # Base -- see database/strategy_db.py). ensure_deployment_tables_exists()
            # is a no-op kept only for import compatibility with older call sites.
            try:
                ensure_strategy_tables_exists()
                ensure_deployment_tables_exists()
            except Exception as e:
                logger.error(f"Failed to initialize Strategy DB: {e}")
            ...
            db_init_functions = [
                ...
                ("Custom Indicator DB", ensure_custom_indicator_tables_exists),
                ("Chart Watchlist DB", ensure_chart_watchlist_tables_exists),
                ("Chart Drawing DB", ensure_chart_drawing_tables_exists),
                ("Indicator Preset DB", ensure_indicator_preset_tables_exists),
                ("AI Settings DB", ensure_ai_settings_tables_exists),
                ...
                ("Webhook Delivery DB", ensure_webhook_delivery_tables_exists),
                ("Leverage DB", ensure_leverage_tables_exists),
                ("Strategy Portfolio DB", ensure_strategy_portfolio_tables_exists),
                ("Instrument Registry DB", ensure_instrument_registry_tables_exists),
                ("Payments DB", ensure_payment_tables_exists),
            ]
            ...
            try:
                ...
                # Recovery: Re-register per-minute polling jobs for any active workflow
                # that has a timeCondition node but whose scheduler job is missing.
                # This handles the case where the server restarted after workflows were
                # activated, or where workflows were activated before the auto-polling
                # fix was deployed (commit fdea390).
                def _restore_time_condition_jobs():
                    try:
                        ...
                        for wf in workflows or []:
                            ...
                            if not api_key:
                                logger.warning(
                                    f"Flow recovery: workflow {wf.id} has no api_key, skipping"
                                )
                                continue

                            has_time_condition = any(
                                n.get("type") == "timeCondition" for n in nodes
                            )
                            if has_time_condition:
                                # Re-register the per-minute polling job (only
                                # if the scheduler doesn't already have it).
                                job_id = f"flow_workflow_{wf.id}"
                                if not scheduler.get_job(job_id):
                                    try:
                                        scheduler.set_api_key(api_key)
                                        new_job_id = scheduler.add_workflow_job(
                                            workflow_id=wf.id,
                                            schedule_type="interval",
                                            interval_value=1,
                                            interval_unit="minutes",
                                        )
                                        set_schedule_job_id(wf.id, new_job_id)
                                        restored += 1
                                        logger.info(
                                            f"Flow recovery: re-registered per-minute polling for "
                                            f"workflow {wf.id} ({wf.name})"
                                        )
                                    except Exception as _je:
                                        logger.warning(
                                            f"Flow recovery: failed to restore job for workflow {wf.id}: {_je}"
                                        )

                            # Re-register price-alert triggers. FlowPriceMonitor
                            # stores alerts in-memory only, so after a restart
                            # an active priceAlert workflow would silently stop
                            # firing while still showing "Active" in the UI.
                            # Rebuild each active workflow's alert from its
                            # priceAlert trigger node so price-triggered
                            # workflows keep working across restarts.
                            price_node = next(
                                (n for n in nodes if n.get("type") == "priceAlert"), None
                            )
                            if price_node:
                                td = price_node.get("data", {})
                                try:
                                    price_monitor.add_alert(
                                        workflow_id=wf.id,
                                        symbol=td.get("symbol", ""),
                                        exchange=td.get("exchange", "NSE"),
                                        condition=td.get("condition", "greater_than"),
                                        target_price=float(td.get("price", 0) or 0),
                                        price_lower=td.get("priceLower"),
                                        price_upper=td.get("priceUpper"),
                                        percentage=td.get("percentage"),
                                        api_key=api_key,
                                    )
                                    restored_alerts += 1
                                    logger.info(
                                        f"Flow recovery: re-registered price alert for "
                                        f"workflow {wf.id} ({wf.name})"
                                    )
                                except Exception as _ae:
                                    logger.warning(
                                        f"Flow recovery: failed to restore price alert for "
                                        f"workflow {wf.id}: {_ae}"
                                    )
                        if restored or restored_alerts:
                            logger.info(
                                f"Flow recovery: restored {restored} timeCondition polling job(s) "
                                f"and {restored_alerts} price alert(s)"
                            )
            ...
            try:
                ...
                start_signal_engine()
                logger.debug("Strategy Deployment and Signal Engines initialized")
            except Exception as e:
                logger.error(f"Failed to initialize Strategy Deployment/Signal Engines: {e}")

            # Sandbox execution engine + square-off scheduler are now
            # ALWAYS-ON shared background workers, not gated on any single
            # user's Analyze Mode toggle -- Analyze Mode moved to a
            # per-user flag (see UserRiskSettings.analyze_mode) precisely
            # because it used to be one global Settings.analyze_mode row,
            # and gating these threads on it meant one user turning their
            # own mode off stopped sandbox order execution and square-off
            # for every OTHER user who still had it on. Both engines
            # already filter their per-user work correctly -- they process
            # whatever sandbox orders/positions actually exist, which is
            # itself gated per-user at order-PLACEMENT time by
            # get_analyze_mode(username) in place_order_service.py -- so
            # they don't need the toggle to decide whether to run at all.
            try:
                ...
                logger.error(f"Error auto-starting sandbox execution engine/scheduler: {e}")
...
if __name__ == "__main__":
    ...
    if not debug or os.environ.get("WERKZEUG_RUN_MAIN") == "true":
        from utils.config import get_websocket_port
        ...
        _wsu = f"ws://{_dip}:{get_websocket_port()}"
        _du = "https://docs.maxalgos.in"
        G, C, M, W, Y, R, BD, DM = (
            "\033[92m",
            "\033[96m",
            "\033[95m",
