...
# Max Algos Domain Change Banner
echo -e "${BLUE}"
echo "  ██████╗ ██████╗ ███████╗███╗   ██╗ █████╗ ██╗      ██████╗  ██████╗ "
echo " ██╔═══██╗██╔══██╗██╔════╝████╗  ██║██╔══██╗██║     ██╔════╝ ██╔═══██╗"
echo " ██║   ██║██████╔╝███████╗██╔██╗ ██║███████║██║     ██║  ███╗██║   ██║"
...
# Max Algos Domain Change Script
# Changes the domain for an existing Max Algos server deployment.
...
# Start logging
log_message "Starting Max Algos domain change log at: $LOG_FILE" "$BLUE"
log_message "----------------------------------------" "$BLUE"
...
# fall back to scanning the legacy /var/python/maxalgos-flask/ tree
...
if [ -f "$SIMPLE_PATH/.env" ]; then
    ...
    SOCKET_FILE="$SIMPLE_PATH/maxalgos.sock"
    SERVICE_NAME="maxalgos"
    ENV_FILE="$MAX_ALGOS_PATH/.env"
    ...
    for dir in "$DEPLOY_BASE"/*/; do
        if [ -d "${dir}maxalgos" ] && [ -f "${dir}maxalgos/.env" ]; then
    ...
    if [ ${#DEPLOYMENTS[@]} -eq 0 ]; then
        log_message "Error: No Max Algos deployments found in $SIMPLE_PATH or $DEPLOY_BASE" "$RED"
        exit 1
    fi
    ...
    SOCKET_FILE="$BASE_PATH/maxalgos.sock"
...
log_message "Install Path:       $MAX_ALGOS_PATH" "$BLUE"
...
BACKUP_DIR="$MAX_ALGOS_PATH/db/domain_change_backup_${TIMESTAMP}"
...
log_message "Your Max Algos instance is now available at:" "$GREEN"
