...
#   1. Detect Docker Compose stacks under /opt/maxalgos (default)
...
# ---------------------------------------------------------------------------
INSTALL_BASE="${INSTALL_BASE:-/opt/maxalgos}"

log "\n[1/5] Detecting Max Algos Docker stacks..." "$BLUE"
...
# the domain (/opt/maxalgos/<domain>/), so derive the suggested
...
$(printf '%b' "${GREEN}=========================================================${NC}")
  ...
  Next steps for connecting from a hosted client (claude.ai, chatgpt.com):
    ...
    3. Approve the new client at /admin/remote-mcp on the dashboard
       (sign in to Max Algos first)
    4. Sign in to Max Algos when prompted to authorize the requested scopes
