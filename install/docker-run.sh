#!/bin/bash
# ============================================================================
# Max Algos Docker Runner for macOS/Linux
# ============================================================================
#
...
#
# Commands:
#   start    - Start Max Algos container (default, runs setup if needed)
...
# Valid brokers list
VALID_BROKERS="fivepaisa,fivepaisaxts,aliceblue,angel,arrow,compositedge,definedge,deltaexchange,dhan,dhan_sandbox,firstock,flattrade,fyers,groww,ibulls,iifl,iiflcapital,indmoney,jainamxts,kotak,motilal,mstock,nubra,paytm,pocketful,rmoney,samco,shoonya,tradejini,upstox,wisdom,zebu,zerodha,bnr"

# Banner
echo ""
echo -e "${BLUE}  ========================================${NC}"
echo -e "${BLUE}       Max Algos Docker Runner${NC}"
...
# Setup function
do_setup() {
    log_info "Setting up Max Algos in $MAX_ALGOS_DIR..."
    echo ""

    # Create db directory
    if [ ! -d "$MAX_ALGOS_DIR/db" ]; then
        log_info "Creating database directory..."
        mkdir -p "$MAX_ALGOS_DIR/db"
        if [ $? -ne 0 ]; then
    ...
    if [ -f "$MAX_ALGOS_DIR/$ENV_FILE" ]; then
        log_warn ".env file already exists at $MAX_ALGOS_DIR/$ENV_FILE"
    ...
    if ! curl -sL "$SAMPLE_ENV_URL" -o "$MAX_ALGOS_DIR/$ENV_FILE"; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS sed syntax
        sed -i '' "s/MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE/$APP_KEY/g" "$MAX_ALGOS_DIR/$ENV_FILE"
        sed -i '' "s/MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE/$API_KEY_PEPPER/g" "$MAX_ALGOS_DIR/$ENV_FILE"
    else
        # Linux sed syntax
        sed -i "s/MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE/$APP_KEY/g" "$MAX_ALGOS_DIR/$ENV_FILE"
        sed -i "s/MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE/$API_KEY_PEPPER/g" "$MAX_ALGOS_DIR/$ENV_FILE"
    ...
    if [ "$(uname)" = "Linux" ]; then
        if [ "$EUID" -ne 0 ] && command -v sudo >/dev/null 2>&1; then
            sudo chown 1000:1000 "$MAX_ALGOS_DIR/$ENV_FILE" 2>/dev/null || true
            sudo chmod 600 "$MAX_ALGOS_DIR/$ENV_FILE" 2>/dev/null || true
        else
            chown 1000:1000 "$MAX_ALGOS_DIR/$ENV_FILE" 2>/dev/null || true
            chmod 600 "$MAX_ALGOS_DIR/$ENV_FILE" 2>/dev/null || true
        fi
    else
        chmod 600 "$MAX_ALGOS_DIR/$ENV_FILE" 2>/dev/null || true
    fi
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS sed syntax
        sed -i '' "s/BROKER_API_KEY = 'YOUR_BROKER_API_KEY'/BROKER_API_KEY = '$BROKER_API_KEY'/g" "$MAX_ALGOS_DIR/$ENV_FILE"
        sed -i '' "s/BROKER_API_SECRET = 'YOUR_BROKER_API_SECRET'/BROKER_API_SECRET = '$BROKER_API_SECRET'/g" "$MAX_ALGOS_DIR/$ENV_FILE"
        sed -i '' "s|<broker>|$BROKER_NAME|g" "$MAX_ALGOS_DIR/$ENV_FILE"

        if [ "$IS_XTS" -eq 1 ]; then
            sed -i '' "s/BROKER_API_KEY_MARKET = 'YOUR_BROKER_MARKET_API_KEY'/BROKER_API_KEY_MARKET = '$BROKER_API_KEY_MARKET'/g" "$MAX_ALGOS_DIR/$ENV_FILE"
            sed -i '' "s/BROKER_API_SECRET_MARKET = 'YOUR_BROKER_MARKET_API_SECRET'/BROKER_API_SECRET_MARKET = '$BROKER_API_SECRET_MARKET'/g" "$MAX_ALGOS_DIR/$ENV_FILE"
        fi
    else
        # Linux sed syntax
        sed -i "s/BROKER_API_KEY = 'YOUR_BROKER_API_KEY'/BROKER_API_KEY = '$BROKER_API_KEY'/g" "$MAX_ALGOS_DIR/$ENV_FILE"
    ...
    if [[ "$OPEN_EDITOR" =~ ^[Yy]$ ]]; then
        if [[ "$OSTYPE" == "darwin"* ]]; then
            open -t "$MAX_ALGOS_DIR/$ENV_FILE"
        elif command -v xdg-open &> /dev/null; then
            # Linux with desktop environment - non-blocking
            xdg-open "$MAX_ALGOS_DIR/$ENV_FILE" &>/dev/null &
        elif command -v gedit &> /dev/null; then
            gedit "$MAX_ALGOS_DIR/$ENV_FILE" &>/dev/null &
        elif command -v code &> /dev/null; then
            code "$MAX_ALGOS_DIR/$ENV_FILE"
        else
            echo "  Edit .env manually: $MAX_ALGOS_DIR/$ENV_FILE"
        fi
    fi

    echo ""
    log_ok "Setup complete! Run './docker-run.sh start' to launch Max Algos."
    echo ""
    return 0
}
...
do_start() {
    ...
    if [ ! -f "$MAX_ALGOS_DIR/$ENV_FILE" ]; then
        ...
        if ! do_setup; then
            echo ""
            log_error "Setup failed. Cannot start Max Algos."
            echo "Please fix the issues above and try again."
            exit 1
        fi
        echo ""
        log_info "Starting Max Algos after setup..."
        echo ""
    fi
    ...
    if [ ! -d "$MAX_ALGOS_DIR/strategies" ]; then
        log_info "Creating strategies directory..."
        mkdir -p "$MAX_ALGOS_DIR/strategies/scripts"
        mkdir -p "$MAX_ALGOS_DIR/strategies/examples"
    fi
    if [ ! -d "$MAX_ALGOS_DIR/log" ]; then
        log_info "Creating log directory..."
        mkdir -p "$MAX_ALGOS_DIR/log/strategies"
    fi
    if [ ! -d "$MAX_ALGOS_DIR/keys" ]; then
        log_info "Creating keys directory..."
        mkdir -p "$MAX_ALGOS_DIR/keys"
    fi
    if [ ! -d "$MAX_ALGOS_DIR/tmp" ]; then
        log_info "Creating temp directory..."
        mkdir -p "$MAX_ALGOS_DIR/tmp"
    fi
    ...
    if docker run -d \
        ...
        -v "$MAX_ALGOS_DIR/tmp:/app/tmp" \
        -v "$MAX_ALGOS_DIR/.env:/app/.env" \
        --restart unless-stopped \
        "$IMAGE"; then

        echo ""
        log_success "Max Algos started successfully!"
        echo ""
        echo -e "${GREEN}  ========================================${NC}"
        echo -e "${GREEN}  Web UI:     http://127.0.0.1:5000${NC}"
        echo -e "${GREEN}  WebSocket:  ws://127.0.0.1:8785${NC}"
        echo -e "${GREEN}  ========================================${NC}"
        echo ""
        echo "  Data directory: $MAX_ALGOS_DIR"
        echo ""
        ...
        echo "    ./docker-run.sh stop     - Stop Max Algos"
        echo "    ./docker-run.sh restart  - Restart Max Algos"
        echo ""
    else
        echo ""
...
# Stop function
do_stop() {
    log_info "Stopping Max Algos..."
    docker stop "$CONTAINER" >/dev/null 2>&1
    docker rm "$CONTAINER" >/dev/null 2>&1
    log_ok "Max Algos stopped."
}

# Restart function
do_restart() {
    log_info "Restarting Max Algos..."
    do_stop
    echo ""
    do_start
}
...
do_status() {
    ...
    if docker ps --filter "name=$CONTAINER" --filter "status=running" | grep -q "$CONTAINER"; then
        echo -e "${GREEN}[STATUS]${NC} Max Algos is running."
        echo ""
        echo "  Web UI: http://127.0.0.1:5000"
    else
        echo -e "${YELLOW}[STATUS]${NC} Max Algos is NOT running."
    fi
    echo ""
    echo "  Data directory: $MAX_ALGOS_DIR"
}
