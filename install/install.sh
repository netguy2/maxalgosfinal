...
# Max Algos Installation and Configuration Script
...
validate_broker() {
    ...
    local valid_brokers="fivepaisa,fivepaisaxts,aliceblue,angel,arrow,compositedge,definedge,deltaexchange,dhan,dhan_sandbox,firstock,flattrade,fyers,groww,ibulls,iifl,iiflcapital,indmoney,jainamxts,kotak,motilal,mstock,nubra,paytm,pocketful,rmoney,samco,shoonya,tradejini,upstox,wisdom,zebu,zerodha,bnr"
...
# Start logging
log_message "Starting Max Algos installation log at: $LOG_FILE" "$BLUE"
log_message "----------------------------------------" "$BLUE"
...
log_message "Max Algos Installation Configuration" "$BLUE"
log_message "----------------------------------------" "$BLUE"
...
while true; do
    ...
    log_message "\nValid brokers: fivepaisa,fivepaisaxts,aliceblue,angel,arrow,compositedge,definedge,deltaexchange,dhan,dhan_sandbox,firstock,flattrade,fyers,groww,ibulls,iifl,iiflcapital,indmoney,jainamxts,kotak,motilal,mstock,nubra,paytm,pocketful,rmoney,samco,shoonya,tradejini,upstox,wisdom,zebu,zerodha,bnr" "$BLUE"
...
log_message "\nRemote MCP lets hosted AI clients (Claude.ai, ChatGPT) connect to Max Algos over HTTPS." "$BLUE"
...
#
#   App, venv, socket, .env all live under /var/python/maxalgos
#   systemd unit:  maxalgos.service
#   nginx vhost:   maxalgos.conf
DEPLOY_NAME="maxalgos"
MAX_ALGOS_PATH="/var/python/maxalgos"
BASE_PATH="$MAX_ALGOS_PATH"
VENV_PATH="$MAX_ALGOS_PATH/.venv"
SOCKET_PATH="$MAX_ALGOS_PATH"
SOCKET_FILE="$SOCKET_PATH/maxalgos.sock"
SERVICE_NAME="maxalgos"
...
NGINX_CONFIG_FILE="$NGINX_AVAILABLE/maxalgos.conf"

log_message "\nStarting Max Algos installation for $DEPLOY_NAME..." "$YELLOW"
...
case "$OS_TYPE" in
    ubuntu | debian | raspbian)
        ...
        # Install Chromium for Kaleido/Plotly static chart rendering.
        ...
        # Non-fatal — if nothing sticks we just warn; the rest of maxalgos still installs fine.
        log_message "\nInstalling Chromium for Kaleido chart rendering..." "$BLUE"
        if sudo apt-get install -y chromium fonts-liberation 2>/dev/null; then
            ...
            log_message "Chromium install failed - Kaleido static chart rendering will not work" "$YELLOW"
    ...
    centos | fedora | rhel | amzn)
        ...
        if command -v dnf >/dev/null 2>&1; then
            if sudo dnf install -y chromium liberation-fonts 2>/dev/null; then
                ...
                log_message "Chromium not available via dnf - Kaleido static chart rendering will not work" "$YELLOW"
            ...
            if sudo yum install -y chromium liberation-fonts 2>/dev/null; then
                ...
                log_message "Chromium not available via yum - Kaleido static chart rendering will not work" "$YELLOW"
...
# Check and handle existing Max Algos installation
handle_existing "$BASE_PATH" "installation directory" "Max Algos directory for $DEPLOY_NAME"
...
sudo $UV_CMD pip install --python $VENV_PATH/bin/python -r $MAX_ALGOS_PATH/requirements-nginx.txt
...
handle_existing "$MAX_ALGOS_PATH/.env" "environment file" ".env file"

sudo cp $MAX_ALGOS_PATH/.sample.env $MAX_ALGOS_PATH/.env
sudo sed -i "s|YOUR_BROKER_API_KEY|$BROKER_API_KEY|g" $MAX_ALGOS_PATH/.env
sudo sed -i "s|YOUR_BROKER_API_SECRET|$BROKER_API_SECRET|g" $MAX_ALGOS_PATH/.env

# Update market data API credentials if the broker is XTS-based
if is_xts_broker "$BROKER_NAME"; then
    sudo sed -i "s|YOUR_BROKER_MARKET_API_KEY|$BROKER_API_KEY_MARKET|g" $MAX_ALGOS_PATH/.env
    sudo sed -i "s|YOUR_BROKER_MARKET_API_SECRET|$BROKER_API_SECRET_MARKET|g" $MAX_ALGOS_PATH/.env
fi

sudo sed -i "s|http://127.0.0.1:5000|https://$DOMAIN|g" $MAX_ALGOS_PATH/.env
# Explicitly set HOST_SERVER in case the default value didn't match
sudo sed -i "s|HOST_SERVER = '.*'|HOST_SERVER = 'https://$DOMAIN'|g" $MAX_ALGOS_PATH/.env
sudo sed -i "s|<broker>|$BROKER_NAME|g" $MAX_ALGOS_PATH/.env
sudo sed -i "s|MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$APP_KEY|g" $MAX_ALGOS_PATH/.env
sudo sed -i "s|MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$API_KEY_PEPPER|g" $MAX_ALGOS_PATH/.env
...
if [ "$DISABLE_SESSION_EXPIRY" = "true" ]; then
    sudo sed -i "s|DISABLE_SESSION_EXPIRY = 'false'|DISABLE_SESSION_EXPIRY = 'true'|g" $MAX_ALGOS_PATH/.env
    log_message "Session auto-logout disabled for crypto broker" "$GREEN"
fi

# Update WebSocket URL for production
sudo sed -i "s|WEBSOCKET_URL='.*'|WEBSOCKET_URL='wss://$DOMAIN/ws'|g" $MAX_ALGOS_PATH/.env
...
if [ "$ENABLE_REMOTE_MCP" = "true" ]; then
    sudo sed -i "s|MCP_HTTP_ENABLED = 'False'|MCP_HTTP_ENABLED = 'True'|g" $MAX_ALGOS_PATH/.env
    sudo sed -i "s|MCP_PUBLIC_URL = ''|MCP_PUBLIC_URL = 'https://$DOMAIN'|g" $MAX_ALGOS_PATH/.env
...
# Remove the existing maxalgos nginx config and any legacy domain-keyed
# files left over from older installs (pre-simple-paths) so the rewrite
# below leaves exactly one maxalgos vhost on disk.
sudo rm -f $NGINX_CONFIG_FILE
sudo rm -f ${NGINX_AVAILABLE}/${DOMAIN} ${NGINX_AVAILABLE}/${DOMAIN}.conf
if [ "$NGINX_CONFIG_MODE" = "sites" ]; then
    sudo rm -f /etc/nginx/sites-enabled/maxalgos.conf
...
server {
    ...
    # Security headers - only Strict-Transport-Security here. X-Frame-Options,
    # X-Content-Type-Options, X-XSS-Protection, Referrer-Policy, and
    # Permissions-Policy are already set by Flask's own CSP middleware
    # (csp.py) on every response. Duplicating them here produces two
    # (sometimes conflicting) values for the same header - e.g.
    # X-Frame-Options: DENY from nginx AND X-Frame-Options: SAMEORIGIN from
...
handle_existing "/etc/systemd/system/$SERVICE_NAME.service" "systemd service" "Max Algos service file"

# Create systemd service with unique name
log_message "\nCreating systemd service..." "$BLUE"
sudo tee /etc/systemd/system/$SERVICE_NAME.service > /dev/null << EOL
[Unit]
Description=Max Algos Gunicorn Daemon ($DEPLOY_NAME)
After=network.target

[Service]
User=$WEB_USER
Group=$WEB_GROUP
WorkingDirectory=$MAX_ALGOS_PATH
# Set HOME so Kaleido/choreographer can write temp files for static chart rendering.
...
Environment="HOME=$MAX_ALGOS_PATH/tmp"
# Environment variables for numba/scipy support
Environment="TMPDIR=$MAX_ALGOS_PATH/tmp"
Environment="NUMBA_CACHE_DIR=$MAX_ALGOS_PATH/tmp/numba_cache"
...
sudo mkdir -p $MAX_ALGOS_PATH/db
sudo mkdir -p $MAX_ALGOS_PATH/tmp/numba_cache
sudo mkdir -p $MAX_ALGOS_PATH/tmp/matplotlib
# Create directories for Python strategy feature
sudo mkdir -p $MAX_ALGOS_PATH/strategies/scripts
sudo mkdir -p $MAX_ALGOS_PATH/strategies/examples
sudo mkdir -p $MAX_ALGOS_PATH/log/strategies
sudo mkdir -p $MAX_ALGOS_PATH/keys
# Set ownership and permissions
sudo chown -R $WEB_USER:$WEB_GROUP $MAX_ALGOS_PATH
sudo chmod -R 755 $MAX_ALGOS_PATH
# Set more restrictive permissions for sensitive directories
sudo chmod 700 $MAX_ALGOS_PATH/keys
...
sudo chmod 600 $MAX_ALGOS_PATH/.env
...
log_message "Installation Directory: $MAX_ALGOS_PATH" "$BLUE"
log_message "Environment File: $MAX_ALGOS_PATH/.env" "$BLUE"
...
log_message "\nNext Steps:" "$YELLOW"
log_message "1. Visit https://$DOMAIN to access your Max Algos instance" "$GREEN"
...
log_message "Restart Max Algos: sudo systemctl restart $SERVICE_NAME" "$BLUE"
