...
```bash
# Windows
docker-run.bat start     # Start Max Algos
docker-run.bat stop      # Stop Max Algos
...
This script provides a simplified, automated installation of Max Algos using Docker on Ubuntu/Debian systems with custom domain and SSL.
...
- Domain name (e.g., demo.maxalgos.in)
...
5. ✅ Clones Max Algos repository to `/opt/maxalgos`
...
| Strategies | Docker volume `maxalgos_strategies` |
| Application Logs | `/opt/maxalgos/log` |
...
sudo docker inspect maxalgos-web --format='{{.State.Health.Status}}'
```
...
sudo netstat -tlnp | grep -E ':(5000|8785)'
...
- **Port 443** (HTTPS) - Open
- **Ports 5000, 8785** - Only accessible via localhost (Docker ports)
...
4. **Setup automated backups**: Create a cron job
   ...
   crontab -e
   # Add: 0 2 * * * /usr/local/bin/maxalgos-backup
   ```
5. **Use strong passwords** for your Max Algos account
...
maxalgos-backup
```
Backups are stored in `/opt/maxalgos-backups/` and include:
- Database
...
sudo tar -xzf /opt/maxalgos-backups/maxalgos_backup_TIMESTAMP.tar.gz -C /opt/maxalgos
...
sudo rm -rf /opt/maxalgos

# Remove backups (optional)
sudo rm -rf /opt/maxalgos-backups
...
sudo rm /usr/local/bin/maxalgos-*
...
│ :5000 │ │  :8785   │   (maxalgos-web)
└───────┘ └──────────┘
    │
    ▼
┌──────────┐
│ SQLite   │ ← Docker Volume
│ Database │   (maxalgos_db)
└──────────┘
```
...
**Q: Can I run multiple Max Algos instances?**
A: Not with this script. Each installation assumes it's the only instance.

**Q: How do I change my broker after installation?**
A: Edit `/opt/maxalgos/.env`, update broker credentials, then run `sudo docker compose restart`.
...
Max Algos is released under the **AGPL V3.0 License**.
...
**Note**: This script is designed for fresh server installations. If you have an existing Max Algos installation or other applications on the server, please review the script and make necessary adjustments to avoid conflicts.
