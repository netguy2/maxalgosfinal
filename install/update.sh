...
# Max Algos Update Script
# Updates an existing Max Algos installation to the latest version using the UV method.
...
# Start logging
log_message "Starting Max Algos update log at: $LOG_FILE" "$BLUE"
log_message "----------------------------------------" "$BLUE"
...
#   1. Simple (current install.sh)   /var/python/maxalgos, service "maxalgos"
#   2. Legacy multi-deploy           /var/python/maxalgos-flask/<deploy>/maxalgos,
#                                    service "maxalgos-<deploy>" (still produced
...
SIMPLE_PATH="/var/python/maxalgos"
DEPLOY_BASE="/var/python/maxalgos-flask"
SERVER_MODE=false
STASHED=false
...
if [ -d "$SIMPLE_PATH/.git" ] && [ -f "$SIMPLE_PATH/.env" ]; then
    SERVER_MODE=true
    SELECTED_DEPLOY="maxalgos"
    BASE_PATH="$SIMPLE_PATH"
    MAX_ALGOS_PATH="$SIMPLE_PATH"
    VENV_PATH="$SIMPLE_PATH/.venv"
    SERVICE_NAME="maxalgos"

    log_message "Found Max Algos install at $SIMPLE_PATH" "$GREEN"
...
if [ "$SERVER_MODE" = false ] && [ ${#DEPLOYMENTS[@]} -gt 0 ]; then
    ...
    MAX_ALGOS_PATH="$BASE_PATH/maxalgos"
    VENV_PATH="$BASE_PATH/venv"
    SERVICE_NAME="maxalgos-$SELECTED_DEPLOY"

    log_message "\nUpdating deployment: $SELECTED_DEPLOY" "$BLUE"
    log_message "Path: $MAX_ALGOS_PATH" "$BLUE"
    log_message "Service: $SERVICE_NAME" "$BLUE"
fi

if [ "$SERVER_MODE" = false ]; then
    # Check if we're in or near an maxalgos git repo (local development)
    if [ -d ".git" ] && [ -f "app.py" ]; then
        MAX_ALGOS_PATH="$(pwd)"
    elif [ -d "$SCRIPT_DIR/../.git" ] && [ -f "$SCRIPT_DIR/../app.py" ]; then
        MAX_ALGOS_PATH="$(cd "$SCRIPT_DIR/.." && pwd)"
    else
        log_message "Error: No Max Algos deployment found." "$RED"
        log_message "For server deployments, ensure install.sh was run first." "$YELLOW"
        log_message "For local development, run this script from the maxalgos directory." "$YELLOW"
        exit 1
    fi

    log_message "Detected local development setup at: $MAX_ALGOS_PATH" "$GREEN"
fi
...
if [ "$SERVER_MODE" = true ]; then
    CURRENT_COMMIT=$(sudo git -C "$MAX_ALGOS_PATH" rev-parse --short HEAD 2>/dev/null || echo "unknown")
    CURRENT_BRANCH=$(sudo git -C "$MAX_ALGOS_PATH" branch --show-current 2>/dev/null || echo "main")
else
...
BACKUP_DIR="$MAX_ALGOS_PATH/db/backup_${TIMESTAMP}"
BACKUP_COUNT=0

if [ -d "$MAX_ALGOS_PATH/db" ]; then
    if [ "$SERVER_MODE" = true ]; then
    ...
    for db_file in maxalgos.db logs.db latency.db sandbox.db; do
        if [ -f "$MAX_ALGOS_PATH/db/$db_file" ]; then
            if [ "$SERVER_MODE" = true ]; then
                sudo cp "$MAX_ALGOS_PATH/db/$db_file" "$BACKUP_DIR/$db_file"
            else
                cp "$MAX_ALGOS_PATH/db/$db_file" "$BACKUP_DIR/$db_file"
            fi
    ...
    if [ -f "$MAX_ALGOS_PATH/db/historify.duckdb" ]; then
        if [ "$SERVER_MODE" = true ]; then
            sudo cp "$MAX_ALGOS_PATH/db/historify.duckdb" "$BACKUP_DIR/historify.duckdb"
        else
            cp "$MAX_ALGOS_PATH/db/historify.duckdb" "$BACKUP_DIR/historify.duckdb"
        fi
...
if [ "$SERVER_MODE" = true ]; then
    LOCAL_CHANGES=$(sudo git -C "$MAX_ALGOS_PATH" status --porcelain 2>/dev/null | grep -v "^??" | head -20)
else
...
if [ -n "$LOCAL_CHANGES" ]; then
    ...
    if [ "$SERVER_MODE" = true ]; then
        sudo git -C "$MAX_ALGOS_PATH" stash push -m "auto-stash before update $TIMESTAMP"
    else
...
# Pull latest code
if [ "$SERVER_MODE" = true ]; then
    sudo git -C "$MAX_ALGOS_PATH" pull origin "$CURRENT_BRANCH"
else
...
# Get new commit hash
if [ "$SERVER_MODE" = true ]; then
    NEW_COMMIT=$(sudo git -C "$MAX_ALGOS_PATH" rev-parse --short HEAD 2>/dev/null || echo "unknown")
else
...
if [ -f "$MAX_ALGOS_PATH/.env" ] && [ -f "$MAX_ALGOS_PATH/.sample.env" ]; then
    # Extract variable names from both files and compare
    SAMPLE_VARS=$(grep -oP "^[A-Z_][A-Z_0-9]+ *=" "$MAX_ALGOS_PATH/.sample.env" 2>/dev/null | sed 's/ *=$//' | sort -u)
    CURRENT_VARS=$(grep -oP "^[A-Z_][A-Z_0-9]+ *=" "$MAX_ALGOS_PATH/.env" 2>/dev/null | sed 's/ *=$//' | sort -u)
...
elif [ ! -f "$MAX_ALGOS_PATH/.env" ]; then
    ...
    if [ "$SERVER_MODE" = true ]; then
        ...
        cp "$MAX_ALGOS_PATH/.sample.env" "$MAX_ALGOS_PATH/.env"
    fi
    ...
    if [ "$SERVER_MODE" = true ]; then
        sudo sed -i "s|MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$NEW_APP_KEY|g" "$MAX_ALGOS_PATH/.env"
        sudo sed -i "s|MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$NEW_PEPPER|g" "$MAX_ALGOS_PATH/.env"
        sudo chmod 600 "$MAX_ALGOS_PATH/.env"
    else
        sed -i.bak "s|MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$NEW_APP_KEY|g" "$MAX_ALGOS_PATH/.env" && rm -f "$MAX_ALGOS_PATH/.env.bak"
        sed -i.bak "s|MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$NEW_PEPPER|g" "$MAX_ALGOS_PATH/.env" && rm -f "$MAX_ALGOS_PATH/.env.bak"
        chmod 600 "$MAX_ALGOS_PATH/.env"
    fi
    log_message "Generated fresh APP_KEY and API_KEY_PEPPER in $MAX_ALGOS_PATH/.env" "$GREEN"
    log_message "Please edit $MAX_ALGOS_PATH/.env with your broker credentials and settings." "$RED"
fi
...
if [ -f "$MAX_ALGOS_PATH/.env" ]; then
    ...
    if [ "$SERVER_MODE" = true ]; then
        ENV_PERMS=$(stat -c '%a' "$MAX_ALGOS_PATH/.env" 2>/dev/null || stat -f '%Lp' "$MAX_ALGOS_PATH/.env" 2>/dev/null)
    ...
    if ! grep -q "^TRUST_PROXY_HEADERS" "$MAX_ALGOS_PATH/.env"; then
        ...
        if [ -d /etc/nginx/sites-enabled ]; then
            if find /etc/nginx/sites-enabled -type f -o -type l 2>/dev/null | xargs grep -l "unix:.*\.sock\|maxalgos\|gunicorn" 2>/dev/null | head -1 | grep -q .; then
                BEHIND_NGINX="true"
            fi
        fi
        if [ "$BEHIND_NGINX" = "false" ] && [ -d /etc/nginx/conf.d ]; then
            if find /etc/nginx/conf.d -type f -name "*.conf" 2>/dev/null | xargs grep -l "unix:.*\.sock\|maxalgos\|gunicorn" 2>/dev/null | head -1 | grep -q .; then
                BEHIND_NGINX="true"
            fi
        fi
...
if [ "$SERVER_MODE" = true ]; then
    ...
    if [ -f "$MAX_ALGOS_PATH/upgrade/migrate_all.py" ]; then
        log_message "Running database migrations..." "$BLUE"
        sudo -u "$WEB_USER" bash -c "source $VENV_PATH/bin/activate && cd $MAX_ALGOS_PATH && python upgrade/migrate_all.py" 2>&1 | tee -a "$LOG_FILE"
        if [ ${PIPESTATUS[0]} -ne 0 ]; then
            log_message "Retrying migrations with elevated permissions..." "$YELLOW"
            sudo bash -c "source $VENV_PATH/bin/activate && cd $MAX_ALGOS_PATH && python upgrade/migrate_all.py" 2>&1 | tee -a "$LOG_FILE"
        fi
...
if [ "$SERVER_MODE" = true ]; then
    ...
    if [ ! -d "$MAX_ALGOS_PATH/frontend/dist" ]; then
        if command -v npm >/dev/null 2>&1; then
            log_message "Building React frontend (dist/ not found)..." "$BLUE"
            cd "$MAX_ALGOS_PATH/frontend"
...
# ============================================
log_message "\n========================================" "$GREEN"
log_message "  Max Algos Update Summary" "$GREEN"
log_message "========================================" "$GREEN"
