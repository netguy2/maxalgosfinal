...
can connect to your Max Algos install over HTTPS using OAuth 2.1 with
...
| `install/install-multi.sh` (native Ubuntu, multiple domains) | `sudo ./install/enable-remote-mcp.sh` — the helper detects all `maxalgos-*` services and asks which one |
...
endpoints live under your existing Max Algos dashboard hostname, e.g.
...
# your dashboard is reachable, run this from the maxalgos project
# root:
...
1. Detects the existing `maxalgos-*` systemd service
...
```bash
# After your container(s) are running, from the maxalgos project root:
sudo ./install/enable-remote-mcp-docker.sh
```

The script:

1. Discovers Compose stacks under `/opt/maxalgos/<domain>/`
...
Then `cd /opt/maxalgos/<domain> && docker compose restart`.
...
2. **Edit the existing nginx config** (the file written by
   ...
   server {
       ...
       location ^~ /.well-known/oauth-authorization-server {
           proxy_pass http://unix:/var/run/maxalgos/<DEPLOY_NAME>.sock;
...
2. The client probes the discovery endpoint, registers itself via DCR,
   and redirects you to Max Algos for OAuth approval.
...
3. **First-time approval gate** — because `MCP_OAUTH_REQUIRE_APPROVAL=True`,
   ...
   - Sign in to Max Algos at `https://yourdomain.com`
...
4. Sign in to your Max Algos dashboard if prompted, review the scopes,
   and click **Authorize**.
...
| OAuth client list | `sqlite3 db/maxalgos.db 'SELECT * FROM oauth_clients'` |
| Active refresh tokens | `sqlite3 db/maxalgos.db 'SELECT id, client_id, family_id, revoked_at FROM oauth_refresh_tokens'` |
| Kill switch (revoke everything) | `sqlite3 db/maxalgos.db "UPDATE oauth_refresh_tokens SET revoked_at=CURRENT_TIMESTAMP WHERE revoked_at IS NULL"` |
...
sudo systemctl restart maxalgos-<deploy-name>
```
