@echo off
REM ============================================================================
REM Max Algos Docker Runner for Windows
REM ============================================================================
REM
...
REM
REM Commands:
REM   start    - Start Max Algos container (default, runs setup if needed)
...
if "%MAX_ALGOS_DIR:~-1%"=="\" set MAX_ALGOS_DIR=%MAX_ALGOS_DIR:~0,-1%
set SETUP_FAILED=0
...
set VALID_BROKERS=fivepaisa,fivepaisaxts,aliceblue,angel,arrow,compositedge,definedge,deltaexchange,dhan,dhan_sandbox,firstock,flattrade,fyers,groww,ibulls,iifl,iiflcapital,indmoney,jainamxts,kotak,motilal,mstock,nubra,paytm,pocketful,rmoney,samco,shoonya,tradejini,upstox,wisdom,zebu,zerodha,bnr

REM Banner
echo.
echo   ========================================
echo        Max Algos Docker Runner
...
:setup
echo [INFO] Setting up Max Algos in %MAX_ALGOS_DIR%...
echo.

REM Create db directory
if not exist "%MAX_ALGOS_DIR%\db\" (
    echo [INFO] Creating database directory...
    md "%MAX_ALGOS_DIR%\db" 2>nul
    if errorlevel 1 (
...
if exist "%MAX_ALGOS_DIR%\%ENV_FILE%" (
    echo [WARNING] .env file already exists at %MAX_ALGOS_DIR%\%ENV_FILE%
...
if errorlevel 1 (
    ...
    powershell -Command "Invoke-WebRequest -Uri '%SAMPLE_ENV_URL%' -OutFile '%MAX_ALGOS_DIR%\%ENV_FILE%'" 2>nul
) else (
    curl.exe -sL "%SAMPLE_ENV_URL%" -o "%MAX_ALGOS_DIR%\%ENV_FILE%" 2>nul
)

REM Check if download succeeded
if not exist "%MAX_ALGOS_DIR%\%ENV_FILE%" (
...
powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\%ENV_FILE%') -replace 'MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE', '%APP_KEY%' | Set-Content '%MAX_ALGOS_DIR%\%ENV_FILE%'"
powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\%ENV_FILE%') -replace 'MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE', '%API_KEY_PEPPER%' | Set-Content '%MAX_ALGOS_DIR%\%ENV_FILE%'"
...
powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\%ENV_FILE%') -replace 'BROKER_API_KEY = ''YOUR_BROKER_API_KEY''', 'BROKER_API_KEY = ''%BROKER_API_KEY%''' | Set-Content '%MAX_ALGOS_DIR%\%ENV_FILE%'"
powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\%ENV_FILE%') -replace 'BROKER_API_SECRET = ''YOUR_BROKER_API_SECRET''', 'BROKER_API_SECRET = ''%BROKER_API_SECRET%''' | Set-Content '%MAX_ALGOS_DIR%\%ENV_FILE%'"

REM Update redirect URL with broker name (replace <broker> placeholder)
powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\%ENV_FILE%') -replace '<broker>', '%BROKER_NAME%' | Set-Content '%MAX_ALGOS_DIR%\%ENV_FILE%'"

REM Update XTS market data credentials if applicable
if "%IS_XTS%"=="1" (
    powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\%ENV_FILE%') -replace 'BROKER_API_KEY_MARKET = ''YOUR_BROKER_MARKET_API_KEY''', 'BROKER_API_KEY_MARKET = ''!BROKER_API_KEY_MARKET!''' | Set-Content '%MAX_ALGOS_DIR%\%ENV_FILE%'"
    powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\%ENV_FILE%') -replace 'BROKER_API_SECRET_MARKET = ''YOUR_BROKER_MARKET_API_SECRET''', 'BROKER_API_SECRET_MARKET = ''!BROKER_API_SECRET_MARKET!''' | Set-Content '%MAX_ALGOS_DIR%\%ENV_FILE%'"
)
...
echo   Data directory: %MAX_ALGOS_DIR%
echo   Config file:    %MAX_ALGOS_DIR%\%ENV_FILE%
echo   Database:       %MAX_ALGOS_DIR%\db\
echo   Strategies:     %MAX_ALGOS_DIR%\strategies\
echo   Logs:           %MAX_ALGOS_DIR%\log\
echo.
echo   Redirect URL for broker portal:
echo   http://127.0.0.1:5000/%BROKER_NAME%/callback
echo.
echo   Documentation: https://docs.maxalgos.in
echo.
set /p OPEN_ENV="Open .env in Notepad for review? (y/n): "
if /i "%OPEN_ENV%"=="y" (
    start notepad "%MAX_ALGOS_DIR%\%ENV_FILE%"
)
echo.
echo [OK] Setup complete! Run 'docker-run.bat start' to launch Max Algos.

:setup_end
exit /b %SETUP_FAILED%

:start
echo [INFO] Starting Max Algos...
echo.
...
if not exist "%MAX_ALGOS_DIR%\%ENV_FILE%" (
    ...
    if errorlevel 1 (
        echo.
        echo [ERROR] Setup failed. Cannot start Max Algos.
        echo Please fix the issues above and try again.
        goto end
    )
    echo.
    echo [INFO] Starting Max Algos after setup...
    echo.
)
...
if not exist "%MAX_ALGOS_DIR%\strategies\" (
    echo [INFO] Creating strategies directory...
    md "%MAX_ALGOS_DIR%\strategies" 2>nul
    md "%MAX_ALGOS_DIR%\strategies\scripts" 2>nul
    md "%MAX_ALGOS_DIR%\strategies\examples" 2>nul
)
if not exist "%MAX_ALGOS_DIR%\log\" (
    echo [INFO] Creating log directory...
    md "%MAX_ALGOS_DIR%\log" 2>nul
    md "%MAX_ALGOS_DIR%\log\strategies" 2>nul
)
if not exist "%MAX_ALGOS_DIR%\keys\" (
    echo [INFO] Creating keys directory...
    md "%MAX_ALGOS_DIR%\keys" 2>nul
)
if not exist "%MAX_ALGOS_DIR%\tmp\" (
    echo [INFO] Creating temp directory...
    md "%MAX_ALGOS_DIR%\tmp" 2>nul
)
...
docker run -d ^
    ...
    -e "TZ=Asia/Kolkata" ^
    -v "%MAX_ALGOS_DIR%\db:/app/db" ^
    -v "%MAX_ALGOS_DIR%\strategies:/app/strategies" ^
    -v "%MAX_ALGOS_DIR%\log:/app/log" ^
    -v "%MAX_ALGOS_DIR%\keys:/app/keys" ^
    -v "%MAX_ALGOS_DIR%\tmp:/app/tmp" ^
    -v "%MAX_ALGOS_DIR%\.env:/app/.env" ^
...
if errorlevel 1 (
    ...
    echo   1. Check if ports 5000 and 8785 are available
    echo   2. Ensure Docker Desktop is running
    echo   3. Check .env file: %MAX_ALGOS_DIR%\%ENV_FILE%
    echo.
    goto end
)

echo.
echo [SUCCESS] Max Algos started successfully!
echo.
echo   ========================================
echo   Web UI:     http://127.0.0.1:5000
echo   WebSocket:  ws://127.0.0.1:8785
echo   ========================================
echo.
echo   Data directory: %MAX_ALGOS_DIR%
echo.
...
echo     docker-run.bat stop     - Stop Max Algos
echo     docker-run.bat restart  - Restart Max Algos
echo.
goto end

:stop
echo [INFO] Stopping Max Algos...
docker stop %CONTAINER% >nul 2>&1
docker rm %CONTAINER% >nul 2>&1
echo [OK] Max Algos stopped.
goto end

:restart
echo [INFO] Restarting Max Algos...
docker stop %CONTAINER% >nul 2>&1
docker rm %CONTAINER% >nul 2>&1
echo [OK] Max Algos stopped.
echo.
...
if errorlevel 1 (
    echo [STATUS] Max Algos is NOT running.
) else (
    echo [STATUS] Max Algos is running.
    echo.
    echo   Web UI: http://127.0.0.1:5000
)
echo.
...
echo.
echo Commands:
echo   start    Start Max Algos (runs setup if needed, default)
