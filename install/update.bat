@echo off
REM ============================================================================
REM Max Algos Update Script for Windows
REM ============================================================================
REM
REM Usage: update.bat
REM
REM This script updates Max Algos to the latest version using the UV method.
REM Run from the install\ directory or the maxalgos project root.
REM
REM Prerequisites:
REM   - Python 3.12+
...
if exist "%SCRIPT_DIR%\app.py" (
    ...
    if exist "app.py" (
        ...
        echo [ERROR] Could not find Max Algos directory.
        echo.
        echo Please run this script from:
        echo   - The maxalgos project root directory, OR
        echo   - The install\ directory within maxalgos
        echo.
        pause
        exit /b 1
    )
...
echo [INFO] Max Algos directory: %MAX_ALGOS_DIR%
echo.

REM Verify git repository
if not exist "%MAX_ALGOS_DIR%\.git" (
    echo [ERROR] Not a git repository: %MAX_ALGOS_DIR%
    echo Please ensure Max Algos was installed via git clone.
    echo.
    pause
    exit /b 1
)
...
set "BACKUP_DIR=%MAX_ALGOS_DIR%\db\backup_%TIMESTAMP%"
set "BACKUP_COUNT=0"

if exist "%MAX_ALGOS_DIR%\db\" (
    md "%BACKUP_DIR%" 2>nul

    for %%f in (maxalgos.db logs.db latency.db sandbox.db) do (
        if exist "%MAX_ALGOS_DIR%\db\%%f" (
            copy /y "%MAX_ALGOS_DIR%\db\%%f" "%BACKUP_DIR%\%%f" >nul 2>&1
            echo   Backed up: %%f
            set /a BACKUP_COUNT+=1
        )
    )

    if exist "%MAX_ALGOS_DIR%\db\historify.duckdb" (
        copy /y "%MAX_ALGOS_DIR%\db\historify.duckdb" "%BACKUP_DIR%\historify.duckdb" >nul 2>&1
...
if not exist "%MAX_ALGOS_DIR%\.env" (
    echo   [WARNING] No .env file found. Creating from .sample.env...
    if exist "%MAX_ALGOS_DIR%\.sample.env" (
        copy /y "%MAX_ALGOS_DIR%\.sample.env" "%MAX_ALGOS_DIR%\.env" >nul
        ...
        powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\.env') -replace 'MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE', '!NEW_APP_KEY!' | Set-Content '%MAX_ALGOS_DIR%\.env'"
        powershell -Command "(Get-Content '%MAX_ALGOS_DIR%\.env') -replace 'MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE', '!NEW_PEPPER!' | Set-Content '%MAX_ALGOS_DIR%\.env'"
...
if exist "%MAX_ALGOS_DIR%\upgrade\migrate_all.py" (
...
REM ========================================
if not exist "%MAX_ALGOS_DIR%\frontend\dist\" (
    where npm >nul 2>&1
    if not errorlevel 1 (
        echo [OPTIONAL] Building React frontend (dist\ not found)...
        pushd "%MAX_ALGOS_DIR%\frontend"
        call npm ci
        call npm run build
        if errorlevel 1 (
...
echo.
echo   ========================================
echo   Max Algos Update Complete!
echo   ========================================
echo.
echo   Version:   %CURRENT_COMMIT% -^> %NEW_COMMIT%
echo   Branch:    %CURRENT_BRANCH%
echo   Directory: %MAX_ALGOS_DIR%
if exist "%BACKUP_DIR%\" (
