...
# Max Algos Docker Installation Script
...
# Start installation
log "Starting Max Algos Docker Installation..." "$GREEN"
log "========================================" "$GREEN"
...
# Get domain name
while true; do
    read -p "Enter your domain name (e.g., demo.maxalgos.in): " DOMAIN
    if [ -z "$DOMAIN" ]; then
...
while true; do
    ...
    echo "rmoney, samco, shoonya, tradejini, upstox, wisdom, zebu, zerodha, bnr"
    echo ""
...
$SUDO sed -i "s|MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$APP_KEY|g" .env
$SUDO sed -i "s|MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$API_KEY_PEPPER|g" .env
...
if ! grep -qE "^MAX_ALGOS_GIT_BRANCH\s*=" .env 2>/dev/null; then
    echo "MAX_ALGOS_GIT_BRANCH = '${GIT_BRANCH}'" | $SUDO tee -a .env > /dev/null
fi
if ! grep -qE "^MAX_ALGOS_GIT_COMMIT\s*=" .env 2>/dev/null; then
    echo "MAX_ALGOS_GIT_COMMIT = '${GIT_COMMIT}'" | $SUDO tee -a .env > /dev/null
fi
...
upstream maxalgos_flask {
    server 127.0.0.1:5000;
    keepalive 64;
}

upstream maxalgos_websocket {
...
server {
    ...
    # X-Frame-Options: SAMEORIGIN from nginx AND X-Frame-Options: DENY from
    ...
    # WebSocket Proxy Server (Port 8785)
    location = /ws {
        proxy_pass http://maxalgos_websocket;
    ...
    # Socket.IO WebSocket
    location /socket.io/ {
        proxy_pass http://maxalgos_flask/socket.io/;
        proxy_http_version 1.1;
        proxy_read_timeout 86400s;
        proxy_send_timeout 86400s;
        proxy_connect_timeout 60s;
    ...
    location /api/ {
        ...
        proxy_pass http://maxalgos_flask;
        proxy_http_version 1.1;
...
CONTAINER_STATUS=$(sudo docker ps --filter "name=maxalgos-web" --format "{{.Status}}")
...
# Status script
$SUDO tee /usr/local/bin/maxalgos-status > /dev/null << 'EOFSCRIPT'
#!/bin/bash
echo "=========================================="
echo "Max Algos Status"
echo "=========================================="
echo ""
echo "Container Status:"
sudo docker ps --filter "name=maxalgos-web" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
echo ""
echo "Container Health:"
sudo docker inspect maxalgos-web --format='{{.State.Health.Status}}' 2>/dev/null || echo "Container not found"
echo ""
echo "Recent Logs:"
sudo docker compose -f /opt/maxalgos/docker-compose.yaml logs --tail=30
EOFSCRIPT

$SUDO chmod +x /usr/local/bin/maxalgos-status

# Restart script
$SUDO tee /usr/local/bin/maxalgos-restart > /dev/null << 'EOFSCRIPT'
#!/bin/bash
echo "Restarting Max Algos..."
cd /opt/maxalgos
sudo docker compose restart
sleep 10
echo "Container Status:"
sudo docker ps --filter "name=maxalgos-web"
EOFSCRIPT

$SUDO chmod +x /usr/local/bin/maxalgos-restart

# Logs script
$SUDO tee /usr/local/bin/maxalgos-logs > /dev/null << 'EOFSCRIPT'
#!/bin/bash
cd /opt/maxalgos
sudo docker compose logs -f --tail=100
EOFSCRIPT

$SUDO chmod +x /usr/local/bin/maxalgos-logs

# Backup script
$SUDO tee /usr/local/bin/maxalgos-backup > /dev/null << 'EOFSCRIPT'
#!/bin/bash
BACKUP_DIR="/opt/maxalgos-backups"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="$BACKUP_DIR/maxalgos_backup_$TIMESTAMP.tar.gz"
mkdir -p $BACKUP_DIR
...
sudo docker run --rm -v maxalgos_db:/data -v $TEMP_DIR:/backup alpine tar -czf /backup/db.tar.gz -C /data . 2>/dev/null
sudo docker run --rm -v maxalgos_strategies:/data -v $TEMP_DIR:/backup alpine tar -czf /backup/strategies.tar.gz -C /data . 2>/dev/null
...
cd $BACKUP_DIR
ls -t maxalgos_backup_*.tar.gz 2>/dev/null | tail -n +8 | xargs -r rm
echo "Backup completed!"
EOFSCRIPT

$SUDO chmod +x /usr/local/bin/maxalgos-backup
...
log "\n============================================" "$GREEN"
log "Max Algos Docker Installation Complete!" "$GREEN"
log "============================================" "$GREEN"
...
log "Broker: $BROKER_NAME" "$BLUE"
log "Installation Path: $INSTALL_PATH" "$BLUE"
log "Container: maxalgos-web" "$BLUE"
if [ "$ENABLE_REMOTE_MCP" = "true" ]; then
...
log "\nNext Steps:" "$YELLOW"
log "1. Visit https://$DOMAIN to access Max Algos" "$GREEN"
log "2. Create your admin account and login" "$GREEN"
log "3. Configure your broker settings" "$GREEN"

log "\nUseful Commands:" "$YELLOW"
log "View status:  maxalgos-status" "$BLUE"
log "View logs:    maxalgos-logs" "$BLUE"
log "Restart:      maxalgos-restart" "$BLUE"
log "Backup:       maxalgos-backup" "$BLUE"
