...
# Max Algos Docker Multi-Instance Installation with Custom SSL
...
get_next_ports() {
    ...
    if [ -d "$INSTALL_BASE" ]; then
        ...
        while IFS= read -r file; do
            ...
            local w_port=$(grep -A 5 "ports:" "$file" | grep ":8785" | cut -d: -f2)
...
for i in "${!CONF_DOMAINS[@]}"; do
    ...
    PROJECT_NAME="maxalgos-${SANITIZED_NAME}"
    ...
    if [ "$IS_UPDATE_MODE" == "true" ] && [ -f "$ENV_FILE" ]; then
        ...
        sed -i "s|MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$APP_KEY|g" "$ENV_FILE"
        sed -i "s|MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE|$PEPPER|g" "$ENV_FILE"
...
upstream maxalgos_flask_${SANITIZED_NAME} {
    server 127.0.0.1:${FLASK_PORT};
    keepalive 64;
}

upstream maxalgos_websocket_${SANITIZED_NAME} {
...
server {
    ...
    # and X-Content-Type-Options are already set by Flask's own CSP
    ...
    # Logic: WebSocket
    location = /ws {
        proxy_pass http://maxalgos_websocket_${SANITIZED_NAME};
        proxy_http_version 1.1;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host \$host;
    ...
    location /socket.io/ {
        proxy_pass http://maxalgos_flask_${SANITIZED_NAME}/socket.io/;
        proxy_http_version 1.1;
        proxy_read_timeout 86400s;
    ...
    # Logic: Main App
    location / {
        proxy_pass http://maxalgos_flask_${SANITIZED_NAME};
        proxy_http_version 1.1;
...
cat <<'EOF' > /usr/local/bin/maxalgos-ctl
#!/bin/bash
# Max Algos Manager
...
usage() {
    echo "Usage: maxalgos-ctl <command> [domain]"
    echo "Commands:"
...
chmod +x /usr/local/bin/maxalgos-ctl


log "\n==============================================" "$GREEN"
log " INSTALLATION COMPLETE" "$GREEN"
log "==============================================" "$GREEN"
log "Management Command: maxalgos-ctl" "$BLUE"
log "  maxalgos-ctl list" "$BLUE"
log "  maxalgos-ctl restart <domain.com>" "$BLUE"
log "  maxalgos-ctl logs <domain.com>" "$BLUE"
