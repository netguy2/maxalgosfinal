...
# Max Algos Multi-Instance Installation Banner
echo -e "${BLUE}"
echo "  ██████╗ ██████╗ ███████╗███╗   ██╗ █████╗ ██╗      ██████╗  ██████╗ "
echo " ██╔═══██╗██╔══██╗██╔════╝████╗  ██║██╔══██╗██║     ██╔════╝ ██╔═══██╗"
echo " ██║   ██║██████╔╝███████╗██╔██╗ ██║███████║██║     ██║  ███╗██║   ██║"
...
# Start logging
log_message "Starting Max Algos Multi-Instance installation" "$BLUE"
...
while true; do
    read -p "How many Max Algos instances do you want to set up? " INSTANCES
...
log_message "Setting up $INSTANCES Max Algos instances" "$GREEN"
...
# Non-fatal — if nothing sticks we warn; the rest of maxalgos still installs fine.
...
for ((i=1; i<=INSTANCES; i++)); do
    ...
    INSTANCE_DIR="$BASE_DIR/maxalgos$i"
    VENV_PATH="$INSTANCE_DIR/venv"
    SOCKET_FILE="$INSTANCE_DIR/maxalgos.sock"
    SERVICE_NAME="maxalgos$i"
    ...
    DB_PATH="sqlite:///db/maxalgos${i}.db"
    ...
    sudo sed -i "s|MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE|$APP_KEY|g" "$ENV_FILE"
...
server {
    ...
    # Security headers - only Strict-Transport-Security here. The rest
    # (X-Frame-Options, X-Content-Type-Options, X-XSS-Protection,
    # Referrer-Policy, Permissions-Policy) are already set by Flask's CSP
    # middleware (csp.py) on every response - duplicating them here produces
    # two conflicting values per header, which can break Chrome's cache
    # validation for static assets. See install.sh for the full explanation.
...
[Unit]
Description=Max Algos Instance $i ($DOMAIN - $BROKER)
...
for ((i=1; i<=INSTANCES; i++)); do
    ...
    log_message "  Service: maxalgos$i" "$BLUE"
    log_message "  Directory: $BASE_DIR/maxalgos$i" "$BLUE"
...
log_message "View all services: systemctl list-units 'maxalgos*'" "$BLUE"
log_message "Restart instance: sudo systemctl restart maxalgos<N>" "$BLUE"
log_message "View logs: sudo journalctl -u maxalgos<N> -f" "$BLUE"
log_message "Check status: sudo systemctl status maxalgos<N>" "$BLUE"
