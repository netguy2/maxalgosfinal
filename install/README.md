...
- Set up the Max Algos application with unique deployment name
...
- Each deployment gets a unique service name (e.g., maxalgos-yourdomain-broker)
...
1. **Check Service Status**
   ...
   sudo systemctl status maxalgos-fyers-yourdomain-fyers
   
   # Example for Zerodha deployment
   sudo systemctl status maxalgos-zerodha-yourdomain-zerodha
   ```
...
2. **Application Not Starting**
   ...
   sudo journalctl -u maxalgos-fyers-yourdomain-fyers    # View logs
   sudo systemctl restart maxalgos-fyers-yourdomain-fyers # Restart service
   
   # Example 2: Zerodha deployment on zerodha.yourdomain.com
   sudo journalctl -u maxalgos-zerodha-yourdomain-zerodha # View logs
   sudo systemctl restart maxalgos-zerodha-yourdomain-zerodha # Restart service
   ```
...
1. **SELinux blocking Nginx**
   ...
   sudo semanage fcontext -a -t httpd_sys_rw_content_t "/var/python/maxalgos-flask(/.*)?"
   sudo restorecon -Rv /var/python/maxalgos-flask
   ```
...
1. **Service Management Examples**
   ...
   # Example outputs:
   # maxalgos-fyers-yourdomain-fyers.service    loaded active running
   # maxalgos-zerodha-yourdomain-zerodha.service loaded active running
   
   # Restart specific deployment
   sudo systemctl restart maxalgos-fyers-yourdomain-fyers
...
2. **Log Management Examples**
   ...
   sudo journalctl -f -u maxalgos-fyers-yourdomain-fyers
   
   # View last 100 lines of Zerodha deployment logs
   sudo journalctl -n 100 -u maxalgos-zerodha-yourdomain-zerodha
   
   # View logs since last hour for specific deployment
   sudo journalctl --since "1 hour ago" -u maxalgos-fyers-yourdomain-fyers
   ```
...
4. **Installation Directory Examples**
   ...
   ls -l /var/python/maxalgos-flask/
   
   # Example structure:
   # /var/python/maxalgos-flask/fyers-yourdomain-fyers/
   # /var/python/maxalgos-flask/zerodha-yourdomain-zerodha/
   
   # Check specific deployment files
   ls -l /var/python/maxalgos-flask/fyers-yourdomain-fyers/
   ```
...
2. **SELinux (RHEL-based systems)**
   - The installation script **automatically configures SELinux** for Max Algos
