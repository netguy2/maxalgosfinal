...
class BaseBrokerWebSocketAdapter(ABC):
    ...
    def _bind_to_available_port(self):
        ...
        with self._port_lock:
            # Try configured default port first
            ...
            for attempt in range(5):
                port = find_free_zmq_port(start_port=get_zmq_port() + 1 + random.randint(0, 1000))
    ...
    def is_connected(self) -> bool:
        """
        Check if the adapter is currently connected to the broker WebSocket
        """
    ...
    def publish_market_data(self, topic, data):
        ...
        try:
            if isinstance(data, dict):
                broker = getattr(self, "broker_name", None) or getattr(self, "broker", None)
                if broker:
                    data.setdefault("broker", broker)
