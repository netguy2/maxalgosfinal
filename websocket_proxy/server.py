...
from utils.config import get_websocket_host, get_websocket_port, get_zmq_host, get_zmq_port
from utils.env_patch import install_getenv_patch
from utils.logging import get_logger, highlight_url

# Install dynamic per-user broker credential patch for standalone proxy process
...
class WebSocketProxy:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 8765):
        ...
        if is_port_in_use(host, port, wait_time=2.0):  # Wait up to 2 seconds for port release
            error_msg = (
                ...
                f"Please:\n"
                f"1. Stop any other Max Algos instances running on port {port}\n"
        ...
        self.adapter_token_hashes = {}  # Maps user_id to MD5 hash of current auth token
    ...
    async def cleanup_client(self, client_id):
            ...
            for sub_json in subscriptions:
                try:
                    ...
                    if (
                        ...
                        broker_name_for_unsub = self.user_broker_mapping.get(user_id)
                        # adapter.unsubscribe() ultimately does a raw blocking
                        # socket send() to the broker's WS (see e.g.
                        # broker/zebu/streaming/zebu_websocket.py's
                        # _send_message) -- calling it inline on this async
                        # handler would block the single shared event loop
                        # for every other connected user's whole process for
                        # however long that send takes. Same class of bug as
                        # _run_broker_call's docstring describes for
                        # initialize()/connect(); route through the same
                        # helper here.
                        await self._run_broker_call(
                            user_id, broker_name_for_unsub, adapter.unsubscribe, symbol, exchange, mode
                        )
                        logger.debug(
        ...
        if client_id in self.user_mapping:
            ...
            if is_last_client and user_id in self.broker_adapters:
                ...
                # For Flattrade, Shoonya, Zebu, and Bnr keep the connection alive and just unsubscribe
                # from data. Full reconnect pays a fresh Noren auth handshake every time, and pages
                # that poll/re-subscribe repeatedly (Option Chain, Max Pain, IV Smile) hit the brief
                # gap between the last client unsubscribing and the next one subscribing on
                # every poll cycle -- that was thrashing Zebu's (and, same Noren family, Bnr's)
                # connection into a permanent reconnect loop and starving those pages of a clean
                # run to finish rendering.
                if broker_name in ["flattrade", "shoonya", "zebu", "bnr"] and hasattr(adapter, "unsubscribe_all"):
                    logger.info(
                        f"{broker_name.title()} adapter for user {user_id}: last client disconnected. Unsubscribing all symbols instead of disconnecting."
                    )
                    # unsubscribe_all()'s direct (non-batched) send path
                    # bottoms out in a raw blocking socket .send() with no
                    # timeout (broker/zebu/streaming/zebu_websocket.py's
                    # _send_message, same pattern in the Noren-family
                    # adapters) -- if the broker's socket is slow to accept
                    # the write at that moment, this call alone can stall
                    # this single-worker eventlet process for minutes,
                    # taking down every other connected user's traffic (and
                    # the Docker healthcheck) along with it. Route through
                    # _run_broker_call so it runs on a real OS thread
                    # instead of inline on the shared event loop.
                    await self._run_broker_call(user_id, broker_name, adapter.unsubscribe_all)
                else:
                    ...
                    # adapter.disconnect() also makes real network calls
                    # (closing the broker WS handshake) -- same blocking
                    # risk as above.
                    await self._run_broker_call(user_id, broker_name, adapter.disconnect)
    ...
    async def get_user_broker_configuration(self, user_id):
        ...
        try:
            ...
            if auth_obj and not auth_obj.is_revoked and auth_obj.broker:
                ...
                # Fallback: first auto-discovered broker (see
                # utils.config.get_valid_brokers - not .env's VALID_BROKERS,
                # which goes stale on servers that never edit .env after a
                # git pull).
                from utils.config import get_valid_brokers

                valid_brokers = get_valid_brokers()
                broker_name = valid_brokers[0] if valid_brokers else "angel"
                logger.warning(
            ...
            # Validate broker is supported - auto-discovered, not .env-driven
            # (see utils.config.get_valid_brokers).
            from utils.config import get_valid_brokers

            valid_brokers_list = get_valid_brokers()
    ...
    @staticmethod
    async def _run_broker_call(user_id: str, broker_name: str, func, *args, **kwargs):
        """Run a synchronous, network-blocking broker adapter call (initialize/
        connect) off the single shared asyncio event loop this whole proxy
        process runs on.

        adapter.initialize()/adapter.connect() are plain `def` methods that make
        real HTTP/TCP calls to the broker (login, token validation, WS
        handshake) -- when awaited directly inline, they block THIS event loop
        for their full duration, which stalls every other connected user's
        authenticate/subscribe/tick fan-out for as long as this one broker call
        takes (commonly 1-3s, longer on the retry paths below). That's a
        multi-second full-process stall per first-connect/reconnect, and it
        compounds when many users reconnect around the same time (market open,
        a network blip).

        asyncio.to_thread() runs func in the default thread pool executor AND
        propagates the current contextvars context into that thread
        automatically (see utils/broker_context.py's module docstring, which
        already documents this exact contextvars.copy_context()+ctx.run()
        pattern for basket_order_service.py's ThreadPoolExecutor) -- so the
        broker_credential_context(...) `with` block set up by the caller just
        before this call remains correctly scoped to (user_id, broker_name)
        inside the worker thread, with no risk of leaking across concurrent
        users the way a plain threading.local()-based override would.
        """
        from utils.broker_context import broker_credential_context

        def _call():
            with broker_credential_context(user_id, broker_name):
                return func(*args, **kwargs)

        return await aio.to_thread(_call)
    ...
    async def authenticate_client(self, client_id, data):
        ...
        # Admission control: this whole proxy runs as one eventlet process
        # (CLAUDE.md's single-worker Gunicorn constraint -- Flask-SocketIO
        # state is in-process and can't be spread across workers), so there
        # was previously no ceiling anywhere in this file on how many
        # concurrent broker adapters it would try to hold open. Without a
        # cap, load past the process's real capacity doesn't fail loudly --
        # it degrades silently (rising tick latency, then eventually FD
        # exhaustion or event-loop starvation) with no clear signal to the
        # user or an operator about why. Reject cleanly instead, once past
        # the configured limit, for any user who doesn't already have an
        # adapter open (an existing user reconnecting a second tab, or the
        # auto-recovery path in _rebuild_adapter_and_resubscribe, isn't new
        # capacity demand and must not be blocked by this).
        if user_id not in self.broker_adapters:
            max_users = int(os.getenv("MAX_CONCURRENT_WS_USERS", "1000"))
            if len(self.broker_adapters) >= max_users:
                logger.warning(
                    f"Rejecting connection for user {user_id}: at capacity "
                    f"({len(self.broker_adapters)}/{max_users} concurrent users)"
                )
        ...
        # Calculate current token hash from database for stale-token detection
        ...
        try:
            ...
            if current_token:
                current_token_hash = hashlib.md5(current_token.encode("utf-8")).hexdigest()
        except Exception as hash_err:
            logger.debug(f"Error computing token hash for user {user_id}: {hash_err}")
        ...
        if user_id in self.broker_adapters:
            ...
            previous_token_hash = self.adapter_token_hashes.get(user_id)
            # An unknown hash on either side means we cannot prove the cached
            # adapter was built on the token currently in the DB. Rebuilding is
            # cheap; serving a silently dead feed is not - so treat
            # "unprovable" as stale rather than as "no change".
            token_changed = previous_token_hash != current_token_hash

            if broker_changed or not cached_connected or token_changed:
                reason = (
                    f"broker changed {previous_broker_name}->{broker_name}"
                    if broker_changed
                    else "cached adapter disconnected"
                    if not cached_connected
                    else f"auth token changed ({previous_token_hash} -> {current_token_hash})"
                )
                logger.info(
        ...
        if user_id not in self.broker_adapters:
            try:
                ...
                initialization_result = await self._run_broker_call(
                    user_id, broker_name, adapter.initialize, broker_name, user_id
                )
                ...
                connect_result = await self._run_broker_call(user_id, broker_name, adapter.connect)
                ...
                if is_error:
                    ...
                    if should_retry:
                        ...
                        try:
                            ...
                            init_retry_result = await self._run_broker_call(
                                user_id, broker_name, adapter.initialize, broker_name, user_id, force=True
                            )
                        except TypeError:
                ...
                # Store the adapter. Record the hash even when it is None -
                # skipping the write would leave previous_token_hash unset on
                # the next auth, permanently disabling stale-token detection.
                self.broker_adapters[user_id] = adapter
                self.adapter_token_hashes[user_id] = current_token_hash
                    ...
                    try:
                        ...
                        if adapter:
                            ...
                            if not init_is_error:
                                connect_result = await self._run_broker_call(
                                    user_id, broker_name, adapter.connect
                                )
                                # Handle both response formats
                                connect_is_error = (
                                    (connect_result and connect_result.get("status") == "error") or
            ...
            # Reusing a cached adapter that survived the staleness checks above.
            # Log it: a silently-reused dead adapter still reports auth success
            # to the client, so this is the only trace of a stalled feed.
            reused = self.broker_adapters[user_id]
            logger.info(
                f"Reusing cached {broker_name} adapter for user {user_id} "
                f"(connected={getattr(reused, 'connected', None)}, "
                f"token_hash={current_token_hash})"
            )
    ...
    async def _rebuild_adapter_and_resubscribe(self, user_id: str, broker_name: str) -> bool:
        """Silently rebuild a torn-down broker adapter and replay every
        already-connected client's stored subscriptions against it, with no
        message sent to the browser and no action required from the user.

        Why this exists: _handle_cache_invalidation tears down
        self.broker_adapters[user_id] on every login/re-login (needed so the
        NEXT broker call uses the fresh token) but previously left it torn
        down forever for any client that was already authenticated and
        streaming -- authenticate_client only rebuilds the adapter when the
        BROWSER sends a fresh "authenticate" message, which an already-open,
        already-streaming tab never does on its own. That gap is why ticks
        silently stopped updating after every broker re-login (including the
        normal ~3 AM IST daily token rollover) until the user manually
        reloaded the page. This closes that gap by treating the adapter
        teardown as a signal to immediately reconnect on the user's behalf.

        Deliberately duplicates (does not share a helper with)
        authenticate_client's adapter build/retry sequence rather than
        refactoring that method -- authenticate_client is the code path
        every user's login goes through today; this method is not, so a bug
        here cannot regress normal login. Keep the two in sync by hand if
        either's retry logic changes.

        Returns True if the adapter was rebuilt and connected, False
        otherwise (already logged; a later invalidation or the user's own
        next reload will retry).
        """
        # This same teardown path fires on BOTH re-login and explicit
        # logout (upsert_auth(..., revoke=True), see utils/session.py and
        # blueprints/auth.py) -- publish_all_cache_invalidation carries no
        # signal distinguishing the two. Auto-reconnecting after a logout
        # would silently keep streaming a session the user explicitly
        # ended, which must never happen. Check the DB directly rather than
        # relying on the adapter-init call chain to fail closed by itself.
        try:
            from database.auth_db import Auth

            auth_row = Auth.query.filter_by(name=user_id).first()
            if not auth_row or auth_row.is_revoked:
                logger.info(
                    f"Auto-recovery: skipping adapter rebuild for user {user_id} -- "
                    f"session is revoked/logged out, not reconnecting"
                )
                return False

            # Indian broker tokens all expire at ~3 AM IST (see CLAUDE.md),
            # so this exact path can fire for hundreds/thousands of users
            # within the same few-minute window every morning. Without
            # spreading them out, every one of those reconnects (each a
            # real broker login + WS handshake) lands on this single
            # eventlet process and on each broker's auth API at once. A
            # small random delay per user costs nothing for the (far more
            # common) single manual re-login case but prevents the daily
            # mass-reconnect from becoming a synchronized thundering herd.
            jitter_max = float(os.getenv("WS_RECONNECT_JITTER_SECONDS", "8"))
            if jitter_max > 0:
                import random

                await aio.sleep(random.uniform(0, jitter_max))
        except Exception as e:
            logger.warning(
                f"Auto-recovery: could not verify session state for user {user_id}, "
                f"skipping rebuild to be safe: {e}"
            )
            return False
        ...
        try:
            ...
            if not adapter:
                logger.warning(
                    f"Auto-recovery: failed to create {broker_name} adapter for user {user_id}"
                )
                return False
            ...
            init_is_error = (
                (initialization_result and initialization_result.get("status") == "error")
                or (initialization_result and initialization_result.get("success") is False)
            )
            if init_is_error:
                error_msg = initialization_result.get(
                    "message", initialization_result.get("error", "Failed to initialize broker adapter")
                ...
                if adapter.is_auth_error(error_msg):
                    # Same as authenticate_client: an auth error here almost
                    # always means OUR cache still has the token from a
                    # moment ago -- retry once with it forced clear.
                    adapter.clear_auth_cache_for_user(user_id)
                    initialization_result = await self._run_broker_call(
                        user_id, broker_name, adapter.initialize, broker_name, user_id
                    )
                ...
                if init_is_error:
                    logger.warning(
                        f"Auto-recovery: failed to initialize {broker_name} adapter for user "
                        f"{user_id}: {error_msg}"
                    )
                    return False
            ...
            if is_error:
                logger.warning(
                    f"Auto-recovery: failed to connect {broker_name} adapter for user {user_id}: "
                    f"{connect_result.get('message', connect_result.get('error', 'unknown error'))}"
                )
                return False

            self.broker_adapters[user_id] = adapter

            # Recompute and store the token hash so authenticate_client's own
            # staleness check (which runs on the user's NEXT real login) sees
            # this freshly-rebuilt adapter as current rather than immediately
            # evicting it again.
            try:
                import hashlib
                ...
                current_token = get_auth_token(user_id, bypass_cache=True)
                if current_token:
                    self.adapter_token_hashes[user_id] = hashlib.md5(
                        current_token.encode("utf-8")
                    ).hexdigest()
            except Exception as hash_err:
                logger.debug(f"Auto-recovery: could not compute token hash for {user_id}: {hash_err}")

            # Replay every already-connected client's subscriptions on the
            # new adapter. self.subscriptions[client_id] holds JSON strings
            # written by subscribe_client -- same shape, decoded the same way.
            affected_clients = [
                cid for cid, uid in self.user_mapping.items() if uid == user_id
            ]
            resubscribed = 0
            for client_id in affected_clients:
                for sub_json in self.subscriptions.get(client_id, set()):
                    try:
                        sub = json.loads(sub_json)
                        response = adapter.subscribe(
                            sub["symbol"], sub["exchange"], sub["mode"], sub.get("depth_level", 5)
                        )
                        if response.get("status") == "success":
                            resubscribed += 1
                        else:
                            logger.warning(
                                f"Auto-recovery: resubscribe failed for user {user_id} "
                                f"{sub.get('symbol')}:{sub.get('exchange')}: {response}"
                            )
                    except Exception as sub_err:
                        logger.warning(
                            f"Auto-recovery: error replaying subscription for user {user_id}: {sub_err}"
                        )

            logger.info(
                f"Auto-recovery: rebuilt {broker_name} adapter for user {user_id} and "
                f"replayed {resubscribed} subscription(s) across {len(affected_clients)} client(s) "
                f"-- live feed restored with no action needed from the user"
            )
            return True

        except Exception as e:
            logger.exception(f"Auto-recovery: unexpected error rebuilding adapter for {user_id}: {e}")
            return False

    async def get_supported_brokers(self, client_id):
        """
        Get list of supported brokers, auto-discovered from broker/*/plugin.json
        (see utils.config.get_valid_brokers) rather than .env's VALID_BROKERS.
        ...
        try:
            ...
            supported_brokers = get_valid_brokers()
    ...
    @staticmethod
    def _log_cache_invalidation_task_error(task: "aio.Task") -> None:
        """Done-callback for the fire-and-forget _handle_cache_invalidation
        task. Without this, an exception raised inside that background task
        is only ever surfaced as an "exception was never retrieved" warning
        (or silently dropped depending on the event loop's exception
        handler) instead of a real log entry -- which would make an
        auto-recovery failure just as invisible as the original bug this
        was written to fix."""
        if task.cancelled():
            return
        exc = task.exception()
        if exc is not None:
            logger.exception(
                "Unhandled error in background cache-invalidation task", exc_info=exc
            )
    ...
    async def _handle_cache_invalidation(self, topic_str: str, data_str: str):
        """
        ...
        Also auto-recovers any already-connected client's live feed after the
        adapter teardown below (see _rebuild_adapter_and_resubscribe) -- the
        adapter is torn down unconditionally on every re-login so the NEXT
        broker call uses the fresh token, but a client whose browser tab was
        already open and streaming never sends a fresh "authenticate"
        message on its own, so without this it would silently receive no
        more ticks until the user manually reloaded the page.
        ...
        try:
            ...
            # Auto-recovery: if this user has a browser tab already
            # authenticated and streaming, rebuild the adapter and replay
            # its subscriptions right now instead of leaving it dead until
            # the user manually reloads. _rebuild_adapter_and_resubscribe
            # itself checks is_revoked and refuses to reconnect a logged-out
            # session, so this is safe to call unconditionally here.
            has_connected_client = any(
                uid == user_id for uid in self.user_mapping.values()
            )
            if has_connected_client and broker_name:
                await self._rebuild_adapter_and_resubscribe(user_id, broker_name)
        ...
        while self.running:
            try:
                ...
                if topic_str.startswith("CACHE_INVALIDATE"):
                    # Fire-and-forget as a background task, NOT awaited inline:
                    # _handle_cache_invalidation may now rebuild a broker
                    # adapter (real network I/O taking 1-3+ seconds, see
                    # _run_broker_call's docstring) to auto-recover an
                    # already-streaming client's feed. Awaiting it here would
                    # stall this single shared event loop's tick fan-out for
                    # EVERY other connected user for that same duration --
                    # exactly the multi-second full-process stall
                    # _run_broker_call itself exists to avoid one layer down.
                    task = aio.ensure_future(
                        self._handle_cache_invalidation(topic_str, data_str)
                    )
                    task.add_done_callback(self._log_cache_invalidation_task_error)
                ...
                broker_name = (
                    market_data.get("broker")
                    or market_data.get("broker_name")
                    or "unknown"
                )
