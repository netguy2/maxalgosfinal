#!/bin/bash
# Max Algos Docker Build and Deployment Script
# This script builds and deploys Max Algos with numba/llvmlite support
...
# Docker Compose wrapper to support both docker-compose and docker compose V2
docker_compose() {
...
# Start container
start_container() {
    print_header "Starting Max Algos Container"
...
health_check() {
    ...
    if docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        ...
        docker_compose logs --tail=50 maxalgos || docker logs ${CONTAINER_NAME}
        exit 1
    fi
...
test_python_deps() {
    ...
    if docker_compose exec -T maxalgos python -c "import numba; import llvmlite; print('SUCCESS')" 2>/dev/null | grep -q "SUCCESS"; then
        print_success "numba, llvmlite imports successful"
    else
        print_error "Failed to import dependencies"
        print_info "Running detailed test..."
        docker_compose exec maxalgos python -c "import numba; import llvmlite; print('SUCCESS')"
        return 1
    fi

    # Test 2: Numba JIT compilation
    print_info "Testing numba JIT compilation..."
    if docker_compose exec -T maxalgos python -c "
...
" 2>/dev/null | grep -q "SUCCESS"; then
    ...
    if docker_compose exec -T maxalgos bash -c "
...
show_access_info() {
    ...
    echo -e "${GREEN}✓ Max Algos is now running${NC}\n"
    ...
    echo -e "  Shell access:     ${YELLOW}docker compose exec maxalgos bash${NC}"
    echo -e "  Run strategy:     ${YELLOW}docker compose exec maxalgos uv run python /app/strategies/scripts/your_script.py${NC}"
...
# Main execution
main() {
    print_header "Max Algos Docker Build & Deploy"
