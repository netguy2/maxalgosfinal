...
class OrderSchema(Schema):
    ...
    # Optional: which of the user's connected brokers to route this order
    # to (see database/auth_db.py's AuthBrokerSession / list_broker_sessions).
    # Omitted -> falls back to the single-row Auth/ApiKeys resolution, same
    # as before this field existed. Lets one Python strategy (or any
    # api_key-authenticated caller) place orders on a broker other than
    # whichever one happens to be that account's primary Auth row.
    broker = fields.Str(missing=None, allow_none=True)
...
class SmartOrderSchema(Schema):
    ...
    # See OrderSchema.broker's comment -- same optional per-call broker override.
    broker = fields.Str(missing=None, allow_none=True)
