"""Regression tests for Max Algos login broker-session resume."""
...
def test_login_clears_expired_existing_session_before_password_flow(monkeypatch):
    ...
    user = SimpleNamespace(
        is_totp_required_for=lambda purpose: False,
        status="ACTIVE",
        username="rajandran",
        # is_admin True so the subscription gate is skipped -- this test is
        # about stale-session clearing, not the platform-fee gate.
        is_admin=True,
    )
    ...
    monkeypatch.setattr(auth_bp_module, "find_user_by_login_identifier", lambda identifier: user)
