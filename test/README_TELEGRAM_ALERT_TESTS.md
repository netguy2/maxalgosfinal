...
**Username Clarification**: The `username` parameter in the API refers to your **Max Algos login username** (the username you use to login to the Max Algos application), NOT your Telegram username (@handle).

- ✅ **Correct**: Use `"rajandran"` (your Max Algos login username)
...
1. **Max Algos must be running**
   - Start Max Algos application
   - Ensure it's accessible at `http://127.0.0.1:5000`

2. **Telegram Bot Must Be Running**
   - Go to Max Algos Telegram settings (`/telegram`)
...
3. **User Must Be Linked**
   ...
   - Username must match your Max Algos account username

4. **API Key and Username**
   - Get your API key from Max Algos settings
   - Replace `"your_api_key_here"` in the test files with your actual API key
   - Replace `"your_username_here"` with your **Max Algos login username** (the username you use to login to Max Algos app)
   - **IMPORTANT**: Use Max Algos username, NOT your Telegram username (@handle)
...
cd D:/maxalgos-sandbox-test/maxalgos/test
...
cd D:/maxalgos-sandbox-test/maxalgos
...
USERNAME = "your_username_here"      # Replace with your Max Algos login username (NOT @telegram_handle)
```

**IMPORTANT**: `USERNAME` should be your Max Algos login username (e.g., "rajandran"), NOT your Telegram username (e.g., "@rajandranr").
...
**Solution**:
1. Go to Max Algos Telegram settings (`/telegram`)
2. Click "Start Bot"
...
4. **Ensure you're using Max Algos login username, NOT Telegram username**
   - ✅ Correct: `"rajandran"` (Max Algos login)
...
**Solution**:
1. Check bot status in Max Algos dashboard
2. Verify username with `/status` in Telegram bot
3. Check Max Algos logs for errors
...
- Check bot status in Max Algos Telegram dashboard
- Verify user linking with `/status` in Telegram
- Review Max Algos logs for detailed error messages
