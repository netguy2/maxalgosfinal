...
# ---------------------------------------------------------------------------
# Expected Max Algos standard symbols (from symbol_Max Algos.md)
# ---------------------------------------------------------------------------
...
def get_database_url(env_vars):
    ...
    if not db_url:
        return None
    # Resolve relative sqlite paths (e.g. sqlite:///db/maxalgos.db)
...
def run_tests(db_url, broker_name):
    ...
    if extra_nse:
        print(f"  Found {len(extra_nse)} extra symbols (not in Max Algos doc)")
    ...
    if extra_bse:
        print(f"  Found {len(extra_bse)} extra symbols (not in Max Algos doc)")
        print(f"  Checking if unlisted symbols were preserved (only basic cleanup applied):\n")
        bse_transformed = []
        bse_preserved = []
    ...
    # -----------------------------------------------------------------------
    # Unlisted symbols log (broker symbols not in Max Algos doc)
    # -----------------------------------------------------------------------
    print_header("UNLISTED SYMBOLS (not in Max Algos doc)")
