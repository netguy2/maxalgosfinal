...
def test_subprocess_env_includes_strategy_exchange(ps_module, monkeypatch, tmp_path):
    """start_strategy_process injects MAX_ALGOS_STRATEGY_EXCHANGE per strategy."""
    ...
    monkeypatch.setattr("database.settings_db.is_kill_switch_active", lambda: False)
    ...
    assert success, msg
    assert captured_env.get("MAX_ALGOS_STRATEGY_EXCHANGE") == "MCX"
