...
* **`client.whatsapp()` in the maxalgos Python SDK (1.0.50)** — One unified call for every common case: send to self, to a single E.164 number, to up to 5 numbers (ToS-safety cap), with text, image, or document payloads. `wait_for_delivery=True` by default so the response carries a real per-recipient delivery report.
...
* **Platform version bump** — `2.0.1.0` → `2.0.1.1`. SDK pin (`maxalgos`) `1.0.49` → `1.0.50`.

***

**WhatsApp Bot — feature deep dive**

WhatsApp brings parity with Telegram for both outbound alerts and interactive command queries, with security choices tuned for the single-user-per-deployment model Max Algos runs under.

**Architecture in one sentence:** the wars (PyO3 over whatsapp-rust) library hosts a fully linked WhatsApp Web device inside the Flask process; outgoing alerts flow event-bus → `whatsapp_subscriber` → `WhatsAppBotThread` → `wars.send()`; incoming slash-commands flow `wars.on_message` → `is_from_me=True` gate → command dispatcher → Max Algos SDK call → reply via the same bot thread.
...
* On pair success: `export_session()` → Fernet-encrypted → persisted to `whatsapp_config.session_blob` in `maxalgos.db` → temp file unlinked
...
* Hard precheck — every send path refuses with `409 "WhatsApp is not paired or not connected. Pair the device first from the /whatsapp page in Max Algos before sending."` if `is_ready()` is false; we explicitly do not queue when unpaired
...
* Attachment paths validated against `WHATSAPP_ATTACHMENT_ROOTS` allowlist (default: `<maxalgos>/db/attachments/`); paths with `..`, paths under `/etc /proc /sys /root /var/log /C:\Windows`, or paths that resolve outside the allowlist are rejected with `400 image_path is not allowed`
...
**maxalgos Python SDK 1.0.50** — released to PyPI alongside this version:
...
* Available at <https://pypi.org/project/maxalgos/1.0.50/>
...
* `61370758` / `54d83c07` — restyle the MCP OAuth consent page to match Max Algos dashboard + fix alignment
...
* Optional: `WHATSAPP_ATTACHMENT_ROOTS` (comma-separated absolute dirs; defaults to `<maxalgos>/db/attachments/`)
...
* SDK pin (`maxalgos`) `1.0.49` → `1.0.50`
...
* `maxalgos==1.0.49` → `maxalgos==1.0.50`
...
* `maxalgos` SDK pin: `1.0.49` → `1.0.50` (PyPI: <https://pypi.org/project/maxalgos/1.0.50/>)
...
* New: `collections/maxalgos/IN_stock/whatsapp_notify.bru` — Bruno collection entry (auto-discovered by `/playground`)
* Updated: `docs/api/README.md` — adds the WhatsApp Services section under the existing service taxonomy
* Updated: `docs/prompt/maxalgos python sdk.md` — full `client.whatsapp(...)` reference with all four recipient forms, image/document attachments, fire-and-forget vs synchronous delivery, and inbound slash-command reference
...
* `f786e21a` — `chore: add Caddyfile for local https://maxalgos.local dev`
...
1. Log in to Max Algos, open `/whatsapp`.
2. Click **Start pairing**. A QR code appears.
3. On your phone: WhatsApp → Settings → Linked devices → Link a device → scan.
4. Done. The bot auto-starts and reconnects on every server boot from the encrypted session in `maxalgos.db`.
...
* **Python SDK on PyPI**: <https://pypi.org/project/maxalgos/1.0.50/>
* **WhatsApp service docs**: <https://docs.maxalgos.in/api-documentation/v1/whatsapp-services>
...
```
GET https://docs.maxalgos.in/change-log/release/version-2.0.1.1-released.md?ask=<question>
```
