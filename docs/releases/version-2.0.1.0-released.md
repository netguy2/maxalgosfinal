...
* **Platform version bump** — `2.0.0.9` → `2.0.1.0`. `.sample.env` `ENV_CONFIG_VERSION` `1.0.6` → `1.0.7`. SDK pin (`maxalgos==1.0.49`) unchanged.

***

**Remote MCP — feature deep dive**

Remote MCP brings the Max Algos MCP toolset to **hosted AI clients** over the public internet. Same 40 tools as the local stdio integration, exposed at `https://yourdomain.com/mcp` with full OAuth 2.1.
...
* `install/enable-remote-mcp.sh` — native Ubuntu enabler. Detects all `maxalgos-*` systemd services, refuses if `FLASK_DEBUG=True`, backs up `.env`, sets the four MCP keys, runs `upgrade/migrate_all.py`, restarts the service, and probes the discovery / JWKS / `/mcp/healthz` endpoints to confirm boot.
* `install/enable-remote-mcp-docker.sh` — Docker enabler. Walks `/opt/maxalgos/*/docker-compose.yaml`, picks a stack, backs up the bind-mounted `.env`, updates keys, restarts the container (whose `start.sh` runs migrations automatically), and runs the same smoke probe.
...
* `version = "2.0.1.0"`
* SDK pin (`maxalgos==1.0.49`) unchanged
...
```bash
cd /var/python/maxalgos-flask/<deploy-name>/maxalgos
...
* **Remote MCP guide**: <https://docs.maxalgos.in/mcp/remote-mcp>
...
```
GET https://docs.maxalgos.in/change-log/release/version-2.0.1.0-released.md?ask=<question>
```
