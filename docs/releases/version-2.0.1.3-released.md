...
* **Session UX on token expiry** — An expired broker token no longer logs you out of Max Algos: the session stays intact, the UI flags the stale token and offers a re-authentication path, and login resume requires a valid broker session.
...
* **Platform version bump** — `2.0.1.2` → `2.0.1.3`. SDK pin (`maxalgos`) `1.0.51` → `2.0.1`, `opengreeks` `0.1.0` → `0.2.0`.
...
* `maxalgos` SDK pin: `1.0.51` → `2.0.0` → **`2.0.1`** (PyPI: <https://pypi.org/project/maxalgos/>).
...
* **Python SDK on PyPI**: <https://pypi.org/project/maxalgos/>
