...
Let a self-hosted Max Algos install expose its MCP tools to **hosted AI clients** (chatgpt.com, claude.ai, claude mobile) in addition to the existing local stdio integration with Claude Desktop / Cursor / Windsurf.

Concrete outcome: after running `install/install.sh` and pointing a domain at the server, the user can connect a hosted MCP client to `https://mcp.<their-domain>/mcp` and use Max Algos tools through standard OAuth.

## Non-goals

- **Multi-user**: a remote MCP server is still single-user, single-broker. The single Max Algos admin authorizes the client; there's no per-user MCP access.
...
- **A SaaS hosted MCP**: nothing runs on infrastructure operated by the Max Algos project.
...
└────────────┬──────────────────┘           │  └─────────────────────────────────┘   │
        ...
        │   mcp = FastMCP("maxalgos")     │◀─┼──│   GET  /mcp (SSE stream)        │   │
        │   @mcp.tool()                   │  │  └─────────────────────────────────┘   │
        │   def place_order(...): ...     │  │                                        │
        │   ... (all tools)               │  │  ┌─────────────────────────────────┐   │
...
6. User logs in to Max Algos (existing session) and approves scopes on a consent page.
...
**Decision: the Max Algos dashboard login (username + password + TOTP), not the API key.**
...
|---|---|---|
| Max Algos login (username + password + TOTP) | Admin user, interactively at the browser | Gates `/oauth/authorize` — proves the human at the keyboard is the Max Algos admin and consents to grant the MCP client access |
| Max Algos API key | Server-side `.env` value | Used **inside** the tool implementations when they call back to `/api/v1/*` — the user never sees or enters it during OAuth |
...
2. /oauth/authorize is gated by @check_session_validity:
   - If no valid session → standard Max Algos login page (username + password + TOTP)
...
- **Recovery story.** A user who loses their API key still has password+TOTP and can revoke MCP access; a user who loses password+TOTP would already be locked out of Max Algos entirely, so MCP is not a new dependency.
...
| Value | Behavior |
|---|---|
| `session` | Any valid Max Algos session passes (least friction) |
...
New file `database/oauth_db.py` with three tables in `db/maxalgos.db`:
...
6. **Tokens hashed with `API_KEY_PEPPER`.** Refresh tokens never persist in plaintext. Same Argon2/HMAC pipeline as Max Algos API keys.
7. **Signing key on disk.** RS256 private key in `keys/mcp_oauth_signing.pem`, chmod `600`, owned by the Max Algos process user. Auto-generated by install scripts. Not in git, not in `.env`.
...
| Compromised Max Algos admin password → MCP takeover | Same blast radius as compromised admin already. Mitigation is on the Max Algos side (Argon2, rate-limited login). MCP doesn't widen this. |
...
```
[?] Enable remote MCP server (allows ChatGPT/Claude.ai to call Max Algos)? [y/N]
...
4. **Single-tenant simplification** — DCR allows arbitrary client registration. For a single-user install we could short-circuit with one pre-approved client per Max Algos install. Default: keep DCR (matches MCP spec); add `MCP_OAUTH_REQUIRE_APPROVAL` for friction-on-demand.
...
- Multi-broker switching via MCP (the current Max Algos session ties one user to one broker)
