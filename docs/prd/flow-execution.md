...
├── flow_maxalgos_client.py     # Max Algos API wrapper (~500 lines)
...
| `FlowMax AlgosClient` | Wraps Max Algos internal APIs |
...
```python
class NodeExecutor:
    def __init__(self, client: FlowMax AlgosClient, context: WorkflowContext, logs: list):
        self.client = client
...
def execute_workflow(
    ...
    # 3. Create executor with Max Algos client
    client = FlowMax AlgosClient(api_key)
...
Wraps internal Max Algos APIs.

```python
class FlowMax AlgosClient:
