...
This guide walks through every step required to add a new broker to Max Algos. It covers the directory structure, authentication patterns, order/data APIs, symbol mapping, WebSocket streaming, master contract database, rate limiting, and all registration points across the codebase.
...
└── your_broker/
    ...
    ├── mapping/
    │   ├── transform_data.py          # Max Algos ↔ broker format mapping (required)
...
```json
{
    ...
    "Plugin URI": "https://maxalgos.in",
    "Description": "YourBroker Max Algos Plugin",
    "Version": "1.0",
    "Author": "Your Name",
    "Author URI": "https://maxalgos.in"
}
```
...
This is the critical translation layer between Max Algos's unified format and the broker's API format.
...
def transform_data(data):
    """
    Transform Max Algos order request to broker-specific format.

    Input (Max Algos format):
        {
            "symbol": "SBIN",
            "exchange": "NSE",
            "action": "BUY",
...
Maps broker order response fields to Max Algos's standardized format:

```python
def map_order_data(order):
    """Map broker order data to Max Algos format."""
    return {
...
This module downloads the broker's instrument/symbol master file and populates the `symtoken` table, which maps Max Algos symbols to broker-specific symbols.
...
def master_contract_download():
    ...
    3. Maps broker-specific symbols to Max Algos's standardized format
...
|----------------|-------------|---------|
| `symbol` | Max Algos standardized symbol | `SBIN-EQ`, `NIFTY24JAN24000CE` |
| `brsymbol` | Broker's native symbol | `SBIN`, `NIFTY24JAN24000CE` |
| `name` | Human-readable name | `State Bank of India` |
| `exchange` | Max Algos exchange | `NSE`, `NFO`, `BSE`, `BFO`, `CDS`, `MCX` |
...
def map_feed_data(raw_data):
    """
    Normalize broker-specific tick data to Max Algos's unified format.
...
Max Algos supports five distinct authentication patterns:
