...
JSON that can be imported into the Max Algos Flow Editor. It covers the
...
`instrumenttype` field which is lowercase to match the Max Algos REST API.
...
| Field | Type | Default | Notes |
|---|---|---|---|
| `symbol` | string | — | Max Algos symbol format. |
...
(if set). The shape returned by each maps onto the Max Algos REST API's
...
Sends a Telegram message via the per-user Telegram bot configured in Max Algos
settings.

| Field | Type | Default | Notes |
|---|---|---|---|
| `username` | string | — | Max Algos login ID linked to a Telegram user. |
| `message` | string | — | Supports `{{vars}}`. |
...
Receive a webhook, then fan out: place an Max Algos order **and** post a
...
```json
{
  ...
  "nodes": [
    ...
    { "id": "node_5", "type": "telegramAlert",  "position": { "x": 100, "y": 540 }, "data": { "username": "rajandran", "message": "[Max Algos] Daily square-off done at {{time}} IST on {{date}}" } }
  ],
  "edges": [
    { "id": "e1", "source": "node_1", "target": "node_2" },
    { "id": "e2", "source": "node_2", "target": "node_3" },
...
Max Algos standardizes broker-specific symbols to the following format. See
