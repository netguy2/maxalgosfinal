...
To install the Max Algos Python library, use pip:
...
pip install maxalgos[indicators]  
```

### Get the Max Algos apikey

Make Sure that your Max Algos Application is running. Login to Max Algos Application with valid credentials and get the Max Algos apikey

For detailed function parameters refer to the [API Documentation](https://docs.maxalgos.in/api-documentation/v1)

### Getting Started with Max Algos

First, import the `api` class from the Max Algos library and initialize it with your API key:
...
### Check Max Algos Version

```python
import maxalgos 
maxalgos.__version__
```

### Examples

Please refer to the documentation on [order constants](https://docs.maxalgos.in/api-documentation/v1/order-constants), and consult the API reference for details on optional parameters
...
Prerequisites: open `/whatsapp` in the Max Algos web UI, click **Pair**, scan the QR with your phone. Pairing is admin-only on purpose — the REST API exposes only the send endpoint so a leaked API key cannot re-pair the device. Once paired, the bot auto-reconnects on every server boot from the encrypted session blob stored in `maxalgos.db`.
...
The path is read from the Max Algos server's filesystem. It must lie under `WHATSAPP_ATTACHMENT_ROOTS` (defaults to `<maxalgos>/db/attachments/`).
...
#### Send to a linked Max Algos user (legacy multi-recipient path)
...
Type slash-commands from your own phone in the **"Message yourself"** chat — the Max Algos linked device sees those as `is_from_me=True` and responds in the same chat. Random contacts who message your number cannot drive the bot.
...
# Initialize Max Algos client
client = api(
    api_key="your_api_key",                  # Replace with your actual Max Algos API key
...
```
GET https://docs.maxalgos.in/trading-platform/python.md?ask=<question>
```
