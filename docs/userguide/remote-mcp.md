...
Lets hosted AI clients — ChatGPT, Claude.ai, Claude mobile — talk to your Max Algos install over the internet so you can ask them to fetch quotes, summarise positions, or place orders in plain English.
...
1. **Max Algos on your own domain with HTTPS.** Dashboard reachable at `https://yourdomain.com`, login + broker auth + orders all working through the web UI. If you're not there yet, start with one of the install scripts: `install/install.sh`, `install/install-multi.sh`, `install/install-docker.sh`, or `install/install-docker-multi-custom-ssl.sh`.
2. **Max Algos 2.0.1.0 or later.** Footer of the dashboard shows the version, or `curl https://yourdomain.com/api/v1/maxalgos-version`. On older builds run `install/update.sh` first.
3. **An Max Algos API key.** Generate one at **Profile → API Keys**. The MCP server uses it server-side; hosted clients never see it — they get OAuth tokens instead.
...
If you said no and want to flip it now, edit `/var/python/maxalgos/.env`:
...
Then `sudo systemctl restart maxalgos`.

### Multi-domain native (`install-multi.sh`)

Edit the per-deploy `.env` (typically `/var/python/maxalgos-flask/<deploy-name>/.env`) with the same two keys, then `sudo systemctl restart maxalgos-<deploy-name>`.
...
| --- | --- |
| Name | `Max Algos` |
| Description | `Max Algos trading server` (optional) |
...
The notice *"CIMD is unavailable…"* is expected — Max Algos advertises DCR. DCR is the right pick.
...
In any new chat, click **+** below the message box → **Connectors** → toggle **Max Algos** ON.

Try:

> *"Using Max Algos, give me the LTP of RELIANCE on NSE."*
...
#### Step 5 — Approve in Max Algos

`https://yourdomain.com/admin/remote-mcp` → **Pending approvals** → **Approve**.

#### Step 6 — Complete OAuth

Back in claude.ai → **Connect** on the connector card → sign in to Max Algos (+ TOTP if on) → consent screen (verify redirect URI is `claude.ai`) → **Authorize**. Card switches to **Disconnect** when you're live.

#### Step 7 — Tool permissions

Click your **Max Algos** connector to expand permissions:
...
In any chat, click the **Tools** icon below the message box → toggle **Max Algos** on.
...
| *"CIMD is unavailable"* in ChatGPT | Max Algos advertises DCR, not CIMD | Expected — pick **DCR** |
| Tools missing from chat | Connector not toggled on for that chat | `+` menu (ChatGPT) or Tools menu (Claude) |
| `bad_arguments` on a tool call | Hosted client guessed parameter names | Update Max Algos (newer builds expose strict tool schemas) |
...
```bash
sudo sed -i "s|MCP_HTTP_ENABLED.*|MCP_HTTP_ENABLED = 'False'|" /var/python/maxalgos/.env
...
```bash
sudo sed -i "s|MCP_HTTP_ENABLED.*|MCP_HTTP_ENABLED = 'False'|" /opt/maxalgos/<domain>/.env
cd /opt/maxalgos/<domain> && sudo docker compose restart
```
...
- Max Algos Symbol Format — how equity / future / option symbols are constructed
...
```
GET https://docs.maxalgos.in/mcp/remote-mcp.md?ask=<question>
```
