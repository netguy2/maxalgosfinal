# Max Algos Symbol Format Guide

## Introduction

Max Algos uses a standardized symbol format across all exchanges and brokers. This uniform symbology eliminates the need for traders to adapt to varied broker-specific formats, streamlining algorithm development and execution.
...
| Company | Base Symbol | Max Algos Symbol |
|---------|-------------|-----------------|
...
Max Algos uses standardized exchange codes to identify trading venues.
...
Max Algos has rolled out a **standardized index symbol set across all supported brokers**. Use exchange code `NSE_INDEX` or `BSE_INDEX` when placing orders or fetching quotes for these symbols — the same symbol works identically on every broker.
...
> The `BSEOIL&GAS` symbol literally contains an ampersand — preserve it exactly as shown when passing the symbol via API, JSON body, or webhook. For the full authoritative reference see [docs.maxalgos.in/symbol-format](https://docs.maxalgos.in/symbol-format).
...
### Method 1: Max Algos Symbol Search

1. Go to Max Algos dashboard
...
Max Algos maintains a master contract database that maps broker symbols to standardized symbols. The database is updated daily.
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Max Algos Symbol Quick Reference                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  TYPE          FORMAT                      EXAMPLE                          │
│  ────          ──────                      ───────                          │
