...
Before diving into Max Algos, let's understand the key terms and concepts you'll encounter. This foundation will make everything else easier to understand.
...
**In Max Algos**: When TradingView wants to place an order, it sends a request to Max Algos's API. Max Algos processes it and sends the order to your broker.

```
TradingView → "Place BUY order for SBIN" → Max Algos API → Broker
```

### 2. API Key

**Simple Explanation**: Your API key is like a password that identifies you. It proves to Max Algos that the request is coming from an authorized source.
...
**Simple Explanation**: A webhook is like a doorbell. When something happens (like a TradingView alert), it "rings" your Max Algos server to notify it.
...
Webhook sends data to your URL
        ↓
Max Algos receives and processes
        ↓
...
**Simple Explanation**: When you log into your broker through Max Algos, you get a temporary pass (token) that lets Max Algos place orders on your behalf.
...
**Simple Explanation**: Every stock has a specific way to write its name that Max Algos understands.

**Examples**:
| What you want | Max Algos Symbol |
|---------------|-----------------|
...
**Simple Explanation**: The period you're logged into Max Algos. Sessions expire for security.

**Browser Session**: Your Max Algos web login
...
**Previous**: [02 - Why Build with Max Algos](../02-why-build-with-maxalgos/README.md)
