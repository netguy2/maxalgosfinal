...
ChartInk is a powerful stock screening platform popular among Indian traders. It can scan thousands of stocks in real-time and send webhook alerts when conditions are met. Max Algos integrates with ChartInk to execute trades automatically based on your screener alerts.

## How It Works

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    ChartInk → Max Algos Flow                                 │
│                                                                              │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌──────────┐  │
│  │  ChartInk   │     │   Webhook   │     │  Max Algos   │     │  Broker  │  │
...
2. Max Algos running and accessible via internet
3. API key generated in Max Algos
4. Broker connected and logged in

## Making Max Algos Accessible for Webhooks

ChartInk webhooks need to reach your Max Algos server from the internet.

### Recommended: Production Server with Domain

Deploy Max Algos on an Ubuntu server using `install.sh` (see [Installation Guide](../04-installation/README.md)):
...
**Important**: Tunnel services are **only for webhooks**, not for running the full application. Always run Max Algos on your own server for production use
...
2. Enable **Webhook**
3. Enter your Max Algos webhook URL
...
```
https://your-maxalgos-url/api/v1/placeorder
```

### ChartInk Webhook Payload

ChartInk sends data in a specific format. You need to configure the payload to match Max Algos's API:
...
1. Enable Analyzer Mode in Max Algos
...
- Check Max Algos dashboard regularly
- Set up Telegram notifications
- Review trades daily

## Troubleshooting

### Alert Not Reaching Max Algos
...
1. Go to **Traffic Logs** in Max Algos
