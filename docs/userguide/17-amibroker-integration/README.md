...
Amibroker is a powerful technical analysis and backtesting software widely used by Indian traders. Max Algos provides seamless integration via its HTTP API, allowing your Amibroker strategies to execute trades automatically.

## How It Works

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Amibroker → Max Algos Flow                                │
│                                                                              │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌──────────┐  │
│  │  Amibroker  │     │    HTTP     │     │  Max Algos   │     │  Broker  │  │
...
2. Max Algos running on same machine or network
...
```afl
function SendMax AlgosOrder(apiKey, strategy, symbol, exchange, action, quantity, priceType, product)
{
    // Build JSON payload
    json = "{";
    json += "\"apikey\": \"" + apiKey + "\",";
...
{
    response = SendMax AlgosOrder(apiKey, strategy, symbol, exchange, "BUY", quantity, priceType, product);
    _TRACE("Buy Order: " + response);
}

if(LastValue(Sell) AND LastValue(Ref(Sell, -1)) == 0)
{
    response = SendMax AlgosOrder(apiKey, strategy, symbol, exchange, "SELL", quantity, priceType, product);
...
```afl
//=============================================================================
// Max Algos Integration Template for Amibroker
// Version: 1.0
//=============================================================================

// Configuration Section
_SECTION_BEGIN("Max Algos Configuration");

apiKey = ParamStr("API Key", "YOUR_API_KEY");
baseUrl = ParamStr("Max Algos URL", "http://127.0.0.1:5000");
...
Always test with Analyzer Mode enabled in Max Algos first.
...
| Issue | Solution |
|-------|----------|
| Connection refused | Check Max Algos is running |
| Invalid API key | Verify key in Max Algos |
