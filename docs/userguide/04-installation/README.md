...
This guide walks you through installing Max Algos on your system. We'll cover Windows, Ubuntu, and macOS installations.
...
#### Step 3: Download Max Algos

Open Command Prompt (search "cmd") and run:

```cmd
# Navigate to where you want Max Algos
...
#### Step 3: Download and Run Max Algos
...
You should see the Max Algos login page.
...
For production use, deploy Max Algos on an Ubuntu server using the automated `install.sh` script. This is the **recommended approach** for live trading.
...
- Unique service name (e.g., maxalgos-yourdomain-broker)
...
1. **Check Service Status**
   ```bash
   sudo systemctl status maxalgos-yourdomain-broker
   ```
...
```bash
# List all Max Algos services
systemctl list-units "maxalgos-*"

# Restart specific deployment
sudo systemctl restart maxalgos-yourdomain-broker

# View real-time logs
sudo journalctl -f -u maxalgos-yourdomain-broker

# View last 100 lines of logs
sudo journalctl -n 100 -u maxalgos-yourdomain-broker
```
...
```bash
# View service logs
sudo journalctl -u maxalgos-yourdomain-broker
...
**Important**: Tunneling is **only for webhooks**. Always run Max Algos on your own server with proper domain setup for production use. Don't run the entire application through a tunnel.
...
│  ┌──────────────────────────────────────────────────────────┐ │
│  │  Nginx (HTTPS) → Gunicorn → Max Algos                     │ │
...
Max Algos can also be deployed using Docker with custom domain and SSL. This is useful if you prefer containerized deployments.
...
5. Clones Max Algos to `/opt/maxalgos`
...
| Item | Location |
|------|----------|
| Installation | `/opt/maxalgos` |
| Configuration | `/opt/maxalgos/.env` |
| Database | Docker volume `maxalgos_db` |
| Nginx Config | `/etc/nginx/sites-available/yourdomain.com` |
| SSL Certificates | `/etc/letsencrypt/live/yourdomain.com/` |
| Backups | `/opt/maxalgos-backups/` |
...
Max Algos can run on Raspberry Pi models 3, 4, or 5 (4GB+ RAM), preferably with Ubuntu 24.04+ server edition.
...
Close all Max Algos instances and restart.
