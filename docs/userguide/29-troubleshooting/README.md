...
This guide helps you diagnose and resolve common issues in Max Algos. Problems are organized by category with step-by-step solutions.
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  □ Is Max Algos running? (Check terminal/service status)                    │
...
**Symptom**: TradingView/ChartInk alerts not reaching Max Algos

**Checklist**:
1. Is Max Algos accessible from internet?
   ```bash
   # Test with curl from external machine
   curl https://your-maxalgos-url/health
   ```
...
**Solution**:
1. Copy API key from Max Algos dashboard
...
**Symptom**: Max Algos consuming too much RAM
...
```bash
# Application logs
tail -f logs/maxalgos.log

# Check for errors
grep -i error logs/maxalgos.log
...
uv run python -c "import maxalgos; print(maxalgos.__version__)"
