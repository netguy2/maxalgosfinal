# Max Algos Real-Time Scanner Service — Architecture & Implementation Guide
...
The service builds candles from Max Algos's live WebSocket tick stream, bootstraps indicator state from Historify (DuckDB) at startup, and distributes candle events to multiple scanner processes via Redis Streams.
...
│  └── Max Algos API: GET /history (source="Db", interval="1m")        │
...
│                                                                     │
│  Max Algos WebSocket Proxy (port 8765)                               │
...
**Purpose:** Single point of connection to Max Algos WebSocket. Receives all ticks and publishes to Redis for downstream consumption.
...
- Authentication: `{"action": "authenticate", "apikey": "<MAX_ALGOS_API_KEY>"}`
...
- Max Algos supports up to 3000 symbols via connection pooling (3 connections x 1000 symbols each)
...
**Data source:** Historify (DuckDB) via Max Algos REST API with `source="Db"`.
...
- Use Max Algos's Historify write API if available
...
- Route through Max Algos's PlaceSmartOrder API
...
2. Max Algos application (WebSocket proxy must be available)
3. tick_receiver (connects to Max Algos WebSocket, starts publishing ticks)
...
08:55  Tick receiver connects to Max Algos WebSocket
       └── Authenticate
...
If WebSocket disconnects:
    ...
    2. Wait 1 second
    3. Reconnect to Max Algos WebSocket
    4. Re-authenticate
...
- Max Algos WebSocket supports 3000 symbols (connection pooling)
...
│   ├── maxalgos_client.py           # Max Algos API wrapper
...
```
# requirements.txt
maxalgos                    # Max Algos SDK — API and WebSocket
