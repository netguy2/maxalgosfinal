...
This document describes the broker factory design that enables Max Algos to work with any of the 29 supported brokers while maintaining a single common interface for the WebSocket proxy system. Max Algos allows one user to connect to one broker at a time, and the broker factory ensures consistent implementation across all supported brokers.
...
All adapters normalize broker data to Max Algos format:
