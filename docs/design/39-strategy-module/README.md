...
The Strategy Module provides a webhook-based system for receiving trading signals from external platforms (TradingView, Amibroker, ChartInk) and executing orders through Max Algos. It features time-based controls, symbol mappings, automatic square-off, and rate-limited order queuing.
...
class StrategySymbolMapping(Base):
    ...
    symbol = Column(String(50))           # Max Algos symbol to trade
...
Allows mapping external symbols to Max Algos format:
