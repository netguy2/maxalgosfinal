...
Max Algos provides a cross-platform Python strategy hosting system that allows users to upload, run, schedule, and manage trading strategies. Each strategy runs in a separate process for complete isolation with support for Windows, Linux, and macOS.
...
4. **Use Max Algos API** - Don't bypass the API layer
