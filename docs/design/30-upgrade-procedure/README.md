...
Guidelines for upgrading Max Algos to new versions while preserving data and configurations.
...
│  □ 5. Stop running Max Algos instance                                       │
...
cp db/maxalgos.db backups/$(date +%Y%m%d)/
...
Max Algos uses automatic database initialization on startup. New tables are created automatically when the application starts - no manual migrations are required.
...
# Restore databases
cp backups/YYYYMMDD/maxalgos.db db/
...
echo "Starting Max Algos upgrade..."
