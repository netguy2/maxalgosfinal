...
Chartink integration allows Max Algos to receive trading signals from Chartink screener alerts via webhooks. When a stock appears in a Chartink scanner, it triggers a webhook that Max Algos processes to place trades automatically.
...
```json
{
    "webhook_id": "your_webhook_id_from_maxalgos",
    "stocks": "{stocks}"
}
```
...
Max Algos: Auto-buy with MIS product, qty=100
```
...
Max Algos: Auto-buy with CNC product for delivery
```
...
Max Algos: Auto-sell to close positions
```
