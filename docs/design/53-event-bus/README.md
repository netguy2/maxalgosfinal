...
Max Algos is a single-user, single-process application using SQLite. External message brokers add infrastructure complexity for zero benefit at this scale. The EventBus is ~70 lines of Python using stdlib `threading` and `concurrent.futures`.
