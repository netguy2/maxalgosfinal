# Max Algos Docker Build Guide

Complete guide to building and deploying Max Algos with numba/llvmlite/scipy support.
...
# Verify it's working
docker-compose exec maxalgos python -c "import numba; import llvmlite; import scipy; print('✓ Success')"
```
...
```bash
# Build the image
docker build --no-cache -t maxalgos:latest .
...
docker run -d \
  ...
  -p 5000:5000 \
  -p 8765:8765 \
  -v maxalgos_db:/app/db \
  -v maxalgos_log:/app/log \
  -v maxalgos_strategies:/app/strategies \
  -v maxalgos_keys:/app/keys \
...
```bash
docker ps | grep maxalgos
```

Expected output:
```
CONTAINER ID   IMAGE             COMMAND          CREATED          STATUS          PORTS                                            NAMES
abc123def456   maxalgos:latest   "/app/start.sh"  10 seconds ago   Up 9 seconds    0.0.0.0:5000->5000/tcp, 0.0.0.0:8765->8765/tcp   maxalgos-web
```
...
**Test imports:**
```bash
docker-compose exec maxalgos python -c "import numba; import llvmlite; import scipy; print('✓ Imports successful')"
```

**Test numba JIT:**
```bash
docker-compose exec maxalgos python -c "
...
docker-compose exec maxalgos ps aux | grep websocket_proxy
```
...
```bash
docker-compose exec maxalgos ls -la /app/.env
docker-compose exec maxalgos head -5 /app/.env
```
...
```bash
docker-compose exec maxalgos ls -la /app/
docker-compose exec maxalgos ls -la /app/tmp/

# Fix if needed (run as root)
docker-compose exec -u root maxalgos chown -R appuser:appuser /app/tmp
docker-compose exec -u root maxalgos chmod -R 755 /app/tmp
```
...
```bash
docker-compose exec maxalgos bash -c '
...
docker buildx build --platform linux/arm64 -t maxalgos:arm64 .

# For AMD64 (Intel/AMD)
docker buildx build --platform linux/amd64 -t maxalgos:amd64 .

# Multi-platform (requires buildx)
docker buildx build --platform linux/amd64,linux/arm64 -t maxalgos:latest .
```
...
```bash
docker-compose build --no-cache
docker tag maxalgos:latest maxalgos:v2.0.0
docker tag maxalgos:latest myregistry.com/maxalgos:latest
```
...
# Tag for registry
docker tag maxalgos:latest your-registry.com/maxalgos:latest

# Push
docker push your-registry.com/maxalgos:latest
```
...
```bash
# Build
docker build -t maxalgos:latest .

# Push to registry
docker tag maxalgos:latest your-registry/maxalgos:latest
docker push your-registry/maxalgos:latest
...
3. **Scan image for vulnerabilities:**
   ...
   docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
     aquasec/trivy:latest image maxalgos:latest
   ```
...
- **Max Algos Documentation:** https://docs.maxalgos.in
...
# Shell access
docker-compose exec maxalgos bash

# Run Python script
docker-compose exec maxalgos uv run python /app/strategies/scripts/your_script.py

# Test dependencies
docker-compose exec maxalgos python -c "import numba; import scipy; print('OK')"
