...
docker run -d \
    ...
    -v "$MAX_ALGOS_DIR/.env:/app/.env:ro" \
...
docker run -d \
    ...
    -v "$MAX_ALGOS_DIR/strategies:/app/strategies" \
    -v "$MAX_ALGOS_DIR/log:/app/log" \
    -v "$MAX_ALGOS_DIR/keys:/app/keys" \                # ← NEW
    -v "$MAX_ALGOS_DIR/tmp:/app/tmp" \                  # ← NEW
    -v "$MAX_ALGOS_DIR/.env:/app/.env:ro" \
    --restart unless-stopped \
    "$IMAGE"
```
...
docker run -d ^
    ...
    -v "%MAX_ALGOS_DIR%\log:/app/log" ^
    -v "%MAX_ALGOS_DIR%\keys:/app/keys" ^                    # ← NEW
    -v "%MAX_ALGOS_DIR%\tmp:/app/tmp" ^                      # ← NEW
    -v "%MAX_ALGOS_DIR%\.env:/app/.env:ro" ^
    --restart unless-stopped ^
    %IMAGE%
```
...
services:
  maxalgos:
    ...
    volumes:
      - maxalgos_db:/app/db
      - maxalgos_logs:/app/logs          # ← EXTRA, UNUSED
...
volumes:
  maxalgos_db:
  maxalgos_logs:                         # ← EXTRA, UNUSED
  maxalgos_log:
  maxalgos_strategies:
  maxalgos_keys:
  # MISSING: maxalgos_tmp
```
...
- [ ] Verify numba import works: `docker exec maxalgos python -c "import numba; print('OK')"`
- [ ] Verify shared memory: `docker inspect maxalgos --format='{{.HostConfig.ShmSize}}'` should show `2147483648`
...
```bash
docker inspect maxalgos --format='{{.HostConfig.ShmSize}}'
# Should show: 2147483648 (2GB in bytes)
```

**Verify volumes:**
```bash
docker inspect maxalgos --format='{{range .Mounts}}{{.Destination}} {{end}}'
