...
2. Add `-v "$MAX_ALGOS_DIR/keys:/app/keys"` volume
3. Add `-v "$MAX_ALGOS_DIR/tmp:/app/tmp"` volume
...
1. Add `--shm-size=2g ^` after `--name %CONTAINER% ^`
2. Add `-v "%MAX_ALGOS_DIR%\keys:/app/keys" ^` volume
3. Add `-v "%MAX_ALGOS_DIR%\tmp:/app/tmp" ^` volume
4. Update setup function to create `keys` and `tmp` directories
...
- ❌ `maxalgos_tmp` volume and mount - Required for numba cache
...
2. Add `maxalgos_tmp:/app/tmp` volume mount
3. Add `maxalgos_tmp:` volume definition
4. Remove duplicate `maxalgos_logs` volume (unused)
...
- [ ] Test numba: `docker exec maxalgos python -c "import numba; print('OK')"`
- [ ] Check shared memory: `docker inspect maxalgos --format='{{.HostConfig.ShmSize}}'`
