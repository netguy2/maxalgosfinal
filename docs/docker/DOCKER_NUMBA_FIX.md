...
When running Max Algos strategies in Docker that use numba/llvmlite (for indicators like Supertrend, EMA, TEMA), you may encounter these errors:
...
# Verify the fix
docker-compose exec maxalgos python -c "import numba; import llvmlite; import scipy; print('✓ All imports successful')"
```
...
docker stop maxalgos-web && docker rm maxalgos-web
...
4. **Run your strategy:**
```bash
docker-compose exec maxalgos uv run python /app/strategies/scripts/your_strategy.py
```
