...
Max Algos connects to your broker's live market feed **once**. That single feed is then distributed locally to three different audiences:
...
3. **External Websockets** — your own Python, JavaScript, Excel, or AmiBroker scripts that connect to Max Algos to receive ticks.
...
If every part of Max Algos opened its own websocket directly to the broker, you'd burn through that budget instantly — the GUI would fight your live algo, and your data-capture script would fight both.

So Max Algos runs **one connection to the broker per session**, demultiplexes it locally, and lets every consumer subscribe to whatever they need without anyone knowing about anyone else.
...
Max Algos's websocket layer is built from four distinct pieces. Each has one job. Understanding them separately makes the rest of this page (and most user questions) much easier.

### 1. The Broker Websocket Adapter

**Job:** speak the broker's proprietary websocket protocol and translate everything into a standard Max Algos tick format.

Every broker has its own websocket — different login flow, different message shape, different way of expressing market depth, different reconnect rules. The adapter (`broker/<broker_name>/streaming/<broker>_adapter.py`) is the *only* code in Max Algos that knows about those quirks. Once a tick has been parsed and normalised, it leaves the adapter looking the same regardless of which broker it came from.
...
- Authenticates them with their Max Algos API key.
...
When you open Max Algos in your browser and look at a live chart, a quote panel, or any ticking number, the browser is connected to the **Websocket Proxy on port 8765** behind the scenes. Each panel asks for the symbols it needs (e.g. NIFTY, BANKNIFTY) and unsubscribes automatically when you close the panel.
...
- **Tab pause.** If you switch away from the Max Algos tab for more than 5 seconds, the UI automatically pauses its subscriptions to save bandwidth. It resumes when you come back.
...
Max Algos exposes the Websocket Proxy as a public WSS endpoint at `ws://<host>:8765`. Any client in any language can connect, authenticate with an Max Algos API key, and subscribe to symbols.
...
External clients talk WSS to port 8765. The browser does too. But internally, Max Algos's own Python code (Flow, watchlists, dashboards, RMS) doesn't speak WSS to itself — that would be wasteful. It uses an in-process facade called **`MarketDataService`** (`services/market_data_service.py`).
...
2. Behind the scenes, Max Algos also subscribes that symbol on the websocket if it isn't already subscribed (this is what the proxy's deduplication handles).
...
- **Use `MarketDataService` directly** if your code runs inside the same Max Algos process. It's the lightest path: one function call, no serialisation, automatic safety gates.
...
This is the question people ask the most. Not every Max Algos feature uses the websocket — many of them poll the broker's REST API instead. **Knowing which is which lets you reason about your websocket budget.**
...
A **broker websocket** is the live TCP connection from Max Algos to your broker's market-data servers. Each broker imposes a cap on how many symbols a single such connection can carry — typically 1000 symbols, sometimes 3000 (Zerodha), occasionally less.
...
| Total cap (Max Algos side)        | 3000    | derived                            |
| LTP throttle                     | 50 ms   | hard-coded                         |
| Connection pooling enabled       | yes     | `ENABLE_CONNECTION_POOLING=true`    |

The actual ceiling is the **lower** of (Max Algos's cap) and (your broker's per-login cap):
...
- It doesn't share connections *across users*. Pooling is per-(broker, user). Each Max Algos deployment is single-user, so in practice you have one pool.
...
Setting these correctly means Max Algos will reject "you've gone too far" cleanly instead of letting the broker cut you off mid-trade.
...
Not built in. You'd write your own client against port 8765 (or, if you're comfortable, against the internal ZMQ bus) and persist the data yourself. There's no parquet/CSV recorder in Max Algos today.
...
**Q: My broker says I can have 2 websocket connections. Does Max Algos use both?**
...
Max Algos also uses **Flask-SocketIO** for control-plane events (order placed/filled/rejected, analyzer updates, master contract loaded, etc.). That is a separate websocket from the market-data Websocket Proxy described here. Don't confuse the two:
