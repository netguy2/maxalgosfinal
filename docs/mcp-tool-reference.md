# Max Algos MCP — Tool Reference & Prompt Examples
...
- **Default strategy tag**: `python mcp` — every MCP-triggered order is tagged so you can filter MCP activity in Max Algos logs and the Analyzer. Override by saying *"use strategy 'my scalper'"* in the prompt.
...
Returns the full standardized Max Algos index symbol list (57 NSE + 40 BSE), rolled out uniformly across every supported broker.
...
### `get_maxalgos_version`

**Prompt:** *"What version of the maxalgos library is running?"*
...
- Start in **analyzer mode** (`analyzer_toggle True`) while you get comfortable — orders look real but never leave Max Algos.
...
- For live trading, set up the Max Algos Telegram bot and ask the assistant to *"send a Telegram alert after every order fill"* — you get a realtime feed without staring at the screen.
...
- [Max Algos Symbol Format](./userguide/symbol-format/README.md) — how equity / future / option symbols are constructed
