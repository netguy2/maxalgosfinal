# ⚙️ How to Integrate Any XTS API-Supported Broker in Max Algos (5-Minute Setup)

Max Algos already supports XTS API through the `compositedge` plugin. Any broker using XTS (like IIFL, Nirmal Bang, Anand Rathi, Jainam, 5paisa, etc.) can be added with **zero code duplication**—just a few config updates.
...
│
└── mapping/
    ├── order_data.py           # Max Algos → XTS order translation
    └── transform_data.py       # XTS → Max Algos data formatting
```
...
```json
{
  ...
  "Description": "CompositedgeMax Algos Plugin",
  "Version": "1.0",
  "Author": "Kalaivani",
...
Thanks to Max Algos’s modular and broker-agnostic design:
