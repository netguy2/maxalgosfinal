...
|------------|-------------|--------------------|---------------|
| apikey | Max Algos API key (string) | Mandatory | - |
...
| symbol | Trading symbol in Max Algos format (string) | Mandatory | - |
...
- **OCO modify atomicity**: Max Algos aims to update both legs of an OCO atomically; some brokers expose a per-leg modify under the hood and may, in rare failure cases, leave the OCO in a half-modified state — re-issue the modify or cancel and re-place if the response indicates partial failure.
