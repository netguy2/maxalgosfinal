...
Max Algos runs one paired WhatsApp Web session per install. Once paired
...
- The paired session blob (~300 KB of Signal Protocol private keys) is
  ...
  `maxalgos.db` file **and** the `.env` secrets can impersonate the
...
`@check_session_validity`, so only the logged-in Max Algos admin can
invoke them.
...
```
/link <YOUR_API_KEY>        # one-time, links this WhatsApp number to your Max Algos user
...
Each command runs against the Max Algos SDK using the linked user's API
