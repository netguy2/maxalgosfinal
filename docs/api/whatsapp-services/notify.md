...
### Send to a linked Max Algos user
...
|-----------|------|-------------|
| `apikey` | string | Max Algos API key. **Mandatory.** |
| `self` | boolean | If `true`, send to the paired device's own number. |
| `username` | string | Max Algos username — resolves through the linked-users table. |
...
- Image / document paths are read from the Max Algos server's filesystem,
...
- **Attachment path allowlist.** For security, only paths inside the
  ...
  accepted. The default (when unset) is `<maxalgos>/db/attachments/` only.
