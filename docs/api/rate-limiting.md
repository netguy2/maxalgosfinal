...
To protect Max Algos from abuse and ensure fair usage, rate limits are enforced at both login and API levels. These limits are configurable via the `.env` file and apply globally per IP address.

## UI Login Rate Limits

Max Algos applies two login-specific rate limits:
...
Max Algos implements differentiated rate limiting for various types of operations:
...
Max Algos uses the **moving-window** strategy for rate limiting, which provides more accurate rate limiting compared to fixed-window approaches.
