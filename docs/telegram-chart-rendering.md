...
maxalgos on Docker, Ubuntu, Debian, RHEL/CentOS, Fedora, or Arch.
...
1. Parse `symbol`, `exchange`, `chart_type`, `interval`, and `days` from the
   user's message.
2. Fetch OHLCV history via the Max Algos Python SDK
...
maxalgos pins `kaleido==1.2.0` in `pyproject.toml`. Kaleido had a major
architectural change between v0.2.x and v1.x, and the switchover is the
single most common reason new maxalgos installs see `/chart` silently fail:
...
```bash
docker exec maxalgos-web /usr/bin/chromium --version
...
```bash
# Docker
docker exec maxalgos-web /app/.venv/bin/python -c '
...
rest of maxalgos still installs and everything except `/chart` works:
...
maxalgos container itself is what runs Chromium, and the container gets it
...
```bash
sudo systemctl restart maxalgos   # or whichever unit name install.sh created
```
...
3. **`loop.run_in_executor(None, ...)` does not save you under eventlet.**
   ...
   maxalgos runs gunicorn with `--worker-class eventlet -w 1` — both in Docker
...
1. **Container/host logs first.**
   ```bash
   # Docker
   docker logs maxalgos-web --since 5m | grep -E "telegram_bot_service|chart|kaleido|chromium"

   # Bare metal
   sudo journalctl -u maxalgos --since "5 minutes ago" | grep -E "telegram_bot_service|chart|kaleido|chromium"
   ```
...
- **Moving off eventlet.** The gunicorn maintainers have deprecated the
  ...
  backward compatibility with how maxalgos integrates Flask-SocketIO). If
  maxalgos ever migrates to `gthread`, `gevent`, or `uvicorn`, the
