...
When deploying Max Algos using `install.sh` on Ubuntu server, most security measures are **automatically configured**. This document covers what's already done and what remains for you.
...
```bash
# View Max Algos logs
sudo journalctl -u maxalgos-yourdomain-broker -f
...
# Update Max Algos dependencies
cd /var/python/maxalgos-flask/*/maxalgos
...
```bash
# Check logs
sudo journalctl -u maxalgos-yourdomain-broker -n 50
...
sudo chown -R www-data:www-data /var/python/maxalgos-flask/*/
sudo chmod -R 755 /var/python/maxalgos-flask/*/
```
...
4. **Review logs**:
   ```bash
   sudo journalctl -u maxalgos-* --since "24 hours ago"
