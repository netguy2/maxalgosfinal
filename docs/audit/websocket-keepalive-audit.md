...
| **Tier 5 — Transport-managed** (Socket.IO / engine.io / NATS handles its own heartbeat invisibly) | compositedge, fivepaisaxts, ibulls, iifl, jainamxts, rmoney, groww | Max Algos has zero visibility into liveness. |
...
- (Fyers also fixes today's commit `5eb7baaa` that was scrambling HSM↔Max Algos symbol mappings — see issue #1093.)
...
- All use `python-socketio.Client`. No explicit `ping_interval`/`ping_timeout` — engine.io transport-level heartbeat handles it (opaque to Max Algos).
...
- **Gap:** no health-check thread. No data-timeout. If engine.io's internal heartbeat hangs, Max Algos learns about it only via `on_disconnect` callback.
...
- The WebSocket Proxy server (`websocket_proxy/server.py`) handles client-initiated `{"action":"ping"}` from SDKs and responds with `{"type":"pong",...}`. That's the *external* keep-alive between SDK clients and Max Algos, separate from this audit which covers the *internal* keep-alive between Max Algos and the brokers. Issue #1101 mentions both; this audit only covers the latter.
