...
The keepalive audit catalogs **what each broker does** for ping/heartbeat. This document identifies **which brokers are broken or fragile under Max Algos's actual self-hosted production workload**, ranked by impact, with concrete defects and a phased remediation plan.

### 1.1 The deployment reality

Max Algos is **self-hosted by individual traders** on their own server (per CLAUDE.md: "Single user per deployment — no multi-user, no privilege escalation. One user, one broker session per instance."). The realistic operational profile:
...
Max Algos runs on Windows, macOS, Docker, and Ubuntu+gunicorn+eventlet. Code that works in dev may break in production due to eventlet's stdlib monkey-patching.
...
- **Token refresh on broker side** — out of Max Algos's control.
