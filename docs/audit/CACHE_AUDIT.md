# Max Algos Cache Architecture Audit

**Date:** 2026-02-22
**Scope:** All in-memory caching, persistence, eviction, concurrency, fault tolerance, and security
**Codebase:** Max Algos (Flask + React 19 algorithmic trading platform)
...
- `db/maxalgos.db` — Auth, settings, strategies, symbols, users
...
| Volumes | `maxalgos_db` persists SQLite databases across container restarts |
...
| L4 | **Static Fernet encryption salt** | `auth_db.py:60` | `b"maxalgos_static_salt"` — functional but reduces KDF diversity | LOW |
