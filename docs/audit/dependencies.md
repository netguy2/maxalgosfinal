...
This assessment reviews third-party packages used by Max Algos for security considerations.
...
These packages historically have more vulnerabilities (not specific to Max Algos):
...
```bash
# Full audit (run from maxalgos directory)
...
Max Algos uses **well-maintained, trusted packages**:
