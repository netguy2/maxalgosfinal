...
Max Algos uses WebSockets for real-time market data streaming. When deployed via `install.sh`, WebSocket traffic is secured through Nginx reverse proxy with TLS.
...
3. **Check logs**:
   ```bash
   sudo journalctl -u maxalgos-* | grep -i websocket
   ```
