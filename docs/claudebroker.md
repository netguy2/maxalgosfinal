# claudebroker.md — Guide to Integrating a New Broker into Max Algos
...
> Golden rule: Max Algos has a **common symbol format, common API, and common
...
**Inspect the LIVE `db/maxalgos.db` `symtoken` table of a working broker** (e.g.
...
broker/<name>/
  ...
  mapping/__init__.py
  mapping/transform_data.py           # Max Algos order -> broker payload + enum maps
  mapping/order_data.py               # broker JSON -> Max Algos normalized rows
...
- **Symbol construction** (Max Algos common format):
  ...
  - INDEX: the Max Algos index symbol (`NIFTY`, `BANKNIFTY`, `SENSEX`), `brsymbol`
...
(Max Algos is single-user), and raise a clear error if neither exists — never
...
- `__init__(self, auth_token)` sets `self.timeframe_map` (Max Algos interval →
...
If the broker uses one `INDEX` pseudo-exchange, translate Max Algos
...
(Max Algos symbol → uppercased brsymbol → raw brsymbol), treat 400 as
...
The Max Algos `/api/v1/history` endpoint itself is rate-limited by
...
`map_*` functions reverse broker codes to Max Algos (product/side/order-type) and
convert broker symbols to Max Algos via `get_oa_symbol(brsymbol, exchange)`. The
service calls `map_*` first (mutates in place) then `transform_*`.

### Order placement mapping (`mapping/transform_data.py`)
- product: Max Algos `CNC/NRML/MIS` → broker codes
...
- [ ] Connect a working broker (e.g. zerodha) once and inspect `db/maxalgos.db` `symtoken`
