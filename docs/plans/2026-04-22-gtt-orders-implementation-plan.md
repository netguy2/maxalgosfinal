...
| 0.8 | ID naming | Max Algos uses `trigger_id` in JSON (broker-neutral); sandbox table column `gtt_id` (internal). |
...
1. **Zerodha broker module**
   - (N) `broker/zerodha/mapping/gtt_data.py` — `transform_place_gtt(data)` builds Kite's `{type, condition, orders}` from Max Algos `{trigger_type, legs, symbol, exchange, last_price}`. Analogous `transform_modify_gtt`.
...
4. **Semi-auto / Action Center integration**
   ...
   **b. Queue payload shape**
   ...
   ```
   `_meta.maxalgos_mode` is the mode-binding value re-validated at approval time (see Phase 0.11).
   ...
   **d. Approval dispatch**
   - (E) `database/action_center_db.py:approve_pending_order` — the existing dispatch lookup must learn about `placegttorder`:
     ...
     ```
   - Before dispatch: re-read the user's current mode via `database.settings_db.get_analyze_mode()`. If it differs from `order_data["_meta"]["maxalgos_mode"]`, return HTTP 409 with `{"error": "mode_mismatch", "queued_in": "live", "current_mode": "analyze"}` and leave the row in `pending` (operator can cancel and re-queue). Phase 0.11 rationale.
   ...
   **e. Audit trail**
   - On queue: `order_logs` row with `api_type = "placegttorder"`, `status = "pending_approval"`, `result_data = {"queue_id": ..., "maxalgos_mode": ...}`.
...
5. **Playground collection**
   - (N) `collections/maxalgos/IN_stock/orders/place_gtt_order.bru` — single-leg + OCO example bodies.
   - (N) `collections/maxalgos/IN_stock/orders/modify_gtt_order.bru`
   - (N) `collections/maxalgos/IN_stock/orders/cancel_gtt_order.bru`
   - (N) `collections/maxalgos/IN_stock/orders/gtt_orderbook.bru`
   - (E) `blueprints/playground.py` — `categorize_endpoint()` routes these to `"orders"`.

### Files touched

- **New:** 2 in `broker/zerodha/`, 4 in `services/`, 4 in `restx_api/`, 4 in `collections/maxalgos/IN_stock/orders/`
...
15. (E) `src/maxalgos/orders.py` (or equivalent client module) — add `placegttorder`, `modifygttorder`, `cancelgttorder`, `gttorderbook` methods with docstrings mirroring `placeorder` style.
16. Pre-request leg-count validation.
17. SDK version bump (minor).

### Task Group E — Flow editor (4-place rule)

18. (E) `services/flow_maxalgos_client.py` — methods `place_gtt`, `modify_gtt`, `cancel_gtt`, `get_gtt_orderbook`.
...
- `pip install maxalgos && python -c "from maxalgos import api; api(...).placegttorder(...)"` — SDK smoke test passes.
...
GTT has feature parity with regular orders across every existing Max Algos surface.
...
| Broker GTT expires silently; Max Algos shows stale `active` | Nightly reconciliation job pulls `gttorderbook` from broker in live mode, diffs against last-seen state, logs & emits events for state changes. (Optional Phase 6 enhancement.) | 6+ |
| Semi-auto queue collides with GTT triggers | Disallow semi-auto for modify/cancel (Phase 0.3). | 2 |
| Operator toggles mode between queue and approve in Action Center | `_meta.maxalgos_mode` captured at queue time, re-validated at approval (Phase 0.11). 409 + leave row pending if changed; operator cancels and re-queues. | 0.11, 2 |
...
| Services | `services/{place,modify,cancel}_gtt_order_service.py`, `services/gtt_orderbook_service.py` | `services/sandbox_service.py`, `services/order_router_service.py`, `services/telegram_alert_service.py`, `services/flow_maxalgos_client.py`, `services/flow_executor_service.py` |
...
| Playground | 4 `.bru` files under `collections/maxalgos/IN_stock/orders/` | `blueprints/playground.py` |
| SDK | — | `src/maxalgos/orders.py` (or client module) |
