...
- **US-2**: As a trader locked out of the dashboard (laptop dead, phone only), I send `/killswitch on` to the Max Algos Telegram bot and achieve the same outcome.
...
Every order path in Max Algos converges at the service-function level, *above* the branch between live broker and sandbox. Put the gate there — one check, both modes covered.
...
Because the flag is persisted in `db/maxalgos.db`, a Gunicorn worker crash (or host reboot) mid-activation leaves the flag as whatever was last committed. On process startup, `is_kill_switch_active()` re-reads from DB. User sees the active state correctly on next page load.
...
- No API key has *more* power than the single user's session — Max Algos's single-user deployment model means we don't need role-based access control for v1.

---

## 18. Telegram Security Considerations

The Telegram bot already operates on the assumption that the user's chat ID is registered in their Max Algos instance. The kill switch commands inherit that trust model:
