...
All tables stored in `db/maxalgos.db` for persistence across restarts.
...
The risk engine, OrderStatusPoller, Flask request threads (webhooks, dashboard API, manual close), and APScheduler all write to `db/maxalgos.db` concurrently. SQLite requires explicit configuration for safe multi-threaded access.
...
Migration follows the existing Max Algos pattern: standalone idempotent scripts in `upgrade/`, registered in `migrate_all.py`, run via `uv run upgrade/migrate_all.py`.
...
#!/usr/bin/env python
"""
Strategy Risk Management Migration Script for Max Algos
...
def upgrade():
    ...
    engine = get_main_db_engine()         # db/maxalgos.db
