...
The Max Algos WhatsApp Bot connects your Max Algos install to a WhatsApp account that you control. It does two things:
...
Unlike Telegram, WhatsApp has no separate "bot account" concept. The Max Algos server runs as a **linked device** on your personal WhatsApp account — the same way WhatsApp Web does. You pair once with a QR scan and the encrypted session lives in `maxalgos.db`.
...
#### 1. Pair Your WhatsApp Device in Max Algos

1. Log in to Max Algos.
...
> **Note:** WhatsApp permits a maximum of four (currently) linked devices per account. If you're already at the cap, remove an unused linked device on your phone before pairing Max Algos.

#### 2. (Optional) Generate an Max Algos API Key

Slash-command queries (`/orderbook`, `/positions`, etc.) execute against the Max Algos SDK using **your own** Max Algos API key, looked up server-side. If you haven't generated one yet:
...
When unset, the default allowlist is `<maxalgos>/db/attachments/` only. Paths containing `..`, paths under sensitive system trees (`/etc`, `/proc`, `/sys`, `/root`, `/var/log`, `C:\Windows`, `C:\Users\Default`), and paths that resolve outside the allowlist are always rejected with `400 image_path is not allowed`.

### How to Send Commands

Commands work differently from Telegram. WhatsApp has no separate bot identity — the bot **is** your own WhatsApp account, running as a linked device on the Max Algos server.
...
4. The linked device on the Max Algos server sees the message as `is_from_me=True`, dispatches it, runs the matching SDK call, and replies in the same chat.
...
* `/mode` — Show whether the Max Algos instance is in `live` or `analyze` (sandbox) mode
...
The bot automatically sends a WhatsApp message to the paired device's own number for every order-related API activity. No additional commands are needed — alerts are sent automatically when orders flow through the Max Algos API.
...
1. WhatsApp device must be paired in Max Algos (`/whatsapp` page in the web UI).
2. The Max Algos server must have rebooted at least once since pairing OR the bot must be currently connected (auto-reconnects on every boot from the encrypted session blob).
3. Orders must be placed through the Max Algos API (REST `/api/v1/*`, the Python SDK, or any tool that ultimately hits the API).

### Sending Messages via API

In addition to the automatic order alerts, you can send arbitrary WhatsApp messages from your own code through the Max Algos REST API or the Python SDK.
...
{
  "status": "error",
  "message": "WhatsApp is not paired or not connected. Pair the device first from the /whatsapp page in Max Algos before sending."
}
```
...
| `username` | string | Max Algos username — resolves via the linked-users table |
...
The QR-scan / pair-code flow lives behind `POST /whatsapp/pair`, which is protected by the Flask **session cookie** (`@check_session_validity`). It is deliberately **not** exposed in the public REST API. An Max Algos API key alone cannot:
...
The paired-device session blob (~300 KB of Signal Protocol private keys, identity material, and registration info from wars/whatsapp-rust) is **Fernet-encrypted** before writing to `maxalgos.db`:
...
| --- | --- |
| `maxalgos.db` only | Useless — ciphertext without key |
| `.env` only | Useless — no ciphertext to decrypt |
| `maxalgos.db` + `.env` | Full impersonation of the linked WhatsApp device |

Keep both off public hosts, off public git, and off any backup destination that mixes the two.

#### Owner-Only Bot Commands

Slash-commands are gated by WhatsApp's own multi-device cryptography. When the operator types `/orderbook` from their primary phone, WhatsApp marks the message as `is_from_me=True` when mirroring it to the linked Max Algos device. Random contacts who message the operator's number arrive with `is_from_me=False`. The bot's handler unconditionally drops the latter — there is no allowlist to maintain or `/link` flow to manage.
...
The bot uses SQLAlchemy ORM with the following tables in `maxalgos.db`:
...
Linked recipient phone numbers and their Max Algos username/api_key mapping. Unused in the standard single-user deployment.
...
2. Check that you have an Max Algos API key generated at `/apikey` (slash-commands need it for SDK calls).
...
2. If the badge says **Connected** but sends still fail, restart the Max Algos server. Auto-reconnect rebuilds the session from the encrypted blob.
...
3. Verify the Max Algos owner has an API key on file (`/apikey` page).
...
1. Move the file to `<maxalgos>/db/attachments/` (the default allowlist), or
2. Add the file's directory to `WHATSAPP_ATTACHMENT_ROOTS` in `.env` and restart Max Algos.
...
* `DATABASE_URL` — Main Max Algos database (WhatsApp tables live here)
...
* `HOST_SERVER` — Max Algos server URL the bot uses for SDK loopback calls (defaults to `http://127.0.0.1:5000`)
* `WHATSAPP_ATTACHMENT_ROOTS` — Optional comma-separated allowlist for media paths. Defaults to `<maxalgos>/db/attachments/` only.
...
Max Algos's WhatsApp integration uses `wars`, an unofficial WhatsApp client. Unofficial clients can get the linked device unlinked, or in rare cases the entire account banned, by Meta's automation. The dominant trigger is send volume and pattern, not the client itself:

* **Low risk (typical Max Algos usage)** — A handful of self-send order alerts per day, occasional `/status` replies, sending charts/reports to a small circle of subscribers. Indistinguishable from a person using WhatsApp normally; well under Meta's automated thresholds.
...
1. Check the server logs (`log/maxalgos_YYYY-MM-DD.log` + `log/errors.jsonl`)
...
```
GET https://docs.maxalgos.in/trading-platform/whatsapp.md?ask=<question>
```
