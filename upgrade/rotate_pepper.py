#!/usr/bin/env python3
"""
Max Algos PEPPER Rotation Migration (DESTRUCTIVE)
=================================================
...
Pre-flight:
  1. Stop Max Algos (kill the running process / systemctl stop maxalgos).
  2. Back up db/maxalgos.db (the script also creates a backup, but
     belt-and-braces).
...
Columns rotated:
  auth_db Fernet (PBKDF2-SHA256, salt=b"maxalgos_static_salt"):
    - auth.auth
    - auth.feed_token
...
def _telegram_db_fernet(pepper: str) -> Fernet:
    ...
    salt = os.getenv("TELEGRAM_KEY_SALT", "telegram-maxalgos-salt").encode()
    kdf = PBKDF2HMAC(
        algorithm=hashes.SHA256(),
        length=32,
        salt=salt,
...
def _settings_db_fernet(pepper: str) -> Fernet:
    ...
    salt = os.getenv("SMTP_KEY_SALT", "smtp-maxalgos-salt").encode()
    kdf = PBKDF2HMAC(
        algorithm=hashes.SHA256(),
        length=32,
        salt=salt,
...
def main():
    ...
    print()
    print("=" * 72)
    print("  Max Algos PEPPER Rotation Migration")
    print("=" * 72)
    ...
    if not args.dry_run:
        ...
        backup_path = os.path.join(backup_dir, f"maxalgos.db.before-rotate-pepper-{ts}")
    ...
    print()
    print("  Next steps:")
    print("    1. Restart Max Algos: uv run app.py  (or systemctl restart …)")
