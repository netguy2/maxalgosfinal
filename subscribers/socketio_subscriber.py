...
def _room_for_event(event) -> str | None:
    ...
    deployment order placement) instead populate `event.username`
    directly (resolved from the strategy/deployment owner) -- fall back to
    that so those orders' status still reaches the owning user rather than
    being silently dropped. Only if NEITHER is set does this return None
    and the caller skips the emit.
    """
    api_key = getattr(event, "api_key", None)
    if not api_key:
        username = getattr(event, "username", None)
        return f"user_{username}" if username else None
    try:
