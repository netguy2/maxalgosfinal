...
Max Algos is a production-ready algorithmic trading platform built with Flask (backend) and React 19 (frontend). It is **four products in one self-hosted instance**, all sharing a single broker session and WebSocket feed:
...
- **Multi-user, each with independent broker connections** — the platform supports multiple user accounts, and each user registers their own broker app credentials (`UserBrokerCredential`, `database/user_db.py`) and logs into their own broker account independently (session token stored per-user in `Auth`/`AuthBrokerSession`, `database/auth_db.py`). `broker/*/api/auth_api.py` modules call `os.getenv("BROKER_API_KEY")` etc., but `os.getenv` is globally monkeypatched (`utils/env_patch.py`, installed in `app.py`) to resolve those keys per requesting user — from an active `broker_credential_context` override or the current session's user/broker — falling back to the literal `.env` value only when no per-user credential row exists (e.g. first-run/admin default). Order placement resolves the correct user's own broker token via their personal API key (`services/place_order_service.py` → `get_auth_token_broker` → `verify_api_key` → that user's `Auth` row), so User A's and User B's orders go to their own independent real broker accounts. The WebSocket proxy's `broker_adapters`/`user_broker_mapping` registries in `websocket_proxy/server.py` are similarly keyed per `user_id`. **The single-worker Gunicorn constraint below (in-process SocketIO state) is the real ceiling on concurrent-user scale, not credential isolation.**
...
- **SEBI static IP mandate** (effective April 1, 2026): All transactional API orders require broker-side static IP whitelisting. Delta Exchange (crypto) also enforces this. Stolen broker credentials CANNOT be used from an attacker's machine — the broker rejects requests from non-registered IPs. However, attacks routed THROUGH the Max Algos server (which has the registered IP) are still viable.
...
- `websocket_proxy/` - Unified WebSocket server (default port 8785, override via `WEBSOCKET_PORT`)

### Database Architecture

Max Algos uses **6 separate databases** for isolation:

- `db/maxalgos.db` - Main database (users, orders, positions, settings)
...
5. `mapping/` - Transform Max Algos format ↔ broker format
...
1. **Broker WebSocket Adapters** (`broker/*/streaming/`): Each broker has a WebSocket adapter that connects to the broker's proprietary feed and normalizes data into Max Algos's internal format. Connection pooling is per-broker: `MAX_SYMBOLS_PER_WEBSOCKET` (default: 1000) x `MAX_WEBSOCKET_CONNECTIONS` (default: 3) = 3000 symbols max.

2. **ZeroMQ Message Bus** (default port 5575, override via `ZMQ_PORT`): Broker adapters publish normalized tick data to a ZeroMQ PUB socket. This decouples the broker feed from client delivery — the broker adapter runs independently and never blocks on slow clients.

3. **Unified WebSocket Proxy Server** (`websocket_proxy/server.py`, default port 8785, override via `WEBSOCKET_PORT`): Subscribes to ZeroMQ, manages client WebSocket connections, handles symbol subscriptions/unsubscriptions, and delivers filtered ticks to each connected client. Includes per-symbol throttling to prevent flooding slow clients.
...
- **Production servers** (clients running Max Algos on Ubuntu/Docker/EC2) **do not need Node.js or npm.** A plain `git pull` from `main` already brings the latest UI artifacts. This is the canonical upgrade path documented at https://docs.maxalgos.in/installation-guidelines/getting-started/upgrade.
...
Max Algos uses an event-driven architecture where state changes are broadcast to the UI in real-time:
...
2. **WebSocket Proxy**: Unified market data streaming (default port 8785, override via `WEBSOCKET_PORT`) — see WebSocket Architecture above.

3. **ZeroMQ PUB/SUB**: Internal message bus between broker adapters and WebSocket proxy (default port 5575, override via `ZMQ_PORT`). Also used for cache invalidation events across modules.
...
This is the Max Algos platform itself. Source of truth: `utils/version.py`. Bumping touches **two files** and regenerates the lockfile — **never** the requirements files.
...
### 2. Max Algos Python SDK pin (e.g. `maxalgos==1.0.49`)

This is a **separate** client library published on PyPI ([`maxalgos`](https://pypi.org/project/maxalgos/)) that the platform uses internally. It has its own release cycle. Bumping the SDK pin touches the dependency lists, **not** `utils/version.py`:

1. `pyproject.toml` — update `maxalgos==X.Y.Z` in the `dependencies` list
2. `requirements.txt` — update the `maxalgos==X.Y.Z` line
3. `requirements-nginx.txt` — update the `maxalgos==X.Y.Z` line
...
**Rule of thumb:** if you are releasing Max Algos, bump #1. If a new SDK is on PyPI with a fix you need, bump #2. They are unrelated.
...
Max Algos uses a standardized symbol format across all 30+ brokers. Broker-specific symbols are mapped via `broker/*/mapping/` modules and stored in the `SymToken` table.
...
**Database schema (`SymToken`):** `symbol` (Max Algos format), `brsymbol` (broker format), `exchange`, `brexchange`, `token` (broker instrument token), `expiry`, `strike`, `lotsize`, `instrumenttype`, `tick_size`
...
2. **File** (if `LOG_TO_FILE=True`): Daily-rotated text logs in `log/maxalgos_YYYY-MM-DD.log`, retained for `LOG_RETENTION` days.
...
4. Check firewall settings for port 8785 (default; override via WEBSOCKET_PORT)
