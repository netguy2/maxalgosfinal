@echo off
REM Max Algos Docker Build and Deployment Script for Windows
REM This script builds and deploys Max Algos with numba/llvmlite support

setlocal enabledelayedexpansion

set IMAGE_NAME=maxalgos
set IMAGE_TAG=latest
set CONTAINER_NAME=maxalgos-web

echo.
echo ========================================
echo Max Algos Docker Build ^& Deploy
echo ========================================
echo.
...
REM Start container
echo.
echo [5/8] Starting Max Algos container...
docker-compose up -d
if %ERRORLEVEL% NEQ 0 (
...
docker-compose exec -T maxalgos python -c "import numba; import llvmlite; print('SUCCESS')" 2>nul | findstr "SUCCESS" >nul
if %ERRORLEVEL% EQU 0 (
...
docker-compose exec -T maxalgos python -c "from numba import jit; import numpy as np; jit(nopython=True)(lambda x: x*2)(np.array([1,2,3])); print('SUCCESS')" 2>nul | findstr "SUCCESS" >nul
if %ERRORLEVEL% EQU 0 (
...
echo   Shell access:     docker-compose exec maxalgos bash
echo.
