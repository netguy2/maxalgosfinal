...
## Overview
A complete web-based strategy hosting and scheduling system for Max Algos, accessible at `/python`.
...
2. The system is already integrated into Max Algos. Access it at:
```
...
# EXCHANGE prefers MAX_ALGOS_STRATEGY_EXCHANGE (set by /python from your
...
# HOST_SERVER is the canonical name in Max Algos's .env (inherited).
# MAX_ALGOS_HOST is a fallback alias the platform setdefaults to 127.0.0.1:5000.
API_HOST = os.getenv('HOST_SERVER') or os.getenv('MAX_ALGOS_HOST', 'http://127.0.0.1:5000')
...
def main():
    ...
    while True:
        try:
            ...
            # 3. Generate signals
            # 4. Place orders via Max Algos API
...
> **Reading `MAX_ALGOS_STRATEGY_EXCHANGE` is optional but strongly recommended.** If your script hardcodes `exchange = "NSE"`, the host will still gate it correctly per its config (e.g. host runs your script during MCX evening session because `exchange=MCX`), but your `client.placeorder(exchange="NSE", ...)` calls will still send NSE orders — and the broker will reject them. Wiring the env var keeps host calendar and script orders aligned.
...
- `MAX_ALGOS_STRATEGY_EXCHANGE` — the exchange picked at upload/edit time (`NSE` / `BSE` / `NFO` / `BFO` / `MCX` / `BCD` / `CDS` / `CRYPTO`). Read this in your script so its trading calls match the calendar the host is gating against
- `MAX_ALGOS_API_KEY` — decrypted API key for this user
- `MAX_ALGOS_HOST` — Max Algos host URL, **set with `setdefault` to `http://127.0.0.1:5000`**. If `MAX_ALGOS_HOST` is already present in `.env` (it usually isn't — `.env` uses `HOST_SERVER`), that value is kept. Treat this as a convenience fallback only

### Inherited from `.env`

Strategies are launched with `os.environ.copy()`, so they inherit **every** variable from Max Algos's `.env`. The relevant ones for connecting to Max Algos:
...
> ```python
> # REST: prefer HOST_SERVER (from .env), fall back to injected MAX_ALGOS_HOST, then a literal
> API_HOST = os.getenv("HOST_SERVER") or os.getenv("MAX_ALGOS_HOST", "http://127.0.0.1:5000")
...
> ```
>
> Note: there is **no `HOST_URL` variable** anywhere in Max Algos. Only `HOST_SERVER` (REST), `MAX_ALGOS_HOST` (injected fallback alias), and `WEBSOCKET_URL` (WS).
...
Example of integrating with Max Algos API in your strategy:
...
class Max AlgosAPI:
    def __init__(self, host=None, api_key=None):
        # HOST_SERVER (from .env) wins; MAX_ALGOS_HOST is the platform-injected fallback.
        self.host = host or os.getenv('HOST_SERVER') or os.getenv('MAX_ALGOS_HOST', 'http://127.0.0.1:5000')
        self.api_key = api_key or os.getenv('MAX_ALGOS_API_KEY', '')
...
8. **Orders rejected with "market closed" while host says strategy is running**: your script's hardcoded `exchange="NSE"` doesn't match the host's `exchange="MCX"`. Read `MAX_ALGOS_STRATEGY_EXCHANGE` in your script (see Strategy Template)
