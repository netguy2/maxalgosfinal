...
#     Without these, any Kaleido-based static chart export silently fails
#     inside Docker.
#
#     The retry loop (up to 3 attempts) guards against transient Debian mirror
#     "connection reset by peer" errors that occasionally drop individual .deb
#     downloads mid-stream. --fix-missing re-fetches only the failed packages
#     rather than restarting the entire download from scratch.
RUN set -eux; \
    for attempt in 1 2 3; do \
        ...
        apt-get install -y --no-install-recommends \
            ...
            fonts-liberation \
        && break \
        || { echo "apt-get attempt $attempt failed, retrying..."; apt-get install -y --fix-missing --no-install-recommends \
            tzdata curl libopenblas0 libgomp1 libgfortran5 chromium fonts-liberation && break; }; \
    done && \
