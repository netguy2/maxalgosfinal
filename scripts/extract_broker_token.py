"""
Extract the broker access token from this Max Algos deployment's own database.

Intended for the OWNER of a self-hosted Max Algos instance who wants to call
broker-native endpoints Max Algos does not proxy (e.g., Upstox option-chain
Greeks). This script reuses Max Algos's existing Fernet decryption — it does
NOT re-implement crypto.

PRECONDITIONS:
  - Run inside the Max Algos venv:    uv run python scripts/extract_broker_token.py
  - .env must contain the same API_KEY_PEPPER used to encrypt the row
  - db/maxalgos.db must contain an active Auth row (i.e. you've logged in today
    via the Max Algos UI — Indian broker tokens expire daily ~3:00 AM IST)
...
SECURITY NOTES (read before using):
  ...
  - Calls you make directly against the broker bypass Max Algos's traffic logs,
  ...
  - Prefer building the missing feature inside Max Algos over scripting against
    a leaked token.
...
# Make sure we can import Max Algos's modules from repo root.
...
def _banner(stream) -> None:
    ...
    stream.write("  - Direct broker calls bypass Max Algos's audit & rate limits\n")
    stream.write("=" * 72 + "\n")
...
def main(as_json: bool = False) -> int:
    ...
    if not rows:
        sys.stderr.write(
            "No auth rows in db/maxalgos.db. Log in via the Max Algos UI to "
