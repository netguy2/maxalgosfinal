# Max Algos MCP Server

This is a Model Context Protocol (MCP) server that provides trading and market data functionality through the Max Algos platform. It enables AI assistants to execute trades, manage positions, and retrieve market data directly from supported brokers.
...
### 1. Max Algos Server Setup

Ensure your Max Algos server is running and properly configured:

1. **Start Max Algos Server**: Your Max Algos server should be running (e.g., on `http://127.0.0.1:5000`)
2. **Verify Connection**: Test that the server is accessible by visiting the web interface.
3. **Broker Authentication**: Ensure your broker credentials are properly configured in Max Algos.

### 2. API Key

To get your Max Algos API key:
1. Open your Max Algos web interface (e.g., `http://127.0.0.1:5000`)
...
```json
{
  "mcpServers": {
    "maxalgos": {
      "command": "D:\\maxalgos-mcp\\maxalgos\\.venv\\Scripts\\python.exe",
      "args": [
        "D:\\maxalgos-mcp\\maxalgos\\mcp\\mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
```json
{
  "mcpServers": {
    "maxalgos": {
      "command": "/Users/your_username/maxalgos/.venv/bin/python3",
      "args": [
        "/Users/your_username/maxalgos/mcp/mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
```json
{
  "mcpServers": {
    "maxalgos": {
      "command": "/home/your_username/maxalgos/.venv/bin/python3",
      "args": [
        "/home/your_username/maxalgos/mcp/mcpserver.py",
        "YOUR_API_KEY_HERE",
        "http://127.0.0.1:5000"
      ]
    }
...
- **Windows**: Replace `D:\\maxalgos-zerodha\\maxalgos` with your actual Max Algos installation path
...
1. **Connection Issues**: Verify Max Algos server is running on `http://127.0.0.1:5000`
...
4. **Order Failures**: Verify broker credentials in Max Algos are valid and active

## Support

For issues related to:
- **Max Algos Platform**: Visit the Max Algos documentation
