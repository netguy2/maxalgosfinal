...
#    client. The Flask app sets MAX_ALGOS_MCP_HTTP_BOOT=1 *before* importing
...
if os.environ.get("MAX_ALGOS_MCP_HTTP_BOOT") == "1":
    ...
    # Initialize Max Algos client with provided arguments
...
def init_for_http(api_key_value: str, host_value: str) -> None:
    ...
    determined the admin's API key and the local Max Algos loopback URL.
...
# Surfaced in Max Algos logs and analyzer views so MCP-driven trades are identifiable.
MCP_STRATEGY = "python mcp"

# Max Algos standardized index symbols (NSE_INDEX / BSE_INDEX) — rolled out across all brokers.
# Source: https://docs.maxalgos.in/symbol-format
NSE_INDEX_SYMBOLS = [
...
def get_historical_data(
    ...
    Args:
        ...
        source: 'api' (default) fetches from broker API. 'db' fetches from the local
                Max Algos Historify DuckDB store (1m/D stored, other intervals computed via SQL).
...
def get_index_symbols(exchange: str = "NSE") -> str:
    """
    Get the Max Algos-standardized index symbols for NSE or BSE.

    These are the common index names rolled out across all supported brokers via the
    Max Algos symbol standardization. Use exchange code 'NSE_INDEX' / 'BSE_INDEX' when
...
@mcp.tool()
def get_maxalgos_version() -> str:
    """Get the Max Algos library version."""
    try:
        import maxalgos

        return f"Max Algos version: {maxalgos.__version__}"
    except Exception as e:
...
def check_holiday(date: str, exchange: str | None = None) -> str:
    ...
    This calls the /api/v1/checkholiday endpoint directly (not yet in the maxalgos SDK).
