...
if [ -f "$ENV_FILE" ] && [ -r "$ENV_FILE" ] && [ -s "$ENV_FILE" ]; then
    echo "[Max Algos] Using existing .env file"
else
    echo "[Max Algos] No .env file found or file is empty. Checking for environment variables..."
    
    # Check if we're on Railway/Cloud (HOST_SERVER is the key indicator)
    if [ -n "$HOST_SERVER" ]; then
        echo "[Max Algos] Environment variables detected. Generating .env file..."
        ...
        if ! touch "$ENV_FILE" 2>/dev/null; then
            echo "[Max Algos] Cannot write to /app/.env, using /tmp/.env"
            ENV_FILE="/tmp/.env"
        fi
        ...
        cat > "$ENV_FILE" << EOF
# Max Algos Environment Configuration File
...
DATABASE_URL = '${DATABASE_URL:-sqlite:///db/maxalgos.db}'
...
WEBSOCKET_PORT = '${WEBSOCKET_PORT:-8785}'
...
EOF
        ...
        echo "[Max Algos] .env file generated at $ENV_FILE"
        echo "[Max Algos] Configuration: HOST_SERVER=${HOST_SERVER}"
        ...
        if [ "$ENV_FILE" = "/tmp/.env" ]; then
            ...
            echo "[Max Algos] Linked .env to /app/.env"
        fi
    else
        echo "============================================"
...
============================================================
[Max Algos] STARTUP BLOCKED — compromised APP_KEY detected
============================================================

Your .env contains the publicly-known sample APP_KEY (and
possibly API_KEY_PEPPER). Max Algos v2.0.0.6+ tries to
...
After this, Max Algos will start cleanly. Existing browser
sessions will need to log in again — APP_KEY rotation
invalidates session cookies, by design.

============================================================
[Max Algos] DO NOT regenerate API_KEY_PEPPER
============================================================
...
if [ -f "/app/upgrade/migrate_all.py" ]; then
    echo "[Max Algos] Running database migrations..."
    /app/.venv/bin/python /app/upgrade/migrate_all.py || echo "[Max Algos] Migration completed (some may have been skipped)"
else
    echo "[Max Algos] No migrations found, skipping..."
fi

# ============================================
# WEBSOCKET PROXY SERVER
# ============================================
echo "[Max Algos] Starting WebSocket proxy server on port ${WEBSOCKET_PORT:-8785}..."
/app/.venv/bin/python -m websocket_proxy.server &
WEBSOCKET_PID=$!
echo "[Max Algos] WebSocket proxy server started with PID $WEBSOCKET_PID"

# ============================================
# CLEANUP HANDLER
# ============================================
cleanup() {
    echo "[Max Algos] Shutting down..."
...
echo "[Max Algos] Starting application on port ${APP_PORT} with eventlet..."
