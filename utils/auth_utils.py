...
from utils.socket_scope import emit_to_user, user_scope
...
def load_existing_master_contract(broker):
    ...
    """
    # Sleep/yield control back to the Eventlet event loop immediately to allow
    # the parent Flask thread to return its JSON HTTP response before doing heavy work.
    time.sleep(0.1)
    try:
...
def async_master_contract_download(broker, username=None):
    ...
    `username` is the user whose broker connect (or manual re-download)
    triggered this. It exists purely to scope the completion notification:
    every broker's own master_contract_download() ends in a
    `master_contract_download` socketio emit, ~25 modules deep on this
    background thread with no Flask session of its own. Without a scope
    those emits broadcast platform-wide, so User A connecting their broker
    popped a "Master Contract: Successfully Downloaded" toast in every
    other logged-in user's browser. Setting user_scope here lets those
    emits resolve the right room without changing 25+ broker signatures;
    when it's None (startup/scheduled downloads with no acting user) the
    emit is dropped rather than broadcast. See utils/socket_scope.py.
    """
    # Sleep/yield control back to the Eventlet event loop immediately to allow
    # the parent Flask thread to return its JSON HTTP response before doing heavy work.
    time.sleep(0.1)
    ...
    with user_scope(username):
        return _run_master_contract_download(broker, start_time)


def _run_master_contract_download(broker, start_time):
    """Body of async_master_contract_download, run inside the caller's
    user_scope so nested broker emits are scoped correctly."""
...
def handle_auth_success(auth_token, user_session_key, broker, feed_token=None, user_id=None):
    ...
    """
    # Multi-broker data-broker rule: the FIRST broker a user connects
    # becomes (and stays) the data broker - it alone drives quotes, the
    # option chain, and the master contract sync. Connecting additional
    # brokers afterwards registers them as order-capable connections
    # (AuthBrokerSession below) WITHOUT silently stealing the data-broker
    # slot out from under whatever is already set. The data broker only
    # changes via an explicit "Set as data broker" action on the Broker
    # Management page (blueprints/broker_credentials.py::set_data_broker).
    # Computed up front (before touching session["broker"]) so the session's
    # data-broker pointer is never overwritten by an additional connect.
    from database.auth_db import get_auth_token_dbquery
    existing_data_broker_obj = get_auth_token_dbquery(user_session_key)
    is_additional_broker = bool(
        existing_data_broker_obj and existing_data_broker_obj.broker != broker
    )
    ...
    if not is_additional_broker:
        # Only the data broker (first connect, or re-auth of the existing
        # data broker) should own these session keys - an additional
        # broker's feed/user_id/session["broker"] must never overwrite the
        # data broker's, since session["broker"] is what app.py's os.getenv
        # patch falls back to for quote/order credential resolution.
        if feed_token:
    ...
    from database.auth_db import register_session, get_active_sessions, is_known_device
    device_info_str = request.headers.get("User-Agent", "")[:500]
    ip_str = get_real_ip()

    # Check before registration (existing rows = known devices; new row about to be added)
    _is_new_device = not is_known_device(user_session_key, device_info_str, ip_str)
    ...
    # room-scoped: this is THIS user's device list. Broadcasting it set every
    # other logged-in user's session-count badge (and the device list behind
    # it) to this user's, and leaked their device/IP metadata across accounts.
    active = get_active_sessions(user_session_key)
    emit_to_user("active_sessions_update", {
        "count": len(active),
        "sessions": active,
    }, user_session_key)

    # Fire a new-device email alert if this IP+UA combo hasn't been seen before.
    # Best-effort: wrapped in try/except so an email failure NEVER blocks login.
    if _is_new_device:
        try:
            from database.user_db import find_user_by_exact_username
            from utils.email_utils import send_new_device_login_email
            import datetime
            user_obj = find_user_by_exact_username(user_session_key)
    ...
    inserted_id = None
    if is_additional_broker:
        logger.info(
            f"{user_session_key} connected {broker} while {existing_data_broker_obj.broker} "
            "is already the data broker - registering as an additional order-capable broker, "
            "data broker unchanged."
        )
    else:
        # Store auth token in database (this is the data broker: either the
        # user's first connection, or they're re-authenticating the SAME
        # broker that's already the data broker).
        inserted_id = upsert_auth(
            user_session_key, auth_token, broker, feed_token=feed_token, user_id=user_id
        )

    # Additive multi-broker support: every successful broker connection also
    # registers as a "connected broker" (see database/auth_db.py's
    # AuthBrokerSession), independent of Auth/data-broker above. This never
    # touches Auth or any of its existing readers - if it fails, the
    # existing single-broker login flow is completely unaffected.
    try:
        from database.auth_db import upsert_broker_session
        upsert_broker_session(
            user_session_key, broker, auth_token, feed_token=feed_token, user_id=user_id
        )
    except Exception as e:
        logger.warning(f"Failed to register broker session for {user_session_key}/{broker}: {e}")

    from database.auth_db import record_activity
    record_activity(user_session_key, "broker", "Broker Connected", f"{broker} connected successfully")

    # Auto-generate a platform API key for the user if they don't have one yet.
    # This ensures the WebSocket data feed and REST fallback work immediately
    # after the first broker connect, without needing a separate /apikey step.
    try:
        from database.auth_db import get_api_key_for_tradingview, upsert_api_key
        if not get_api_key_for_tradingview(user_session_key):
            import secrets as _secrets
            auto_key = _secrets.token_hex(32)
            upsert_api_key(user_session_key, auto_key)
            logger.info(f"Auto-generated platform API key for new user {user_session_key}")
    except Exception as _e:
        logger.warning(f"Failed to auto-generate platform API key for {user_session_key}: {_e}")

    if is_additional_broker:
        # Connected successfully as an order-only broker. Master contract
        # sync stays scoped to the data broker only (see design note above),
        # so skip init_broker_status/download for this one.
        if is_ajax_request():
            return jsonify(
                {
                    "status": "success",
                    "message": f"{broker} connected. Data broker remains {existing_data_broker_obj.broker}.",
                    "redirect": "/broker/manage",
                }
            ), 200
        else:
            return redirect("/broker/manage")
    ...
    if inserted_id:
        ...
        if should_download:
            ...
            # user_session_key scopes the completion toast to the user who
            # actually connected this broker (see async_master_contract_download).
            thread = Thread(
                target=async_master_contract_download,
                args=(broker, user_session_key),
                daemon=True,
            )
            thread.start()
        else:
...
def handle_auth_failure(error_message, forward_url="broker.html"):
    ...
    if is_ajax_request():
        ...
        # For OAuth callbacks, redirect to broker selection with the error
        # carried in the query string. broker_login() checks for this to
        # avoid bouncing straight to /dashboard when the user still has a
        # valid session with their PREVIOUS broker (e.g. switching from
        # BNR to Zebu, Zebu's OAuth fails) - without this, the failure was
        # silently swallowed and the user was returned to the dashboard
        # with the old broker still shown, with no indication anything
        # went wrong.
        return redirect(url_for("auth.broker_login", auth_error=error_message))
