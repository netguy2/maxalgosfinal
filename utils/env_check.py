...
# Placeholder values shipped in .sample.env. maxalgos detects these on startup
...
PLACEHOLDER_APP_KEY = "MAX_ALGOS_PLACEHOLDER_APP_KEY_REGENERATE_BEFORE_USE"
PLACEHOLDER_PEPPER = "MAX_ALGOS_PLACEHOLDER_API_KEY_PEPPER_REGENERATE_BEFORE_USE"
            ...
            for line in f:
                ...
                if len(parts) >= 4 and parts[1] == '/tmp':
                    ...
                    if 'noexec' in mount_options:
                        ...
                        print("")
                        print("   maxalgos has auto-configured alternative paths:")
...
def _db_has_user_data(env_dir: str) -> bool:
    ...
    Args:
        env_dir: Absolute directory containing the .env file. Used to
            resolve a relative DATABASE_URL such as ``sqlite:///db/maxalgos.db``
...
PLACEHOLDER_FERNET_SALT = "MAX_ALGOS_PLACEHOLDER_FERNET_SALT_REGENERATE_BEFORE_USE"
...
def _ensure_fernet_salt(env_path: str) -> None:
    ...
    Background:
        ...
        (``b"maxalgos_static_salt"``). Identical salt across every maxalgos
        ...
        placeholder ``MAX_ALGOS_PLACEHOLDER_FERNET_SALT_REGENERATE_BEFORE_USE``
        in that exact spot).
    ...
    old_fernet = _make_fernet(b"maxalgos_static_salt")
    ...
    if db_path and os.path.exists(db_path):
        ...
        if sample_cts:
            ...
            if not decryptable:
                sys.stderr.write(
                    "\n\033[91m\033[1m[maxalgos Fernet salt]\033[0m\n"
...
def _resolve_sqlite_path(db_url: str, env_path: str) -> str | None:
    """Return absolute path to the maxalgos.db SQLite file, or None for non-SQLite."""
    m = re.match(r"sqlite:///(.+)", db_url)
    if not m:
        return None
    db_path = m.group(1)
...
def _migrate_fernet_db(env_path: str, pepper: str, new_salt: str) -> None:
    """Re-encrypt every Fernet-protected column in maxalgos.db.
    ...
    if not is_reloader_parent and (migrated or skipped):
        print(
            "\n\033[92m\033[1m[maxalgos Fernet salt rotation]\033[0m "
...
def _generate_keys_on_first_run(env_path: str) -> None:
    ...
    if pairs:
        try:
            ...
            if db_populated:
                ...
                sys.stderr.write(
                    "\n\033[91m\033[1m[maxalgos security]\033[0m\n"
    ...
    if rotated_names and not db_populated and not is_reloader_parent:
        print(
            "\n\033[92m\033[1m[maxalgos first-run setup]\033[0m "
        ...
        print(
            "\n\033[93m\033[1m[maxalgos security]\033[0m "
...
def load_and_check_env_variables() -> None:
    ...
    if broker_name == "fivepaisa":
        if ":::" not in broker_api_key or broker_api_key.count(":::") != 2:
            ...
            print("  https://docs.maxalgos.in/connect-brokers/brokers/5paisa")
            sys.exit(1)
    ...
    # Validate flattrade API key format
        ...
        if ":::" not in broker_api_key or broker_api_key.count(":::") != 1:
            ...
            print("\nFor detailed instructions, please refer to:")
            print("  https://docs.maxalgos.in/connect-brokers/brokers/flattrade")
            sys.exit(1)
    ...
    # Validate dhan API key format
        ...
        if ":::" not in broker_api_key or broker_api_key.count(":::") != 1:
            ...
            print("\nFor detailed instructions, please refer to:")
            print("  https://docs.maxalgos.in/connect-brokers/brokers/dhan")
            sys.exit(1)
    ...
    # Validate broker name - auto-discovered from broker/*/plugin.json rather
    # than requiring .env's VALID_BROKERS to be manually kept in sync with
    # every broker shipped in code (see utils.config.get_valid_brokers).
    from utils.config import get_valid_brokers

    valid_brokers = set(get_valid_brokers())
    if not valid_brokers:
        print("\nError: No broker plugins found under broker/*/plugin.json.")
    ...
    # WEBSOCKET_URL is optional (only used to override the reverse-proxied
    # client URL); WEBSOCKET_HOST/PORT and ZMQ_HOST/PORT are hardcoded in
    # utils/config.py and no longer read from .env. Validate the format
    # only when an operator has explicitly set an override.
    websocket_url = os.getenv("WEBSOCKET_URL", "")
    if websocket_url and not websocket_url.startswith(("ws://", "wss://")):
        print("\nError: WEBSOCKET_URL must start with 'ws://' or 'wss://'")
        print("Example: WEBSOCKET_URL='ws://localhost:8785'")
        sys.exit(1)
