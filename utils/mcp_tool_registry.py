...
HTTP / SSE (new):
    ...
    ``MAX_ALGOS_MCP_HTTP_BOOT=1``. We expose:
...
TOOL_SCOPES: dict[str, str] = {
    ...
    "get_maxalgos_version": SCOPE_READ_MARKET,
...
def _load_mcpserver_module():
    ...
    spec = importlib.util.spec_from_file_location("maxalgos_mcp_server", target)
    if spec is None or spec.loader is None:
        logger.error("Could not build spec for mcp/mcpserver.py")
        return None
    module = importlib.util.module_from_spec(spec)
    sys.modules["maxalgos_mcp_server"] = module  # so its decorators bind
...
def get_tool_callable(tool_name: str) -> Callable | None:
    ...
    ``MAX_ALGOS_MCP_HTTP_BOOT=1`` before this module is loaded so the
