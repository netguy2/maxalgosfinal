...
def api_get_websocket_config():
    ...
    from utils.config import get_websocket_url

    websocket_url = get_websocket_url()

    # If the request host is a remote domain (not local), automatically route to
    # the reverse-proxied /ws path on the incoming host with wss:// secure scheme.
    host_lower = request.host.lower()
    is_local_host = "localhost" in host_lower or "127.0.0.1" in host_lower or "::1" in host_lower

    # Detect if the request came through HTTPS (either directly or via proxy headers)
    is_secure_request = (
        request.is_secure
    ...
    if not is_local_host:
        websocket_url = f"wss://{request.host}/ws"
        logger.info(f"Dynamically routed remote secure WebSocket URL to: {websocket_url}")
    elif is_secure_request and websocket_url.startswith("ws://"):
    ...
    return jsonify(
        {
            ...
            "original_url": get_websocket_url(),
        }
    ), 200
