...
def init_http_transport() -> None:
    ...
    the admin's existing Max Algos API key (stored in db/maxalgos.db)
    ...
    # Loopback target the bundled maxalgos SDK uses to call back into
    ...
    if api_key is None:
        logger.warning(
            "[MCP HTTP] No Max Algos API key found in db/maxalgos.db. "
...
def _unauthorized(error_code: str, description: str = "") -> Response:
    ...
    challenge = f'Bearer realm="maxalgos-mcp", error="{error_code}"'
    if description:
    ...
    if method == "initialize":
        ...
        return _jsonrpc_result(
            rpc_id,
            {
                ...
                "serverInfo": {"name": "maxalgos", "version": _maxalgos_version()},
...
def _dispatch_tool_call(
    *,
    ...
    # MCP content blocks per spec — tools return a string per Max Algos
...
def healthz():
    ...
    return jsonify({"status": "ok", "service": "maxalgos-mcp"}), 200
