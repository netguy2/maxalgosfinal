...
@historify_bp.route("/api/live-data", methods=["GET"])
@check_session_validity
def get_live_chart_data():
    """Get OHLCV data for charting, fetched live from the broker (not
    Historify's stored/downloaded catalog). Unlike /api/data, this works
    immediately for any valid symbol/exchange without requiring the user to
    first add it to the Historify watchlist and download it — used by the
    Charts page (pages/Charts.tsx), which is meant to chart any symbol on
    demand. Rate-limited server-side to ~3 requests/sec against the broker
    (see services/history_service.py's _enforce_rate_limit)."""
    try:
        from database.auth_db import get_api_key_for_tradingview
        ...
        # Broker history APIs require a bounded date range -- default to a
        # window wide enough for indicator warm-up (MACD needs 26+ bars) on
        # any interval, but not so wide that intraday intervals blow past
        # what most brokers will return in one call.
        ...
        # Normalize to the {status, data: [...], count} shape the frontend
        # chart component already expects from /api/data.
        if success:
        ...
        logger.exception(f"Error getting live chart data: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500


@historify_bp.route("/api/indicators", methods=["POST"])
@check_session_validity
def get_indicator_series():
    """Compute indicator overlay series (+ crossover signals) for charting,
    over a caller-supplied list of OHLCV bars -- POST (not GET) because the
    bars array can be large. The Charts page sends back the exact same bars
    it just fetched live from the broker, so indicators are guaranteed to
    line up with the candles rendered on screen and work immediately for
    any symbol (no Historify pre-download step).

    Body:
        bars - list of {timestamp, open, high, low, close, volume}
        indicators - comma-separated list like "EMA:20,SMA:50,RSI:14,MACD,BOLLINGER:20:2"
                     (period(s) after the colon; omitted params use the same
                     defaults as services/indicator_registry.py's plugins)
    """
    try:
        from services.historify_service import get_indicator_overlays_from_bars

        payload = request.get_json(silent=True) or {}
        bars = payload.get("bars", [])
        indicators_param = payload.get("indicators", "")

        if not indicators_param or not bars:
            return jsonify({"status": "success", "data": {}, "signals": {}}), 200

        success, response, status_code = get_indicator_overlays_from_bars(
            bars=bars,
            indicators_param=indicators_param,
        )
        return jsonify(response), status_code
    except Exception as e:
        logger.exception(f"Error getting indicator series: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500


@historify_bp.route("/api/support-resistance", methods=["POST"])
@check_session_validity
def get_support_resistance():
    """Detect support/resistance levels from real swing highs/lows in a
    caller-supplied list of OHLCV bars -- POST for the same reason as
    /api/indicators (the bars array can be large). See
    services/indicator_series_service.py::compute_support_resistance for
    the algorithm (fractal swing-point detection + price clustering, not
    a fixed formula).
    ...
    """
    try:
        from services.indicator_series_service import compute_support_resistance_from_bars

        payload = request.get_json(silent=True) or {}
        bars = payload.get("bars", [])

        if not bars:
            return jsonify({"status": "success", "data": {"resistance": [], "support": []}}), 200

        levels = compute_support_resistance_from_bars(bars)
        return jsonify({"status": "success", "data": levels}), 200
    except Exception as e:
        logger.exception(f"Error computing support/resistance: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500
