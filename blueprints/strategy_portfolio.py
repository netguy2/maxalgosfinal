...
fixed watchlists: `mytrades` and `simulation`. Session-authed (no /api/v1
exposure — UI-only), scoped per user (see database/strategy_portfolio_db.py).
"""
...
def list_strategies():
    """List the current user's saved strategies; optional ?watchlist= filter."""
    ...
    items = list_portfolio(session.get("user"), watchlist)
...
def get_strategy(entry_id: int):
    entry = get_portfolio_entry(entry_id, session.get("user"))
    if not entry:
...
def delete_strategy(entry_id: int):
    ok = delete_portfolio_entry(entry_id, session.get("user"))
    if not ok:
        return jsonify({"status": "error", "message": "not found"}), 404
