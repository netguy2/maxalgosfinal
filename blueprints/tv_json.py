...
from services.signal_normalizer import normalize_tradingview
    ...
    if request.method == "POST":
        try:
            ...
            if mode == "line":
                ...
                # Build a normalized Signal, then project it into the same
                # OrderedDict key order the client previously received directly
                # (see services/signal_normalizer.py's normalize_tradingview).
                signal = normalize_tradingview(request.json, symbol_data, api_key, mode)
                ...
                # Placeholders below ({{strategy.order.action}} etc.) are
                # TradingView's own template syntax, substituted client-side by
                # TradingView -- NOT resolved here. normalize_tradingview passes
                # them through unchanged inside the Signal.
                signal = normalize_tradingview(request.json, symbol_data, api_key, mode)
                json_data = OrderedDict(
                    [
                        ("apikey", api_key),
