...
def get_session_status():
    ...
    if session.get("logged_in") and session.get("broker"):
        ...
        # Resolve the broker to REPORT from the Auth table row, not the
        # session["broker"] scalar -- the scalar can go stale (e.g. set at
        # OAuth-callback time, then never updated/cleared if the user later
        # starts-but-abandons connecting a *different* broker), which
        # otherwise makes this endpoint (and the header pill it feeds via
        # AuthSync -> useAuthStore) report a broker name that doesn't match
        # which broker's token is actually live. Same fix as
        # /auth/dashboard-data and mirrors /auth/broker-status, which
        # already derives from this row correctly.
        _auth_row = get_auth_token_dbquery(session.get("user"))
        reported_broker = (
            _auth_row.broker if _auth_row and not _auth_row.is_revoked else session.get("broker")
        )
