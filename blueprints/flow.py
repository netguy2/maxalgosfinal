...
def require_workflow_ownership(view_func):
    """Decorator for routes with a <int:workflow_id> param: 404s unless the
    session user owns that workflow (or it's a legacy row with no owner).

    FlowWorkflow gained a user_id column so one user can't read/edit/
    execute/delete another user's workflows -- previously every per-workflow
    route only checked session validity, never ownership, so any logged-in
    user could act on any workflow by guessing its id. Returns 404 (not 403)
    so a probing user can't even confirm a foreign workflow id exists.
    """

    @wraps(view_func)
    def wrapper(workflow_id, *args, **kwargs):
        from database.flow_db import user_owns_workflow

        username = session.get("user")
        if not username:
            return jsonify({"error": "Session expired"}), 401
        if not user_owns_workflow(workflow_id, username):
            return jsonify({"error": "Workflow not found"}), 404
        return view_func(workflow_id, *args, **kwargs)
...
def list_workflows():
    ...
    workflows = get_all_workflows(user_id=session.get("user"))
    items = []
...
def update_workflow(workflow_id):
    ...
    # If workflow is active, automatically re-sync trigger schedule with APScheduler
    if workflow.is_active:
        try:
            activate_workflow(workflow_id)
        except Exception as e:
            logger.warning(f"Could not auto-resync schedule for workflow {workflow_id}: {e}")
...
def activate_workflow(workflow_id):
    ...
    # Check if workflow uses timeCondition nodes (these need polling to evaluate)
    has_time_condition = any(n.get("type") == "timeCondition" for n in nodes)
    ...
    try:
        if trigger_type == "start":
            ...
            # If workflow has timeCondition nodes but no automatic schedule, auto-schedule
            # per-minute polling so the timeCondition node evaluates on every tick.
            # timeCondition nodes are passive filters — they only fire when execute_workflow()
            # is called. Without a recurring scheduler job, they never evaluate.
            if has_time_condition and (not schedule_type or schedule_type == "manual"):
                scheduler = get_flow_scheduler()
                ...
                logger.info(
                    f"Auto-scheduled per-minute polling for workflow {workflow_id} "
                    f"(timeCondition node detected)"
                )
            elif schedule_type and schedule_type != "manual":
                scheduler = get_flow_scheduler()
                scheduler.set_api_key(api_key)
...
def _execute_webhook(token, webhook_data=None, url_secret=None):
    ...
    if not api_key:
        api_key = os.getenv("MAX_ALGOS_API_KEY")  # Fallback to environment variable
