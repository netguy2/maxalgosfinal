...
def prometheus_metrics():
    ...
    Exposes platform health, worker pool usage, and system stats in standard Prometheus text format.
    """
    try:
        from services.strategy_worker_pool import get_strategy_worker_pool
        from utils.redis_client import is_redis_available

        pool_status = get_strategy_worker_pool().get_status()
        redis_ok = 1 if is_redis_available() else 0
        health_status = get_cached_health_status()
        status_val = 1 if health_status.get("status") == "pass" else 0

        lines = [
            "# HELP maxalgos_up Overall application health status (1 = pass, 0 = degrade/fail)",
            "# TYPE maxalgos_up gauge",
            f"maxalgos_up {status_val}",
            "# HELP maxalgos_strategy_workers_active Currently active strategy execution worker threads",
            "# TYPE maxalgos_strategy_workers_active gauge",
            f"maxalgos_strategy_workers_active {pool_status.get('active_workers', 0)}",
            "# HELP maxalgos_strategy_workers_max Maximum strategy execution worker thread limit",
            "# TYPE maxalgos_strategy_workers_max gauge",
            f"maxalgos_strategy_workers_max {pool_status.get('max_workers', 32)}",
            "# HELP maxalgos_redis_connected Redis connection status (1 = connected, 0 = local mode)",
            "# TYPE maxalgos_redis_connected gauge",
            f"maxalgos_redis_connected {redis_ok}",
        ]
