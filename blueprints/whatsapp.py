...
serve the logged-in Max Algos admin user (auth via Flask session cookie).
...
def start_pair():
    ...
    Captures the logged-in Max Algos admin's identity (user_id + username) and
...
@limiter.limit(WHATSAPP_MESSAGE_RATE_LIMIT)
def test_message():
    """Send a test message — to the linked-user for the logged-in Max Algos
    ...
    try:
        ...
        if wa_user:
            ...
            test_msg = (
                f"*Test from Max Algos (admin: {username})*\n"
                "Your WhatsApp integration is working."
            )
