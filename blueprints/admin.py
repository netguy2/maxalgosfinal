...
def _get_current_user_admin_status():
    """Helper: returns (current_username, is_admin) from session + DB lookup."""
    from flask import session as flask_session

    from database.user_db import find_user_by_exact_username

    # session["user"] is always a plain username string in this app
...
def api_users_list():
    ...
    from database.user_db import User, get_active_platform_subscription
            ...
            "data": [
                {
                    ...
                    "has_active_subscription": get_active_platform_subscription(u.username) is not None,
                }
                for u in users
            ],
        })
...
def api_users_create():
    ...
    is_new_admin = bool(data.get("is_admin", False))
    ...
    user = add_user(username, email, password, is_admin=is_new_admin)
    if user is None:
        return jsonify({"status": "error", "message": "Username or email already exists"}), 409

    logger.info(f"Admin created new user: {username} (admin={is_new_admin})")
    return jsonify({
        "status": "success",
...
def api_users_delete(user_id):
    ...
    from database.user_db import db_session as user_db_session
    ...
    try:
        ...
        username_del = target_user.username
        user_db_session.delete(target_user)
        user_db_session.commit()
        logger.info(f"Admin deleted user: {username_del}")
        return jsonify({"status": "success", "message": f"User '{username_del}' deleted"})
    except Exception as e:
...
@admin_bp.route("/api/users/<int:user_id>/sessions", methods=["GET"])
@check_session_validity
@limiter.limit(API_RATE_LIMIT)
def api_user_sessions(user_id):
    """List active device sessions for a user (admin only)."""
    ...
    return jsonify({"status": "success", "data": get_active_sessions(target_user.username)})


@admin_bp.route("/api/users/<int:user_id>/login-history", methods=["GET"])
@check_session_validity
@limiter.limit(API_RATE_LIMIT)
def api_user_login_history(user_id):
    """List recent login attempts for a user (admin only)."""
    ...
    return jsonify({
        "status": "success",
        "data": get_login_attempts(limit=limit, username=target_user.username),
    })
...
def api_user_suspend(user_id):
    """Suspend a user account (admin only, cannot suspend self). Suspended
    accounts are rejected at login before the password check even runs
    (see blueprints/auth.py login())."""
    ...
    user_db_session.commit()
    clear_user_sessions(target_user.username)

    logger.info(f"Admin suspended user: {target_user.username}")
    return jsonify({"status": "success", "message": f"User '{target_user.username}' suspended"})


@admin_bp.route("/api/users/<int:user_id>/reactivate", methods=["POST"])
@check_session_validity
@limiter.limit("10 per minute")
def api_user_reactivate(user_id):
    ...
    logger.info(f"Admin reactivated user: {target_user.username}")
    return jsonify({"status": "success", "message": f"User '{target_user.username}' reactivated"})


# ============================================================================
# SMTP / Email Settings API Endpoints
# ============================================================================
# SMTP is a platform-wide setting (one row, shared by every registration/
# password-reset email sent from this install) -- it must be admin-only.
# blueprints/auth.py's legacy /auth/smtp-config route (used by the Profile
# page's SMTP tab) predates multi-tenant self-registration and was reachable
# by ANY logged-in user; it is now also admin-gated (see auth.py) but this
# is the canonical admin-panel path going forward, mirroring
# /payments/admin/settings' GET+POST pattern.
...
@limiter.limit(API_RATE_LIMIT)
def api_smtp_get():
    """Return SMTP configuration (admin only). Password is masked -- the
    frontend only ever learns whether one is set, never its value."""
    ...
    return jsonify({
        ...
        "data": {
            ...
            "smtp_helo_hostname": settings.get("smtp_helo_hostname"),
        },
    })
...
def api_smtp_set():
    ...
    from database.settings_db import set_smtp_settings

    current_username, is_admin = _get_current_user_admin_status()
    if not is_admin:
        return jsonify({"status": "error", "message": "Admin access required"}), 403

    # The admin form always submits every transport field together (it is
    # not a partial patch), so an empty string here means "the admin cleared
    # this field" and must actually clear the stored value -- NOT "leave the
    # existing value alone". Collapsing "" to None before calling
    # set_smtp_settings() (which treats None as "field omitted, keep
    # existing") made every one of these fields un-clearable: e.g. clearing
    # HELO Hostname and clicking Save silently kept whatever stale value
    # (smtp.office365.com from an earlier Microsoft 365 attempt) was already
    # in the database, because the empty submission never reached the
    # column. Send "" through as "" (set_smtp_settings normalizes "" -> None
    # for storage) so clearing a field in the UI actually clears it.
    data = request.get_json(silent=True) or {}
    ...
    smtp_helo_hostname = (data.get("smtp_helo_hostname") or "").strip()
    # Unlike the other fields, do NOT strip() this into an empty-string ->
    # None normalization (a password can legitimately contain leading/
    # trailing spaces), but DO strip stray whitespace that a browser
    # autofill/copy-paste from a provider's "here is your password" page
    # commonly appends -- an invisible trailing space/newline in a masked
    # password field was silently saved verbatim while every other field
    # got cleaned, so the mailbox login (typed fresh in webmail) worked but
    # the saved SMTP password didn't, surfacing as Titan's
    # "535 5.7.8 Authentication failed" even though the credentials were
    # "the same".
    smtp_password = (data.get("smtp_password") or "").strip()
    try:
        smtp_port = int(data.get("smtp_port") or 587)
    except (TypeError, ValueError):
        return jsonify({"status": "error", "message": "SMTP port must be a number"}), 400
    ...
    # same "leave blank to keep existing" convention as the legacy Profile
    # SMTP form, so re-saving other fields never wipes a working password.
    if smtp_password.strip():
        kwargs["smtp_password"] = smtp_password

    try:
        set_smtp_settings(**kwargs)
    except Exception as e:
        logger.exception(f"Error updating SMTP settings: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500

    logger.info(f"SMTP settings updated by admin: {current_username}")
    return jsonify({"status": "success", "message": "SMTP settings updated successfully"})


@admin_bp.route("/api/smtp/identities", methods=["GET"])
@check_session_validity
@limiter.limit(API_RATE_LIMIT)
def api_smtp_identities_get():
    """Return the platform email identities (which "From" address each
    email category uses) -- admin only. Distinct from /api/smtp above,
    which is the shared SMTP transport (server/port/credentials/TLS)."""
    from database.settings_db import get_email_identities

    _current_username, is_admin = _get_current_user_admin_status()
    if not is_admin:
        return jsonify({"status": "error", "message": "Admin access required"}), 403

    return jsonify({"status": "success", "data": get_email_identities()})


@admin_bp.route("/api/smtp/identities", methods=["POST"])
@check_session_validity
@limiter.limit("10 per minute")
def api_smtp_identities_set():
    """Update the platform email identities (admin only)."""
    from database.settings_db import set_email_identities
    ...
    try:
        set_email_identities(**kwargs)
    except Exception as e:
        logger.exception(f"Error updating platform email identities: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500

    logger.info(f"Platform email identities updated by admin: {current_username}")
    return jsonify({"status": "success", "message": "Platform email identities updated successfully"})
...
@limiter.limit("10 per minute")
def api_smtp_test():
    """Send a test email using the CURRENTLY SAVED SMTP settings (admin
    only). Reuses the same send path as the legacy /auth/test-smtp route."""
    ...
    if not test_email:
        return jsonify({"status": "error", "message": "Please provide a test email address."}), 400
    ...
    result = send_test_email(test_email, sender_name=current_username)
    if result["success"]:
...
def _build_info():
    ...
    info = {
        "maxalgos_version": None,
        "maxalgos_sdk_version": None,
        "git_branch": None,
        "git_commit": None,
    ...
    try:
        ...
        info["maxalgos_version"] = get_version()
    except Exception:
        pass
    try:
        from importlib import metadata as _metadata

        info["maxalgos_sdk_version"] = _metadata.version("maxalgos")
    except Exception:
        pass
    ...
    if not info["git_branch"]:
        env_branch = os.getenv("MAX_ALGOS_GIT_BRANCH")
        if env_branch:
            info["git_branch"] = env_branch.strip()[:64]
    if not info["git_commit"]:
        env_commit = os.getenv("MAX_ALGOS_GIT_COMMIT")
        if env_commit:
...
def _safe_config_snapshot():
    ...
    from utils.config import get_valid_brokers, get_websocket_host, get_websocket_port
...
def _check_db_read():
    """Run SELECT 1 through the main DB engine and time it. Returns ms or error.

    Uses database.auth_db's engine (the shared main-DB engine, dialect-agnostic
    via database.engine_factory) rather than a raw sqlite3 connection, so this
    probe works whether the main DB is SQLite or PostgreSQL.
    """
    import time

    from sqlalchemy import text

    from database.auth_db import DATABASE_URL, engine
    ...
    try:
        ...
        return {"name": "DB read (maxalgos.db)", "ok": True, "ms": elapsed, "detail": "OK"}
    except Exception as e:
        return {"name": "DB read (maxalgos.db)", "ok": False, "ms": None, "detail": str(e)[:200]}
...
def _check_loopback_http():
    ...
    # FLASK_PORT is the canonical maxalgos var; PORT is the Docker/Railway
...
def _check_websocket_proxy():
    ...
    from utils.config import get_websocket_host, get_websocket_port

    host = get_websocket_host()
...
def _render_report(payload, errors_summary, errors_recent, fmt):
    ...
    lines = []
    lines.append(f"{h1}maxalgos System Report")
    lines.append("")
    ...
    lines.append(_md_kv("maxalgos", build.get("maxalgos_version")))
    lines.append(_md_kv("maxalgos SDK", build.get("maxalgos_sdk_version")))
...
def api_system_report():
    ...
    try:
        ...
        filename = f"maxalgos-system-report-{datetime.now().strftime('%Y%m%d-%H%M%S')}.{fmt}"
...
# restart (sudo systemctl restart maxalgos) before they take effect —
...
def _resolve_env_path() -> Path:
    ...
    systemd's WorkingDirectory points at MAX_ALGOS_PATH for the production
    ...
    return jsonify(
        {
            ...
            "restart_command": "sudo systemctl restart maxalgos",
