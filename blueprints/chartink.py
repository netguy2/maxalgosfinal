...
from services.signal_normalizer import normalize_chartink, signal_to_order_dict
    ...
    try:
        ...
        if not strategy:
            ...
            ingest(
                ...
                dedup=False,
            ).rejected("unknown_webhook", "No ChartInk strategy matches this webhook ID")
            return jsonify({"status": "error", "error": "Invalid webhook ID"}), 404

        # Kill-switch gate. Checked HERE rather than at the top of the
        # handler because the switch is per-user (see
        # database/settings_db.py) and the owning user is only known once
        # the strategy has been resolved from the webhook ID -- checking
        # earlier would mean reading some other account's flag.
        #
        # Fast 403 instead of running the full scan-name/time-window
        # validation below only to have the eventual place_order()/
        # place_smart_order() call rejected by the gate anyway -- that gate
        # remains the real backstop for this path since this handler
        # proxies to the REST API via requests.post() rather than calling
        # the service functions directly.
        if is_kill_switch_active(strategy.user_id):
            return jsonify(
                {
                    "status": "error",
                    "code": "KILL_SWITCH_ACTIVE",
                    "message": "Max Algos kill switch is active. Orders are not being accepted.",
                }
            ), 403
        ...
        # Audited before any validation, so a signal that never becomes an order
        # still leaves a trace of why. Deduplication guards against ChartInk
        # retrying the same scan alert -- each retry would otherwise place a
        # second live order.
        delivery = ingest(
            webhook_id=webhook_id,
            payload=data or {},
            ...
            signal=(data or {}).get("scan_name"),
            remote_ip=request.remote_addr,
        )

        if delivery.is_duplicate:
            # 200 so ChartInk does not treat this as a failure and retry again.
            return jsonify(
                {
                    "status": "duplicate",
                    "message": "Identical scan alert already received; suppressed to prevent a duplicate order",
                }
            ), 200
        ...
        # Resolution lives in services/chartink_action_resolver.py: it matches
        # on word boundaries (so "BUYBACK"/"COVERED" no longer collide with
        # BUY/COVER) and REJECTS a name containing conflicting keywords rather
        # than silently taking the first branch. A structured "action" field in
        # the payload, when present, bypasses name parsing entirely.
        from services.chartink_action_resolver import ActionResolutionError, resolve_action

        scan_name = data.get("scan_name", "")
        try:
            action, use_smart_order, is_entry_order = resolve_action(
                scan_name, explicit_action=data.get("action")
            )
        except ActionResolutionError as e:
            logger.error(f"ChartInk action resolution failed for scan '{scan_name}': {e}")
            delivery.rejected(e.code, str(e))
            return jsonify({"status": "error", "error": str(e)}), 400
        ...
        if strategy.is_intraday:
            ...
            if current_time < start_time:
                ...
                delivery.rejected(
                    "before_start_time", f"Signal at {current_time} precedes start {start_time}"
                )
                return jsonify(
            ...
            if current_time >= squareoff_time:
                ...
                delivery.rejected(
                    "after_squareoff_time",
                    f"Signal at {current_time} is past squareoff {squareoff_time}",
                )
                return jsonify(
            ...
            if is_entry_order and current_time >= end_time:
                ...
                delivery.rejected(
                    "after_end_time", f"Entry at {current_time} is past end time {end_time}"
                )
                return jsonify(
        ...
        if not symbols:
            ...
            delivery.rejected("no_symbols", "Payload contained no 'stocks' entries")
        ...
        if not mappings:
            ...
            delivery.rejected("no_mappings", "Strategy has no symbol mappings configured")
        ...
        if not api_key:
            ...
            delivery.rejected("no_api_key", f"No API key configured for user {strategy.user_id}")
        ...
        for symbol in symbols:
            ...
            # Build a normalized Signal from this scan-webhook symbol entry,
            # then convert it to the flat placeorder/placesmartorder payload
            # shape via the shared signal_normalizer layer (services/signal_normalizer.py).
        ...
        if processed_symbols:
            ...
            record_activity(
                ...
                "system",
                "Webhook Received",
                f"{strategy.name}: {len(processed_symbols)} order(s) queued",
            )
            delivery.stage(
                "queue", "continue", f"{len(processed_symbols)} order(s) queued: {', '.join(processed_symbols)}"
            )
            # Terminal here rather than on fill: this handler proxies to the REST
            # API via a background queue, so the request has no visibility into
            # what the broker ultimately did with each order.
            delivery.processed(detail=f"Queued {action} for {len(processed_symbols)} symbol(s)")
            return jsonify(
                {
                    "status": "success",
            ...
            delivery.rejected(
                "no_matching_symbols",
                "No scan symbol matched a configured mapping; nothing was queued",
            )
