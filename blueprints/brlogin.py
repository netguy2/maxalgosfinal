...
@brlogin_bp.route("/bnr-callback", defaults={"broker": "bnr"}, methods=["POST", "GET"])
...
def broker_callback(broker, para=None):
    ...
    # Load user-configured credentials from database into environment variables dynamically
    # to support multi-user settings and database overrides during authentication.
    username = session.get("user") or "admin"
    # This broker's OWN stored credentials, resolved explicitly by broker
    # name. Used below when building OAuth authorize URLs so we never read
    # the ambient BROKER_API_KEY (which app.py's os.getenv patch resolves
    # for the session's CURRENT/data broker, not the one being connected -
    # that mixup sent Zebu's OAuth to MYNT with BNR's client_id).
    this_broker_creds = {}
    try:
        from database.user_db import get_user_broker_credentials
        db_creds = get_user_broker_credentials(username, broker)
        this_broker_creds = db_creds or {}
        if db_creds:
            if db_creds.get("broker_api_key"):
                os.environ["BROKER_API_KEY"] = db_creds["broker_api_key"]
            if db_creds.get("broker_api_secret"):
                os.environ["BROKER_API_SECRET"] = db_creds["broker_api_secret"]
            if db_creds.get("broker_api_key_market"):
                os.environ["BROKER_API_KEY_MARKET"] = db_creds["broker_api_key_market"]
            if db_creds.get("broker_api_secret_market"):
                os.environ["BROKER_API_SECRET_MARKET"] = db_creds["broker_api_secret_market"]
            if db_creds.get("redirect_url"):
                os.environ["REDIRECT_URL"] = db_creds["redirect_url"]
            logger.info(f"Loaded credentials from database for user {username} into environment variables")
    except Exception as e:
        logger.warning(f"Failed to load credentials from database for {username}: {e}")
    ...
    # Only bypass/redirect if already fully broker-authenticated for this specific broker
    # and no new authentication parameters are supplied.
    has_auth_params = (
    ...
    if broker == "fivepaisa":
        ...
        if authCode and userId:
            # Callback from AliceBlue with authorization code. Force
            # credential resolution to THIS broker during token exchange -
            # otherwise auth_function's os.environ.get reads the session
            # data broker's key/secret (the same Zebu-using-BNR-creds bug,
            # here for AliceBlue).
            ...
            # Initial visit — redirect to AliceBlue login page. Resolve
            # from THIS broker's own stored creds (not os.getenv/os.environ,
            # which resolve the session's current data broker's key) - same
            # fix as BNR/Zebu above, so AliceBlue never picks up another
            # connected broker's appcode.
            logger.info("Redirecting to AliceBlue login page")
            appcode = this_broker_creds.get("broker_api_key") or os.environ.get("BROKER_API_KEY")
            if not appcode:
                return handle_auth_failure(
                    "AliceBlue API key not configured. Please enter your credentials on the Credentials tab first.",
                    forward_url="broker.html",
                )
        ...
        if request.method == "GET":
            # Initial hit from Max Algos broker page has no callback parameters.
            ...
            # Callback reached Max Algos but required params were not provided.
        ...
        if code:
            ...
            # Force credential resolution to THIS broker during token
            # exchange - otherwise auth_function's os.getenv reads the
            # session data broker's key/secret (the Zebu-using-BNR-creds bug).
            from utils.broker_context import broker_credential_context
            with broker_credential_context(username, broker):
                res = auth_function(code)
                if isinstance(res, tuple) and len(res) == 3:
                    auth_token, user_id, error_message = res
                else:
                    auth_token, error_message = res
                    user_id = None
            forward_url = "broker.html"
        else:
            # Initial visit — redirect to Zebu OAuth login page.
            # Resolve from THIS broker's own stored creds only -- NOT
            # os.getenv("BROKER_API_KEY"), which (via utils/env_patch.py)
            # resolves the session's CURRENT/data broker's key, not
            # necessarily Zebu's. Falling back to it here previously leaked
            # another broker's client_id into Zebu's OAuth URL whenever
            # Zebu's own UserBrokerCredential row was empty.
            logger.info("Redirecting to Zebu OAuth login page")
            # BROKER_API_KEY format: userid:::client_id
            full_api_key = this_broker_creds.get("broker_api_key")
            if not full_api_key:
                return handle_auth_failure(
                    "Zebu API key not configured. Please enter your credentials on the Credentials tab first.",
                    forward_url="broker.html",
                )
            parts = full_api_key.split(":::", 1)
            client_id = parts[1].strip() if len(parts) > 1 and parts[1].strip() else parts[0].strip()
            if not client_id:
                return handle_auth_failure(
                    "Zebu API key or Client ID missing. Please update your credentials on Broker Management.",
                    forward_url="broker.html",
                )
        ...
        if code:
            code = code.strip()
            logger.debug(f"BNR broker - OAuth callback with code: {code}")
            from utils.broker_context import broker_credential_context
            with broker_credential_context(username, broker):
                # broker/bnr/api/auth_api.py's authenticate_broker() returns
                # (access_token, resp_userid, error_msg) on its token-exchange
                # path -- 3 values -- but (None, error_msg) -- 2 values -- on
                # its early validation-failure paths. Same inconsistency
                # already handled for Zebu above; mirrors that fix so BNR's
                # real client/actid (resp_userid) is captured instead of
            ...
            # Initial visit — redirect to BNR OAuth login page.
            # Resolve from THIS broker's own stored creds only -- NOT
            # os.getenv("BROKER_API_KEY"), which (via utils/env_patch.py)
            # resolves the session's CURRENT/data broker's key, not
            # necessarily BNR's. Falling back to it here previously sent
            # another broker's (e.g. Zebu's) client_id to BNR's OAuth flow.
            logger.info("Redirecting to BNR OAuth login page")
            # BROKER_API_KEY format: userid:::client_id
            full_api_key = this_broker_creds.get("broker_api_key")
            if not full_api_key:
                return handle_auth_failure(
                    "BNR API key not configured. Please enter your credentials on the Credentials tab first.",
                    forward_url="broker.html",
                )
            parts = full_api_key.split(":::", 1)
            if len(parts) != 2 or not parts[1]:
                return handle_auth_failure(
                    "BNR API key must be configured in format 'userid:::client_id' (e.g. ITC1441:::ITC1441_U). Please update it on the Credentials tab.",
                    forward_url="broker.html",
                )
            client_id = parts[1]  # OAuth client_id
            bnr_login_url = f"https://prime.bnrsecurities.com/OAuthlogin/authorize/oauth?client_id={client_id}"
            return redirect(bnr_login_url)
        ...
        if code:
            ...
            # os.getenv("BROKER_API_KEY"), which (via utils/env_patch.py)
            # resolves the session's CURRENT/data broker's key, not
            # necessarily Shoonya's. Falling back to it here previously sent
            # another broker's client_id to Shoonya's OAuth flow.
        ...
        # Sharekhan's callback query param name for the encrypted request
        # token is not documented in its public SDK/README - check every
        # plausible spelling so the token is never silently dropped if it
        # comes back under a name other than the one assumed here.
        code = (
        ...
        logger.debug(f"Sharekhan broker - request token present: {bool(code)}")
        # Force credential resolution to THIS broker during the AES-GCM
        # token decrypt/exchange - otherwise app.py's os.getenv patch reads
        # the session's CURRENT DATA BROKER's secret key (e.g. Zebu's, not
        # Sharekhan's), since it always resolves via session["broker"] and
        # that isn't updated to "sharekhan" until AFTER this call succeeds.
        # This produced a silent, permanent AES-GCM InvalidTag/decrypt
        # failure even with perfectly correct Sharekhan credentials saved -
        # the code was decrypting every request_token with the wrong
        # broker's key. Same fix already applied to Zebu/BNR/AliceBlue's
        # callback branches above.
        from utils.broker_context import broker_credential_context
        with broker_credential_context(username, broker):
            auth_token, error_message = auth_function(code)
        forward_url = "broker.html"
    ...
    if auth_token:
        ...
        # For OAuth / multi-broker callbacks, handle missing session user
        if broker in ("compositedge", "rmoney", "iiflcapital") and "user" not in session:
