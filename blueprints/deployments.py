...
def create_new_deployment():
    ...
    required_fields = ["name", "strategy_id", "capital"]
...
def update_deployment_broker(id):
    """Change which broker(s) a deployment routes orders/quotes through.
    ...
    option was deleting and recreating it. This lets a user correct the
    broker selection in place.

    Accepts either `brokers` (a list -- the multi-broker case, one order
    fanned out to each) or the legacy single `broker` string. Validates
    every new broker is genuinely connected (mirrors
    place_order_service.py's Case-1-specific-broker check) rather than
    trusting client-supplied names outright -- otherwise this endpoint
    ...
    data = request.json or {}
    brokers_list = data.get("brokers")
    if brokers_list is not None:
        brokers = [str(b).strip() for b in brokers_list if str(b).strip()]
    else:
        single = str(data.get("broker") or "").strip()
        brokers = [single] if single else []

    if not brokers:
        return jsonify({"status": "error", "message": "broker or brokers is required"}), 400
    ...
    deployment.broker = brokers[0]
    deployment.brokers = json.dumps(brokers)
    db_session.commit()

    # Give it a clean run against the corrected broker(s) rather than
    # leaving a stale "Error" from the old misconfigured one sitting on
    # screen.
    label = ", ".join(brokers)
    update_deployment_status(
        id, "Waiting", f"Broker(s) changed to {label} by user -- resuming evaluation."
    )

    return jsonify({
        "status": "success",
        "message": "Deployment broker(s) updated",
        "broker": brokers[0],
        "brokers": brokers,
    })
