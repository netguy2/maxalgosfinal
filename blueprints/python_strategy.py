...
def ensure_directories():
    ...
    try:
        ...
        if STRATEGIES_DIR.exists() and LOGS_DIR.exists():
            ...
            temp_base = Path(tempfile.gettempdir()) / "maxalgos"
...
def start_strategy_process(strategy_id):
    ...
    with PROCESS_LOCK:  # Thread-safe operation
        ...
        # Kill-switch gate, scoped to the strategy's OWNER. The switch is
        # per-user (see database/settings_db.py), so another account's
        # emergency stop must not prevent this user starting their own
        # strategies. Checked after the config lookup because that is where
        # the owner becomes known.
        if is_kill_switch_active(config.get("user_id")):
            return False, "Kill switch is active -- cannot start strategies"
        ...
        try:
            ...
            # (per strategies/README.md: STRATEGY_ID, STRATEGY_NAME, MAX_ALGOS_API_KEY, MAX_ALGOS_HOST)
            strategy_env = os.environ.copy()
            strategy_env["STRATEGY_ID"] = strategy_id
            strategy_env["STRATEGY_NAME"] = config.get("name", strategy_id)
            strategy_env["MAX_ALGOS_STRATEGY_EXCHANGE"] = normalize_exchange(
                config.get("exchange")
            )
            # Comma-separated list of brokers the user selected for this
            # strategy at creation time (informational -- see brokers=
            # comment in new_strategy()). The script reads this to know
            # which of its placeorder(broker=...) calls are valid without
            # hardcoding broker names; not enforced by the platform.
            _brokers = config.get("brokers") or []
            strategy_env["MAX_ALGOS_BROKERS"] = ",".join(_brokers)
            # MAX_ALGOS_HOST must point at THIS running server so the
            # subprocess's `from maxalgos import api` client reaches the real
            # REST endpoints. Derive it from HOST_SERVER (the app's own
            # configured base URL, e.g. http://127.0.0.1:5001) rather than
            # hardcoding :5000 -- a hardcoded :5000 would silently send every
            # strategy's order/quote call to the wrong port (this deployment
            # runs on FLASK_PORT=5001), where nothing answers, so every SDK
            # call would fail with a connection error the user only sees
            # buried in the strategy log.
            _host_server = os.getenv("HOST_SERVER", "http://127.0.0.1:5000")
            strategy_env["MAX_ALGOS_HOST"] = _host_server
            try:
                from database.auth_db import get_api_key_for_tradingview
                user_id = config.get("user_id")
                _api_key = get_api_key_for_tradingview(user_id) if user_id else None
                if _api_key:
                    strategy_env["MAX_ALGOS_API_KEY"] = _api_key
                else:
                    # Fail loudly before launch instead of starting a
                    # subprocess that can never authenticate. Without a real
                    # API key, every SDK call the strategy makes gets
                    # rejected with "Invalid maxalgos apikey" -- the strategy
                    # would "run" (process alive, logging) but never place a
                    # single order, with no upfront signal to the user.
                    close_log_handle_safely({"log_handle": log_handle})
                    return (
                        False,
                        "No API key found for this account. Generate one from the API Key "
                        "page, then start this strategy.",
                    )
            except Exception as e:
                logger.warning(f"Could not inject API key for strategy {strategy_id}: {e}")

            # Inject project root into PYTHONPATH so `from maxalgos import api` is resolved reliably
            project_root = str(Path.cwd().absolute())
            existing_pp = strategy_env.get("PYTHONPATH", "")
            strategy_env["PYTHONPATH"] = f"{project_root}{os.pathsep}{existing_pp}" if existing_pp else project_root
            ...
            if isinstance(process, subprocess.Popen):
                ...
                if IS_WINDOWS:
                    ...
                    try:
                        ...
                        # taskkill /F /T already force-killed the process
                        # tree above -- this wait() is just to reap the
                        # zombie, not a condition of success. If it's still
                        # not fully reaped within 2s (slow tree teardown),
                        # that must NOT be treated as a stop failure: the
                        # outer except below would report "Failed to stop
                        # strategy" even though taskkill succeeded, leaving
                        # the strategy stuck as "running" in the UI until
                        # the 60s cleanup_dead_processes reaper catches it.
                        try:
                            process.wait(timeout=2)
                        except subprocess.TimeoutExpired:
                            logger.warning(
                                f"Strategy {strategy_id} (PID {pid}) not yet reaped 2s after "
                                "taskkill /F /T -- treating as stopped since the force-kill "
                                "already succeeded; cleanup_dead_processes will reap it."
                            )
                else:
                    # Unix-like systems (Linux, macOS)
                    try:
...
def new_strategy():
    ...
    if request.method == "POST":
        ...
        if file and file.filename.endswith(".py"):
            # Validate the uploaded file is syntactically valid Python BEFORE
            # saving it -- otherwise a script with a syntax error "uploads
            # successfully", and the error only surfaces (silently, in a log
            # file) when the user later tries to start it, with no clue at
            # upload time that the file was broken. Read the bytes, compile,
            # then rewind so file.save() below still writes the full content.
            try:
                _content_bytes = file.read()
                file.seek(0)
                ast.parse(_content_bytes.decode("utf-8"))
            except SyntaxError as e:
                msg = f"Python syntax error on line {e.lineno}: {e.msg}"
                if is_ajax:
                    return jsonify({"status": "error", "message": msg}), 400
                flash(msg, "error")
                return redirect(request.url)
            except UnicodeDecodeError:
                msg = "File is not valid UTF-8 text -- please upload a plain Python (.py) file."
                if is_ajax:
                    return jsonify({"status": "error", "message": msg}), 400
                flash(msg, "error")
                return redirect(request.url)
            ...
            # Which of the user's connected brokers this strategy is allowed
            # to trade on. Purely informational to the platform (exposed to
            # the script as MAX_ALGOS_BROKERS below) -- the script itself
            # picks a specific broker per-order via placeorder(broker=...);
            # this list is not enforced server-side. Empty/omitted means
            # "no restriction stated", matching pre-multi-broker behavior
            # (falls back to the account's default/primary broker).
            brokers_json = request.form.get("brokers", "[]")
            try:
            ...
            # Marketplace catalog id / signalId this script was generated
            # from, if the upload came from StrategyConfigurator.tsx's
            # template deploy path -- purely for display ("from {template}
            # template" in My Strategies). Both None for hand-written
            # uploads (NewPythonStrategy.tsx never sends these fields).
            source_template_id = request.form.get("source_template_id") or None
            source_signal_id = request.form.get("source_signal_id") or None
...
def start_strategy(strategy_id):
    ...
    # Locked: this is a read-modify-write on the same config dict that
    # schedule_strategy_route mutates under PROCESS_LOCK -- without the
    # lock here too, a /start racing a concurrent /schedule request on the
    # same never-before-scheduled strategy could interleave and clobber
    # each other's writes to is_scheduled/schedule_start/schedule_stop.
    with PROCESS_LOCK:
        config = STRATEGY_CONFIGS.get(strategy_id, {})
        if not config.get("is_scheduled"):
            ...
            config["schedule_days"] = config.get(
    ...
    # Check schedule constraints. Use the same is_within_schedule_time() the
    # rest of the file relies on (market_hours_enforcer, is_within_schedule_time
    # docstring) instead of re-deriving the window here -- a prior duplicated
    # version only compared the user's raw schedule_start/schedule_stop
    # against wall-clock time, without intersecting it with the exchange's
    # actual session window (get_effective_session_window). That meant a
    # strategy on an exchange with a partial/special session (e.g. MCX
    # evening-only on an NSE holiday) could be told it was "within hours"
    # while the exchange itself had no session live, or vice versa.
    schedule_days = config.get("schedule_days", [])
    now = datetime.now(IST)
    day_names = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"]
    today_day = day_names[now.weekday()]
    ...
    if not is_scheduled_day or not is_within_hours or is_holiday:
        ...
        if is_holiday:
            ...
            try:
                ...
                next_start = (
                    f"today at {schedule_start} IST"
                    if now < start_time
                    else f"next scheduled day at {schedule_start} IST"
                )
...
def schedule_strategy_route(strategy_id):
    ...
    # Hold PROCESS_LOCK across the running-check and the config mutation so
    # this can't race with start_strategy_process/stop_strategy_process
    # (which mutate is_running/pid under the same lock) -- without it, a
    # concurrent /start request between the check below and
    # schedule_strategy()'s writes could either flip is_running after this
    # route already decided to proceed, or have its own is_running write
    # clobbered by schedule_strategy()'s unlocked config mutation.
    with PROCESS_LOCK:
        config = STRATEGY_CONFIGS.get(strategy_id)
        if config is None:
            return jsonify({"status": "error", "message": "Strategy not found"}), 404
...
def api_get_strategies():
    """API: Get all strategies as JSON, scoped to the requesting user.

    Previously returned every strategy across every user unfiltered --
    any authenticated user could see every other user's strategy names,
    schedules, and running status. Filtered the same way
    verify_strategy_ownership() checks a single strategy: legacy rows with
    no user_id (pre-dating the ownership column) are still shown, matching
    that function's "allow access if strategy has no owner" fallback.
    """
    user_id = session.get("user")
    if not user_id:
        return jsonify({"status": "error", "message": "Session expired"}), 401
        ...
        strategies.append(
            {
                ...
                "source_signal_id": config.get("source_signal_id"),
            }
        )
...
@python_strategy_bp.route("/api/logs/all")
@check_session_validity
def api_get_all_log_files():
    """API: List every log file across all of the current user's Python
    strategies, newest first -- the data source for the aggregate "All
    Logs" view. Each entry is tagged with strategy_id/strategy_name so the
    UI can filter/group without a second round-trip per strategy."""
    user_id = session.get("user")
    if not user_id:
        return jsonify({"status": "error", "message": "Session expired"}), 401
    ...
    owned_ids = {
        ...
        if not cfg.get("user_id") or cfg.get("user_id") == user_id
    }

    logs = []
    try:
        for log_file in LOGS_DIR.glob("*.log"):
            # Log filenames are {strategy_id}_{timestamp}_IST.log -- match
            # against owned strategy_ids by prefix (same convention
            # api_get_log_content already relies on for ownership checks).
            matched_id = next((sid for sid in owned_ids if log_file.name.startswith(f"{sid}_")), None)
            if not matched_id:
                continue
            stats = log_file.stat()
            logs.append(
                {
                    ...
                    "strategy_name": STRATEGY_CONFIGS.get(matched_id, {}).get("name", matched_id),
                    "size_kb": stats.st_size / 1024,
                    "last_modified": datetime.fromtimestamp(stats.st_mtime, tz=IST).isoformat(),
                }
            )
    except Exception as e:
        logger.exception(f"Error listing all log files: {e}")

    logs.sort(key=lambda x: x["last_modified"], reverse=True)
    return jsonify({"logs": logs})


@python_strategy_bp.route("/api/logs/all/combined")
@check_session_validity
def api_get_combined_log_content():
    """API: Merge each owned strategy's MOST RECENT log file into one
    chronologically-interleaved feed, every line tagged with which
    strategy produced it. This is what makes "20-30 strategies running at
    once" actually reviewable without opening each one's log tab
    individually.

    Capped at `limit` lines per source file (most recent lines kept) to
    avoid loading unbounded log volume into one response; the per-strategy
    log view remains the place to read a single strategy's full history.
    """
    user_id = session.get("user")
    if not user_id:
        return jsonify({"status": "error", "message": "Session expired"}), 401
    ...
    try:
        limit_per_strategy = min(int(request.args.get("limit", 500)), 2000)
    except (TypeError, ValueError):
        limit_per_strategy = 500

    owned = {
        sid: cfg.get("name", sid)
        for sid, cfg in STRATEGY_CONFIGS.items()
        if not cfg.get("user_id") or cfg.get("user_id") == user_id
    }

    # For each owned strategy, find its most recently modified log file.
    latest_log_by_strategy = {}
    try:
        for log_file in LOGS_DIR.glob("*.log"):
            matched_id = next((sid for sid in owned if log_file.name.startswith(f"{sid}_")), None)
            if not matched_id:
                continue
            mtime = log_file.stat().st_mtime
            current = latest_log_by_strategy.get(matched_id)
            if current is None or mtime > current[1]:
                latest_log_by_strategy[matched_id] = (log_file, mtime)
    except Exception as e:
        logger.exception(f"Error scanning logs for combined view: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500

    # Parse each strategy's tail into (timestamp, strategy_name, line) rows.
    # Log lines don't carry per-line timestamps today (they're plain print()
    # output), so exact interleave order falls back to file mtime -- good
    # enough to group by strategy while still roughly time-ordering across
    # strategies; the per-strategy view remains authoritative for exact
    # within-strategy ordering.
    combined = []
    for strategy_id, (log_file, mtime) in latest_log_by_strategy.items():
        try:
            content = log_file.read_text(encoding="utf-8", errors="replace")
            lines = content.splitlines()[-limit_per_strategy:]
            strategy_name = owned.get(strategy_id, strategy_id)
            for line in lines:
                if not line.strip():
                    continue
                combined.append(
            ...
            logger.warning(f"Error reading log {log_file} for combined view: {e}")

    # Stable sort by source file mtime keeps each strategy's own lines in
    # their original (already-chronological) order while still grouping
    # roughly by recency across strategies.
    combined.sort(key=lambda x: x["file_mtime"])

    return jsonify(
        {
            "entries": combined,
            "strategy_count": len(latest_log_by_strategy),
...
def save_strategy(strategy_id):
    ...
    # Reject a syntactically invalid edit before overwriting the file. The
    # editor lets users paste/edit arbitrary Python; saving a broken script
    # would otherwise "succeed" here and only fail later (silently, in a log
    # file) when the strategy is started. Surface the exact line/message now.
    try:
...
def restore_running_strategy_process(strategy_id, config):
    ...
    try:
        ...
        # Exact-argv match on the resolved absolute path, not a substring
        # check on the joined command line: after a PID has been reused by
        # an unrelated process (e.g. after a host reboot), a naive
        # `strategy_file in " ".join(cmdline)` could misattribute that
        # unrelated process as this strategy's process if its command line
        # happens to contain this path as a substring (e.g. a similarly-
        # named/prefixed strategy file) -- then later terminate/manage a
        # process that isn't actually ours. Resolve to absolute on both
        # sides since config["file_path"] is stored relative
        # (STRATEGIES_DIR is a relative Path) while the process was
        # actually launched with file_path.absolute() as its argv.
        if not strategy_file:
            logger.debug(f"PID {pid} exists but strategy {strategy_id} has no file_path recorded")
            return False

        strategy_file_abs = str(Path(strategy_file).absolute())
        cmdline_args_abs = {str(Path(arg).absolute()) for arg in process.cmdline()}
        if strategy_file_abs not in cmdline_args_abs:
