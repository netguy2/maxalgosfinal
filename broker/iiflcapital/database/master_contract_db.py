...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "iiflcapital").delete(synchronize_session=False)
    db_session.commit()
...
def copy_from_dataframe(df):
    ...
    for row in data_dict:
        row["broker"] = "iiflcapital"

    existing_tokens = {
        result.token
        for result in db_session.query(SymToken.token)
        .filter(SymToken.broker == "iiflcapital")
        .all()
    }
...
def format_strike(strike_val):
    """Format strike price for Max Algos symbol. Integer if whole number, else decimal."""
    try:
...
def process_iiflcapital_indices_csv(path):
    ...
    # Determine Max Algos exchange from CSV Exchange column
