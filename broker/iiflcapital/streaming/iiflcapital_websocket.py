...
broker-feed surface that other Max Algos adapters consume (Zerodha-style):
...
Max Algos's WebSocket proxy still wants three distinct modes (LTP/Quote/Depth),
...
# Max Algos's mode ints — the adapter layer translates 1/2/3 → "ltp"/"quote"/"full".
MODE_LTP = "ltp"
MODE_QUOTE = "quote"
MODE_FULL = "full"
...
class IiflcapitalWebSocket:
    ...
    def subscribe_instruments(
        ...
        Args:
            ...
            mode: "ltp" / "quote" / "full" — affects which Max Algos modes the
    ...
    def _dispatch_oi(self, key: str, payload: bytes) -> None:
        ...
        # hours). This keeps the Max Algos proxy's mode set simple.
