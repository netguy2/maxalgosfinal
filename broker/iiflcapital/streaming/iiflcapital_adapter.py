"""
IIFL Capital WebSocket adapter — connects Max Algos's WebSocket proxy to the
...
class IiflcapitalWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """
    Max Algos broker adapter for IIFL Capital's MQTT market-data feed.
    ...
    decoded dict into three Max Algos-shaped payloads on the way out, exactly
    like Zerodha's `full` → `ltp/quote/full` fan-out.
    """

    # Max Algos mode ints → internal IIFL feed mode strings.
    ...
    def __init__(self) -> None:
        ...
        # {
        #   "exchange": str,           # Max Algos exchange (e.g. NSE_INDEX)
        #   "symbol": str,
        #   "segment": str,            # IIFL brexchange (NSEEQ, NSEFO, …)
        #   "token": str,
        #   "mode": int,               # Max Algos mode int (1/2/3)
        #   "is_index": bool,
        # }
        self.subscribed_symbols: dict[str, dict] = {}

        # Reverse lookup: f"{segment}/{token}" → (symbol, exchange) — used to
        # rebuild the Max Algos topic when a tick comes back from the broker.
    ...
    def _handle_ticks(self, ticks: list[dict]) -> None:
        ...
        the Max Algos ZeroMQ bus, slicing the single broker packet into LTP /
