...
def get_margin_data(auth_token):
    ...
    # Shoonya's API can return an empty body or an HTML error page instead
    # of a JSON error payload -- same Noren-API-vendor bug already fixed in
    # broker/zebu/api/funds.py and broker/bnr/api/funds.py.
    body_text = response.text
    if not body_text or not body_text.strip():
        raise ValueError(
            f"Shoonya API returned an empty response for /Limits (HTTP {response.status_code})"
        )
    try:
        margin_data = json.loads(body_text)
    except json.JSONDecodeError as e:
        raise ValueError(
            f"Shoonya API returned a non-JSON response for /Limits "
