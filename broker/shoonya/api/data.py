...
from broker.shoonya.api.rate_limiter import throttle_shoonya_request
...
def get_api_response(endpoint, auth, method="POST", payload=None):
    ...
    throttle_shoonya_request()  # shared per-account rate limit -- see rate_limiter.py
...
class BrokerData:
    ...
    def get_multiquotes(self, symbols: list) -> list:
        ...
        """
        try:
            # Shoonya is a Noren-API broker (10/sec + 120/min shared per
            # account). Pacing is handled precisely by the shared
            # throttle_shoonya_request() inside each per-symbol fetch, so
            # there is NO blind inter-batch sleep -- we stay under the caps
            # without ever waiting longer than necessary (latency-critical
            # for the option tools). BATCH_SIZE matches the per-second cap so
            # the ThreadPoolExecutor fan-out doesn't pile 20 threads onto the
            # throttle lock at once.
            BATCH_SIZE = 8
    ...
    def _fetch_single_quote_sync(
        ...
        try:
            ...
            # ThreadPoolExecutor fan-out (max_workers=20) would blow past
            # Shoonya's 120/min cap. See broker/shoonya/api/rate_limiter.py.
            throttle_shoonya_request()

            # Use httpx.post for sync requests
            http_response = httpx.post(url, content=payload_str, headers=headers, timeout=10.0)
            # Shoonya's GetQuotes endpoint can return an empty body or an
            # HTML error page instead of a JSON error payload -- this batch
            # path builds its own request and bypasses get_api_response(),
            # so it needs its own guard (same fix as broker/zebu's equivalent).
            body_text = http_response.text
            if not body_text or not body_text.strip():
                return {
                    "symbol": symbol,
                    "exchange": exchange,
                    "error": f"Shoonya API returned an empty response (HTTP {http_response.status_code})",
                }
            try:
                ...
                return {
                    "symbol": symbol,
                    "exchange": exchange,
                    "error": f"Shoonya API returned a non-JSON response (HTTP {http_response.status_code})",
                }
