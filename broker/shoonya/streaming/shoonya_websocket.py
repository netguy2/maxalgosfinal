...
class ShoonyaWebSocket:
    ...
    def __init__(
        ...
        # and the subscribe/unsubscribe/auth callers can both write to the
        # same underlying socket concurrently -- two heartbeat mechanisms
        # plus arbitrary app writes, unsynchronized on one socket. That race
        # corrupts the outgoing frame stream, which the broker's Noren
        # server responds to by dropping the raw TCP connection ("WebSocket
        # closed: None - None" -- no status/message, i.e. not a graceful
        # close) roughly every 30-60s, wiping the tick cache and
        # subscriptions on every cycle. Plain thread-safety bug in
        # websocket-client usage (see
        # broker/zebu/streaming/zebu_websocket.py for the same fix and full
        # analysis), not a broker API limit.
