...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "shoonya").delete(synchronize_session=False)
    db_session.commit()
...
def copy_from_dataframe(df):
    ...
    existing_token_exchange = {
        ...
        for result in db_session.query(SymToken.token, SymToken.exchange)
        .filter(SymToken.broker == "shoonya")
        .all()
    }
...
def process_shoonya_nse_data(output_path):
    """
    Processes the shoonya NSE data (NSE_symbols.txt) to generate Max Algos symbols.
...
def process_shoonya_nfo_data(output_path):
    """
    Processes the shoonya NFO data (NFO_symbols.txt) to generate Max Algos symbols.
...
def process_shoonya_cds_data(output_path):
    """
    Processes the shoonya CDS data (CDS_symbols.txt) to generate Max Algos symbols.
    Handles both futures and options formatting.
    """
...
def process_shoonya_mcx_data(output_path):
    """
    Processes the shoonya MCX data (MCX_symbols.txt) to generate Max Algos symbols.
    Handles both futures and options formatting.
    """
...
def process_shoonya_bse_data(output_path):
    """
    Processes the shoonya BSE data (BSE_symbols.txt) to generate Max Algos symbols.
...
def process_shoonya_bfo_data(output_path):
    """
    Processes the shoonya BFO data (BFO_symbols.txt) to generate Max Algos symbols and correctly extract the name column.
