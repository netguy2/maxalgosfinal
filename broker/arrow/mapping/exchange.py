...
#
# Central exchange-code translation between Max Algos and Arrow, plus index
...
#   NSECO           NSE commodities (Max Algos exchange NCO)
...
# Max Algos index exchanges (quote-only).
OA_INDEX_EXCHANGES = {"NSE_INDEX", "BSE_INDEX", "MCX_INDEX"}

# Arrow ExchSeg -> Max Algos exchange. Verified against the live CSV.
ARROW_EXCHSEG_TO_OA = {
    "NSECM": "NSE",
    "BSECM": "BSE",
    "NSEFO": "NFO",
...
# Max Algos exchange -> Arrow exchange code used by the /info/quote* endpoints.
...
# Max Algos exchange -> Arrow historical-API path segment (lowercase). Indices
...
# ("Nifty 50", "BSE IT", "CrudeOil"). Max Algos standardizes these to the
...
# NSE index display names (normalized) -> Max Algos symbol.
_NSE_INDEX_MAP = {
    # Major indices
    "NIFTY50": "NIFTY",
...
# BSE index short codes (normalized CSV `Symbol`) -> Max Algos symbol.
_BSE_INDEX_MAP = {
    "SENSEX": "SENSEX",
    "BANKEX": "BANKEX",
    "SENSEX50": "SENSEX50",
...
# MCX iCOMDEX index names (normalized CSV `Symbol`) -> Max Algos symbol.
...
# Max Algos name yet, so they get a consistent MCX-prefixed symbol.
_MCX_INDEX_MAP = {
...
def to_arrow_quote_exchange(oa_exchange):
    """Max Algos exchange -> Arrow quote exchange (indices -> INDEX)."""
    return _OA_TO_ARROW_QUOTE.get(oa_exchange, oa_exchange)


def to_arrow_history_exchange(oa_exchange):
    """Max Algos exchange -> Arrow historical path segment (lowercase)."""
...
def arrow_exchange_to_oa(exchseg, segment=None, exchange=None):
    """Map an Arrow instrument's raw exchange fields to an Max Algos exchange.
