...
#
# Arrow uses short, single/short codes that differ from Max Algos's vocabulary:
#   side    : BUY/SELL          -> B / S
...
# --- Order / price type -------------------------------------------------
# Max Algos pricetype -> Arrow `order` value.
_ORDER_TYPE_MAP = {
    "MARKET": "MKT",
    "LIMIT": "LMT",
    "SL": "SL-LMT",
...
# --- Product type -------------------------------------------------------
# Max Algos product -> Arrow `product` value.
...
# Arrow `product` -> Max Algos product (used when normalizing broker responses).
...
def map_order_type(pricetype):
    """Map Max Algos pricetype to Arrow order type. Defaults to MKT."""
    return _ORDER_TYPE_MAP.get(pricetype, "MKT")


def map_product_type(product):
    """Map Max Algos product to Arrow product code. Defaults to I (intraday)."""
    return _PRODUCT_MAP.get(product, "I")


def reverse_map_product_type(exchange, product):
    """Map Arrow product code back to Max Algos product."""
    return _REVERSE_PRODUCT_MAP.get(product)


def map_transaction_type(action):
    """Map Max Algos BUY/SELL to Arrow B/S."""
    return "B" if str(action).upper() == "BUY" else "S"


def transform_data(data):
    """Transform an Max Algos order dict into the Arrow place-order payload.

    Max Algos order fields: symbol, exchange, action, pricetype, quantity,
    ...
    transformed = {
        ...
        "remarks": str(data.get("strategy", "maxalgos"))[:16],
    }
...
def transform_modify_order_data(data):
    """Transform an Max Algos modify-order dict into the Arrow modify payload
