# Mapping Max Algos margin request <-> Arrow margin API (edge.arrow.trade/margin/order).
#
...
# Max Algos response reports span_margin/exposure_margin as 0 and
...
# Max Algos product -> Arrow product code (pyarrow-client ProductType enum:
...
def _map_margin_order(pricetype):
    """Max Algos pricetype -> Arrow margin order type. The margin endpoint
    documents only LMT/MKT."""
    return "MKT" if pricetype == "MARKET" else "LMT"


def transform_margin_positions(positions):
    """Transform Max Algos positions into Arrow /margin/order request bodies."""
    bodies = []
    skipped = []
...
def parse_margin_response(order_data_list):
    """Aggregate Arrow per-order /margin/order `data` dicts into the Max Algos
...
def parse_basket_margin_response(data):
    """Map Arrow's /margin/basket response `data` to the Max Algos standard.
