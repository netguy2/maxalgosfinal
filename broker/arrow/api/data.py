...
class BrokerData:
    def __init__(self, auth_token):
        ...
        # Max Algos timeframe -> Arrow interval string. Keys are what
    ...
    def _lookup(self, symbol, exchange):
        """Resolve an Max Algos (symbol, exchange) to the Arrow instrument.
    ...
    def _quote_index(self, mode, symbol, br_symbol, token):
        ...
        Tries the Max Algos symbol, then the uppercased display name, then the
    ...
    def get_quotes(self, symbol, exchange):
        """Return the Max Algos quote dict. Uses Arrow `full` mode so bid/ask are
    ...
    def get_depth(self, symbol, exchange):
        """Return Max Algos 5-level market depth. Indices supported via INDEX."""
        try:
            q = self._fetch_quote("full", symbol, exchange)
    ...
    def get_history(self, symbol, exchange, timeframe, from_date, to_date):
        """Historical candles -> Max Algos DataFrame (timestamp, open, high, low,
