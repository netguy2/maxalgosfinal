...
#
# Builds the Max Algos SymToken table from Arrow's instrument master.
#
...
# standardized to the documented Max Algos index symbols in mapping/exchange.py.
...
def _broker_auth_token():
    ...
    Max Algos is single-user, so there is at most one.
    """
...
# Max Algos exchanges whose instruments can be futures (used by the
...
def process_arrow_csv(df):
    ...
    # NOTE: Max Algos stores indices with instrumenttype "EQ" (verified against
    ...
    # "CrudeOil"); standardize to the documented Max Algos index symbols.
    ...
    # Drop rows that failed to map to a valid Max Algos exchange.
        ...
        rows.append(
            {
                ...
                "strike": 0.0,
                "lotsize": 0,
                # Max Algos stores indices as instrumenttype "EQ" (the
