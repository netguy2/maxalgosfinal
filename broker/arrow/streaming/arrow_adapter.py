"""Arrow WebSocket adapter -> Max Algos unified streaming.

Subclasses BaseBrokerWebSocketAdapter. Resolves Max Algos (symbol, exchange) to
...
master contract). The publish topic EXCHANGE_SYMBOL_MODE keeps the Max Algos
...
# Arrow subscription mode -> Max Algos topic suffix. The proxy fans a higher mode
...
class ArrowWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def _normalize(self, tick, symbol, exchange, topic_mode):
        """Build the Max Algos normalized tick (same key set as the Zerodha
