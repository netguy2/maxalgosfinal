"""
Bnr WebSocket Adapter for Max Algos
Handles market data streaming from Bnr broker
"""
...
class LTPNormalizer:
    ...
    def normalize(data: dict[str, Any], msg_type: str) -> dict[str, Any]:
        ...
        result = {
            ...
            "ltp": ltp,
            "bnr_timestamp": safe_int(data.get("ft")),
        }
        # Same change/change_percent computation as QuoteNormalizer/
        # DepthNormalizer below -- mirrors the identical fix in
        # broker/zebu/streaming/zebu_adapter.py's LTPNormalizer. "data" is
        # the MarketDataCache-merged dict, so "c" is present once any
        # message for this token has carried it. LTP is the default
        # subscription mode (frontend's useMarketData), so omitting this
...
class QuoteNormalizer:
    ...
    def normalize(data: dict[str, Any], msg_type: str) -> dict[str, Any]:
        ...
        # Bnr only sends "pc" (its own percent-change field), not "change"
        # or "change_percent" -- every consumer (MarketDataManager.ts, the
        # Dashboard index cards) reads those exact key names. Same fix as
        # broker/zebu/streaming/zebu_adapter.py (identical Noren-vendor bug).
        if close:
            change = ltp - close
            result["change"] = round(change, 2)
            result["change_percent"] = round(change / close * 100, 2)
...
class BnrWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Bnr WebSocket adapter with improved structure and error handling"""
    ...
    def __init__(self):
        ...
        self.logger.info(
            f"Bnr adapter initialized - Expected ZMQ port: {actual_zmq_port}, Actual bound port: {self.zmq_port}"
        )
    ...
    def initialize(
        ...
        """Initialize connection with Bnr WebSocket API"""
        self.user_id = user_id
        self.broker_name = broker_name

        # Get Bnr credentials from environment
        ...
        if full_api_key and ":::" in full_api_key:
            ...
            self.logger.info(f"Using Bnr user ID from BROKER_API_KEY: {self.actid}")
        elif full_api_key:
        ...
        if not self.actid or not self.susertoken:
            self.logger.error(f"Missing Bnr credentials for user {user_id}")
            raise ValueError(f"Missing Bnr credentials for user {user_id}")

        self.logger.info(f"Using Bnr credentials - User ID: {self.actid}")

        # Initialize WebSocket client
        self.ws_client = BnrWebSocket(
            user_id=self.actid,  # Both user_id and actid should be the Bnr account ID
            actid=self.actid,
    ...
    def connect(self) -> None:
        """Establish connection to Bnr WebSocket endpoint"""
        if not self.ws_client:
            self.logger.error("WebSocket client not initialized. Call initialize() first.")
            return

        self.logger.info("Connecting to Bnr WebSocket...")
        connected = self.ws_client.connect()

        if connected:
            self.connected = True
            self.reconnect_attempts = 0
            self.logger.info("Connected to Bnr WebSocket successfully")
        else:
            raise ConnectionError("Failed to connect to Bnr WebSocket")

    def disconnect(self) -> None:
        """Disconnect from Bnr WebSocket endpoint"""
        ...
        self.logger.info("Disconnected from Bnr WebSocket")
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            self.cleanup_zmq()
            self.connected = False
            self.logger.info("Bnr adapter cleanup complete")
        except Exception as e:
    ...
    def _create_subscription(
        ...
        bnr_exchange = BnrExchangeMapper.to_bnr_exchange(brexchange)
        scrip = f"{bnr_exchange}|{token}"
    ...
    # Bnr's calling convention: subscribe() calls _websocket_subscribe
    ...
    def _on_open(self, ws):
        ...
        self.logger.info("Connected to Bnr WebSocket")
        self.connected = True
        self._resubscribe_all()

    def _on_error(self, ws, error):
        """Handle WebSocket connection error"""
        self.logger.error(f"Bnr WebSocket error: {error}")
        self._handle_websocket_error(error)

    def _on_close(self, ws, close_status_code, close_msg):
        """Handle WebSocket connection close"""
        self.logger.info(f"Bnr WebSocket connection closed: {close_status_code} - {close_msg}")
        self.connected = False
    ...
    def unsubscribe_all(self) -> dict[str, Any]:
        ...
        the WebSocket. Mirrors broker/zebu/streaming/zebu_adapter.py's
        ...
        which thrashed the connection into a reconnect loop and starved
        those pages of a clean run to finish rendering. Keeping the socket
        alive and just clearing subscriptions avoids that.
        """
        try:
            with self.lock:
