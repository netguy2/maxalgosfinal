...
class BnrWebSocket:
    ...
    def _heartbeat_worker(self) -> None:
        ...
        # TCP black-hole with no proper close frame). Without this,
        # `self.connected` can be stuck True forever once this loop exits,
        # since the higher-level adapter (bnr_adapter.py) only ever hears
        # about a dead connection via the on_close callback forwarded
        # below -- it never polls `connected` itself -- so the adapter
        # (and the WS proxy's ConnectionPool, which trusts the ADAPTER's
        # own connected flag) would otherwise never learn this session
        # died and never reconnect.
        if self.running and self.connected:
            self.logger.warning(
            ...
            # bnr_adapter.py's reconnection logic.
            self.connected = False
            self._heartbeat_thread = None
