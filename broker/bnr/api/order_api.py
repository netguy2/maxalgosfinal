...
from broker.bnr.mapping.transform_data import (
    map_product_type,
...
def get_api_response(endpoint, auth, method="GET", payload=""):
    ...
    # endpoint, so order calls must respect the same budget that quote/
    # depth polling consumes, not a private counter of their own.
    ...
    # JSON error payload -- guard the same way as broker/bnr/api/data.py's
    # get_api_response() and broker/bnr/api/funds.py's get_margin_data().
...
def place_order_api(data, auth):
    ...
    # Shared throttle with data.py/get_api_response - see
    # broker/bnr/api/rate_limiter.py. This call builds its own HTTP
    # request rather than going through get_api_response, so it needs its
    # own explicit throttle call to respect the same account-wide quota.
    throttle_bnr_request()

    # Get the shared httpx client
    client = get_httpx_client()
    url = "https://prime.bnrsecurities.com/NorenWClientAPI/PlaceOrder"
...
def cancel_order(orderid, auth):
    ...
    # Get the shared httpx client
    client = get_httpx_client()
    url = "https://prime.bnrsecurities.com/NorenWClientAPI/CancelOrder"
...
def modify_order(data, auth):
    ...
    # Get the shared httpx client
    client = get_httpx_client()
    url = "https://prime.bnrsecurities.com/NorenWClientAPI/ModifyOrder"
