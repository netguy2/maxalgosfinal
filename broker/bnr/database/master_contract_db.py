...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "bnr").delete(synchronize_session=False)
    db_session.commit()
...
# Define the Bnr URLs for downloading the symbol files
bnr_urls = {
    "NSE": "https://prime.bnrsecurities.com/NSE_symbols.txt.zip",
    "NFO": "https://prime.bnrsecurities.com/NFO_symbols.txt.zip",
    "CDS": "https://prime.bnrsecurities.com/CDS_symbols.txt.zip",
    "MCX": "https://prime.bnrsecurities.com/MCX_symbols.txt.zip",
    "BSE": "https://prime.bnrsecurities.com/BSE_symbols.txt.zip",
    "BFO": "https://prime.bnrsecurities.com/BFO_symbols.txt.zip",
}


def download_and_unzip_bnr_data(output_path):
    """
    Downloads and unzips the Bnr text files to the tmp folder using httpx with connection pooling.
    """
    logger.info("Downloading and Unzipping Bnr Data")
    ...
    # Iterate through the Bnr URLs and download/unzip files
    for key, url in bnr_urls.items():
        try:
...
def process_bnr_nse_data(output_path):
    """
    Processes the Bnr NSE data (NSE_symbols.txt) to generate Max Algos symbols.
    Separates EQ, BE symbols, and Index symbols.
    """
    logger.info("Processing Bnr NSE Data")
    ...
    # Convert token to string to ensure compatibility with Bnr API
    ...
    # Apply transformation for Max Algos symbols
    def get_maxalgos_symbol(broker_symbol):
        ...
        if "-EQ" in broker_symbol:
            ...
            # For other symbols (including index), Max Algos symbol remains the same as broker symbol
            return broker_symbol

    # Update the 'symbol' column
    df["symbol"] = df["brsymbol"].apply(get_maxalgos_symbol)
    ...
    # Broker exchange should always be NSE for Bnr API calls
    df["brexchange"] = "NSE"
    ...
    # Map common NSE index symbols to Max Algos format
    nse_index_mapping = {
        "NIFTY INDEX": "NIFTY",
...
def process_bnr_nfo_data(output_path):
    """
    Processes the Bnr NFO data (NFO_symbols.txt) to generate Max Algos symbols.
    Handles both futures and options formatting.
    """
    logger.info("Processing Bnr NFO Data")
...
def process_bnr_cds_data(output_path):
    """
    Processes the Bnr CDS data (CDS_symbols.txt) to generate Max Algos symbols.
    Handles both futures and options formatting.
    """
    logger.info("Processing Bnr CDS Data")
...
def process_bnr_mcx_data(output_path):
    """
    Processes the Bnr MCX data (MCX_symbols.txt) to generate Max Algos symbols.
    Handles both futures and options formatting.
    """
    logger.info("Processing Bnr MCX Data")
...
def process_bnr_bse_data(output_path):
    """
    Processes the Bnr BSE data (BSE_symbols.txt) to generate Max Algos symbols.
    Ensures that the instrument type is always 'EQ' (no BSE index symbols available from Bnr).
    """
    logger.info("Processing Bnr BSE Data")
    ...
    # Apply transformation for Max Algos symbols (no special logic needed here)
    def get_maxalgos_symbol(broker_symbol):
        return broker_symbol
    ...
    # Set Exchange: 'BSE' for all rows (no BSE index symbols from Bnr)
    df["exchange"] = "BSE"
...
def process_bnr_bfo_data(output_path):
    """
    Processes the Bnr BFO data (BFO_symbols.txt) to generate Max Algos symbols and correctly extract the name column.
    Handles both futures and options formatting, ensuring strike prices are handled as either float or integer.
    """
    logger.info("Processing Bnr BFO Data")
    file_path = f"{output_path}/BFO_symbols.txt"

    if not os.path.exists(file_path):
        logger.warning(f"Bnr BFO data file not found: {file_path}. Skipping.")
...
def delete_bnr_temp_data(output_path):
    """
    Deletes the Bnr symbol files from the tmp folder after processing.
    """
...
def master_contract_download():
    """
    Downloads, processes, and deletes Bnr data.
    """
    logger.info("Downloading Bnr Master Contract")

    output_path = "tmp"
    try:
        download_and_unzip_bnr_data(output_path)
        delete_symtoken_table()

        # Placeholders for processing different exchanges
        token_df = process_bnr_nse_data(output_path)
        copy_from_dataframe(token_df)
        token_df = process_bnr_bse_data(output_path)
        copy_from_dataframe(token_df)
        token_df = process_bnr_nfo_data(output_path)
        copy_from_dataframe(token_df)
        token_df = process_bnr_cds_data(output_path)
        copy_from_dataframe(token_df)
        token_df = process_bnr_mcx_data(output_path)
        copy_from_dataframe(token_df)
        token_df = process_bnr_bfo_data(output_path)
        copy_from_dataframe(token_df)

        delete_bnr_temp_data(output_path)
