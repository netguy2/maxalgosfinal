...
def transform_order_data(orders):
    ...
    for order in orders:
        ...
        # Map Bnr status to Max Algos status
...
def transform_tradebook_data(tradebook_data):
    ...
    for trade in tradebook_data:
        # Parse the timestamp from Bnr format "HH:MM:SS DD-MM-YYYY" to just "HH:MM:SS"
...
def transform_positions_data(positions_data):
    ...
    for position in positions_data:
        ...
        lp = float(position.get("lp", 0.0))  # Last Price from Bnr
        rpnl = float(position.get("rpnl", 0.0))  # Realized P&L from Bnr
        urmtom = float(position.get("urmtom", 0.0))  # Unrealized MTM from Bnr

        # For closed positions, Bnr zeroes out netavgprc
    ...
    for portfolio in portfolio_data:
        ...
        for exch_tsym in portfolio.get("exch_tsym", []):
            ...
            if symbol_from_db:
                ...
                logger.info(f"Bnr Portfolio - Product Value for {symbol} Not Found or Changed.")
