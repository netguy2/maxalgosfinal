...
def get_margin_data(auth_token):
    ...
    # instead of a JSON error payload -- guard the same way order_api.py's
    # get_api_response() does in this same broker plugin.
    try:
        margin_data = response.json()
    except json.JSONDecodeError as e:
        raise ValueError(
            f"Ibulls API returned a non-JSON response for /user/balance "
