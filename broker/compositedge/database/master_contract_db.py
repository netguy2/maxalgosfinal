...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "compositedge").delete(synchronize_session=False)
    db_session.commit()
...
def copy_from_dataframe(df):
    ...
    for row in data_dict:
        row["broker"] = "compositedge"

    # Retrieve existing tokens to filter them out from the insert
    existing_tokens = {
        result.token
        for result in db_session.query(SymToken.token)
        .filter(SymToken.broker == "compositedge")
        .all()
    }
...
def normalize_bse_index_symbols(symbol_series: pd.Series) -> pd.Series:
    """Normalize raw BSE index symbols to Max Algos naming and format."""
...
def process_compositedge_bse_csv(path):
    ...
    # Normalize BSE index short codes from SPOT rows to Max Algos index symbols
...
def process_index_data(index_data):
    """
    Processes index data from API to fit the Max Algos database schema.
    ...
    # NSE index mapping (raw broker index names -> Max Algos symbols)
    nse_index_map = {
        "NIFTY 50": "NIFTY",
