"""
Fyers WebSocket Adapter for Max Algos WebSocket Proxy
Integrates with the Max Algos WebSocket proxy system
"""
...
class FyersWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Fyers-specific implementation of the WebSocket adapter for Max Algos proxy"""
    ...
    def __init__(self):
        ...
        self.tbt_symbol_to_ticker = {}  # Max Algos symbol -> Fyers ticker
        self.tbt_ticker_to_symbol = {}  # Fyers ticker -> Max Algos symbol
    ...
    def _subscribe_tbt_depth(
        ...
        Args:
            symbol: Max Algos symbol (actual symbol without suffix)
            exchange: Exchange name
    ...
    def _unsubscribe_tbt_depth(self, symbol: str, exchange: str) -> bool:
        ...
        Args:
            symbol: Max Algos symbol (may include :50 suffix)
            exchange: Exchange name
    ...
    def _convert_to_fyers_ticker(self, symbol: str, exchange: str) -> str | None:
        """
        Convert Max Algos symbol to Fyers ticker format using database lookup

        Args:
            symbol: Max Algos symbol (e.g., 'RELIANCE', 'NIFTY24DEC25000CE')
    ...
    def _on_tbt_depth_update(self, ticker: str, depth_data: dict[str, Any]):
        ...
        try:
            ...
            # Map to Max Algos format
            symbol = subscription["symbol"]
            exchange = subscription["exchange"]

            self.logger.debug(f"Mapping TBT depth for {exchange}:{symbol}")

            mapped_data = self.data_mapper.map_tbt_depth_to_maxalgos(
    ...
    def _map_fyers_to_maxalgos(
        self, fyers_data: dict[str, Any], mode: int
    ) -> dict[str, Any] | None:
        """
        Map Fyers data to Max Algos WebSocket format
        ...
        Returns:
            Mapped data in Max Algos format
        """
        try:
            if not fyers_data:
                return None
            ...
            if mode == 1:  # LTP
                ...
                maxalgos_data.update({"ltp": converted_ltp, "data_type": "LTP"})
            elif mode == 2:  # Quote
