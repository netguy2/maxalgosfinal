"""
Fyers Data Mapping
Maps Fyers HSM data to Max Algos format for compatibility
"""
...
class FyersDataMapper:
    """
    Maps Fyers HSM WebSocket data to Max Algos format
    """
    ...
    def map_to_maxalgos_ltp(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to Max Algos LTP format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            Max Algos LTP format dict or None if mapping fails
        """
        try:
            ...
            return maxalgos_data

        except Exception as e:
            logger.debug(f"Error mapping LTP data: {e}")
            return None

    def map_to_maxalgos_quote(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers data to Max Algos Quote format

        Args:
            fyers_data: Raw data from Fyers HSM WebSocket

        Returns:
            Max Algos Quote format dict or None if mapping fails
        """
        try:
            if not fyers_data:
                return None
    ...
    def map_to_maxalgos_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers depth data to Max Algos Depth format

        Args:
            fyers_data: Raw depth data from Fyers HSM WebSocket

        Returns:
            Max Algos Depth format dict or None if mapping fails
        """
        try:
            ...
            # Map to Max Algos Depth format (matching other brokers)
            maxalgos_data = {
                "symbol": f"{exchange}:{symbol_name}",
                "exchange": exchange,
                "token": fyers_data.get("exchange_token", ""),
    ...
    def map_index_to_synthetic_depth(self, fyers_data: dict[str, Any]) -> dict[str, Any] | None:
        """
        Map Fyers index data to synthetic Max Algos Depth format
    ...
    def map_fyers_data(
        ...
        """
        Map Fyers data to appropriate Max Algos format based on requested type
        ...
        Returns:
            Mapped Max Algos data or None if mapping fails
        """
        if not fyers_data:
            return None
        ...
        if requested_type == "LTP":
            return self.map_to_maxalgos_ltp(fyers_data)
        elif requested_type == "Quote":
            return self.map_to_maxalgos_quote(fyers_data)
        elif requested_type == "Depth" and fyers_type == "dp":
            return self.map_to_maxalgos_depth(fyers_data)
    ...
    def extract_symbol_info(self, symbol: str) -> dict[str, str]:
        """
        Extract exchange and symbol from Max Algos format
