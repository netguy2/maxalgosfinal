"""
Fyers WebSocket Adapter for Max Algos
...
class FyersAdapter:
    """
    Fyers WebSocket adapter for Max Algos streaming service
    Follows Max Algos adapter pattern similar to Angel, Zerodha etc.
    """
    ...
    def subscribe_symbols(self, symbols: list[dict[str, str]], data_type: str, callback: Callable):
        ...
        try:
            with self.lock:
                ...
                self.logger.debug(
                    f"Converting {len(valid_symbols)} Max Algos symbols to HSM format using database lookup..."
                )

                # Convert Max Algos symbols directly to HSM tokens using database lookup
                hsm_tokens, token_mappings, invalid_symbols = (
                    self.token_converter.convert_maxalgos_symbols_to_hsm(valid_symbols, data_type)
                )
                ...
                # Build the HSM<->Max Algos mapping by JOINING through brsymbol.
                #
                ...
                # map from valid_symbols lets us recover the correct Max Algos
                ...
                brsymbol_to_maxalgos: dict[str, tuple[str, str]] = {}
                for s in valid_symbols:
                    br = get_br_symbol(s["symbol"], s["exchange"])
                    if br:
                        brsymbol_to_maxalgos[br] = (s["exchange"], s["symbol"])
                ...
                for hsm_token in hsm_tokens:
                    ...
                    if not brsym:
                        ...
                        continue
                    pair = brsymbol_to_maxalgos.get(brsym)
                    if not pair:
                        self.logger.warning(
    ...
    def _on_message(self, fyers_data: dict[str, Any]):
        ...
        try:
            ...
            # Map to Max Algos format first to get symbol info
            ...
            callback = None
            maxalgos_data_type = "Quote"  # Default
            ...
            for cb, maxalgos_data_type in dispatches:
