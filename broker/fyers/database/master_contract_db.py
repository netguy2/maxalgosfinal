...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "fyers").delete(synchronize_session=False)
    db_session.commit()
...
def reformat_symbol_detail(s):
    ...
    # Reorder and format the parts to match the Max Algos standard symbol format
...
def process_fyers_nse_csv(path):
    ...
    # Override specific symbols where normalized name differs from Max Algos standard
...
def process_fyers_bse_csv(path):
    ...
    # Normalize BSE_INDEX symbols to Max Algos standard format
...
def process_fyers_cds_json(path):
    ...
    # Map JSON fields to Max Algos schema
    df["token"] = df["fyToken"]
