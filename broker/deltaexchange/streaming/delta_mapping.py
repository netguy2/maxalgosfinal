...
class DeltaExchangeMapper:
    """Maps Max Algos exchange codes to Delta Exchange equivalents.

    Delta Exchange uses plain symbol strings (e.g. "BTCUSD").
    All products trade on a single exchange named "CRYPTO" in Max Algos.
    """

    # Max Algos exchange code → Delta Exchange exchange code
    EXCHANGE_SEGMENTS = {
        "CRYPTO": "CRYPTO",
...
class DeltaModeMapper:
    """Maps Max Algos subscription mode integers to Delta Exchange channel names."""

    # Max Algos mode → Delta WS channel name
    MODE_CHANNELS = {
