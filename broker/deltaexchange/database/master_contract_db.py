...
def init_db():
    ...
    try:
        ...
        logger.error(
            ...
            f"the column is added. Run: sqlite3 db/maxalgos.db "
...
def _to_canonical_symbol(delta_symbol: str, instrument_type: str, expiry: str) -> str:
    """
    Convert a Delta Exchange native symbol to the Max Algos canonical CRYPTO format.
    ...
    Returns:
        Max Algos canonical symbol string.
    """
...
# Maps Delta Exchange contract_type values to Max Algos instrument type codes
CONTRACT_TYPE_MAP = {
...
def process_delta_products(products):
    ...
    Max Algos SymToken schema.  Only live + operational products are included.

    Field mapping (from GET /v2/products response):
        token          ← id                    (int → str)
        brsymbol       ← symbol                (Delta-native, e.g. "C-BTC-80000-280225")
        symbol         ← canonical             (Max Algos format, e.g. "BTC28FEB2580000CE")
        name           ← description
        exchange       ← "CRYPTO"              (Max Algos exchange abstraction)
    ...
    for p in products:
        ...
        # Build Max Algos canonical symbol (exchange = CRYPTO, broker-agnostic format)
        canonical_symbol = _to_canonical_symbol(symbol_str, instrument_type, expiry)

        rows.append(
            {
                "token": str(p["id"]),
                "symbol": canonical_symbol,   # Max Algos canonical (e.g. BTC28FEB2580000CE)
