# Mapping Max Algos API Request to Delta Exchange API Parameters
...
def transform_data(data, token):
    """
    Transforms the Max Algos API request structure to Delta Exchange POST /v2/orders payload.
...
def transform_modify_order_data(data):
    """
    Transforms Max Algos modify order data to Delta Exchange PUT /v2/orders payload.
...
def map_order_type(pricetype):
    """Maps Max Algos pricetype to Delta Exchange order_type string."""
    mapping = {
        "MARKET": "market_order",
        "LIMIT": "limit_order",
        "SL": "limit_order",
...
def reverse_map_product_type(br_product):
    """
    Maps Delta Exchange position category back to Max Algos product type.
...
def map_exchange(br_exchange):
    """
    Maps a Delta Exchange broker exchange field back to Max Algos exchange code.
...
def map_exchange_type(exchange):
    """
    Maps an Max Algos exchange code to the Delta Exchange context.
