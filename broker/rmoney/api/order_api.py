...
def get_api_response(endpoint, auth, method="GET", payload=""):
    ...
    # RMoney's XTS-vendor API can return an empty body or an HTML error
    # page instead of a JSON error payload -- guard the same way the
    # per-function place/cancel/modify order calls already do in this file.
    try:
        return response.json()
    except json.JSONDecodeError as e:
        raise ValueError(
            f"RMoney API returned a non-JSON response for {endpoint} "
...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    # Map exchange from Max Algos format to XTS format
    exchange_mapping = {
        "NSE": "NSECM",
        "BSE": "BSECM",
        "NFO": "NSEFO",
