...
def map_order_data(order_data):
    ...
    for order in orders:
        ...
        # Check if symbol was found; if so, update with Max Algos format
        if symbol:
            # Convert to Max Algos symbol format
...
def transform_order_data(orders):
    """
    Transform order data from Pocketful API format to Max Algos standard format.
...
def map_trade_data(trade_data):
    ...
    for trade in trades:
        ...
        if symbol:
            ...
            # Convert to Max Algos symbol format if exchange is available
            if exchange:
                oa_symbol = get_oa_symbol(symbol, exchange)
...
def transform_tradebook_data(tradebook_data):
    """
    Transform tradebook data from Pocketful API format to Max Algos standard format.
...
def transform_holdings_data(holdings_data):
    """
    Transform holdings data from Pocketful API format to Max Algos standard format.
    ...
    for holding in holdings_data:
        ...
        # Convert to Max Algos symbol format if needed
        if tradingsymbol and exchange:
            ...
            # Try to convert to Max Algos symbol format
            try:
                logger.info(
                    f"DEBUG - Converting symbol '{tradingsymbol}' for exchange '{exchange}' to Max Algos format"
                )
                oa_symbol = get_oa_symbol(tradingsymbol, exchange)
                if oa_symbol:
                    tradingsymbol = oa_symbol
                    logger.debug(f"DEBUG - Converted to Max Algos symbol: {tradingsymbol}")
                else:
                    logger.debug(
                        f"DEBUG - Could not convert to Max Algos symbol, using original: {tradingsymbol}"
                    )
            except Exception as e:
                logger.error(f"DEBUG - Error converting symbol to Max Algos format: {e}")
