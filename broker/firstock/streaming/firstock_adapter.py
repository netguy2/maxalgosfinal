    ...
    def initialize(
        ...
        if not auth_data:
            ...
            if broker_api_key:
                ...
                # Fallback to Max Algos user ID if BROKER_API_KEY not set
                self.firstock_user_id = user_id
                self.logger.warning(
                    f"BROKER_API_KEY not found in environment. Using Max Algos user ID '{user_id}' which may fail authentication"
                )
