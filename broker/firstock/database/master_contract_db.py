...
def process_firstock_nse_data(output_path):
    """
    Processes the Firstock NSE data (NSE_symbols.csv) to generate Max Algos symbols.
    ...
    # For index rows, normalize to Max Algos-standard symbol using the
    # comprehensive mapping (symbol_maxalgos.md). Unlisted indices fall
    # through unchanged.
    index_mask = df["is_index"]
    if index_mask.any():
        df.loc[index_mask, "symbol"] = df.loc[index_mask].apply(
            lambda r: map_to_maxalgos_index_symbol(
...
def process_firstock_nfo_data(output_path):
    """
    Processes the Firstock NFO data (NFO_symbols.csv) to generate Max Algos symbols.
...
def process_firstock_bse_data(output_path):
    """
    Processes the Firstock BSE data (BSE_symbols.csv) to generate Max Algos symbols.

    Indices (SENSEX, SENSEX50, etc.) are identified by empty ISIN + zero
    TickSize + zero FreezeQty (same heuristic as NSE) and routed to
    exchange='BSE_INDEX' with Max Algos-standard symbol normalization.
    ...
    # For index rows, normalize to Max Algos-standard symbol (e.g. SENSEX,
...
def process_firstock_bfo_data(output_path):
    """
    Processes the Firstock BFO data (BFO_symbols.csv) to generate Max Algos symbols.
...
# Max Algos standard index symbols per symbol_maxalgos.md.
# Matching any fetched idxname/tradingSymbol against these (case- and
# whitespace-insensitive) normalizes to the Max Algos format; unmatched
# indices pass through unchanged so new/custom indices still get stored.
MAX_ALGOS_NSE_INDICES = {
...
# Explicit aliases for broker/exchange names that diverge from the Max Algos
...
# Pre-computed normalized lookup dicts: normalized_key -> canonical Max Algos symbol.
# Built once at module load so tolerant matching is a single dict lookup.
_NSE_INDEX_NORMALIZED_LOOKUP = {
    _normalize_index_key(s): s for s in MAX_ALGOS_NSE_INDICES
}
_BSE_INDEX_NORMALIZED_LOOKUP = {
    _normalize_index_key(s): s for s in MAX_ALGOS_BSE_INDICES
}
...
def _basic_index_cleanup(value):
    ...
    the stored symbol is a valid Max Algos-format token.
    """
    if not value:
        return ""
...
def map_to_maxalgos_index_symbol(trading_symbol, idxname, br_exchange):
    """
    Resolve the Max Algos-standard index symbol for a fetched Firstock index.

    Match order:
      1. tradingSymbol is already an Max Algos canonical (NIFTY, SENSEX, ...)
      2. tradingSymbol/idxname normalizes to an explicit alias (e.g. BSEIT)
      3. tradingSymbol/idxname normalizes to a canonical Max Algos symbol
...
def fetch_firstock_indices():
    ...
    try:
        ...
        for item in indices:
            ...
            maxalgos_symbol = map_to_maxalgos_index_symbol(
