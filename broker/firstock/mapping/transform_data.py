...
def transform_data(data, token, auth_token=None):
    """
    Transforms the Max Algos API request to Firstock's V1 /placeOrder structure.

    For MARKET and SL-M orders, applies client-side Market Price Protection
    (MPP): fetch LTP, compute a protected price via the Max Algos MPP slab,
    ...
    Args:
        data: Order data dictionary (Max Algos format).
...
def transform_modify_order_data(data, token, auth_token=None):
    ...
    used by transform_data so the price is owned by Max Algos. When we can't
    ...
    Expects data["symbol"] to be in Max Algos format; the broker symbol is
...
def map_order_type(pricetype):
    """
    Maps the Max Algos pricetype to Firstock's order type.
    """
...
def map_product_type(product):
    """
    Maps the Max Algos product type to Firstock's product type.
    """
...
def reverse_map_product_type(product):
    """
    Maps Firstock's product type to Max Algos product type.
    """
