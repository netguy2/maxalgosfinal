...
class BrokerData:
    ...
    def _fetch_single_quote_sync(
        ...
        try:
            ...
            # Flattrade's GetQuotes endpoint can return an empty body or an
            ...
            if not body_text or not body_text.strip():
                return {
                    "symbol": symbol,
                    "exchange": exchange,
                    "error": f"Flattrade API returned an empty response (HTTP {http_response.status_code})",
                }
            try:
                ...
                return {
                    "symbol": symbol,
                    "exchange": exchange,
                    "error": f"Flattrade API returned a non-JSON response (HTTP {http_response.status_code})",
                }
    ...
    async def _fetch_single_quote_async(
        ...
        try:
            ...
            if not body_text or not body_text.strip():
                logger.warning(
                    f"Flattrade returned empty response for {symbol}@{exchange} "
            ...
            try:
                ...
                logger.warning(
                    f"Flattrade returned non-JSON response for {symbol}@{exchange} "
                    f"(HTTP {http_response.status_code})"
                )
                return {
                    "symbol": symbol,
