...
def get_api_response(endpoint, auth, method="GET", payload=""):
    ...
    # Flattrade's API can return an empty body or an HTML error page
    # instead of a JSON error payload -- same Noren/PiConnect-vendor bug
    # already fixed in broker/zebu and broker/bnr's equivalent functions.
    if not data or not data.strip():
        raise ValueError(
            f"Flattrade API returned an empty response for {endpoint} (HTTP {response.status_code})"
        )
    try:
        return json.loads(data)
    except json.JSONDecodeError as e:
        raise ValueError(
            f"Flattrade API returned a non-JSON response for {endpoint} "
