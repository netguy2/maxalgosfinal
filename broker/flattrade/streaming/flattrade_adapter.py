"""
Flattrade WebSocket Adapter for Max Algos
...
class QuoteNormalizer:
    ...
    def normalize(data: dict[str, Any], msg_type: str) -> dict[str, Any]:
        ...
        # Flattrade only sends "pc" (its own percent-change field), not
        # "change"/"change_percent" -- every consumer (MarketDataManager.ts,
        # the Dashboard index cards) reads those exact key names. Same fix
        # as broker/zebu, broker/bnr, and broker/shoonya's streaming
        # adapters (identical Noren/PiConnect-vendor bug).
        if close:
            change = ltp - close
            result["change"] = round(change, 2)
            result["change_percent"] = round(change / close * 100, 2)
