...
class FlattradeExchangeMapper:
    """Maps between Max Algos exchange names and Flattrade exchange codes"""

    # Max Algos to Flattrade exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    # Reverse mapping
    FLATTRADE_TO_MAX_ALGOS = {v: k for k, v in EXCHANGE_MAP.items()}

    @classmethod
    def to_flattrade_exchange(cls, oa_exchange: str) -> str | None:
        """Convert Max Algos exchange to Flattrade exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, flattrade_exchange: str) -> str | None:
        """Convert Flattrade exchange to Max Algos exchange format"""
        return cls.FLATTRADE_TO_MAX_ALGOS.get(flattrade_exchange.upper())
