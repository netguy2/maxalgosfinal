...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "flattrade").delete(synchronize_session=False)
    db_session.commit()
...
def process_flattrade_nse_data(output_path):
    """
    Processes the Flattrade NSE data (NSE_Equity.csv) to generate Max Algos symbols.
...
def process_flattrade_nfo_data(output_path):
    """
    Processes the Flattrade NFO data (NFO.csv) to generate Max Algos symbols.
...
def process_flattrade_cds_data(output_path):
    """
    Processes the Flattrade CDS data (Currency_Derivatives.csv) to generate Max Algos symbols.
    Handles both futures and options formatting.
    """
...
def process_flattrade_mcx_data(output_path):
    """
    Processes the Flattrade MCX data (Commodity.csv) to generate Max Algos symbols.
    Handles both futures and options formatting.
    """
...
def process_flattrade_bse_data(output_path):
    """
    Processes the Flattrade BSE data (BSE_Equity.csv) to generate Max Algos symbols.
...
def process_flattrade_bfo_data(output_path):
    """
    Processes the Flattrade BFO data (BFO.csv) to generate Max Algos symbols.
    Handles both futures and options formatting.
    """
