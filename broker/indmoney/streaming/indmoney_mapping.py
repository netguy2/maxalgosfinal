...
class IndmoneyExchangeMapper:
    """Maps Max Algos exchange codes to INDmoney-specific segment codes"""
...
class IndmoneyModeMapper:
    ...
    # Mode mapping: Max Algos mode number -> INDmoney mode string
    MODE_MAP = {
    ...
    def get_indmoney_mode(mode):
        """
        Convert Max Algos mode number to INDmoney mode string

        Args:
            mode (int): Max Algos mode (1: LTP, 2: Quote)
    ...
    @staticmethod
    def get_maxalgos_mode(indmoney_mode):
        """
        Convert INDmoney mode string to Max Algos mode number

        Args:
            indmoney_mode (str): INDmoney mode ('ltp' or 'quote')

        Returns:
            int: Max Algos mode number
        """
