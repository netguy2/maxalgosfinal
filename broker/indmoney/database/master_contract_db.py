...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "indmoney").delete(synchronize_session=False)
    db_session.commit()
...
def copy_from_dataframe(df, broker="indmoney"):
    ...
    existing_tokens = {
        ...
        .filter(SymToken.broker == "indmoney")
        .all()
    }
...
def reformat_symbol(row, file_segment=None):
    """
    Reformat symbols according to Max Algos standards based on Indmoney data structure
    """
    ...
    # Format expiry date for Max Algos format (DDMMMYY)
...
def process_indmoney_csv(path):
    ...
    for segment in segments:
        ...
        # Generate Max Algos formatted symbol
