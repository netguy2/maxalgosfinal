...
def transform_margin_positions(positions):
    """
    Transform Max Algos margin position format to IndMoney margin format.
...
def map_segment(exchange):
    """
    Maps Max Algos exchange to IndMoney segment.

    Max Algos: NSE, BSE, NFO, BFO, CDS, BCD, MCX
...
def map_exchange_type(exchange):
    """
    Maps Max Algos exchange to IndMoney exchange format.
...
def map_product_type(product):
    """
    Maps Max Algos product type to IndMoney product type.

    Max Algos: CNC, NRML, MIS
...
def parse_margin_response(response_data):
    """
    Parse IndMoney margin calculator response to Max Algos standard format.

    Args:
        response_data: Raw response from IndMoney margin calculator API

    Returns:
        Standardized margin response matching Max Algos format
    """
    try:
        ...
        # Extract only the three required values as per Max Algos API specification
        total_margin = data.get("total_margin", 0)
        span_margin = data.get("span_margin", 0)
        exposure_margin = data.get("exposure_margin", 0)

        # Return standardized format matching Max Algos API specification
