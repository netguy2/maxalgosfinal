# Dhan Forever Order payload transforms (Max Algos ⇄ Dhan).
...
# Dhan Forever Order status → Max Algos GTT status.
_STATUS_MAP = {
    "TRANSIT": "active",
    "PENDING": "active",
    "CONFIRM": "active",
...
def transform_place_gtt(data):
    """Transform an Max Algos flat place-GTT payload into Dhan's POST /forever/orders body.
    ...
    # Max Algos's ``strategy`` doubles as Dhan's correlationId (max 30 chars).
...
def transform_modify_gtt(data, leg_name):
    """Transform an Max Algos modify-GTT payload into Dhan's PUT /forever/orders/{id} body.

    Dhan modifies one leg at a time. Field semantics dispatch by
    ``trigger_type`` (the Max Algos flag), not by ``leg_name`` — Dhan's leg
    ...
    for Max Algos a SINGLE always carries its values in ``price`` and the
    resolved trigger.
...
def map_gtt_book(gtt_list):
    """Normalise Dhan's GET /forever/orders response into an Max Algos-shaped list.
    ...
    one Max Algos entry per order. ``last_price`` is not returned by Dhan, so
