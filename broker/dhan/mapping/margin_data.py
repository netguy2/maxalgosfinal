...
def transform_margin_position(position, client_id):
    """
    Transform a single Max Algos margin position to Dhan margin format.
...
def map_product_type_for_margin(product):
    """
    Maps Max Algos product type to Dhan product type for margin calculation.
...
def parse_margin_response(response_data):
    """
    Parse Dhan single-order margin response to Max Algos standard format.

    Args:
        response_data: Raw response from Dhan API

    Returns:
        Standardized margin response matching Max Algos RESTX margin format
    """
    try:
