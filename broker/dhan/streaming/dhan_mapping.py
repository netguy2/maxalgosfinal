...
class DhanExchangeMapper:
    """Maps between Max Algos exchange names and Dhan exchange codes"""

    # Max Algos to Dhan exchange mapping
    EXCHANGE_MAP = {
        "NSE": "NSE_EQ",
        "BSE": "BSE_EQ",
        "NFO": "NSE_FNO",
    ...
    # Dhan exchange segment codes (numeric) to Max Algos exchange mapping
    ...
    # Reverse mappings
    DHAN_TO_MAX_ALGOS = {v: k for k, v in EXCHANGE_MAP.items()}
    EXCHANGE_TO_SEGMENT = {v: k for k, v in SEGMENT_TO_EXCHANGE.items()}

    @classmethod
    def get_dhan_exchange(cls, maxalgos_exchange: str) -> str | None:
        """Convert Max Algos exchange to Dhan exchange format"""
        return cls.EXCHANGE_MAP.get(maxalgos_exchange)

    @classmethod
    def get_maxalgos_exchange(cls, dhan_exchange: str) -> str | None:
        """Convert Dhan exchange to Max Algos exchange format"""
        return cls.DHAN_TO_MAX_ALGOS.get(dhan_exchange)

    @classmethod
    def get_exchange_from_segment(cls, segment_code: int) -> str | None:
        """Convert Dhan exchange segment code to Max Algos exchange"""
    ...
    def get_segment_from_exchange(cls, exchange: str) -> int | None:
        """Convert Max Algos exchange to Dhan exchange segment code"""
