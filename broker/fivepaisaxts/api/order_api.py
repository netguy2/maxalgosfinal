...
def get_api_response(endpoint, auth, method="GET", payload=""):
    ...
    # 5paisa XTS's API can return an empty body or an HTML error page
    # instead of a JSON error payload -- guard the same way the
    # per-function place/cancel/modify order calls already do in this file.
    try:
        return response.json()
    except json.JSONDecodeError as e:
        raise ValueError(
            f"5paisa XTS API returned a non-JSON response for {endpoint} "
