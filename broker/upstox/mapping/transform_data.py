...
def transform_data(data, token):
    ...
    transformed = {
        ...
        # Was the literal placeholder "string" (leftover from API docs
        # example) for EVERY order -- now a genuinely unique per-order
        # tag. See utils/order_tag.py.
        "tag": generate_order_tag(data.get("strategy")),
...
def reverse_map_product_type(exchange, product):
    """
    Reverse maps the broker product type to the Max Algos product type, considering the exchange.
    """
    # Exchange to Max Algos product type mapping for 'D'
    ...
    if product == "D":
        maxalgos_product = exchange_mapping_for_d.get(exchange)
        if not maxalgos_product:
            logger.warning(
                f"Could not reverse map product type 'D' for unknown exchange '{exchange}'."
            )
        return maxalgos_product
    elif product == "I":
        return "MIS"
    else:
