...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "zerodha").delete(synchronize_session=False)
    db_session.commit()
...
def copy_from_dataframe(df):
    ...
    existing_tokens = {
        ...
        .filter(SymToken.broker == "zerodha")
        .all()
    }
...
def process_zerodha_csv(path):
    ...
    # BEFORE we remap to Max Algos names, so brexchange retains the broker's
    ...
    # NSE Index Symbol Mapping (Zerodha tradingsymbol → Max Algos format)
