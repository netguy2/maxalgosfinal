...
def place_gtt_order(data, auth):
    ...
    # Zerodha's API can return an empty body or an HTML error page instead
    # of a JSON error payload -- guard the same way modify_gtt_order/
    # cancel_gtt_order/get_gtt_book already do below in this same file
    # (place_gtt_order alone was missing it).
    try:
...
def get_gtt_book(auth):
    ...
    Max Algos-normalised GTT objects (see ``map_gtt_book``).
    """
    client = get_httpx_client()
