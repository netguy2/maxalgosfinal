...
and Max Algos's standard format.
"""
...
class ZerodhaExchangeMapper:
    """Maps exchange codes between Zerodha and Max Algos formats"""

    # Map Max Algos exchange codes to Zerodha exchange codes
    ...
    # Map Zerodha exchange codes to Max Algos exchange codes
    ...
    def to_zerodha_exchange(cls, oa_exchange: str) -> str:
        """
        Convert Max Algos exchange code to Zerodha exchange code.

        Args:
            oa_exchange: Max Algos exchange code (e.g., 'NSE', 'NFO')
    ...
    def to_oa_exchange(cls, zerodha_exchange: str) -> str:
        """
        Convert Zerodha exchange code to Max Algos exchange code.

        Args:
            zerodha_exchange: Zerodha exchange code

        Returns:
            Max Algos exchange code
        """
...
class ZerodhaCapabilityRegistry:
    ...
    # Map Max Algos capability flags to Zerodha subscription modes
    CAPABILITY_MAP = {
        "LTP": "ltp",
        "QUOTE": "quote",
        "DEPTH": "full",
    ...
    def get_zerodha_mode(cls, capability: str) -> str:
        ...
        Args:
            capability: Max Algos capability (LTP, QUOTE, DEPTH)
...
class ZerodhaDataTransformer:
    """Transforms data between Zerodha and Max Algos formats"""
    ...
    def transform_tick(self, tick_data: dict, symbol: str, exchange: str) -> dict:
        """
        Transform Zerodha tick data to Max Algos format.
        ...
        Returns:
            Transformed tick data in Max Algos format
        """
        try:
            if not tick_data:
                return {}
    ...
    def transform_order_update(self, order_data: dict) -> dict:
        """
        Transform Zerodha order update to Max Algos format.

        Args:
            order_data: Raw order data from Zerodha WebSocket

        Returns:
            Transformed order data in Max Algos format
        """
        try:
            ...
            # Map Zerodha status to Max Algos status
            status_map = {
                "OPEN": "open",
                "COMPLETE": "complete",
                "CANCELLED": "cancelled",
    ...
    def transform_position(self, position_data: dict) -> dict:
        """
        Transform Zerodha position data to Max Algos format.

        Args:
            position_data: Raw position data from Zerodha

        Returns:
            Transformed position data in Max Algos format
        """
        try:
