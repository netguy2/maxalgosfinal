# Zerodha GTT payload transforms (Max Algos ⇄ Kite).
...
def transform_place_gtt(data):
    """Transform an Max Algos flat place-GTT payload into Kite's `{type, condition, orders}`.
...
def transform_modify_gtt(data):
    """Transform an Max Algos modify-GTT payload (flat shape) into Kite's body.
...
def map_gtt_book(gtt_data):
    """Normalise Kite's GET /gtt/triggers response into an Max Algos-shaped list.
    ...
    Kite tradingsymbol back to Max Algos's symbol.
    """
