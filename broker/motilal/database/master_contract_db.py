...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "motilal").delete(synchronize_session=False)
    db_session.commit()
...
def copy_from_dataframe(df):
    ...
    existing_tokens = {
        ...
        .filter(SymToken.broker == "motilal")
        .all()
    }
...
# "SNXT50", ...) while Max Algos needs canonical symbols per symbol_maxalgos.md.
# The mapping is kept local to this broker loader so other brokers — which
# already feed clean strings — aren't affected.

# Broker-house-style NSE index name -> Max Algos canonical symbol. Keys are
...
# Broker-house-style BSE index name -> Max Algos canonical symbol. Keys are
...
def standardize_index_symbols(df):
    """
    Standardize NSE_INDEX and BSE_INDEX symbol names to Max Algos canonical form
...
def process_motilal_index_csv(df, exchange_name):
    """
    Processes the Motilal Index CSV file to fit the Max Algos database schema.
    ...
    # Map exchange to Max Algos format with _INDEX suffix
    ...
    # Standardize index symbols to Max Algos format
...
def process_motilal_csv(df, exchange_name):
    """
    Processes the Motilal CSV file to fit the Max Algos database schema.
    ...
    # Rename columns based on Motilal API format to Max Algos schema
    df = df.rename(
        columns={
            "scripcode": "token",
            "scripname": "symbol",
    ...
    # Map Motilal exchange names to Max Algos exchange names
    exchange_map = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NSEFO": "NFO",
    ...
    # Format symbols according to Max Algos standards
