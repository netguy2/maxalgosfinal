...
def get_margin_data(auth_token):
    ...
    # Motilal's API can return an empty body or an HTML error page instead
    # of a JSON error payload -- guard the same way as order_api.py's
    # place/cancel/modify order functions in this same broker plugin.
    try:
        margin_data = response.json()
    except json.JSONDecodeError as e:
        raise ValueError(
            f"Motilal API returned a non-JSON response for getreportmargindetail "
    ...
    if margin_data.get("status") == "SUCCESS" and margin_data.get("data"):
        ...
        # Map Motilal Oswal fields to Max Algos standard fields
        margin_dict = {}
