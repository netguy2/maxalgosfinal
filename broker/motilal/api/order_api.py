...
def get_open_position(tradingsymbol, exchange, producttype, auth):
    ...
    # Map exchange from Max Algos format to Motilal format for comparison
...
def place_order_api(data, auth):
    ...
    # Parse the JSON response. Motilal's gateway can return an empty body
    # or an HTML error page on a transient failure -- guard the same way
    # as get_api_response() elsewhere in this file so a single failed call
    # never surfaces as an opaque JSONDecodeError.
    try:
        response_data = response.json()
    except json.JSONDecodeError as e:
        raise ValueError(
            f"Motilal API returned a non-JSON response for placeorder "
...
def close_all_positions(current_api_key, auth):
    ...
    if positions_response.get("status") == "SUCCESS":
        ...
        for position in positions_response["data"]:
            ...
            # Convert Motilal exchange to Max Algos exchange for symbol lookup
            motilal_exchange = position["exchange"]
            maxalgos_exchange = reverse_map_exchange(motilal_exchange)

            # Get maxalgos symbol to send to placeorder function
            symbol = get_symbol(position["symboltoken"], maxalgos_exchange)
            logger.debug(f"The Symbol is {symbol}")

            if not symbol:
                logger.error(
                    f"Symbol not found for token {position['symboltoken']} and exchange {maxalgos_exchange}"
                )
                continue
            ...
            place_order_payload = {
                ...
                "symbol": symbol,
                "action": action,
                "exchange": maxalgos_exchange,  # Use Max Algos exchange format
                "pricetype": "MARKET",
                "product": reverse_map_product_type(position["productname"], maxalgos_exchange),
...
def cancel_order(orderid, auth):
    ...
    try:
        ...
        raise ValueError(
            f"Motilal API returned a non-JSON response for cancelorder "
            f"(HTTP {response.status_code}): {response.text[:200]!r}"
        ) from e
...
def modify_order(data, auth):
    ...
    try:
        ...
        raise ValueError(
            f"Motilal API returned a non-JSON response for modifyorder "
            f"(HTTP {response.status_code}): {response.text[:200]!r}"
        ) from e
