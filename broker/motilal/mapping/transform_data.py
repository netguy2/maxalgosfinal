...
from utils.order_tag import generate_order_tag
...
def map_exchange(exchange):
    """
    Maps Max Algos exchange names to Motilal Oswal exchange names.

    Max Algos uses: NSE, BSE, NFO, CDS, MCX, BFO
...
def reverse_map_exchange(exchange):
    """
    Reverse maps Motilal Oswal exchange names to Max Algos exchange names.
    """
...
def transform_data(data, token, auth_token=None):
    """
    Transforms the Max Algos API request structure to Motilal Oswal expected structure.
    ...
    maxalgos_exchange = data["exchange"]
    motilal_exchange = map_exchange(maxalgos_exchange)
    ...
    if pricetype == "MARKET":
        logger.info(
            ...
            f"Exchange={maxalgos_exchange}, Action={action}"
        )
        try:
            if auth_token:
                ...
                quote_data = broker_data.get_quotes(data["symbol"], maxalgos_exchange)
                ltp = float(quote_data.get("ltp", 0)) if quote_data else 0

                if ltp > 0:
                    instrument_type = get_instrument_type_from_symbol(data["symbol"])
                    tick_size = None
                    symbol_info = get_symbol_info(data["symbol"], maxalgos_exchange)
    ...
    transformed = {
        ...
        "producttype": map_product_type(
            data["product"], maxalgos_exchange
        ),  # Pass Max Algos exchange for context
        ...
        # algoid intentionally left blank -- this is the Exchange-issued
        # algo ID from SEBI empanelment/registration (a business/legal
        # process with the Exchange via the broker), not something this
        # code can generate. Filling it with a fake value would be worse
        # than leaving it blank (misrepresenting an unregistered order as
        # Exchange-registered).
        "algoid": "",  # Algo Id or Blank for Non-Algo Orders
        "goodtilldate": "",  # DD-MMM-YYYY format if GTD
        # Unique per-order tag (was a hardcoded "" for every order) --
        # see utils/order_tag.py. Motilal caps this at 10 characters.
        "tag": generate_order_tag(data.get("strategy"))[:10],
...
def transform_modify_order_data(data, token, lastmodifiedtime, qtytradedtoday):
    ...
    Args:
        data: Max Algos modify order request data
...
def map_order_type(pricetype):
    """
    Maps Max Algos pricetype to Motilal Oswal order type.
...
def map_product_type(product, exchange=None):
    """
    Maps Max Algos product type to Motilal Oswal product type based on exchange.
    ...
    Args:
        product: Max Algos product type (CNC, MIS, NRML)
        exchange: Max Algos exchange name (NSE, BSE, NFO, CDS, MCX, BFO)
    """
    # Determine if this is a cash segment or derivative segment
    # Using Max Algos exchange names
...
def reverse_map_product_type(product, exchange=None):
    """
    Reverse maps Motilal Oswal product type to Max Algos product type.
    Context:
