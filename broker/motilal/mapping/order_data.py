...
def map_order_data(order_data):
    ...
    if order_data:
        for order in order_data:
            ...
            # Convert Motilal exchange (NSEFO) to Max Algos exchange (NFO) for database lookup
            maxalgos_exchange = reverse_map_exchange(motilal_exchange)

            # Use the get_symbol function to fetch the symbol from the database
            # Use Max Algos exchange format for lookup
            symbol_from_db = get_symbol(symboltoken, maxalgos_exchange)

            # Check if a symbol was found; if so, update the trading_symbol in the current order
            if symbol_from_db:
                order["symbol"] = symbol_from_db  # Motilal uses 'symbol' field
                # Convert exchange to Max Algos format for display
                order["exchange"] = maxalgos_exchange

                # Map Motilal product types to Max Algos format
                if maxalgos_exchange in ["NSE", "BSE"]:
                ...
                elif maxalgos_exchange in ["NFO", "MCX", "CDS", "BFO"]:
                ...
                logger.info(
                    f"Symbol not found for token {symboltoken} and exchange {maxalgos_exchange}. Keeping original trading symbol."
                )
                # Still convert exchange to Max Algos format
                order["exchange"] = maxalgos_exchange
...
def transform_order_data(orders):
    ...
    for order in orders:
        ...
        # Map Motilal order types to Max Algos standard format
        # Motilal returns: Market, Limit, Stoploss (title case)
        # Max Algos standard: MARKET, LIMIT, SL, SL-M (uppercase)
        ...
        # Map Motilal order status to Max Algos standard format
        # Motilal returns: Traded, Confirm, Sent, Error, Rejected, Cancel (title case)
        # Max Algos standard: complete, open, rejected, cancelled (lowercase)
...
def map_trade_data(trade_data):
    ...
    if trade_data:
        for order in trade_data:
            ...
            motilal_exchange = order["exchange"]
            # Convert Motilal exchange to Max Algos exchange for database lookup
            maxalgos_exchange = reverse_map_exchange(motilal_exchange)

            # Use the get_symbol function to fetch the symbol from the database
            symbol_from_db = get_oa_symbol(symbol, maxalgos_exchange)

            # Check if a symbol was found; if so, update the trading_symbol in the current order
            if symbol_from_db:
                order["symbol"] = symbol_from_db
                # Convert exchange to Max Algos format
                order["exchange"] = maxalgos_exchange
                ...
                logger.info(
                    f"Unable to find the symbol {symbol} and exchange {maxalgos_exchange}. Keeping original trading symbol."
                )
                order["exchange"] = maxalgos_exchange
...
def transform_tradebook_data(tradebook_data):
    """
    Transforms Motilal Oswal tradebook data to Max Algos format.
    ...
    if position_data_list:
        for position in position_data_list:
            ...
            if symbol_from_db:
                ...
                # Convert exchange to Max Algos format
                position["exchange"] = maxalgos_exchange
                ...
                logger.info(
                    f"Symbol not found for token {symboltoken} and exchange {maxalgos_exchange}. Keeping original symbol."
                )
                position["exchange"] = maxalgos_exchange
...
def transform_positions_data(positions_data):
    """
    Transforms Motilal Oswal positions data to Max Algos format.
...
def transform_holdings_data(holdings_data):
    """
    Transforms Motilal Oswal holdings data to Max Algos format.
    ...
    for holdings in holdings_list:
        # Get the mapped Max Algos symbol and exchange from map_portfolio_data
...
def map_portfolio_data(portfolio_data):
    ...
    Returns:
    - The modified portfolio_data with mapped fields (Max Algos symbols and exchange).
    """
    ...
    if isinstance(data, list):
        for idx, holding in enumerate(data):
            ...
            # Use get_symbol to fetch the Max Algos symbol from database
