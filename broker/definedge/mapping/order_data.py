...
def map_order_data(order_data):
    ...
    if orders:
        for order in orders:
            ...
            # Convert broker symbol to Max Algos format
            if symbol and exchange:
                oa_symbol = get_oa_symbol(brsymbol=symbol, exchange=exchange)
                if oa_symbol:
                    order["tradingsymbol"] = oa_symbol

                    # Map product types to Max Algos constants following Angel pattern
...
def transform_order_data(orders):
    ...
    for order in orders:
        ...
        # Map DefinedGe order status to Max Algos format
        status = order.get("order_status", "").upper()
        if status in ["COMPLETE", "EXECUTED"]:
...
def map_trade_data(trade_data):
    ...
    if trades:
        for trade in trades:
            ...
            # Map product types to Max Algos constants (following Max Algos order constants)
...
def transform_tradebook_data(tradebook_data):
    """
    Transform DefinedGe tradebook data to Max Algos format.

    Parameters:
    - tradebook_data: List of trade dictionaries from DefinedGe API

    Returns:
    - List of transformed trade dictionaries matching Max Algos format
    """
    transformed_data = []
    ...
    for trade in tradebook_data:
        ...
        # Map product type to Max Algos format
        product_type = trade.get("product_type", "")
        if product_type == "INTRADAY":
            product_type = "MIS"
        elif product_type == "NORMAL":
        ...
        transformed_trade = {
            "symbol": trade.get("tradingsymbol", ""),
            "exchange": trade.get("exchange", ""),
            "product": product_type,  # Mapped to Max Algos constants
...
def map_position_data(position_data):
    ...
    if positions:
        for position in positions:
            ...
            # Map product types to Max Algos constants
...
def transform_positions_data(positions_data):
    """
    Transform DefinedGe positions data to Max Algos format.
...
def transform_holdings_data(holdings_data):
    """
    Transform DefinedGe holdings data to Max Algos format.
    Following Angel's pattern for consistency.
    """
    transformed_data = []
