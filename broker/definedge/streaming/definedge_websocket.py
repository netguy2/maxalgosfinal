...
class DefinedGeWebSocket:
    ...
    def __init__(self, auth_data, token_provider=None):
        ...
        # Serializes every ws.send() call (auth, heartbeat, subscribe,
        # unsubscribe) against every other one. These are called from at
        # least two threads (the heartbeat thread and the caller thread for
        # subscribe/unsubscribe), and unsynchronized concurrent writes to
        # the same socket can corrupt the outgoing frame stream. This mirrors
        # the fix applied to the other Noren-family adapters (Zebu, BNR,
        # Shoonya, Flattrade) after that exact race was confirmed in
        # production causing dropped connections roughly every 30-60s. This
        # adapter's run_forever() doesn't pass ping_interval (so
        # websocket-client's own protocol ping is already off by default,
        # unlike those four), but the same lock is added here for the
        # remaining app-level writer race.
