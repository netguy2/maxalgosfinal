...
def close_all_positions(current_api_key, auth):
    ...
    for position in positions_to_close:
        try:
            ...
            if not symbol:
                logger.error(f"Failed to get Max Algos symbol for {tradingsymbol} on {exchange}")
                symbol = tradingsymbol  # Use original as fallback

            logger.debug(f"Max Algos symbol: {symbol}")
...
def modify_order(data, auth):
    ...
    # Same JSON-decode guard as place_order_api above -- Definedge's API can
    # return an empty body or an HTML error page instead of a JSON error
    # payload, which modify_order was missing while place_order_api and
    # cancel_order already had it.
    try:
