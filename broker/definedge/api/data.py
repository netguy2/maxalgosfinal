...
class BrokerData:
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            # Map Definedge response fields to Max Algos format
    ...
    def _fetch_single_quote_sync(
        ...
        try:
            ...
            # A 200 status doesn't guarantee a JSON body -- guard the parse
            # itself too (empty body / HTML error page), same fix already
            # applied to the Noren-family brokers' equivalent batch-quote
            # functions.
            try:
                ...
                return {
                    "symbol": symbol,
                    "exchange": exchange,
                    "error": f"Definedge API returned a non-JSON response (HTTP {http_response.status_code})",
                }
    ...
    def get_history(
        ...
        try:
            ...
            if interval == "D":
                ...
                # This ensures the Max Algos client interprets them correctly
            ...
            # Reorder columns to match Max Algos format (timestamp should be 5th column)
