...
def transform_margin_position(position, client_id):
    """
    Transform a single Max Algos margin position to Dhan Sandbox margin format.

    Note: Dhan Sandbox margin calculator API accepts only one order at a time, not a batch.

    Args:
        position: Position in Max Algos format
...
def map_product_type_for_margin(product):
    """
    Maps Max Algos product type to Dhan Sandbox product type for margin calculation.
...
def parse_margin_response(response_data):
    """
    Parse Dhan Sandbox margin response to Max Algos standard format.
    ...
    try:
        ...
        # Return standardized format with Max Algos-compatible aliases.
        return {
            "status": "success",
            "data": {
...
def parse_multi_margin_response(response_data):
    """
    Parse Dhan Sandbox /v2/margincalculator/multi response to Max Algos standard format.
