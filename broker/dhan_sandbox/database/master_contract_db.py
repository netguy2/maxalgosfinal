...
def delete_symtoken_table():
    ...
    SymToken.query.filter(SymToken.broker == "dhan_sandbox").delete(synchronize_session=False)
    db_session.commit()
...
def copy_from_dataframe(df):
    ...
    for row in data_dict:
        row["broker"] = "dhan_sandbox"

    # Retrieve existing tokens to filter them out from the insert
    existing_tokens = {
        result.token
        for result in db_session.query(SymToken.token)
        .filter(SymToken.broker == "dhan_sandbox")
        .all()
    }
...
def process_dhan_csv(path):
    ...
    # Normalize NSE_INDEX symbols: uppercase, remove spaces, only for symbols in Max Algos docs
    ...
    # Revert symbols not in the Max Algos standard set
