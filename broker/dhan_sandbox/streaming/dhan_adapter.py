"""
Fixed Dhan WebSocket adapter for Max Algos.
...
from .dhan_mapping import get_dhan_exchange, get_maxalgos_exchange
...
class DhanWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    Implements Max Algos WebSocket proxy interface with proper mode mapping.
    """
    ...
    def __init__(self):
        ...
        # Extended mode mapping to handle all possible Max Algos modes
        self.mode_map = {
            # Standard Max Algos modes
    ...
    def subscribe(
        ...
        try:
            ...
            # Map Max Algos mode to Dhan mode with explicit handling of all possible modes
            if mode == 1:
                # Standard LTP mode
                dhan_mode = DhanWebSocket.MODE_LTP
                self.logger.info(f"Mapped Max Algos mode {mode} (LTP) to Dhan mode '{dhan_mode}'")
            elif mode == 2:
                # Standard QUOTE mode
                dhan_mode = DhanWebSocket.MODE_QUOTE
                self.logger.info(f"Mapped Max Algos mode {mode} (QUOTE) to Dhan mode '{dhan_mode}'")
            elif mode == 3:
                # Standard DEPTH mode
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(f"Mapped Max Algos mode {mode} (DEPTH) to Dhan mode '{dhan_mode}'")
            else:
                # All other modes (4-8) map to FULL/DEPTH
                dhan_mode = DhanWebSocket.MODE_FULL
                self.logger.info(
                    f"Mapped Max Algos mode {mode} (DEPTH/FULL) to Dhan mode '{dhan_mode}'"
                )

            # Add symbol to subscription tracking
            with self.lock:
                self.subscribed_symbols[symbol] = {
                    "exchange": exchange,  # Store original Max Algos exchange
            ...
            # Map Max Algos exchange to Dhan exchange code
    ...
    def _on_ticks(self, ticks: list[dict[str, Any]]):
        ...
        try:
            ...
            for tick in ticks:
                ...
                # Convert to Max Algos format for data field
                ...
                # Normalize tick format to Max Algos standard
    ...
    def _generate_topic(self, symbol: str, exchange: str, mode_str: str) -> str:
        ...
        Args:
            ...
            exchange: The exchange code in Max Algos format
    ...
    def _normalize_tick(self, tick: dict[str, Any]) -> dict[str, Any]:
        """
        Normalize Dhan tick data to Max Algos format.
        ...
        try:
            ...
            # Ensure exchange is in Max Algos format
            exchange = tick.get("exchange")
            if exchange:
                maxalgos_exchange = get_maxalgos_exchange(exchange)
            else:
                maxalgos_exchange = exchange
    ...
    def validate_tick_parsing(self, ticker_symbols=None, timeout=30):
        ...
        try:
            ...
            original_callbacks = {}
            for mode in [1, 2, 3, 4, 5]:  # Max Algos modes
