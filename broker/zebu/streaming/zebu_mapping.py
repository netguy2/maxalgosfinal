...
class ZebuExchangeMapper:
    """Maps between Max Algos exchange names and Zebu exchange codes"""

    # Max Algos to Zebu exchange mapping (same as Flattrade/Noren)
    EXCHANGE_MAP = {
        "NSE": "NSE",
        "BSE": "BSE",
        "NFO": "NFO",
    ...
    # Reverse mapping
    ZEBU_TO_MAX_ALGOS = {v: k for k, v in EXCHANGE_MAP.items()}

    @classmethod
    def to_zebu_exchange(cls, oa_exchange: str) -> str | None:
        """Convert Max Algos exchange to Zebu exchange format"""
        return cls.EXCHANGE_MAP.get(oa_exchange.upper())

    @classmethod
    def to_oa_exchange(cls, zebu_exchange: str) -> str | None:
        """Convert Zebu exchange to Max Algos exchange format"""
        return cls.ZEBU_TO_MAX_ALGOS.get(zebu_exchange.upper())
