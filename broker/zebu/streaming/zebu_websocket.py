...
class ZebuWebSocket:
    ...
    # Batched subscription API config -- mirrors
    # broker/shoonya/streaming/shoonya_websocket.py. Large scrip lists (e.g.
    # an option-chain-sized burst) are chunked into MAX_SCRIPS_PER_BATCH-sized
    # '#'-joined messages with SUBSCRIPTION_DELAY between sends, drained by a
    # single worker thread rather than sent as one message per scrip.
    ...
    def __init__(
        ...
        # can both write to the same underlying socket concurrently with
        # websocket-client's own internal ping thread (run_forever's
        # ping_interval=30/ping_timeout=10) -- two heartbeat mechanisms plus
        # arbitrary app writes, all unsynchronized on one socket. That race
        # corrupts the outgoing frame stream, which Zebu's Noren server
        # responds to by dropping the raw TCP connection (observed in prod
        # as "WebSocket closed: None - None" -- no status code/message, i.e.
        # not a graceful protocol-level close) roughly every 30-60s, wiping
        # the tick cache and subscriptions on every cycle. This is a plain
        # thread-safety bug in websocket-client usage, not a Zebu API limit.
        self._send_lock = threading.Lock()

        # Batched subscription queue -- see MAX_SCRIPS_PER_BATCH above.
    ...
    def _run_websocket(self) -> None:
        ...
        try:
            ...
            # heartbeat ({"t":"h"} every HEARTBEAT_INTERVAL, see
            # _heartbeat_worker/_check_connection_health) which is what
            # Zebu's server actually expects and acknowledges -- running
            # BOTH meant two independent, unsynchronized writers could hit
            # the same socket at once (this thread's internal ping plus the
            # heartbeat-worker thread's send), corrupting the frame stream.
            # That surfaced in prod as the connection being dropped roughly
            # every 30-60s with "WebSocket closed: None - None" (a raw TCP
            # drop, no proper close frame) -- wiping ticks/subscriptions on
            # every cycle and looking like frozen/stale live data in the UI.
    ...
    def _heartbeat_worker(self) -> None:
        ...
        # A dead-socket send failure or a health-check timeout means the
        # connection is gone even if the underlying websocket-client
        # library's on_close callback never fires for it (e.g. a silent
        # TCP black-hole with no proper close frame -- exactly what
        # ping_interval=0 above works around for OUR outgoing pings, but
        # doesn't guarantee for a wedged socket object on close()). Without
        # this, `self.connected` can be stuck True forever once this loop
        # exits, since nothing else re-checks it: the higher-level adapter
        # (zebu_adapter.py) only ever hears about a dead connection via the
        # on_close callback forwarded below -- it never polls `connected`
        # itself -- so the adapter (and the WS proxy's ConnectionPool,
        # which trusts the ADAPTER's own connected flag) would otherwise
        # never learn this session died and never reconnect.
        if self.running and self.connected:
            self.logger.warning(
                "Heartbeat worker detected a dead connection -- forcing "
                "disconnected state and notifying the adapter to reconnect."
            )
            # Not _on_close()/_cleanup_connection_state(): both call
            # _stop_heartbeat(), which would try to join() this very
            # thread from itself (RuntimeError, since we ARE the heartbeat
            # thread). Set the flag this thread's own loop condition reads
            # directly, then forward to the adapter exactly like a real
            # close would -- that callback is what actually triggers
            # zebu_adapter.py's reconnection logic.
            self.connected = False
            self._heartbeat_thread = None
            self._call_external_callback(self.on_close, self.ws, None, "heartbeat timeout")
