...
# Global cache to prevent hitting Alice Blue API repeatedly and hanging requests
_QUOTES_CACHE = {}
_CACHE_LOCK = Lock()
_CACHE_TTL = 10.0  # Cache quotes for 10 seconds

# Global WebSocket connection to share a single connection and prevent thread leaks
...
class BrokerData:
    ...
    def get_websocket(self, force_new=False):
        ...
        with _WS_LOCK:
            ...
            if _GLOBAL_WEBSOCKET:
                if hasattr(_GLOBAL_WEBSOCKET, "is_websocket_connected"):
                    is_conn = _GLOBAL_WEBSOCKET.is_websocket_connected()
                else:
                    is_conn = getattr(_GLOBAL_WEBSOCKET, "is_connected", False)

            if not force_new and _GLOBAL_WEBSOCKET and is_conn:
            ...
            try:
                ...
                logger.info("Creating new global WebSocket connection for AliceBlue")
                _GLOBAL_WEBSOCKET = AliceBlueWebSocket(user_id, self.session_id)
                _GLOBAL_WEBSOCKET.connect()

                # Wait for connection to establish
                wait_time = 0
                max_wait = 10  # Maximum 10 seconds to wait
                while wait_time < max_wait and not _GLOBAL_WEBSOCKET.is_connected:
                    time.sleep(0.5)
                    wait_time += 0.5

                if not _GLOBAL_WEBSOCKET.is_connected:
                ...
                logger.exception("Error in get_websocket")
                return None
    ...
    def _map_exchange(self, exchange: str) -> str:
        """Map Max Algos exchange codes to AliceBlue API exchange codes."""
        exchange_map = {
            "NSE_INDEX": "NSE",
            "BSE_INDEX": "BSE",
            "MCX_INDEX": "MCX",
    ...
    def get_quotes(self, symbol: str, exchange: str) -> dict:
        ...
        Returns:
            dict: Quote data in Max Algos standard format
        """
        # Check cache first
        ...
        try:
            ...
            if symbols_to_fetch:
                ...
                if len(symbols_to_fetch) > BATCH_SIZE:
                    logger.info(f"Processing {len(symbols_to_fetch)} symbols in batches of {BATCH_SIZE}")
                    ...
                    fetched_results = self._process_multiquotes_batch(symbols_to_fetch)
            ...
            logger.info(f"Successfully processed {len(final_results)} quotes")
    ...
    def get_depth(self, symbol: str, exchange: str) -> dict:
        ...
        Returns:
            dict: Market depth data in Max Algos standard format
        """
        try:
            ...
            # Return in Max Algos standard format (matching Angel broker)
            return {
                "bids": bids,
                "asks": asks,
