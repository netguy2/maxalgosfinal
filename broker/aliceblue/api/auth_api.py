...
def authenticate_broker(userid, authCode):
    ...
    try:
        ...
        # NOTE: this is the vendor API host (a3.aliceblueonline.com), which
        # is DIFFERENT from the login/redirect UI host (ant.aliceblueonline.com)
        # used to build the OAuth login URL. Posting the checksum request to
        # the wrong (ant) host caused "Invalid vendor name or vendor key" -
        # per AliceBlue's own docs (ant.aliceblueonline.com/productdocumentation/Authentication/),
        # the vendor/getUserDetails endpoint lives on a3.aliceblueonline.com.
        ...
        # TEMPORARY diagnostic: force this into log/errors.jsonl (always-on
        # JSON error log, unlike console/file logs which are gated by
        # LOG_TO_FILE) so the raw response shape survives a retry attempt
        # for debugging the "Failed to extract session" issue. Remove once
        # the actual response shape is confirmed and parsing is fixed.
        logger.error(f"[TEMP DIAGNOSTIC] AliceBlue raw response: {json.dumps(data_dict)}")
