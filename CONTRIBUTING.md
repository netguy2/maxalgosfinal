# Contributing to Max Algos

## Let's democratize algorithmic trading, together!

We're thrilled that you're interested in contributing to Max Algos! This guide will help you get started, whether you're fixing a bug, adding a new broker, improving documentation, or building new features.

Below you'll find everything you need to set up Max Algos on your computer and start contributing.

---

## Our Mission

Max Algos is built **by traders, for traders**. We believe in democratizing algorithmic trading by providing a broker-agnostic, open-source platform that puts control back in the hands of traders. Every contribution, no matter how small, helps us achieve this mission.
...
Max Algos uses a **Python Flask** backend with a **React 19** single-page application frontend.
...
# Clone your fork
git clone https://github.com/YOUR_USERNAME/maxalgos.git
cd maxalgos

# Add upstream remote
git remote add upstream https://github.com/marketcalls/maxalgos.git
...
>
> After forking, go to your fork's **Settings → Actions → General** (`https://github.com/YOUR_USERNAME/maxalgos/settings/actions`) and select **"Disable actions"** under Actions permissions. This prevents CI workflows (frontend builds, Docker pushes) from running on your fork unnecessarily — those workflows are only meant to run on the upstream repository.
...
#### Order-Placement Paths (Safety-Critical)

If you add or modify code that sends an order to a broker, it **must** call
`check_order_allowed()` from `services/order_gate.py` immediately before the
broker call and **after** any analyze/sandbox branch. This is the single
enforcement point for the master kill switch; a path that skips it will place
live orders while the UI reports trading as halted.

Blocked orders must also publish `OrderFailedEvent` / `GTTFailedEvent` — the
`order.failed` subscribers drive the user-facing error toast and notification
drawer, so returning an error without publishing blocks the order *silently*.

`test/test_order_gate.py` enforces both rules statically and will fail CI if a
new order path is added without them. See
`docs/audit/order-path-safety-gates.md` for background and open items.

**Auditing "did we get every order path?"** — grep by *intent* (what actually
reaches a broker), not by function name or import pattern. Naming in this area
is inconsistent (`place_order_api`, `place_smartorder_api`, `place_gtt_order`;
`import_broker_module`, `import_broker_gtt_module`), and three successive
name-based greps each under-reported the real set. Enumerate the full surface:
...
Max Algos follows a strict **incremental contribution** standard. We require all contributions to be submitted as:
...
Max Algos supports **a growing list of brokers**, and every change must be validated across this broad surface area. Large integrations submitted in a single PR require extensive manual testing and verification that is not practical for the maintainers to review all at once.
...
**If you have a large integration or project built on Max Algos:**

We appreciate and encourage projects built on top of Max Algos (it's why we're open-source!). However, we cannot merge large codebases as a single contribution. Instead, extract individual improvements, fixes, or self-contained features and submit them separately. This gives each contribution a much better chance of being reviewed and accepted.
...
2. **Bug Fixes**
   - Check [issues labeled "good first issue"](https://github.com/marketcalls/maxalgos/labels/good%20first%20issue)
...
├── mapping/
│   ├── order_data.py         # Transform Max Algos format to broker format
...
Max Algos uses [shadcn/ui](https://ui.shadcn.com/) built on Radix UI primitives with Tailwind CSS:
...
1. **Python Docstrings** - Use Google-style:
   ...
   def place_order(symbol, quantity, price, order_type):
       ...
       Args:
           symbol (str): Trading symbol in Max Algos format
...
- **GitHub Discussions**: Ask questions in [GitHub Discussions](https://github.com/marketcalls/maxalgos/discussions)
- **Documentation**: Check [docs.maxalgos.in](https://docs.maxalgos.in)
- **GitHub Issues**: Report bugs in [Issues](https://github.com/marketcalls/maxalgos/issues)

### Before Asking for Help

1. **Search existing issues** — your question might already be answered
2. **Check documentation** — review docs at docs.maxalgos.in
...
- **Part of the Max Algos community** on Discord
...
- **Repository**: [github.com/marketcalls/maxalgos](https://github.com/marketcalls/maxalgos)
- **Issue Tracker**: [github.com/marketcalls/maxalgos/issues](https://github.com/marketcalls/maxalgos/issues)
...
- **PyPI Package**: [pypi.org/project/maxalgos](https://pypi.org/project/maxalgos)
- **YouTube**: [youtube.com/@maxalgosHQ](https://youtube.com/@maxalgosHQ)
- **Twitter/X**: [@maxalgosHQ](https://twitter.com/maxalgosHQ)

---

## License

Max Algos is released under the **AGPL v3.0 License**. See the [LICENSE](License.md) file for details.

By contributing to Max Algos, you agree that your contributions will be licensed under the AGPL v3.0 License.

---

## Thank You!

Thank you for contributing to Max Algos! Your efforts help democratize algorithmic trading and empower traders worldwide. Every line of code, documentation improvement, and bug report makes a difference.

**Happy coding, and welcome to the Max Algos community!**
