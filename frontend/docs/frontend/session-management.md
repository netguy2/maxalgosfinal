...
Max Algos supports multi-device login with automatic session synchronization. A user can authenticate once with their broker (OAuth) and then access Max Algos from additional devices without repeating the broker login flow.
