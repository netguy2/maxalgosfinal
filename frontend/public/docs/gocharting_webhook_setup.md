...
GoCharting is a professional HTML5 charting platform optimized for Indian markets. With a **Premium Plan**, you can set up webhook alerts to automate your trading strategies with Max Algos.
...
### 2. Max Algos Setup
- Max Algos instance running and accessible
- API key configured in Max Algos
...
If you're running Max Algos locally (on your computer) and want to test webhooks, you'll need to expose your local server to the internet using a tunneling service.
...
2. Start the tunnel pointing to port 5000 (Max Algos's default port)
3. Copy the public URL provided by the tunneling service
4. Update your `.env` file:
   ```env
   HOST_SERVER=https://your-tunnel-url.com
   ```
5. Restart Max Algos application
...
1. **Login to Max Algos**
   - Navigate to your Max Algos dashboard
...
5. **Configure Delivery Methods**
   ...
   - ☑️ **Webhook**: Enable this for Max Algos integration

   **Webhook URL**: Paste your Max Algos webhook URL
   ```
   https://yourdomain.com/api/v1/placeorder
   ```
...
1. **Enable Sandbox Mode in Max Algos**
...
1. Enable **Analyze Mode** in Max Algos (toggle in navbar)
2. Create a test alert in GoCharting with a condition that will trigger immediately
3. Check the **API Analyzer** in Max Algos to see the incoming request
...
```json
{
  "apikey": "your_api_key_here",      // Your Max Algos API key
...
|-------|----------|-------------|--------------|
| apikey | Yes | Your Max Algos API key | String |
...
4. Ensure Max Algos is running and accessible from internet

### Issue: Orders not placing

**Solutions:**
1. Verify API key is correct in the JSON payload
2. Check broker connection in Max Algos dashboard
3. Verify sufficient funds in trading account
4. Check symbol format matches your broker's convention
5. Review logs in Max Algos (Logs menu)

### Issue: "API key not found" error

**Solutions:**
1. Go to Max Algos → API Key section
...
4. **Test tunnel accessibility**
   ...
   - You should see Max Algos login page
...
### 3. Monitor Activity
- Regularly check order logs in Max Algos
...
Max Algos has rate limiting to prevent abuse:
...
### Documentation
- Max Algos Docs: https://docs.maxalgos.in
- GoCharting Help: https://gocharting.com/help

### Community
- Max Algos Website: https://maxalgos.in
- Community & Support: https://maxalgos.in/discord

### Video Tutorials
- Check Max Algos YouTube channel for video guides
...
A: Check the Logs section in Max Algos or use the API Analyzer in Analyze Mode.
...
**Happy Trading with GoCharting and Max Algos! 🚀**
