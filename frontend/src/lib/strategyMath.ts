...
export type Segment = 'OPTION' | 'FUTURE' | 'EQUITY'
...
export interface StrategyLeg {
  ...
  lotSize: number
  /**
   * Max Algos format, e.g. 28APR26. Not applicable to EQUITY legs (plain
   * cash stock has no expiry) — empty string for those.
   */
  expiry: string
  ...
  price: number
  /** Live IV (%) at the time of building. 0 if unknown. Unused for EQUITY. */
  iv: number
  active: boolean
  /** Symbol for display / Greeks lookup */
  symbol: string
  /** Cash exchange (NSE/BSE) — only set for EQUITY legs. */
  ...
  exitPrice?: number
  /**
   * EQUITY only — planning fields for the Trade Preview's R:R display.
   * Informational: this platform has no GTT/bracket order-placement API
   * today, so these are not wired to a live order.
   */
  equityProduct?: 'CNC' | 'MIS'
  equityOrderType?: 'MARKET' | 'LIMIT'
...
export function legPnlAt(
  ...
  // EQUITY legs track a DIFFERENT underlying than the option chain's spot
  // (e.g. INFY vs. NIFTY) — they must not be re-priced against `underlying`
  // here. The payoff/simulator sweep is specific to the option chain's
  // underlying, so an equity leg's contribution is held flat at 0 in this
  // curve; its real live P&L is shown separately in PnLTab via its own
  // WebSocket/quote price, same mechanism already used for entry/current
  // price comparison there.
  if (leg.segment === 'EQUITY') return 0
...
/** Format symbol per Max Algos standard: BASE[DDMMMYY][STRIKE][CE|PE]. */
