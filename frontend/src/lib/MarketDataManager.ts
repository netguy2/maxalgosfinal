...
export class MarketDataManager {
  ...
  async connect(): Promise<void> {
    ...
    // Reset failure counters so a fresh connect attempt (e.g., after re-login)
    // doesn't immediately re-trigger the fallback mode that was engaged by a
    // prior session's errors.  disableFallbackMode() also stops any polling
    // loop that was running on behalf of the old session.
      ...
      socket.onopen = async () => {
        ...
        try {
          ...
          if (apiKeyData.status === 'success' && apiKeyData.api_key) {
            ...
            // No platform API key yet — try to get one from session-status
            // (which now auto-generates it on first broker connect) then
            // immediately switch to REST fallback so data still flows.
            try {
              const ssResp = await fetch('/auth/session-status', { credentials: 'include' })
              const ssData = await ssResp.json()
              if (ssData.api_key) {
                this.apiKey = ssData.api_key
                this.setConnectionState('authenticating')
                socket.send(JSON.stringify({ action: 'authenticate', api_key: ssData.api_key }))
              } else {
                // Still no API key — enable REST fallback mode silently
                socket.close(1000, 'No API key — switching to REST fallback')
    ...
    try {
      ...
      switch (type) {
        ...
        case 'error':
          ...
          // A broker-level error (BROKER_ERROR / BROKER_INIT_ERROR /
          // BROKER_CONNECTION_ERROR from websocket_proxy/server.py's
          // authenticate_client) means this connection will never deliver
          // live ticks for this user's broker session -- the socket itself
          // stays open (the server never closes it after send_error), so
          // onclose/onerror's consecutiveFailures counter never increments
          // and REST fallback never engages. Without this, a genuine
          // broker-side failure (stale token, broker feed down, etc.) left
          // the page stuck showing neither live data nor falling back to
          // REST -- worse than either outcome alone. Count it as a
          // connection failure so the existing fallback machinery kicks in.
          this.consecutiveFailures++
          if (this.consecutiveFailures >= this.maxConsecutiveFailures) {
            this.enableFallbackMode()
          }
