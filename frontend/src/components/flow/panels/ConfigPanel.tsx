...
const NODE_TITLES: Record<string, string> = {
  ...
  indicatorCondition: 'Indicator Condition',
  positionCheck: 'Position Check',
  basketPnlCheck: 'Master SL / Target',
      ...
      <div className="flex-1 min-h-0">
        <ScrollArea className="h-full">
          <div className="space-y-4 p-4">
            ...
            {/* ===== BASKET CLOSE ("Master SL" close side) ===== */}
            {nodeType === 'basketClose' && (
              <>
                <div className="space-y-2">
                  <Label className="text-xs">Symbols (SYMBOL,EXCHANGE,PRODUCT)</Label>
                  <Textarea
                    className="min-h-[100px] text-xs font-mono"
                    placeholder="RELIANCE,NSE,MIS&#10;INFY,NSE,MIS&#10;NIFTY24DEC24500CE,NFO,NRML"
            ...
            {nodeType === 'indicatorCondition' && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={(nodeData.indicator as string) || 'RSI'}
                    onValueChange={(v) => handleDataChange('indicator', v)}
                  >
                    <SelectTrigger className="h-8">
                      <SelectValue />
                    </SelectTrigger>
                ...
                {nodeData.indicator !== 'VWAP' && (
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className="h-8"
                      placeholder="14"
                      value={(nodeData.period as number) || ''}
                      onChange={(e) =>
                        handleDataChange('period', parseInt(e.target.value, 10) || 14)
                      }
                    />
                  </div>
                )}
            ...
            {/* ===== MASTER SL / TARGET (basket P&L check) ===== */}
            {nodeType === 'basketPnlCheck' && (
              <>
                <div className="space-y-2">
                  <Label className="text-xs">Basket Symbols (SYMBOL,EXCHANGE,PRODUCT)</Label>
                  <Textarea
                    className="min-h-[100px] text-xs font-mono"
                    placeholder="RELIANCE,NSE,MIS&#10;INFY,NSE,MIS&#10;NIFTY24DEC24500CE,NFO,NRML"
                    value={(nodeData.symbols as string) || ''}
                ...
                <div className="space-y-2">
                  <Label className="text-xs">Condition</Label>
                  <Select
                    value={(nodeData.condition as string) || 'pnl_below'}
                    onValueChange={(v) => handleDataChange('condition', v)}
                  >
                    <SelectTrigger className="h-8">
                      <SelectValue />
                    ...
                    <SelectContent>
                      <SelectItem value="pnl_below">Combined P&L Below (Master SL)</SelectItem>
                      <SelectItem value="pnl_above">Combined P&L Above (Master Target)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
