...
export const HttpRequestNode = memo(({ data, selected }: HttpRequestNodeProps) => {
  ...
  return (
    <div className={cn('workflow-node min-w-[140px] border-l-primary', selected && 'selected')}>
      ...
      <div className="p-2">
        <div className="mb-1.5 flex items-center gap-1.5">
          ...
          <div>
            ...
            <div className="text-[9px] text-muted-foreground">API Call</div>
          </div>
        </div>
        <div className="space-y-1">
          <div className="flex items-center gap-1.5">
            ...
            {displayUrl && (
              <span className="truncate text-[9px] text-muted-foreground">{displayUrl}</span>
            )}
          </div>
          {data.outputVariable && (
