...
export function PlaceOrderDialog({
  ...
  // Fetch connected brokers whenever the dialog opens, defaulting the
  // selection to the current data broker (matches today's single-broker
  // behavior when only one broker is connected).
  useEffect(() => {
    if (!open) return
    const loadConnections = async () => {
      try {
        ...
        // Non-fatal — falls back to the single-broker /placeorder path.
      }
    }
    loadConnections()
  }, [open])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // placeOrderMulti to ensure exact broker targeting for every selected broker.
      if (selectedBrokers.length > 0) {
        const multiResponse = await tradingApi.placeOrderMulti(orderRequest, selectedBrokers)
        const results = multiResponse.results ?? []
        const successCount = results.filter((r) => r.status === 'success').length
        const failCount = results.length - successCount
        const brokerNames = selectedBrokers.join(', ')
        if (failCount === 0) {
          const actionStr = orderRequest.action.toUpperCase()
          showToast.info(
            `${actionStr} Order Sent to ${brokerNames}: ${orderRequest.symbol} — confirming fill…`,
            'orders'
          )
          onSuccess?.(results[0]?.orderid ?? '')
          onOpenChange(false)
          // Fill/rejection confirmation is reported by the backend's
          // async_verify_order_fill (services/order_verification_helper.py)
          // via SocketIO order_event/order_notification, consumed in
          // useSocket.ts. A frontend REST poll used to run here too, which
          // produced duplicate toasts/notifications for the same outcome
          // (one from this poll, one from the backend's SocketIO push) --
          // removed in favor of the single backend-driven path.
        } else {
          const failedBrokers = results
            .filter((r) => r.status === 'error' || (r as any).rejection_reason)
            .map(
              (r) =>
                `${r.broker} (${(r as any).rejection_reason || r.message || 'Broker Rejected Order'})`
            )
            .join(', ')
          const errorMsg = `Order Errors on ${brokerNames}: ${failedBrokers || `${failCount} failed`}`
      ...
      const isRejected =
        rawRes.status === 'error' ||
        rawRes.status === 'rejected' ||
        rawRes.status === 'failed' ||
        Boolean(rawRes.rejection_reason)

      if (response.status === 'success' && orderid && !isRejected) {
        const brokerName = selectedBrokers[0] ?? ''
        const actionStr = orderRequest.action.toUpperCase()
        showToast.info(
          `${actionStr} Order Sent to Broker: ${orderRequest.symbol}${
        ...
        const errorMsg =
          rawRes.rejection_reason || rawRes.message || rawRes.text || 'Order placement failed'
        showToast.error(`Broker Error: ${errorMsg}`, 'orders')
        onError?.(errorMsg)
      }
    } catch (err: unknown) {
      ...
      <DialogContent className="sm:max-w-[420px]" aria-describedby={undefined}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            <span className={formAction === 'BUY' ? 'text-profit' : 'text-loss'}>{formAction}</span>
        ...
        <div className="space-y-4 py-2">
          ...
          {/* Broker chip-select — defaults to the data broker. Selecting
              more than one fans the order out sequentially via
              /placeordermulti; a single selection keeps using the existing
              single-broker /placeorder path unchanged. */}
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className={cn('flex-1', formAction === 'BUY' && 'bg-profit hover:bg-profit')}
              ...
              <Button
                ...
                className={cn('flex-1', formAction === 'SELL' && 'bg-loss hover:bg-loss')}
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button
            ...
            className={cn(
              formAction === 'BUY' ? 'bg-profit hover:bg-profit' : 'bg-loss hover:bg-loss'
            )}
          >
