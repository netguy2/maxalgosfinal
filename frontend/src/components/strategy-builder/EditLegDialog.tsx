...
export function EditLegDialog({
  ...
  const syncEntryPriceFromChain = async (
    ...
    if (!leg || isClosed) return

    // ── Equity: re-fetch LTP for its own (fixed) symbol/exchange, no expiry ──
    if (leg.segment === 'EQUITY') {
      if (!apiKey || !leg.symbol || !leg.equityExchange) return
      await fetchAndSetLtp(leg.symbol, leg.equityExchange)
      return
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          {/* Symbol (equity only — read-only, no strike/expiry to pick) */}
          {leg.segment === 'EQUITY' && (
          ...
          <div className="flex items-center gap-6">
            <label className="flex cursor-pointer items-center gap-2 text-sm">
              ...
              />
              <span className={cn(side === 'BUY' && 'font-semibold text-profit')}>Buy</span>
            </label>
            <label className="flex cursor-pointer items-center gap-2 text-sm">
              <input
                type="radio"
                ...
                className="h-4 w-4 accent-loss"
              />
              <span className={cn(side === 'SELL' && 'font-semibold text-loss')}>Sell</span>
            </label>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-muted-foreground">
              {isPriceLoading
                ? 'Fetching live LTP…'
                : `Modify ${leg.segment === 'FUTURE' ? 'Futures' : leg.segment === 'EQUITY' ? 'Equity' : 'Option'} Entry Price`}
            </p>
          </div>
        ...
        <DialogFooter className="flex-row items-center justify-between gap-2 sm:justify-between">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="h-9 w-9 text-loss hover:bg-loss/10 hover:text-loss"
              onClick={() => {
                onDelete(leg.id)
                onOpenChange(false)
              }}
