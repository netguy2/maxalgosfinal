...
export function PnLTab({ legs, fnoExchange, fallbackPrices }: PnLTabProps) {
  ...
  const legSubscriptions = useMemo(() => {
    ...
    for (const leg of openLegs) {
      // EQUITY legs track their own cash exchange (NSE/BSE), never the F&O
      // exchange the option/future legs subscribe under.
      const exchange = leg.segment === 'EQUITY' ? (leg.equityExchange ?? 'NSE') : fnoExchange
      const key = `${exchange}:${leg.symbol}`
      if (seen.has(key)) continue
      seen.add(key)
      out.push({ symbol: leg.symbol, exchange })
    }
    return out
    ...
    return legs
      ...
      .map((leg) => {
        ...
        if (!isClosed && leg.symbol) {
          const exchange = leg.segment === 'EQUITY' ? (leg.equityExchange ?? 'NSE') : fnoExchange
          const ws = marketData.get(`${exchange}:${leg.symbol}`)
            ...
            >
              <TableCell className="text-center">
                <span
                  className={cn(
                    ...
                    leg.side === 'BUY'
                      ? 'bg-profit/15 text-profit ring-profit/20'
                      : 'bg-loss/15 text-loss ring-loss/20'
                  )}
              ...
              <TableCell className="min-w-0 text-xs font-medium">
                <span className="flex min-w-0 items-center gap-2 truncate">
                  ...
                  {isClosed && (
                    <span className="rounded bg-loss/15 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-loss">
                      Closed
                    </span>
                  )}
                </span>
