import { FolderOpen, Layers, Pencil, RotateCw, Save, Send, Trash2 } from 'lucide-react'
...
export interface PositionsPanelProps {
  /** Hides options-only metrics (breakevens, net credit, premium, PoP) when
   * 'Equity' — none of those concepts apply to a plain cash position. */
  ...
  onExecute?: () => void
  /** Opens the Deploy Strategy drawer. Button hidden if omitted. */
  onDeploy?: () => void
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border bg-card shadow-sm">
        ...
        <div className="flex items-center gap-2.5 border-b px-4 py-2">
          <Checkbox
            ...
            className={cn(
              ...
              'data-[state=checked]:border-profit data-[state=checked]:bg-profit data-[state=checked]:text-white',
              'data-[state=indeterminate]:border-profit data-[state=indeterminate]:bg-profit data-[state=indeterminate]:text-white'
            )}
          />
                ...
                >
                  ...
                  <Checkbox
                    ...
                    className={cn(
                      'h-4 w-4 border-border',
                      'data-[state=checked]:border-profit data-[state=checked]:bg-profit data-[state=checked]:text-white',
                      'hover:border-foreground/40 data-[state=checked]:hover:bg-profit'
                    )}
                  />
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      isClosed
                        ? 'cursor-not-allowed bg-loss/15 text-loss'
                        : leg.side === 'BUY'
                          ? 'cursor-pointer bg-profit/15 text-profit ring-1 ring-inset ring-profit/20 hover:bg-profit/25'
                          : 'cursor-pointer bg-loss/15 text-loss ring-1 ring-inset ring-loss/20 hover:bg-loss/25'
                    )}
                  >
                    {leg.side === 'BUY' ? 'B' : 'S'}
                  </button>
                  ...
                  <div
                    className={cn(
                      'min-w-0 flex-1',
                      isClosed && 'text-loss/80 line-through',
      ...
      {(onSaveStrategy || onExecute) && (
        <div className="flex items-center justify-end gap-2 border-t bg-muted/10 px-3.5 py-2.5">
          <Button
            ...
            variant="ghost"
            size="sm"
            className="h-9 gap-1.5 text-xs font-semibold mr-auto"
          >
            <Link to="/strategybuilder/portfolio">
      ...
      <div className="border-t">
        ...
        {/* Breakevens — full width chip row (options only) */}
        {!isEquity && breakevens.length > 0 && (
