...
export default function MultiStrikeOITab({
  ...
  // All chart colors derive from the active theme tokens (light/dark/analyzer
  // /sandbox follow automatically). Alpha suffixes are appended to the opaque
  // hex values returned by useChartTheme.
  const colors = useMemo(
    () => ({
      text: chartTheme.mutedForeground,
      grid: `${chartTheme.mutedForeground}1a`,
      border: `${chartTheme.mutedForeground}33`,
      crosshair: `${chartTheme.mutedForeground}80`,
      crosshairLabel: chartTheme.primary,
      underlying: chartTheme.brand,
      watermark: `${chartTheme.mutedForeground}1f`,
      tooltipBg: `${chartTheme.card}f0`,
      tooltipBorder: `${chartTheme.mutedForeground}33`,
      tooltipText: chartTheme.foreground,
      tooltipMuted: chartTheme.mutedForeground,
    }),
    [chartTheme]
  )
  ...
  return (
    <div className="rounded-xl border bg-card p-4 shadow-sm">
      ...
      {missingOI > 0 && (
        <div className="mb-2 rounded-md border border-warning/30 bg-warning/10 px-3 py-1.5 text-[11px] text-warning">
      ...
      {chartData && !chartData.underlying_available && (
        <div className="mb-2 rounded-md border border-info/30 bg-info/10 px-3 py-1.5 text-2xs text-info">
