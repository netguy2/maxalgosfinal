...
/** Basket Close - squares off only a specified list of positions, not everything */
export interface BasketCloseNodeData {
  label?: string
  /** One "SYMBOL,EXCHANGE,PRODUCT" per line (PRODUCT optional, defaults to MIS) */
  symbols: string
  strategy?: string
}
...
/** Basket P&L Check ("Master SL / Target") - checks COMBINED P&L across a
 * list of positions, not a single symbol. Pairs with Basket Close to build
 * a real master stop-loss/target that squares off the whole basket the
 * moment their combined P&L crosses a threshold. */
export interface BasketPnlCheckNodeData {
  label?: string
  /** One "SYMBOL,EXCHANGE,PRODUCT" per line (PRODUCT optional) */
  symbols: string
  condition: 'pnl_above' | 'pnl_below'
  threshold: number
}
...
/** Indicator Condition - gate a chain on a real technical indicator (RSI,
 * EMA, SMA, MACD, ATR, VWAP) computed from historical candle data, rather
 * than raw price. Pair with Multi-Leg Options to build an indicator-
 * triggered spread entry (e.g. "RSI < 30 -> Bull Call Spread"). */
export interface IndicatorConditionNodeData {
  label?: string
  symbol: string
  exchange: string
  indicator: 'RSI' | 'EMA' | 'SMA' | 'MACD' | 'ATR' | 'VWAP'
  /** Ignored by VWAP (always same-session). */
  period?: number
  operator: '>' | '<' | '==' | '>=' | '<=' | '!='
  value: number
}
...
export const NODE_TYPES = {
  ...
  BASKET_CLOSE: 'basketClose',
  // Conditions
  CONDITION: 'condition',
  POSITION_CHECK: 'positionCheck',
  BASKET_PNL_CHECK: 'basketPnlCheck',
...
export interface SettingsState {
  maxalgos_host: string
  maxalgos_ws_url: string
