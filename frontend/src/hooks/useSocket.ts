...
interface OrderEventData {
  ...
  /** Strategy tag passed as `strategy=` to client.placeorder()/placesmartorder()
   * (Python Strategy Host, MaxHook webhooks, Flow, etc). Empty for orders
   * placed without one (manual REST calls, UI order tickets). */
  strategy?: string
}
  ...
  useEffect(() => {
    ...
    socket.on('cancel_order_event', (data: CancelOrderEventData) => {
      ...
      useNotificationStore.getState().addNotification({
        ...
        message: data.orderid ? `Order ${data.orderid} cancelled` : 'Order cancelled',
        type: 'warning',
      })
    })

    // Modify order notification - only play sound, UI handles toast
    socket.on('modify_order_event', (data: ModifyOrderEventData) => {
      playAlertSound('orders')
      useNotificationStore.getState().addNotification({
        title: 'Order Modified',
        message: data.orderid ? `Order ${data.orderid} modified` : 'Order modified',
        type: 'info',
      })
    })
    ...
    socket.on('close_position_event', (data: ClosePositionEventData) => {
      ...
      useNotificationStore.getState().addNotification({
        title: 'Positions Squared Off',
        message: data.message || 'All open positions squared off',
        type: 'success',
      })
    })
    ...
    socket.on('order_event', (data: OrderEventData) => {
      ...
      const isRejected =
        ...
        Boolean(data.rejection_reason)

      const rejectionText = data.rejection_reason || data.message || data.text
      const strategyTag = data.strategy ? ` [${data.strategy}]` : ''

      if (isRejected) {
        const message = `Order REJECTED for ${data.symbol || 'Symbol'}${strategyTag}${
        ...
        useNotificationStore.getState().addNotification({
          title: `${data.action ? `${data.action.toUpperCase()} ` : ''}Order Rejected`,
          message: message,
          type: 'error',
        })
      } else {
        const actionStr = (data.action || 'BUY').toUpperCase()
        const isBasket = data.batch_order || (data.action && data.action.includes('orders'))
        const message = isBasket
          ? `Basket Sent to Broker: ${data.action} placed (ID: ${data.orderid})`
          : `${actionStr} Order Sent to Broker: ${data.symbol}${strategyTag}${
        ...
        useNotificationStore.getState().addNotification({
          title: isBasket ? 'Basket Sent to Broker' : `${actionStr} Order Sent to Broker`,
          message: `${message} — Check Order Book for fill/rejection status`,
          type: 'success',
        })
      }
    })
    ...
    socket.on(
      ...
      (data: { symbol?: string; status?: string; message?: string }) => {
        ...
        if (statusLower.includes('success')) {
          ...
          type = 'error'
        } else if (statusLower.includes('pending') || statusLower.includes('confirming')) {
          type = 'warning'
        }
    ...
    socket.on(
      ...
      (data: { count: number; sessions?: import('@/api/sessions').ActiveSession[]; current_session_id?: string }) => {
        ...
        if (data.sessions) {
          setActiveSessions(data.sessions, data.current_session_id)
        }
      }
    )
