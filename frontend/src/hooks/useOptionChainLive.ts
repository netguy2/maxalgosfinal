...
export function useOptionChainLive(
  ...
  const {
    ...
    pauseWhenHidden,
    poll: false, // Force single-fetch mode to reduce API requests
  })
  ...
  const {
    ...
    isFallbackMode: isWsFallbackMode,
    error: wsError,
  } = useMarketData({
    symbols: wsSymbols,
  ...
  return {
    data: mergedData,
    isLoading,
    isConnected: isPollingConnected || isStreaming,
    isStreaming,
    isConnecting,
    isPolling,
    isPaused,
    error,
    // Distinct from `error` (the REST chain-fetch failure, which replaces
    // the whole page with an error card) -- this is *why* isPolling/
    // isWsFallbackMode is true, e.g. "WebSocket error: Failed to connect
    // to broker: Zebu session expired". Previously silently dropped here,
    // so the "Polling" badge gave no indication of why live data wasn't
    // streaming.
    wsError,
    lastUpdate,
