...
function MetricCard({ title, icon: Icon, value, subtitle, status, loading }: MetricCardProps) {
  ...
  const iconColors = {
    pass: 'text-profit opacity-20',
    warn: 'text-warning opacity-20',
    fail: 'text-loss opacity-20',
...
export default function HealthMonitor() {
  ...
  useEffect(() => {
    ...
    const textColor = chartTheme.mutedForeground
    const gridColor = chartTheme.border
    const borderColor = chartTheme.border
      ...
      {currentMetrics && (
        <div
          className={cn(
            ...
            currentMetrics.overall_status === 'pass' && 'border-profit/50 bg-profit/10',
            currentMetrics.overall_status === 'warn' && 'border-warning/50 bg-warning/10',
            currentMetrics.overall_status === 'fail' && 'border-loss/50 bg-loss/10'
          )}
        >
              ...
              {alerts.map((alert) => (
                <div
                  ...
                  className={cn(
                    ...
                    alert.severity === 'fail' && 'border-loss/50 bg-loss/10',
