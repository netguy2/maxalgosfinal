...
export default function IVChart() {
  ...
  const makeChartOptions = useCallback(
    ...
    [chartTheme, selectedDays]
  )

  const addWatermark = useCallback(
    (container: HTMLDivElement) => {
      const el = document.createElement('div')
      el.style.cssText = `position:absolute;z-index:2;font-family:Arial,sans-serif;font-size:28px;font-weight:bold;user-select:none;pointer-events:none;color:${chartTheme.mutedForeground}1f`
      el.textContent = 'Max Algos'
  ...
  useEffect(() => {
    ...
    for (const metric of METRICS) {
      for (const type of ['ce', 'pe'] as const) {
        ...
        const color = type === 'ce' ? chartTheme.profit : chartTheme.loss
  ...
  }, [makeChartOptions, addWatermark, updateAllCharts, chartTheme.profit, chartTheme.loss])
