...
export default function Dashboard() {
  ...
  //
  // Right after a broker disconnect+reconnect, a freshly-issued session
  // token can take a few seconds to become live on the broker's own
  // servers (see broker/zebu/api/funds.py::get_margin_data's comment) --
  // the FIRST poll or two after reconnecting can genuinely fail with a
  // transient "not ready yet" error even though the connection is fine.
  // Auto-retry a bounded few times (a few seconds apart) before settling
  // into a hard error state, so the user doesn't have to manually refresh
  // for something that resolves itself within seconds.
  const fetchFundsData = useCallback(async (retriesLeft = 3) => {
    setIsLoading(true)
    ...
    if (retriesLeft > 0) {
      setTimeout(() => fetchFundsData(retriesLeft - 1), 2000)
      return
    }
