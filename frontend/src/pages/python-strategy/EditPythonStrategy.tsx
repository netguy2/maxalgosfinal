...
export default function EditPythonStrategy() {
  ...
  // Warn on tab close/refresh/external navigation with unsaved edits --
  // without this, Ctrl+S is the only thing standing between a user and
  // silently losing in-progress strategy code (no autosave, no draft
  // persistence). In-app navigation (the "Back" link, router pushes) is
  // guarded separately below since beforeunload doesn't fire for those.
  useEffect(() => {
  ...
  // Guards in-app navigation away from the editor (the "Back" link) --
  // beforeunload only covers full page unload, not client-side route
  // changes.
  const confirmDiscardIfDirty = (e: React.MouseEvent) => {
    if (!hasChanges) return
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-3">
            ...
            {isRunning && <Badge className="bg-profit">Running</Badge>}
          </h1>
