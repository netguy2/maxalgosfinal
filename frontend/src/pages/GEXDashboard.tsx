...
export default function GEXDashboard() {
  ...
  // Theme colors for Plotly — derived from active theme tokens
  const themeColors = useMemo(
    () => ({
      bg: 'rgba(0,0,0,0)',
      paper: 'rgba(0,0,0,0)',
      text: chartTheme.foreground,
      grid: `${chartTheme.mutedForeground}1f`,
      ceBar: chartTheme.loss,
      peBar: chartTheme.profit,
      positiveGex: chartTheme.buy,
      negativeGex: chartTheme.sell,
      atmLine: `${chartTheme.foreground}99`,
      hoverBg: chartTheme.card,
      hoverFont: chartTheme.foreground,
      hoverBorder: chartTheme.border,
    }),
    [chartTheme]
  )
              ...
              <div className="space-y-1">
                {topStrikes.topCallOI.map((item, idx) => (
                  ...
                  >
                    ...
                    <span className="font-medium text-loss">{formatNumber(item.value)}</span>
                  </div>
                ))}
              </div>
            </CardContent>
              ...
              <div className="space-y-1">
                {topStrikes.topPutOI.map((item, idx) => (
                  ...
                  >
                    ...
                    <span className="font-medium text-profit">{formatNumber(item.value)}</span>
                  </div>
                ))}
              </div>
            </CardContent>
              ...
              <table className="w-full text-sm">
                  ...
                  {gexData.chain.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-1.5 px-3 text-right text-loss">
                          {item.ce_gex.toFixed(2)}
                        </td>
                        <td className="py-1.5 px-3 text-right text-profit">
                ...
                <tfoot className="sticky bottom-0 bg-background border-t-2 border-border">
                  <tr className="font-semibold">
                    ...
                    <td className="py-2 px-3 text-right text-loss">
                      {(gexData.total_ce_gex || 0).toFixed(2)}
                    </td>
                    <td className="py-2 px-3 text-right text-profit">
