...
function StatusIcon({ status }: { status: string | null }) {
  switch (status) {
    ...
    case 'completed_with_errors':
      // Graph walk finished but at least one order-action node (place/
      // modify/cancel/close) was rejected by the broker -- distinct from
      // both a clean 'completed' and an execution-level 'failed'.
            ...
            <div className="flex items-center gap-3">
              <div
                className={cn(
                  ...
                  workflow.is_active ? 'bg-profit' : 'bg-muted-foreground'
                )}
              />
