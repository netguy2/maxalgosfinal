...
import { fetchCSRFToken, invalidateCSRFToken } from '@/api/client'
...
/**
 * POST to /auth/reset-password with CSRF protection, retrying once with a
 * forced-fresh token if the first attempt is rejected as a CSRF failure.
 *
 * This page predates and bypasses the shared webClient/apiClient (which
 * already invalidate the CSRF cache and let the *next* mutation recover —
 * see api/client.ts's response interceptor) by calling fetch() directly,
 * so a stale cached token here had no recovery path: the very first
 * mutation on a fresh page load could fail outright with no way to
 * retry short of a manual page refresh. A stale cache can happen simply
 * from browser/extension cookie handling delaying when the session
 * cookie set by GET /auth/csrf-token is actually available for the
 * following POST to send back, independent of any server-side bug.
 */
async function postResetPassword(body: Record<string, unknown>): Promise<{
  status?: string
  message?: string
  token?: string
}> {
  const attempt = async (forceFreshToken: boolean) => {
    const csrfToken = await fetchCSRFToken(forceFreshToken)
  ...
  // Flask-WTF CSRF failures land as 400 with a message mentioning the
  // security token (see app.py's csrf_error handler) — retry exactly
  // once with a forced-fresh token instead of surfacing this to the user.
  const looksLikeCsrfFailure =
    response.status === 400 &&
    typeof data?.message === 'string' &&
    /token/i.test(data.message) &&
    /expired|invalid|missing/i.test(data.message)
...
export default function ResetPassword() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await postResetPassword({ step: 'email', email })
  ...
  const handleMethodSelect = async (method: 'totp' | 'email') => {
    ...
    try {
      const data = await postResetPassword({ step: `select_${method}`, email })
  ...
  const handleTotpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await postResetPassword({ step: 'totp', email, totp_code: totpCode })
