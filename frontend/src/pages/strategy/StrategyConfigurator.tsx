...
export default function StrategyConfigurator() {
  ...
  const schema: StrategyTemplateSchema = getSchemaForTemplate(catalogTemplate?.signalId ?? templateId)
  ...
  // connected brokers below; the dropdown itself only ever lists brokers
  ...
  // DeployStrategyDrawer, and PlaceOrderDialog already use.
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      try {
        ...
        if (connected.length > 0) {
          setSelectedBroker((prev) => (prev && connected.includes(prev) ? prev : connected[0]))
        }
      } catch {
        /* non-fatal -- dropdown just stays empty, execution mode defaults to paper */
      }
    })()
    return () => {
      cancelled = true
  ...
  const handleSaveStrategy = async (deployNow = false) => {
    ...
    if (deployNow && executionMode === 'live' && !selectedBroker) {
      showToast.error(
      ...
      if (deployNow) {
        ...
        const deployRes = await fetch('/api/v1/deployments', {
          ...
          body: JSON.stringify({
            strategy_id: newId,
            name: `${name} (Active)`,
            broker: executionMode === 'paper' ? 'Paper Trading' : selectedBroker,
            capital,
              ...
              <Card className="border border-border">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 pt-1 border-t border-border/40">
                    ...
                    <button
                      type="button"
                      onClick={() => { setStartTime('09:15'); setEndTime('15:10'); setSquareoffTime('15:15'); }}
                    ...
                    <button
                      type="button"
                      onClick={() => { setStartTime('09:30'); setEndTime('15:10'); setSquareoffTime('15:15'); }}
                      className="px-2 py-0.5 rounded text-[10px] font-medium bg-muted hover:bg-accent border border-border/50 text-foreground transition"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => { setStartTime('10:00'); setEndTime('14:30'); setSquareoffTime('15:00'); }}
                      className="px-2 py-0.5 rounded text-[10px] font-medium bg-muted hover:bg-accent border border-border/50 text-foreground transition"
                    >
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      disabled={executionMode === 'paper' || connectedBrokers.length === 0}
                      className="w-full px-3 py-2 rounded-md border border-border bg-background text-xs focus:outline-none font-semibold disabled:opacity-50"
                    >
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div className="p-3 bg-muted/30 rounded-lg space-y-1.5 border border-border/50">
                    ...
                    <div>Exchange: <span className="font-semibold">{exchange}</span></div>
                    <div>Symbol: <span className="font-bold text-primary">{symbol}</span></div>
                    <div>Timeframe: <span className="font-semibold">{timeframe}</span></div>
                    <div>Window: <span className="font-mono">{startTime} - {endTime}</span></div>
                    <div>Square-Off: <span className="font-mono">{squareoffTime}</span></div>
                  </div>
                  ...
                  <div className="p-3 bg-muted/30 rounded-lg space-y-1.5 border border-border/50">
                    ...
                    <div>Capital: <span className="font-bold">₹{capital.toLocaleString()}</span></div>
                    <div>Sizing: <span className="font-semibold">{sizingMode}</span></div>
                    <div>Stop Loss: <span className="font-semibold text-loss">{slValue} {slType}</span></div>
                    <div>Target: <span className="font-semibold text-profit">{targetValue} {targetType}</span></div>
                    <div>Mode: <span className="font-bold text-violet-400">{executionMode.toUpperCase()} ({selectedBroker})</span></div>
                  </div>
                </div>
