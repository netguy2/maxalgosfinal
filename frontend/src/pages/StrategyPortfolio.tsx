...
function describeLeg(leg: PortfolioEntry['legs'][number]): string {
  ...
  if (leg.segment === 'EQUITY') {
    return `${sign}${leg.lots}× ${leg.symbol}`
  }
  const desc =
...
export default function StrategyPortfolio() {
  ...
  const symbolsToStream = useMemo(() => {
    ...
    for (const entry of [...myTrades, ...simulation]) {
      ...
      for (const leg of entry.legs) {
        ...
        // EQUITY legs stream against their own cash exchange (NSE/BSE), not
        // the entry's F&O exchange used for option/future legs.
        const legEx = leg.segment === 'EQUITY' ? (leg.equityExchange ?? 'NSE') : ex
        const key = `${legEx}:${leg.symbol}`
        if (seen.has(key)) continue
        seen.add(key)
        out.push({ symbol: leg.symbol, exchange: legEx })
      }
    }
    return out
  }, [myTrades, simulation])
  ...
  const pricesBySymbol = useMemo(() => {
    ...
    for (const entry of [...myTrades, ...simulation]) {
      ...
      for (const leg of entry.legs) {
        if (!leg.symbol) continue
        const legEx = leg.segment === 'EQUITY' ? (leg.equityExchange ?? 'NSE') : ex
        const ws = marketData.get(`${legEx}:${leg.symbol}`)
  ...
  const openInBuilder = (entry: PortfolioEntry) => {
    // /strategybuilder is a legacy alias that redirects to /visual-builder
    // via <Navigate to="/visual-builder" replace /> (App.tsx) -- that
    // redirect does NOT forward query params, so navigating through the
    // alias would silently drop ?load=. Use the canonical route directly.
    navigate(`/visual-builder?load=${entry.id}`)
  }

  // Opens the strategy in the builder canvas and immediately surfaces the
  // same basket-order dialog the canvas's own "Execute" button uses (see
  // StrategyBuilder.tsx's ?load=/?execute= handling) -- places live/paper
  // broker orders for every leg. There is no separate webhook/Deployment
  // attach path for saved option strategies; this reuses the existing,
  // already-working order-placement flow instead of duplicating it here.
  const executeNow = (entry: PortfolioEntry) => {
    navigate(`/visual-builder?load=${entry.id}&execute=1`)
  }
