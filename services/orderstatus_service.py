...
from utils.socket_scope import emit_to_user, username_from_api_key

# Initialize logger
logger = get_logger(__name__)


def _emit_analyzer_update(payload: dict[str, Any], api_key: str | None) -> None:
    """Emit analyzer_update to the requesting user's own clients only.

    These used to be bare `socketio.emit(...)` broadcasts, so one user's
    analyze-mode order-status checks surfaced in every other logged-in
    user's browser. `api_key` comes from the untouched original request
    (the sanitized copy has it stripped for logging), and resolves to the
    owning username via the same verify_api_key() lookup used everywhere
    else. Unresolvable -> dropped, never broadcast.
    """
    username = username_from_api_key(api_key)
    socketio.start_background_task(emit_to_user, "analyzer_update", payload, username)
...
def emit_analyzer_error(request_data: dict[str, Any], error_message: str) -> dict[str, Any]:
    ...
    # Emit socket event asynchronously (non-blocking), scoped to the caller
    _emit_analyzer_update(
        {"request": analyzer_request, "response": error_response},
...
def get_order_status_with_auth(
    ...
    if is_analyze_mode:
        ...
        # Emit socket event for toast notification asynchronously
        # (non-blocking), scoped to the requesting user
        _emit_analyzer_update(
