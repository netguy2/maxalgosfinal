...
process. Max Algos already runs `gunicorn -w 1` for SocketIO compatibility,
...
Lifecycle:
    ...
    [unpaired]  --start_pair-->  [pairing: temp DB + QR loop]
                              ...
                              on_connected from wars
                                          |
                                          v
                        export_session bytes -> encrypt -> maxalgos.db
                                          |
                                          v
...
# the Max Algos process can read (/etc/passwd, env files, broker token DB,
...
def _session_username() -> str | None:
    """The logged-in username, or None when called off a request thread.

    Safe to call from the bot's worker threads: `has_request_context()`
    returns False there rather than raising, so this simply yields None and
    ...
    try:
        ...
        logger.debug("WhatsApp: session lookup failed", exc_info=True)
    return None
...
class WhatsAppBotService:
    ...
    def __init__(self) -> None:
        ...
        # Who this instance's Socket.IO events belong to.
        #
        # Every emit in this service fires from a background worker thread
        # (WhatsAppPairThread / WhatsAppBotThread), where there is no Flask
        # request and therefore no session to read. And during pairing the
        # owner is not yet in the DB either -- save_session_blob() only
        # writes owner_username at the END of a successful pair. So the
        # owner must be captured on the REQUEST thread in start_pair() and
        # held here for the worker threads to use.
        #
        # Without this, _emit() could resolve no recipient and dropped
        # every pairing event: the QR code and pair code never reached the
        # browser, so pairing appeared to hang forever.
    ...
    def start_pair(
        ...
        with self._lock:
            ...
            # have the bytes.
            fd, tmp_path = tempfile.mkstemp(suffix=".db", prefix="maxalgos_wa_pair_")
            os.close(fd)
            try:
            ...
            # Captured HERE, on the request thread, while the Flask session
            # still exists -- the pairing worker below has no request context
            # and the DB has no owner_username until pairing completes. This
            # is what lets _emit() route the QR/pair-code to the admin who
            # clicked Pair. Falls back to the session directly so a caller
            # that didn't pass owner_username still works.
            self._owner_username = owner_username or _session_username()
    ...
    def _finalize_pair(
        ...
        blob into maxalgos.db, then disconnect the temp client and auto-start
        ...
        try:
            ...
            logger.info("WhatsApp pair: session blob encrypted and saved to maxalgos.db")
    ...
    def unlink(self) -> tuple[bool, str]:
        ...
        # Emit BEFORE clearing the cached owner -- this event still belongs
        # to the admin who just unlinked.
        self._emit("whatsapp_status", {"is_running": False, "is_paired": False})
        with self._lock:
            self._is_paired = False
            # Drop the cached owner: the row it came from is gone, and a
            # subsequent re-pair by a DIFFERENT admin must not route their
            # QR code to the previous owner's browser.
        ...
        def _handle(msg) -> None:
            try:
                ...
                if not text or not text.startswith("/"):
                    return
                # Single-user Max Algos: the only identity allowed to drive
    ...
    # SDK helpers. Single-user Max Algos — the operator who paired the device
    ...
    def _sdk_client_for_owner(self):
        ...
        if not owner_username:
            return None, (
                ...
                "/whatsapp page while logged in to Max Algos."
            )
        try:
            ...
            return None, "Could not load Max Algos API key for the owner."
        if not api_key:
            return None, (
        ...
        try:
            from maxalgos import api as maxalgos_api  # type: ignore
        except Exception:
            return None, "maxalgos SDK not available on this server."
        try:
            return maxalgos_api(api_key=api_key, host=host_url), None
        except Exception as e:
            logger.exception("Failed to create maxalgos client")
    ...
    def _resolve_emit_username(self) -> str | None:
        """Who should receive this service's Socket.IO events.

        Order matters, and each step exists for a concrete reason:

          1. `self._owner_username` -- captured on the REQUEST thread in
             start_pair(). This is the only source that works during
             pairing, because the worker thread has no Flask session and
             the DB has no owner_username until pairing finishes.
          2. the persisted `owner_username` -- for events raised by the
             long-lived bot thread after a restart, where step 1 is empty
             because this process never ran start_pair().
          3. the live Flask session -- for the rare emit that happens on a
             request thread (e.g. stop_bot() called from the UI).
        """
        if self._owner_username:
            return self._owner_username

        try:
            owner = get_bot_config().get("owner_username")
            if owner:
                # Cache it so later emits on this thread skip the DB read.
                self._owner_username = owner
                return owner
        except Exception:
            logger.debug("WhatsApp: owner lookup for emit scoping failed", exc_info=True)

        return _session_username()

    def _emit(self, event: str, payload: Any) -> None:
        """Push a pairing/status event to the device owner's browser only.

        Scoped rather than broadcast because this carries the WhatsApp
        pairing QR code and pair code -- as a platform-wide broadcast, any
        other logged-in user could scan/enter it and bind THEIR phone to
        this instance's bot.

        If no owner can be resolved the event is BROADCAST rather than
        dropped. That is a deliberate trade-off specific to this service:
        a dropped QR code makes pairing impossible (the UI has nothing to
        render and appears to hang), whereas the pairing window is short,
        operator-initiated, and single-use. Correct scoping is attempted
        first and virtually always succeeds -- see _resolve_emit_username.
        """
        try:
            username = self._resolve_emit_username()
            if username:
            ...
            logger.warning(
                "WhatsApp: no owner resolved for '%s' -- broadcasting so pairing "
                "is not silently broken. Re-pair while logged in to scope it.",
                event,
            )
