...
class HistorifyScheduler:
    ...
    def init(self, db_url: str = None, api_key: str = None, socketio=None):
        ...
        with self._lock:
            ...
            if db_url is None:
                db_url = os.getenv("DATABASE_URL", "sqlite:///db/maxalgos.db")
    ...
    def _emit_schedule_event(self, event: str, schedule_id: str, data: dict = None):
        """Emit a Socket.IO event for schedule updates, scoped to the user
        who owns the schedule (resolved from the active user_scope set by
        execute_schedule). Dropped rather than broadcast when unresolvable
        -- see utils/socket_scope.py."""
        try:
...
def execute_schedule(schedule_id: str, api_key: str = None):
    ...
    Wrapper that attributes the whole run to the owning user, so the
    schedule/progress Socket.IO events emitted underneath reach only that
    user's browser rather than every connected client. APScheduler runs
    this on a worker thread with no Flask session of its own, so the
    ContextVar set here is the only attribution signal available
    downstream (see utils/socket_scope.py).

    The API key is resolved the same way the body resolves it, so the
    scope matches whichever key the run actually uses.
    """
    from database.auth_db import get_first_available_api_key
    from utils.socket_scope import user_scope, username_from_api_key

    effective_api_key = api_key or get_historify_scheduler().api_key
    if not effective_api_key:
        effective_api_key = get_first_available_api_key()

    with user_scope(username_from_api_key(effective_api_key)):
        return _execute_schedule(schedule_id, api_key)


def _execute_schedule(schedule_id: str, api_key: str = None):
    """Body of execute_schedule, run inside the owning user's scope."""
    ...
    try:
        ...
        if success:
            ...
            # Emit Socket.IO event, scoped to the schedule's owner
            from utils.socket_scope import emit_to_current_user
