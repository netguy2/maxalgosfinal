...
def resolve_expiry_type(
    ...
    "current_month"/"next_month") into a real DDMMMYY expiry string, e.g.
    "current_week" -> "31JUL25".

    This is the shared, service-to-service equivalent of
    flow_executor_service.py's FlowExecutor._resolve_expiry_date, which
    does the same date selection but goes through an HTTP-wrapper client.
    MaxHook's live option/futures resolution (services/signal_engine.py)
    ...
    try:
        ...
        if not expiry_list:
            logger.error(f"No expiry dates found for {symbol} on {exchange}")
            return None
        ...
        def fmt(expiry_str: str) -> str:
            """'10-JUL-25' -> '10JUL25'"""
            return expiry_str.replace("-", "").upper() if expiry_str else ""

        if expiry_type == "current_week":
            return fmt(sorted_expiries[0]) if sorted_expiries else None
        ...
        logger.exception(f"Error resolving expiry type '{expiry_type}' for {symbol}: {e}")
        return None
