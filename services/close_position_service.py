...
def close_position(
    ...
    if api_key and not (auth_token and broker):
        ...
        requested_broker = position_data.get("broker")
        if requested_broker:
            # Pin to a SPECIFIC one of the caller's connected brokers instead
            # of whichever is their primary Auth session -- same override
            # convention as place_order_service.py's Case 1 order_data["broker"].
