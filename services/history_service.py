...
def get_history(
    ...
    if api_key and not (auth_token and broker):
        # See quotes_service.get_quotes's identical branch: a caller that
        # knows WHICH connected broker it wants (deployment_service.py's
        # evaluation loop, via dep.broker) must not fall through to
        # "whichever broker happens to be the primary Auth session" below,
        # or every indicator's historical-candle fallback silently ignores
        # the deployment's configured broker.
        if broker:
