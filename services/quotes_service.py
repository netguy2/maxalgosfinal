...
def get_quotes(
    ...
    if api_key and not (auth_token and broker):
        # A caller that also knows WHICH of the user's connected brokers it
        # wants (e.g. services/deployment_service.py's evaluation loop,
        # which knows dep.broker) must not fall through to "whichever
        # broker happens to be the user's primary Auth session" below --
        # that silently ignored the deployment's configured broker and
        # returned quotes for (or failed against) an unrelated session.
        # Mirrors place_order_service.py's identical "Case 1, specific
        # broker requested" branch.
        if broker:
            from database.auth_db import get_broker_session, verify_api_key

            user_id = verify_api_key(api_key)
            session_info = get_broker_session(user_id, broker) if user_id else None
            if not session_info:
                return (
                    False,
                    {
            ...
            AUTH_TOKEN, FEED_TOKEN, _br_user_id = session_info
