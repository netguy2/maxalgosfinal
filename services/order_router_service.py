...
def acquire_broker_rate_limit(broker_name: str, user_id: str, timeout: float = 5.0) -> bool:
    """
    Acquire token bucket rate limit permission before placing an order to smooth API bursts.
    Returns True if token acquired within timeout.
    """
    try:
        from utils.rate_limiter import get_broker_rate_limiter

        limiter_instance = get_broker_rate_limiter()
        return limiter_instance.acquire_order_token(broker_name, user_id, timeout=timeout)
    except Exception as e:
        logger.debug(f"Rate limiter acquire error for {broker_name}/{user_id}: {e}")
        return True  # Fall back to immediate execution on error
