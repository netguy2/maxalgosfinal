...
class TelegramBotService:
    """Service class for managing Telegram bot operations with Max Algos SDK integration"""
    ...
    def __init__(self):
        ...
        self.sdk_clients = {}  # Cache for Max Algos SDK clients per user
        self._stop_event = original_threading.Event()  # Thread-safe stop signal
        # Set by _start_bot_isolated when startup fails for a reason more
        # specific than "network error" (e.g. an invalid bot token) --
        # start_bot()'s polling loop only observes is_running, so without
        # this the real failure reason was logged but never reached the
        # caller/UI, which showed a misleading "Bot failed to start within
        # timeout" for what was actually an invalid-token error.
        self.start_error: str | None = None

    def _get_sdk_client(self, telegram_id: int) -> maxalgos_api | None:
        """Get or create Max Algos SDK client for a user"""
        from maxalgos import api as maxalgos_api
        ...
        try:
            ...
            # Create SDK client
            client = maxalgos_api(api_key=api_key, host=host_url)
    ...
    def _render_plotly_png(self, fig) -> bytes:
        ...
        t = original_threading.Thread(
            target=_worker, daemon=True, name="maxalgos-kaleido-render"
        )
        t.start()
        t.join()
    ...
    async def _start_bot_isolated(self):
        ...
        while retry_count < max_retries:
            try:
                ...
                # Not a network error -- retrying would just fail the same
                # way max_retries times. Surface the real reason so
                # start_bot()'s caller/UI can show "invalid bot token"
                # instead of a generic timeout.
                logger.error(f"Telegram bot token is invalid: {e}")
                self.start_error = (
                    "Invalid bot token. Get a valid token from @BotFather and save it "
    ...
    def start_bot(self) -> tuple[bool, str]:
        ...
        try:
            ...
            for _ in range(10):  # Wait up to 5 seconds
                ...
                # A specific failure (e.g. invalid token) sets start_error
                # and is_running=False well before the polling window ends
                # -- return immediately with the real reason instead of
                # waiting out the rest of the timeout.
                if self.start_error:
                    return False, self.start_error
                time.sleep(0.5)

            return False, self.start_error or "Bot failed to start within timeout"
    ...
    async def cmd_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if telegram_user:
            ...
            await update.message.reply_text(
                f"Welcome to Max Algos Bot, {user.first_name}! 🚀\n\n"
                "To get started, link your Max Algos account:\n"
...
*Account Management:*
/link `<api_key> <host_url>` - Link your Max Algos account
...
"""
    ...
    async def cmd_link(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            test_client = maxalgos_api(api_key=api_key, host=host_url)
            ...
            if test_response and test_response.get("status") == "success":
                ...
                # Get the actual Max Algos username from the API key
                maxalgos_username = None
                try:
                    maxalgos_username = get_username_by_apikey(api_key)
                    logger.info(f"API key lookup returned: '{maxalgos_username}'")
                except Exception as e:
                ...
                if not maxalgos_username and test_response.get("data"):
                    ...
                    if isinstance(data, dict):
                        ...
                        if maxalgos_username:
                            logger.info(f"Got username from funds response: {maxalgos_username}")

                # Log for debugging
                logger.info(
                    f"Linking Telegram user {user.id} (@{user.username}) with Max Algos username: '{maxalgos_username}'"
                )

                # If we still can't get username, DON'T use telegram username with @
                # Use a proper fallback
                if not maxalgos_username:
                    # Try to get from session or use telegram ID
                    maxalgos_username = f"user_{user.id}"
                    logger.warning(
                        f"Could not get Max Algos username, using fallback: {maxalgos_username}"
                    )
                else:
                    logger.info(f"Successfully retrieved Max Algos username: {maxalgos_username}")
                ...
                create_or_update_telegram_user(
                    telegram_id=user.id,
                    username=maxalgos_username,  # Use the actual Max Algos username
                ...
                logger.info(f"Database updated - Username stored as: {maxalgos_username}")
    ...
    async def cmd_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if telegram_user:
            ...
            # Get display name (prefer telegram_username, fallback to maxalgos_username)
            display_name = (
                telegram_user.get("telegram_username")
                or telegram_user.get("maxalgos_username")
                or "N/A"
            )
            ...
            await update.message.reply_text(
                "❌ No linked account found.\nUse /link to connect your Max Algos account.",
                parse_mode=ParseMode.MARKDOWN,
            )
    ...
    async def cmd_orderbook(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        client = self._get_sdk_client(user.id)
        if not client:
            await update.message.reply_text("❌ Failed to connect to Max Algos")
            return
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if callback_data == "confirm_closeall":
            ...
            client = self._get_sdk_client(user.id)
            if not client:
                await query.edit_message_text("❌ Failed to connect to Max Algos")
                return
        ...
        client = self._get_sdk_client(user.id)
        if not client:
            await context.bot.send_message(chat_id=chat_id, text="❌ Failed to connect to Max Algos")
            return
    ...
    async def broadcast_message(self, message: str, filters: dict = None) -> tuple[int, int]:
        ...
        try:
            ...
            if filters:
                ...
                if filters.get("maxalgos_username"):
                    users = [
                        u
                        for u in users
                        if u.get("maxalgos_username") == filters["maxalgos_username"]
                    ]
