...
# Indicator names -> (series_service function, whether it returns a single
# series or a dict of named sub-series like MACD's macd/signal/histogram).
_INDICATOR_SPECS = {
    "EMA": ("single", "compute_ema", ["period"]),
    "SMA": ("single", "compute_sma", ["period"]),
    "VWAP": ("single", "compute_vwap", []),
    "RSI": ("single", "compute_rsi", ["period"]),
    "MACD": ("multi", "compute_macd", ["fast_period", "slow_period", "signal_period"]),
    "ATR": ("single", "compute_atr", ["period"]),
    "BOLLINGER": ("multi", "compute_bollinger", ["period", "mult"]),
    "STOCH": ("multi", "compute_stochastic", ["k_period", "k_smooth", "d_period"]),
}


def _parse_indicator_spec(spec: str) -> tuple[str, list[str]] | None:
    """Parses one comma-separated entry like "EMA:20" or "BOLLINGER:20:2"
    into (name, [param strings]). Returns None for an unrecognized name
    rather than raising, so one bad entry in the query string doesn't fail
    the whole request."""
...
def get_indicator_overlays_from_bars(
    bars: list[dict[str, Any]], indicators_param: str
) -> tuple[bool, dict[str, Any], int]:
    """
    Compute indicator overlay series (+ crossover signals) for a chart,
    from a caller-supplied list of OHLCV bars (dicts with timestamp, open,
    high, low, close, volume) rather than reading Historify's stored DB.
    This is what pages/Charts.tsx uses — it already fetched bars live from
    the broker (works for any symbol immediately, no pre-download step),
    and this function computes indicators over exactly those same bars so
    the overlay lines are guaranteed to line up with the candles on screen.

    indicators_param is a comma-separated list like
    "EMA:20,SMA:50,RSI:14,MACD,BOLLINGER:20:2" — see _INDICATOR_SPECS for
    the recognized names and their positional params (all optional, using
    the same defaults as services/indicator_registry.py's plugins).

    Returns:
        Tuple of (success, response_data, status_code). response_data["data"]
        is keyed by a display name like "EMA_20" or "MACD" (a dict of
        sub-series for multi-output indicators like MACD/BOLLINGER).
        response_data["signals"] holds crossover markers for the specific
        pairs this function knows how to detect (EMA x SMA, RSI 30/70,
        price vs Bollinger bands) when the requested indicator set makes
        that pair available.
    """
    ...
    try:
        ...
        if df.empty:
            return True, {"status": "success", "data": {}, "signals": {}}, 200
        df = df.sort_values("timestamp").reset_index(drop=True)

        data: dict[str, Any] = {}
        computed_series: dict[str, Any] = {}  # name -> raw series, for signal detection below

        for raw_spec in indicators_param.split(","):
            parsed = _parse_indicator_spec(raw_spec)
            if parsed is None:
                continue
            name, params = parsed
            kind, func_name, param_names = _INDICATOR_SPECS[name]
            func = getattr(iss, func_name)

            kwargs = {}
            for i, pname in enumerate(param_names):
                if i < len(params) and params[i] != "":
                    kwargs[pname] = float(params[i]) if pname == "mult" else int(params[i])

            if kind == "single":
                period = kwargs.get("period")
                display_name = f"{name}_{period}" if period is not None else name
                series = func(df, **kwargs)
                data[display_name] = series
                computed_series[name] = {"params": kwargs, "series": series}
            else:
                result = func(df, **kwargs)
                data[name] = result
                computed_series[name] = {"params": kwargs, "series": result}

        signals: dict[str, list[dict]] = {}

        # EMA x SMA crossover -- only if both were requested (with any period).
        ema_entries = [v for k, v in computed_series.items() if k == "EMA"]
        sma_entries = [v for k, v in computed_series.items() if k == "SMA"]
        if ema_entries and sma_entries:
            signals["EMA_SMA_CROSS"] = iss.detect_crossover_signals(
                ema_entries[0]["series"], sma_entries[0]["series"]
            )

        # RSI crossing the classic 30/70 oversold/overbought thresholds.
        if "RSI" in computed_series:
            rsi_series = computed_series["RSI"]["series"]
            signals["RSI_OVERSOLD"] = iss.detect_threshold_crossover_signals(
                rsi_series, threshold=30, invert=False
            )
            signals["RSI_OVERBOUGHT"] = iss.detect_threshold_crossover_signals(
                rsi_series, threshold=70, invert=True
            )

        # Price crossing the Bollinger bands.
        if "BOLLINGER" in computed_series:
            bands = computed_series["BOLLINGER"]["series"]
            if bands.get("upper") and bands.get("lower"):
                close_series = [
                    {"time": int(t), "value": float(c)}
                    for t, c in zip(df["timestamp"], df["close"], strict=True)
                ]
                signals["BOLLINGER_UPPER_CROSS"] = iss.detect_crossover_signals(
                    close_series, bands["upper"]
                )
                signals["BOLLINGER_LOWER_CROSS"] = iss.detect_crossover_signals(
                    bands["lower"], close_series
                )
        ...
        logger.exception(f"Error computing indicator overlays: {e}")
        return False, {"status": "error", "message": str(e)}, 500
...
def _process_download_job(job_id: str, api_key: str):
    ...
    The whole body runs inside a `user_scope` resolved from `api_key`, so
    the historify_progress / _job_complete / _paused / _cancelled emits
    below reach only the user who started the download. They were bare
    broadcasts, so one user's bulk historical download drove the progress
    bar in every other logged-in user's Historify page.
    """
    from utils.socket_scope import user_scope, username_from_api_key

    with user_scope(username_from_api_key(api_key)):
        _process_download_job_inner(job_id, api_key)


def _process_download_job_inner(job_id: str, api_key: str):
    import json
...
def _emit_progress(job_id: str, current: int, total: int, symbol: str):
    """Emit Socket.IO progress event to the job's owner only."""
    try:
        from utils.socket_scope import emit_to_current_user

        emit_to_current_user(
            "historify_progress",
            {
                "job_id": job_id,
                "current": current,
...
def _emit_job_complete(job_id: str, completed: int, failed: int, total: int):
    """Emit Socket.IO job completion event to the job's owner only."""
    try:
        from utils.socket_scope import emit_to_current_user
...
def _emit_job_paused(job_id: str, current: int, total: int):
    """Emit Socket.IO job paused event to the job's owner only."""
    try:
        from utils.socket_scope import emit_to_current_user
...
def _emit_job_cancelled(job_id: str):
    """Emit Socket.IO job cancelled event to the job's owner only."""
    try:
        from utils.socket_scope import emit_to_current_user

        emit_to_current_user("historify_job_cancelled", {"job_id": job_id, "status": "cancelled"})
    except Exception as e:
