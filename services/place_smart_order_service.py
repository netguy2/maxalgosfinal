...
def place_smart_order_with_auth(
    ...
    # smart orders stay exempt. This blocks both opening and closing legs of a
    # position-aware smart order, which matches the switch's semantics: it
    # halts all new broker traffic, and kill_switch_close_positions_enabled is
    # the separate lever for unwinding. See services/order_gate.py.
    from services.order_gate import check_order_allowed

    allowed, gate_error, gate_status = check_order_allowed("smart order", api_key=api_key)
    if not allowed:
        bus.publish(OrderFailedEvent(
            mode="live", api_type="placesmartorder",
            request_data=order_request_data, response_data=gate_error,
    ...
    try:
        ...
        if res and res.status == 200 and order_id:
...
def place_smart_order(
    ...
    if api_key and not (auth_token and broker):
        ...
        if requested_broker:
            # See place_order_service.py's matching comment -- lets the
            # caller pick a specific one of their connected brokers instead
            # of always using the primary Auth row for this api_key.
