...
def construct_option_symbol(
    ...
    """
    Construct option symbol in Max Algos format.
...
def construct_crypto_option_symbol(
    ...
    """
    Construct crypto option symbol in Max Algos canonical format.
...
def get_option_symbol(
    ...
    try:
        ...
        if underlying_ltp is not None:
            ...
            if ltp == 0.0:
                logger.error(
                    f"LTP returned as 0.0 for {quote_symbol}. "
                    "Data broker does not support live quotes."
                )
                return (
                    False,
                    {
                        "status": "error",
                        "message": (
                            "Live market data is not available for the current broker. "
                            "Please connect a data broker that supports live quotes "
                            "(e.g. AliceBlue, Zebu, BNR) and click 'Set as data broker' "
...
_VALID_STRIKE_SELECTION_MODES = ("premium", "delta", "oi")


def get_option_symbol_by_metric(
    underlying: str,
    exchange: str,
    expiry_date: str | None,
    option_type: str,
    ...
    """
    Resolve an option strike by live PREMIUM, DELTA, or OPEN INTEREST
    instead of a fixed ATM/ITM/OTM offset (see get_option_symbol for that
    path, unchanged and still the default).

    Unlike get_option_symbol's index-walk math (one underlying LTP fetch,
    zero option-contract quote calls), this needs live premium/OI (and for
    "delta", computed Greeks) for a RANGE of candidate strikes -- it
    delegates to services/option_chain_service.py::get_option_chain for
    that batched fetch (same broker multiquote call oi_tracker_service.py
    already relies on), then picks the best-matching strike:
      - "premium": strike whose live LTP is closest to target_value
      - "delta": strike whose |delta| (via option_greeks_service, one more
        batched call over the SAME chain window) is closest to target_value
      - "oi": strike with the highest open interest in the window

    `strike_window` mirrors option_chain_service's `strike_count` (strikes
    above/below ATM) -- defaults to 15 (30 strikes x 2 sides = 60 symbols),
    comfortably under the ~100-symbol native-multiquote batch ceiling
    oi_tracker_service.py already established as safe for this codebase.

    Returns the same response shape as get_option_symbol
    ({status, symbol, exchange, lotsize, tick_size, freeze_qty}) so both
    callers (blueprints/strategy.py's config-time validation,
    services/signal_engine.py's signal-time resolution) can treat this as
    a drop-in alternative resolver. Never silently falls back to ATM --
    any failure (no live quotes, broker without batch-quote support, empty
    window) returns success=False with a clear message, mirroring
    get_option_chain's own "connect a data broker that supports live
    quotes" error convention.
    """
    try:
        mode = (mode or "").lower()
        if mode not in _VALID_STRIKE_SELECTION_MODES:
            return False, {"status": "error", "message": f"Invalid strike selection mode: {mode}"}, 400
        if mode in ("premium", "delta") and target_value is None:
            return (
                False,
                {"status": "error", "message": f"strike selection mode '{mode}' requires a target value"},
                400,
            )

        option_type = (option_type or "").upper()
        if option_type not in ("CE", "PE"):
            return False, {"status": "error", "message": f"Invalid option_type: {option_type}"}, 400
        ...
        if not final_expiry:
            return (
                ...
                {
                    "status": "error",
                    "message": "Expiry date required. Provide via expiry_date parameter or embed in underlying.",
                },
                400,
            )
        ...
        chain_success, chain_response, chain_status = get_option_chain(
            underlying, exchange, final_expiry, strike_window, api_key
        )
        if not chain_success:
            return False, chain_response, chain_status

        chain = chain_response.get("chain", [])
        side_key = "ce" if option_type == "CE" else "pe"
        candidates = [row for row in chain if row.get(side_key) and row[side_key].get("symbol")]
        if not candidates:
            return (
                False,
                {
                    "status": "error",
                    "message": f"No {option_type} strikes found for {base_symbol} expiring {final_expiry}.",
                },
                404,
            )

        # get_option_chain silently substitutes a hardcoded placeholder spot
        # price (e.g. NIFTY=24200.0) when no broker session/live quote is
        # available, specifically so its own UI can still paint strikes
        # (see its own "for initial paint" comment). That's fine for a
        # display-only chain view, but never safe to resolve a real order
        # against -- if NOT ONE candidate strike in the window has a live
        # (non-zero) premium, the whole chain (spot price included) is
        # almost certainly that fallback, not real market data. Fail
        # closed rather than silently pick a strike (or, for "oi", pick an
        # arbitrary strike from an all-zero max()) against fabricated
        # prices.
        has_live_quotes = any(row[side_key].get("ltp") for row in candidates)
        if not has_live_quotes:
            return (
                False,
                {
                    "status": "error",
                    "message": (
                        "Live market data is not available for the current broker -- cannot "
                        "resolve a strike by premium/delta/OI without real quotes. Please "
                        "connect a data broker that supports live quotes (e.g. AliceBlue, Zebu, "
                        "BNR) and click 'Set as data broker' in Broker Management."
                    ),
                },
                503,
            )
        ...
        if mode == "premium":
            scored = [row for row in candidates if row[side_key].get("ltp")]
            if not scored:
                return (
                    False,
                    {"status": "error", "message": "No live premium quotes available for these strikes."},
                    503,
                )
            best = min(scored, key=lambda row: abs(row[side_key]["ltp"] - target_value))

        elif mode == "oi":
            scored = [row for row in candidates if row[side_key].get("oi")]
            if not scored:
                return (
                    False,
                    {"status": "error", "message": "No live open interest data available for these strikes."},
                    503,
                )
            best = max(scored, key=lambda row: row[side_key]["oi"])

        else:  # "delta"
            from services.option_greeks_service import get_multi_option_greeks

            greeks_symbols = [
                {"symbol": row[side_key]["symbol"], "exchange": row[side_key].get("exchange") or exchange}
                for row in candidates
            ]
            greeks_success, greeks_response, greeks_status = get_multi_option_greeks(
                symbols=greeks_symbols, api_key=api_key
            )
            if not greeks_success and greeks_response.get("status") == "error":
                return False, greeks_response, greeks_status

            delta_by_symbol = {
                r["symbol"]: r["delta"]
                for r in greeks_response.get("data", [])
                if r.get("status") == "success" and r.get("delta") is not None
            }
            scored = [row for row in candidates if row[side_key]["symbol"] in delta_by_symbol]
            if not scored:
                return (
                    False,
                    {"status": "error", "message": "Could not compute Delta for these strikes."},
                    503,
                )
            best = min(
                scored,
                key=lambda row: abs(abs(delta_by_symbol[row[side_key]["symbol"]]) - target_value),
            )

        picked = best[side_key]
        picked_exchange = picked.get("exchange") or exchange

        from database.qty_freeze_db import get_freeze_qty_for_option

        freeze_qty = get_freeze_qty_for_option(picked["symbol"], picked_exchange)
        ...
        return (
            True,
            {
                ...
                "exchange": picked_exchange,
                "lotsize": picked.get("lotsize"),
                "tick_size": picked.get("tick_size"),
                "freeze_qty": freeze_qty,
        ...
        logger.exception(f"Error in get_option_symbol_by_metric: {e}")
        return (
            False,
            {
                "status": "error",
                "message": f"An error occurred while resolving strike by {mode}: {str(e)}",
            },
            500,
        )
...
def get_futures_symbol(
    ...
    This is the canonical futures-resolution implementation, promoted here
    from services/oi_profile_service.py's private _find_futures_symbol so
    MaxHook's live FUT resolution (services/signal_engine.py) reuses the
    exact same lookup instead of a third copy. Falls back to the nearest
    upcoming expiry if no contract matches the requested expiry exactly.

    Returns dict with 'symbol' and 'exchange' keys, or None on failure.
    """
    try:
        from database.token_db_enhanced import fno_search_symbols
        from utils.constants import INSTRUMENT_PERPFUT
