...
Executes workflow nodes using internal Max Algos services (synchronous Flask version)
"""
...
from services.flow_maxalgos_client import FlowMaxAlgosClient, get_flow_client
...
# Node types that place, modify, or cancel a real order/position at the
# broker. A diamond-shaped graph (two condition chains both wired to the
# same order node) or a cycle can reach one of these nodes more than once
# in a single execute_workflow() pass -- unlike read-only/data nodes
# (quotes, indicators, symbol lookups), re-running one of these is not
# idempotent, so execute_node_chain skips a second execution of the SAME
# node id within one run instead of placing/cancelling the order twice.
ORDER_ACTION_NODE_TYPES = {
    "placeOrder", "smartOrder", "optionsOrder", "modifyOrder",
    "cancelOrder", "cancelAllOrders", "closePositions", "basketClose",
...
class NodeExecutor:
    ...
    def __init__(self, client: FlowMaxAlgosClient, context: WorkflowContext, logs: list):
        self.client = client
    ...
    def _resolve_expiry_date(self, symbol: str, exchange: str, expiry_type: str) -> str | None:
        ...
        Thin wrapper around the shared services/expiry_service.py::
        resolve_expiry_type(), which does the actual date lookup/parsing/
        selection service-to-service (no HTTP round-trip). Kept as a
        method here so every existing call site in this class is
        unaffected; MaxHook's signal_engine.py calls the shared function
        directly.
        """
        from services.expiry_service import resolve_expiry_type

        result = resolve_expiry_type(symbol, exchange, expiry_type, self.client.api_key)
        if result is None:
            self.log(f"Failed to resolve expiry '{expiry_type}' for {symbol} on {exchange}", "error")
        return result
    ...
    def execute_basket_close(self, node_data: dict) -> dict:
        """Execute Basket Close node — squares off only a caller-specified list
        of positions ("the basket"), unlike execute_close_positions which is
        broker-wide with no selectivity. Pairs with execute_basket_pnl_check:
        wire that node's `condition` output into this node to build a real
        Master SL / Master Target — square off just the basket whose combined
        P&L breached a threshold, leaving every other open position untouched.

        UI fields: symbols — one "SYMBOL,EXCHANGE,PRODUCT" per line (same
        format execute_basket_pnl_check reads), strategy — tag recorded on
        each close call, defaults to "flow_basket_close".

        Closes each position via the already-implemented client.close_position()
        (services/close_position_service.py) sequentially, one broker call per
        symbol — no new broker plumbing, this was already implemented and
        callable but had no Flow node exposing it until now.
        """
        symbols_raw = node_data.get("symbols", "")
        strategy = self.get_str(node_data, "strategy", "flow_basket_close")

        basket: list[tuple[str, str, str]] = []
        if isinstance(symbols_raw, str):
            for line in symbols_raw.strip().split("\n"):
                line = line.strip()
                if not line:
                    continue
                ...
                if len(parts) >= 2:
                    ...
                    product = self.context.interpolate(parts[2]) if len(parts) >= 3 else "MIS"
                    basket.append((symbol, exchange, product))
                else:
                    self.log(
                        f"Skipping invalid basket line '{line}': expected SYMBOL,EXCHANGE[,PRODUCT]",
                        "warning",
                    )
                ...
                if isinstance(item, dict):
                    basket.append(
                        (
                            item.get("symbol", ""),
                            item.get("exchange", ""),
                            item.get("product", "MIS"),
                        )
                    )

        if not basket:
            error_result = {"status": "error", "message": "No valid basket symbols configured"}
            self.log("Basket close failed: no valid symbols", "error")
            return error_result

        self.log(f"Closing basket of {len(basket)} position(s)")
        results = []
        for symbol, exchange, product in basket:
            close_result = self.client.close_position(
                symbol=symbol, exchange=exchange, product_type=product, strategy=strategy
            )
            ok = close_result.get("status") == "success"
            self.log(
                f"  Close {symbol}/{exchange}/{product}: {close_result}",
                "info" if ok else "error",
            )
            results.append({"symbol": symbol, "exchange": exchange, **close_result})

        success_count = sum(1 for r in results if r.get("status") == "success")
        overall_status = "success" if success_count == len(results) else (
            "partial" if success_count > 0 else "error"
        )
        self.log(
            f"Basket close complete: {success_count}/{len(results)} succeeded",
            "info" if overall_status == "success" else "error",
        )
        result = {"status": overall_status, "results": results}
        self.store_output(node_data, result)
        return result
    ...
    def execute_basket_pnl_check(self, node_data: dict) -> dict:
        """Execute Basket P&L Check node — aggregates P&L across a caller-specified
        list of positions ("the basket"), not a single symbol. This is the
        "Master SL / Master Target" primitive: wire this node's `condition`
        output into a closePositions-style node to square off the whole basket
        the moment their COMBINED P&L crosses a threshold, instead of watching
        each leg's SL individually.

        UI fields:
          symbols: one "SYMBOL,EXCHANGE,PRODUCT" per line (PRODUCT optional,
            defaults to MIS). Matches the SYMBOL,EXCHANGE,ACTION,QTY CSV-line
            convention already used by execute_basket_order above.
          condition: "pnl_above" | "pnl_below" (mirrors execute_position_check's
            condition vocabulary so the two nodes read the same in the UI).
          threshold: the combined P&L level to compare against.

        Reuses the existing positionbook() call (already fetches ALL open
        positions with per-position pnl in one broker round-trip) rather than
        calling get_open_position() once per symbol — one network call
        regardless of basket size.
        """
        symbols_raw = node_data.get("symbols", "")
        condition = self.get_str(node_data, "condition", "pnl_below")
        threshold = self.get_float(node_data, "threshold", 0.0)

        basket: list[tuple[str, str, str | None]] = []
        if isinstance(symbols_raw, str):
            for line in symbols_raw.strip().split("\n"):
                line = line.strip()
                if not line:
                ...
                if len(parts) >= 2:
                    symbol = self.context.interpolate(parts[0])
                    exchange = self.context.interpolate(parts[1])
                    product = self.context.interpolate(parts[2]) if len(parts) >= 3 else None
                    basket.append((symbol, exchange, product))
                else:
                    self.log(
                        f"Skipping invalid basket line '{line}': expected SYMBOL,EXCHANGE[,PRODUCT]",
            ...
            for item in symbols_raw:
                if isinstance(item, dict):
                    basket.append(
                        (item.get("symbol", ""), item.get("exchange", ""), item.get("product"))
                    )

        if not basket:
            error_result = {"status": "error", "message": "No valid basket symbols configured"}
            self.log("Basket P&L check failed: no valid symbols", "error")
            return error_result

        self.log(f"Checking combined P&L for basket of {len(basket)} position(s)")
        result = self.client.positionbook()
        if result.get("status") != "success":
            self.log(f"Basket P&L check failed: could not fetch positionbook: {result}", "error")
            return result
        ...
        for symbol, exchange, product in basket:
            for pos in positions:
                if pos.get("symbol") != symbol or pos.get("exchange") != exchange:
                    continue
                pos_product = pos.get("product") or pos.get("product_type")
                if product and pos_product != product:
                    continue
        ...
        if condition == "pnl_above":
            condition_met = total_pnl > threshold
        elif condition == "pnl_below":
            condition_met = total_pnl < threshold
        else:
            condition_met = False
            self.log(f"Basket P&L check: unknown condition '{condition}' — returning False", "error")

        self.log(
            f"Basket P&L check: {matched}/{len(basket)} position(s) matched, "
            f"combined pnl={total_pnl} {condition} {threshold} = {condition_met}"
        )
        result = {
            "status": "success",
            "condition": condition_met,
    ...
    def execute_indicator_condition(self, node_data: dict) -> dict:
        """Execute Indicator Condition node — gate a chain on a real technical
        indicator (RSI, EMA, SMA, MACD, ATR, VWAP) rather than raw price.

        This is the trigger half of "enter a Bull Call Spread when RSI < 30":
        wire this node's True output into an optionsMultiOrder node (which
        already places real multi-leg broker orders) to get a genuinely
        indicator-triggered spread entry.

        UI fields: symbol, exchange, indicator (RSI/EMA/SMA/MACD/ATR/VWAP),
        period (ignored by VWAP), operator (>, <, ==, >=, <=, !=), value
        (the threshold). Same field-naming convention as execute_price_condition.

        Indicator values come from services.indicator_engine.calculate_indicator,
        which reads real historical candle data via database.historify_db
        (see services/indicator_registry.py) — not the placeholder
        ltp-multiplier math this platform used before.
        """
        from services.indicator_engine import calculate_indicator

        symbol = self.get_str(node_data, "symbol", "")
        exchange = self.get_str(node_data, "exchange", "NSE")
        indicator = self.get_str(node_data, "indicator", "RSI")
        ...
        self.log(f"Checking {indicator}({period}) for: {symbol}")
        indicator_value = calculate_indicator(indicator, symbol, exchange, period=period)

        condition_met = self._compare(indicator_value, operator, threshold)
        self.log(
            f"Indicator check: {indicator}({period})={indicator_value} {operator} {threshold} = {condition_met}"
        )
        return {
            "status": "success",
            "condition": condition_met,
    ...
    def execute_subscribe_ltp(self, node_data: dict) -> dict:
        ...
        Connects to Max Algos WebSocket server and subscribes to LTP updates.
    ...
    def execute_subscribe_quote(self, node_data: dict) -> dict:
        ...
        Connects to Max Algos WebSocket and subscribes to quote updates (OHLC + volume).
        Falls back to REST API if WebSocket fails or times out.
        """
        symbol = self.get_str(node_data, "symbol", "")
        exchange = self.get_str(node_data, "exchange", "NSE")
    ...
    def execute_subscribe_depth(self, node_data: dict) -> dict:
        ...
        Connects to Max Algos WebSocket and subscribes to depth updates (order book).
        Falls back to REST API if WebSocket fails or times out.
        """
        symbol = self.get_str(node_data, "symbol", "")
        exchange = self.get_str(node_data, "exchange", "NSE")
...
def execute_node_chain(
    ...
    depth: int = 0,
    executed_order_nodes: set | None = None,
    order_failures: list | None = None,
):
    ...
    # A diamond graph (two branches both wired to this same order node) or
    # a cycle can reach an order-placing/modifying/cancelling node more
    # than once in one run. Re-executing it would place/cancel the same
    # order twice -- skip the second and later visits instead.
    if node_type in ORDER_ACTION_NODE_TYPES and executed_order_nodes is not None:
        if node_id in executed_order_nodes:
            executor.log(
                f"Skipping node {node_id} ({node_type}): already executed once in "
                f"this run (reached again via another path/cycle) -- an order "
                f"action node runs at most once per execution to avoid placing "
                f"the same order twice",
                "warning",
            )
            return
        executed_order_nodes.add(node_id)
    ...
    elif node_type == "basketClose":
        result = executor.execute_basket_close(node_data)
    ...
    elif node_type == "basketPnlCheck":
        result = executor.execute_basket_pnl_check(node_data)
    ...
    elif node_type == "indicatorCondition":
        result = executor.execute_indicator_condition(node_data)
    ...
    # Record an order-action failure precisely (checking result.status
    # directly, not by re-parsing log message text) so execute_workflow can
    # report the run as completed_with_errors instead of a bare
    # "completed" when a broker rejected an order deep in the graph.
    if (
        node_type in ORDER_ACTION_NODE_TYPES
        and order_failures is not None
        and isinstance(result, dict)
        and result.get("status") == "error"
    ):
        order_failures.append({"node_id": node_id, "node_type": node_type, "result": result})
...
def execute_workflow(
    ...
    # Scoped to the workflow owner's own kill switch (resolved from the
    # api_key this run was invoked with). The switch is per-user, so another
    # account's emergency stop must not abort this user's workflows.
    if is_kill_switch_active(username_from_api_key(api_key)):
        return {
            "status": "error",
    ...
    with lock:
        ...
        try:
            ...
            client = get_flow_client(api_key, broker=workflow.broker)
            ...
            # A broker rejection/failure inside an order-action node (e.g.
            # execute_place_order) does not raise -- it's caught and logged
            # by the node executor itself, then the graph walk continues.
            # So reaching here doesn't mean every order succeeded;
            # order_failures (populated directly from each order-action
            # node's result.status inside execute_node_chain) is the
            # precise signal, rather than reaching the end of the walk.
            had_order_failure = len(order_failures) > 0
            final_status = "completed_with_errors" if had_order_failure else "completed"
            update_execution_status(execution.id, final_status)
            save_execution_logs(execution.id, logs)
            return {
                "status": "success" if not had_order_failure else "error",
                "message": "Workflow executed successfully"
                if not had_order_failure
                else f"Workflow completed, but {len(order_failures)} order action(s) failed",
