...
def place_single_order(
    ...
    try:
        ...
        # return HTTP 200 regardless of whether the broker accepted the order -
        # the real outcome is only visible via order_id (None on failure). Require
        # both the HTTP status and a returned order_id for a call to count as success.
        if res.status == 200 and order_id:
            try:
                ...
                async_verify_order_fill(
                    ...
                    broker=getattr(broker_module, "__name__", "broker").split(".")[-1],
                    orderid=str(order_id),
                    symbol=order_data.get("symbol", ""),
                    action=order_data.get("action", "BUY"),
                    # Each basket leg carries the basket's apikey (merged in
                    # by build_basket_orders), so the rejection toast is
                    # attributed to the basket's owner even though this runs
                    # on a ThreadPoolExecutor worker.
                    username=username_from_api_key(order_data.get("apikey")),
                )
            except Exception as _ve:
                logger.debug(f"Could not queue basket order fill verification: {_ve}")
...
def process_basket_order_with_auth(
    ...
    # Kill-switch gate, after the analyze/sandbox branch above so sandbox
    # baskets stay exempt. See services/order_gate.py.
    from services.order_gate import check_order_allowed

    allowed, gate_error, gate_status = check_order_allowed(
        "basket order", api_key=basket_data.get("apikey", "")
    )
    if not allowed:
        # Must publish, not just return: order.failed drives the red toast and
        # the notification-drawer entry in the React UI (see
        # subscribers/socketio_subscriber.py::on_order_failed). Returning the
        # error alone would block the basket silently from the user's view.
    ...
    for batch_start in range(0, total_orders, BATCH_SIZE):
        ...
        # Explicitly propagate this thread's contextvars (in particular
        # utils.broker_context's active override, if a multi-broker fan-out
        # is in progress) into each worker thread. ThreadPoolExecutor does
        # NOT do this automatically - a plain executor.submit(...) call
        # would leave workers with no override and silently fall back to
        # the wrong (data broker's) credentials. See utils/broker_context.py
        # module docstring for the full history of why this matters.
        #
        # IMPORTANT: a single contextvars.Context object can only be
        # "entered" (running) in one place at a time - reusing the SAME
        # copied context across multiple concurrent ctx.run() calls (one
        # per worker thread, submitted together) raises "RuntimeError:
        # cannot enter context: ... is already entered" the moment two
        # workers race. Each submission must get its OWN copy_context()
        # call (all copies carry the same values, but are distinct Context
        # objects that can each be entered independently and concurrently).
