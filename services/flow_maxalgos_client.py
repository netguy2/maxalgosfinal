# services/flow_maxalgos_client.py
"""
Max Algos Client Wrapper for Flow
Provides SDK-like interface using internal Max Algos services
"""
...
class FlowMaxAlgosClient:
    """
    Client wrapper that provides SDK-like interface to Max Algos services.
    Used by Flow workflow executor to interact with trading functionality.
    """

    def __init__(self, api_key: str, broker: str | None = None):
        """
        Initialize the client with an API key.

        Args:
            api_key: The Max Algos API key for authentication
            broker: Optional specific connected broker to pin order/position
                calls to (see FlowWorkflow.broker). None (default) preserves
                today's behavior of resolving whichever broker is the user's
                current primary Auth session.
        """
    ...
    def positionbook(self) -> dict[str, Any]:
        ...
        success, response, status_code = get_positionbook(
            api_key=self.api_key, requested_broker=self.broker
        )
        return self._handle_response(success, response, status_code)
...
def get_flow_client(api_key: str, broker: str | None = None) -> FlowMaxAlgosClient:
    """
    Factory function to create a Flow Max Algos client.

    Args:
        api_key: The Max Algos API key
        broker: Optional specific connected broker to pin this workflow's
            order/position calls to (see FlowWorkflow.broker).

    Returns:
        FlowMaxAlgosClient instance
    """
    return FlowMaxAlgosClient(api_key, broker=broker)
