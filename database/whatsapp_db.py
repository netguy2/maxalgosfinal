...
Anyone with the maxalgos.db file AND the API_KEY_PEPPER + FERNET_SALT (i.e.
...
# Tables live in the main maxalgos.db by default. DATABASE_URL is whatever
...
def _resolve_whatsapp_salt() -> bytes:
    ...
    if raw and len(raw) >= 32:
        try:
            ...
            pass
    return b"maxalgos_static_salt:whatsapp-session"
...
# SQLAlchemy engine — same NullPool pattern as the rest of Max Algos SQLite usage.
engine = create_db_engine(DATABASE_URL, pool_recycle=3600)
...
class WhatsAppConfig(Base):
    ...
    # Single-user Max Algos: the operator who paired the device is the bot's
...
class WhatsAppUser(Base):
    """Linked recipient — a WhatsApp number associated with an Max Algos user.
    ...
    maxalgos_username = Column(String(255), nullable=False, index=True)
...
def create_or_update_whatsapp_user(
    ...
    try:
        ...
        if user:
            user.maxalgos_username = username
...
def get_whatsapp_user(whatsapp_jid: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        result = {
            ...
            "maxalgos_username": user.maxalgos_username,
...
def get_whatsapp_user_by_username(username: str) -> dict[str, Any] | None:
    ...
    try:
        user = (
            db_session.query(WhatsAppUser)
            .filter_by(maxalgos_username=username, is_active=True)
            .first()
        )
        if not user:
            return None
...
def get_user_credentials(whatsapp_jid: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        result = {
            ...
            "username": user.maxalgos_username,
            "broker": user.broker,
        }
        ...
        return [
            {
                ...
                "maxalgos_username": u.maxalgos_username,
