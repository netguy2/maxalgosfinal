...
latency_engine = create_db_engine(LATENCY_DATABASE_URL)
...
class OrderLatency(LatencyBase):
    ...
    def log_latency(
        ...
        try:
            # error/request_body/response_body can arrive as a dict -- e.g. a
            # validation failure passes response_data["message"], which for a
            # marshmallow error is a dict like {"apikey": ["Missing..."]}.
            # The `error` column is TEXT, so binding a dict raises
            # "type 'dict' is not supported" and the whole latency insert is
            # lost. Coerce non-string values to str so a failed order still
            # gets logged instead of throwing a traceback into the error log
...
def init_latency_db():
    ...
    # Extract directory from database URL and create it if missing -- only
    # meaningful for a file-backed SQLite URL; a PostgreSQL URL has no
    # filesystem directory to create.
    if LATENCY_DATABASE_URL and LATENCY_DATABASE_URL.startswith("sqlite:///"):
