...
# Per-user scoped: `cache_loaded` is a consequence of ONE user's broker
# connect / master-contract refresh, and this module is always reached from
# inside that user's `user_scope` (see utils/auth_utils.py's
# async_master_contract_download). As a bare broadcast it fired a
# "master contract connected" state change in every logged-in user's UI.
from utils.socket_scope import scoped_socketio as socketio

logger = get_logger(__name__)


def _record_activity_for_session(category, title, message):
    """Best-effort activity log entry for the logged-in user, if any.

    Guarded because this hook also runs from non-request contexts
    (e.g. scheduled/background master contract refreshes).
    """
    try:
        ...
        record_activity(username, category, title, message)
    except Exception:
        pass
...
def load_symbols_to_cache(broker: str) -> bool:
    ...
    try:
        ...
        if success:
            ...
            _record_activity_for_session(
                "system",
                "System",
                f"Master contract connected ({stats['total_symbols']} symbols)",
            )
            _record_activity_for_session("broker", "Platform", f"{broker} live feed connected")
