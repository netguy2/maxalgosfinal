...
DB_PATH = os.getenv("DATABASE_URL", "sqlite:///db/maxalgos.db")

# Ensure the directory exists -- only meaningful for a file-backed SQLite URL;
# a PostgreSQL URL has no filesystem directory to create.
...
def update_status(broker, status, message, total_symbols=None):
    ...
    try:
        ...
        if broker_status:
            ...
            if status == "error":
                broker_status.total_symbols = "0"
                broker_status.exchange_stats = None
                broker_status.last_download_time = None
                broker_status.download_date = None
                broker_status.download_duration_seconds = None
            else:
