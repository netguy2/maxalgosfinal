...
# NullPool (SQLite) / QueuePool (other backends) is handled by create_db_engine.
# Each checkout creates a fresh connection that is closed immediately, so no
# FD leak risk. Session cleanup (which also prevents FD leaks) is handled by:
...
def init_logs_db():
    ...
    if LOGS_DATABASE_URL and LOGS_DATABASE_URL.startswith("sqlite:///"):
